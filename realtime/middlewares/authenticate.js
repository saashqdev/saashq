...
function authenticate_with_saashq(socket, next) {
	...
	socket.saashq_request = (path, args = {}, opts = {}) => {
	...
	socket
		.saashq_request("/api/method/saashq.realtime.get_user_info")
		.then((res) => res.json())
		.then(({ message }) => {
...
function get_site_name(socket) {
	...
	} else if (socket.request.headers["x-saashq-site-name"]) {
		socket.site_name = get_hostname(socket.request.headers["x-saashq-site-name"]);
	} else if (
		conf.default_site &&
...
module.exports = authenticate_with_saashq;
