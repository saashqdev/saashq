...
def load_address_and_contact(doc, key=None) -> None:
	...
	from saashq.contacts.doctype.address.address import get_address_display_list
	from saashq.contacts.doctype.contact.contact import get_contact_display_list
...
def has_permission(doc, ptype, user):
	...
	for df in links.get("permitted_links") + links.get("not_permitted_links"):
		...
		if name and saashq.has_permission(doctype, ptype, doc=name):
			return True
...
def delete_contact_and_address(doctype: str, docname: str) -> None:
	for parenttype in ("Contact", "Address"):
		for name in saashq.get_all(
			...
			doc = saashq.get_doc(parenttype, name)
			if len(doc.links) == 1:
				doc.delete()
			else:
...
def filter_dynamic_link_doctypes(
	...
	_doctypes_from_df = saashq.get_all(
		"DocField",
		filters=filters,
		pluck="parent",
		distinct=True,
	...
	_doctypes_from_cdf = saashq.get_all(
...
def set_link_title(doc):
	...
	for link in doc.links:
		if not link.link_title:
			linked_doc = saashq.get_doc(link.link_doctype, link.link_name)
