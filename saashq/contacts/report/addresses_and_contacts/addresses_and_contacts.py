...
def get_reference_addresses_and_contact(reference_doctype, reference_name):
	data = []
	filters = None
	reference_details = saashq._dict()
	...
	reference_list = [
		d[0] for d in saashq.get_list(reference_doctype, filters=filters, fields=["name"], as_list=True)
	]

	for d in reference_list:
		reference_details.setdefault(d, saashq._dict())
...
def get_reference_details(reference_doctype, doctype, reference_list, reference_details):
	...
	records = saashq.get_list(doctype, filters=filters, fields=fields, as_list=True)
	temp_records = [d[1:] for d in records]

	if not reference_list:
		saashq.throw(_("No records present in {0}").format(reference_doctype))

	reference_details[reference_list[0]][saashq.scrub(doctype)] = temp_records
