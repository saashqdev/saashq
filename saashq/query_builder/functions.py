...
import saashq
from saashq.query_builder.custom import GROUP_CONCAT, MATCH, STRING_AGG, TO_TSVECTOR
from saashq.query_builder.utils import ImportMapper, db_type_is
...
class Cast_(Function):
	def __init__(self, value, as_type, alias=None):
		if saashq.db.db_type == "mariadb" and (
...
def _aggregate(function, dt, fieldname, filters, **kwargs):
	return (
		saashq.qb.get_query(dt, filters=filters, fields=[function(PseudoColumn(fieldname))]).run(**kwargs)[0][
			0
		]
		or 0
	)
