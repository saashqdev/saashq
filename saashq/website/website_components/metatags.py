...
class MetaTags:
	def __init__(self, path, context):
		...
		self.tags = saashq._dict(self.context.get("metatags") or {})
	...
	def init_metatags_from_context(self):
		...
		if self.tags.get("image"):
			self.tags["image"] = saashq.utils.get_url(self.tags["image"])

		self.tags["language"] = saashq.local.lang or "en"
	...
	def set_metatags_from_website_route_meta(self):
		...
		if route == "":
			# homepage
			route = saashq.db.get_single_value("Website Settings", "home_page")

		route_exists = (
			route and not route.endswith((".js", ".css")) and saashq.db.exists("Website Route Meta", route)
		)

		if route_exists:
			website_route_meta = saashq.get_doc("Website Route Meta", route)
