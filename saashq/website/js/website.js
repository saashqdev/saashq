...
saashq.provide("website");
saashq.provide("saashq.awesome_bar_path");
window.cur_frm = null;
...
$.extend(saashq, {
	...
	require: async function (links, callback) {
		...
		links = links.map((link) => saashq.bundled_asset(link));
	...
	add_asset_to_head(link) {
		return new Promise((resolve) => {
			if (saashq._assets_loaded.includes(link)) return resolve();
			let el;
			...
			el.onload = () => {
				saashq._assets_loaded.push(link);
				resolve();
			};
		});
	},
	...
	call: function (opts) {
		...
		return $.ajax({
			...
			statusCode: opts.statusCode || {
				404: function () {
					saashq.msgprint(__("Not found"));
				},
				403: function () {
					saashq.msgprint(__("Not permitted"));
				},
				200: function (data) {
			...
			saashq.process_response(opts, data);
		});
	},
	...
	process_response: function (opts, data) {
		...
		if (data._server_messages) {
			...
			if (opts.error_msg) {
				...
				saashq.msgprint(server_messages);
			}
		}
	...
	has_permission: function (doctype, docname, perm_type, callback) {
		return saashq.call({
			type: "GET",
			method: "saashq.client.has_permission",
			no_spinner: true,
	...
	render_user: function () {
		if (saashq.is_user_logged_in()) {
			$(".btn-login-area").toggle(false);
			$(".logged-in").toggle(true);
			$(".user-image").attr("src", saashq.get_cookie("user_image"));

			$(".user-image-wrapper").html(
				saashq.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-sidebar").html(
				saashq.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-myaccount").html(
				saashq.avatar(null, "avatar-large", null, null, null, true)
			);
		}
	},
	freeze_count: 0,
	...
	unfreeze: function () {
		if (!saashq.freeze_count) return; // anything open?
		saashq.freeze_count--;
		if (!saashq.freeze_count) {
	...
	trigger_ready: function () {
		saashq.ready_events.forEach(function (fn) {
			fn();
		});
	},
	...
	bind_filters: function () {
		...
		$(".filter").each(function () {
			...
			var val = saashq.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function () {
		saashq.get_navbar_search().on("keypress", function (e) {
			var val = $(this).val();
			if (e.which === 13 && val) {
				$(this).val("").blur();
				saashq.do_search(val);
				return false;
			}
		});
	},
	do_search: function (val) {
		var path =
			(saashq.awesome_bar_path && saashq.awesome_bar_path[location.pathname]) ||
			window.search_path ||
			location.pathname;
	...
	set_search_path: function (path) {
		saashq.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function () {
		return saashq.get_cookie("user_id") !== "Guest" && saashq.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (saashq.session.user === "Guest" && window.show_language_picker) {
			saashq
				.call("saashq.translate.get_all_languages", {
					with_language_name: true,
				})
				.then((res) => {
					let language_list = res.message;
					let language = saashq.get_cookie("preferred_language");
					let language_codes = [];
...
saashq.setup_search = function (target, search_scope) {
	...
	$input.on(
		...
		saashq.utils.debounce(() => {
			...
			saashq
				.call({
					method: "saashq.search.web_search",
					args: {
...
window.valid_email = function (id) {
	// copied regex from saashq/utils.js validate_type
...
$(document).ready(function () {
	window.full_name = saashq.get_cookie("full_name");
	var logged_in = saashq.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	saashq.bind_navbar_search();

	// add apps link
	let apps = saashq.boot?.apps_data?.apps;
	let obj = {
		label: __("Apps"),
		route: "/apps",
	};
	if (apps?.length) {
		...
		let is_desk_apps = saashq.boot?.apps_data?.is_desk_apps;
		!is_desk_apps && saashq.add_apps(obj);
	}

	// switch to app link
	if (saashq.get_cookie("system_user") === "yes" && logged_in) {
		saashq.add_switch_to_desk();
	}
...
$(document).on("page-change", function () {
	...
	$.extend(saashq, saashq.get_cookies());
	saashq.session = { user: saashq.user_id };

	saashq.datetime.refresh_when();
	saashq.trigger_ready();
	saashq.bind_filters();
	saashq.highlight_code_blocks();
	saashq.add_link_to_headings();
	saashq.make_navbar_active();
	// scroll to hash
...
saashq.ready(function () {
	saashq.show_language_picker();
	saashq.setup_videos();
	saashq.realtime.init(window.socketio_port, true); // lazy connection
});
