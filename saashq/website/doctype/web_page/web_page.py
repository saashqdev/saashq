...
from saashq.utils import get_datetime, now, quoted, strip_html
from saashq.utils.caching import redis_cache
from saashq.utils.jinja import render_template
from saashq.utils.safe_exec import safe_exec
from saashq.website.doctype.website_slideshow.website_slideshow import get_slideshow
...
class WebPage(WebsiteGenerator):
	...
	if TYPE_CHECKING:
		...
		from saashq.website.doctype.web_page_block.web_page_block import WebPageBlock
	...
	def get_context(self, context):
		...
		if self.context_script:
			_locals = dict(context=saashq._dict())
	...
	def render_dynamic(self, context):
		...
		if is_jinja:
			saashq.flags.web_block_scripts = {}
			saashq.flags.web_block_styles = {}
			try:
	...
	def check_for_redirect(self, context):
		if "<!-- redirect:" in context.main_section:
			saashq.local.flags.redirect_location = (
	...
	def validate_dates(self):
		if self.end_date:
			...
			if self.published and now() > self.end_date:
				...
				saashq.msgprint(_("Clearing end date, as it cannot be in the past for published pages."))


def check_publish_status():
	# called via daily scheduler
	web_pages = saashq.get_all("Web Page", fields=["name", "published", "start_date", "end_date"])
	...
	for page in web_pages:
		...
		if page.published:
			...
			if (start_date and now_date < start_date) or (end_date and now_date > end_date):
				saashq.db.set_value("Web Page", page.name, "published", 0)
		else:
			# Publish pages that are inside the set date ranges
			if start_date:
				if not end_date or (end_date and now_date < end_date):
					saashq.db.set_value("Web Page", page.name, "published", 1)


def get_web_blocks_html(blocks):
	"""Convert a list of blocks into Raw HTML and extract out their scripts for deduplication."""

	out = saashq._dict(html="", scripts={}, styles={})
	extracted_scripts = {}
	extracted_styles = {}
	for block in blocks:
		web_template = saashq.get_cached_doc("Web Template", block.web_template)
		rendered_html = saashq.render_template(
...
def get_dynamic_web_pages() -> dict[str, str]:
	...
	get_web_pages_with_dynamic_routes = saashq.get_hooks("get_web_pages_with_dynamic_routes") or []
	for method in get_web_pages_with_dynamic_routes:
		pages.extend(saashq.get_attr(method)())
	return pages
