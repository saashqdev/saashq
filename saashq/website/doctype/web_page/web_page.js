...
saashq.ui.form.on("Web Page", {
	layout: function (frm) {
		if (frm.is_new()) {
			if (frm.doc.insert_code) {
				if (!frm.doc.javascript) {
					frm.set_value("javascript", `saashq.ready(() => {\n\t\n});`);
				}
			}
		}
	},
	...
	validate: function (frm) {
		if (frm.doc.title && !frm.doc.route) {
			frm.set_value("route", saashq.scrub(frm.doc.title, "-"));
		}
	},
	...
	published: function (frm) {
		...
		if (!frm.doc.published && frm.doc.end_date) {
			var end_date = saashq.datetime.str_to_obj(saashq.datetime.now_datetime());
	...
	set_meta_tags(frm) {
		saashq.utils.set_meta_tag(frm.doc.route);
	},
});

saashq.ui.form.on("Web Page Block", {
	edit_values(frm, cdt, cdn) {
		...
		open_web_template_values_editor(row.web_template, values).then((new_values) => {
			saashq.model.set_value(cdt, cdn, "web_template_values", JSON.stringify(new_values));
		});
	},
});

saashq.tour["Web Page"] = [
	{
		fieldname: "title",
	...
	},
	{
		...
		description: `${__("You can select one from the following,")} <br>
					<ul>
						...
						<li><b>${__("Page Builder")}</b>: ${__("Saashq page builder using components")}</li>
					</ul>
					`,
	},
	{
