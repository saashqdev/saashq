...
from saashq.website.doctype.website_theme.website_theme import (
	after_migrate,
	get_active_theme,
	get_scss_paths,
)
...
def website_theme_fixture(**theme):
	...
	saashq.delete_doc_if_exists("Website Theme", test_theme)
	theme = saashq.get_doc(doctype="Website Theme", theme=test_theme, **theme)
	theme.insert()
	yield theme
	saashq.db.set_single_value("Website Settings", "website_theme", "Standard")
	theme.delete()


def get_theme_file(theme):
	return Path(saashq.get_site_path("public", theme.theme_url[1:]))
...
class TestWebsiteTheme(IntegrationTestCase):
	...
	def test_get_scss_paths(self):
		self.assertIn("saashq/public/scss/website.bundle", get_scss_paths())

	def test_imports_to_ignore(self):
		with website_theme_fixture(ignored_apps=[{"app": "saashq"}]) as theme:
			self.assertTrue('@import "saashq/public/scss/website"' not in theme.theme_scss)
