...
from saashq.website.utils import (
	clear_cache,
	find_first_image,
	get_comment_list,
...
class BlogPost(WebsiteGenerator):
	...
	def make_route(self):
		if not self.route:
			return (
				saashq.db.get_value("Blog Category", self.blog_category, "route")
				+ "/"
	...
	def validate(self):
		...
		if self.featured:
			if not self.meta_image:
				saashq.throw(_("A featured post must have a cover image"))
	...
	def reset_featured_for_other_blogs(self):
		all_posts = saashq.get_all("Blog Post", {"featured": 1})
		for post in all_posts:
			saashq.db.set_value("Blog Post", post.name, "featured", 0)
	...
	def get_context(self, context):
		...
		context.enable_cta = not self.hide_cta and saashq.db.get_single_value(
			"Blog Settings", "show_cta_in_blog", cache=True
		)

		if self.blogger:
			context.blogger_info = saashq.get_doc("Blogger", self.blogger).as_dict()
		...
		context.category = saashq.db.get_value(
		...
		context.guest_allowed = saashq.db.get_single_value("Blog Settings", "allow_guest_to_comment")

	def fetch_cta(self):
		if saashq.db.get_single_value("Blog Settings", "show_cta_in_blog", cache=True):
			blog_settings = saashq.get_cached_doc("Blog Settings")
	...
	def fetch_social_links_info(self):
		if not saashq.db.get_single_value("Blog Settings", "enable_social_sharing", cache=True):
			return []

		url = saashq.local.site + "/" + self.route
	...
	def load_likes(self, context):
		...
		context.like_count = saashq.db.count("Comment", filters)
		...
		context.like = saashq.db.count("Comment", filters)
...
def get_list_context(context=None):
	...
	blog_settings = saashq.get_doc("Blog Settings").as_dict(no_default_fields=True)
	list_context.update(blog_settings)

	category_name = saashq.utils.escape_html(
		saashq.local.form_dict.blog_category or saashq.local.form_dict.category
	)
	if category_name:
		category = saashq.get_doc("Blog Category", category_name)
	...
	elif saashq.local.form_dict.blogger:
		blogger = saashq.db.get_value("Blogger", {"name": saashq.local.form_dict.blogger}, "full_name")
		list_context.sub_title = _("Posts by {0}").format(blogger)
		list_context.title = blogger

	elif saashq.local.form_dict.txt:
		list_context.sub_title = _('Filtered by "{0}"').format(sanitize_html(saashq.local.form_dict.txt))
...
def get_blog_categories():
	...
	post, category = saashq.qb.DocType("Blog Post"), saashq.qb.DocType("Blog Category")
	return (
		saashq.qb.from_(category)
		.select(category.name, category.route, category.title)
		.where(
			(category.published == 1)
			& ExistsCriterion(
				saashq.qb.from_(post)
				.select("name")
...
def clear_blog_cache():
	for blog in saashq.db.get_list("Blog Post", fields=["route"], pluck="route", filters={"published": True}):
		clear_cache(blog)
...
def get_blog_list(doctype, txt=None, filters=None, limit_start=0, limit_page_length=20, order_by=None):
	...
	if filters and filters.get("blog_category"):
		...
		category = saashq.utils.escape_html(
			saashq.local.form_dict.blog_category or saashq.local.form_dict.category
		)

	if filters and filters.get("blogger"):
		conditions.append("t1.blogger=%s" % saashq.db.escape(filters.get("blogger")))

	if category:
		conditions.append("t1.blog_category=%s" % saashq.db.escape(category))

	if txt:
		conditions.append(
			'(t1.content like {0} or t1.title like {0}")'.format(saashq.db.escape("%" + txt + "%"))
		)

	if conditions:
		saashq.local.no_cache = 1
	...
	posts = saashq.db.sql(query, as_dict=1)
	...
	for post in posts:
		...
		post.category = saashq.db.get_value(
