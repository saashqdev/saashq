...
from saashq.utils import cint, encode, get_request_site_address
from saashq.website.utils import get_boot_data
...
class WebsiteSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from saashq.website.doctype.top_bar_item.top_bar_item import TopBarItem
		from saashq.website.doctype.website_route_redirect.website_route_redirect import (
	...
	def validate_google_settings(self):
		if self.enable_google_indexing and not saashq.db.get_single_value("Google Settings", "enable"):
		...
		for idx, row in enumerate(self.route_redirects):
			try:
				...
				if not saashq.flags.in_migrate:
					saashq.throw(_("Invalid redirect regex in row #{}: {}").format(idx, str(e)))
	...
	def get_access_token(self):
		...
		if not self.indexing_refresh_token:
			button_label = saashq.bold(_("Allow API Indexing Access"))
...
def get_website_settings(context=None):
	...
	context = saashq._dict(context or {})
	settings: "WebsiteSettings" = saashq.get_cached_doc("Website Settings")
	...
	if context.disable_website_theme:
		context.theme = saashq._dict()

	else:
		from saashq.website.doctype.website_theme.website_theme import get_active_theme

		context.theme = get_active_theme() or saashq._dict()

	if not context.get("favicon"):
		context["favicon"] = "/assets/saashq/images/saashq-favicon.svg"
	...
	context.read_only_mode = saashq.flags.read_only
...
def get_auto_account_deletion():
	return saashq.db.get_single_value("Website Settings", "auto_account_deletion")
