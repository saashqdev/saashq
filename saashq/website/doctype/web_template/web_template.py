...
from saashq.modules.export_file import get_module_path, scrub_dt_dn, write_document_file
...
class WebTemplate(Document):
	...
	if TYPE_CHECKING:
		...
		from saashq.website.doctype.web_template_field.web_template_field import WebTemplateField
	...
	def validate(self):
		if self.standard and not saashq.conf.developer_mode and not saashq.flags.in_patch:
			saashq.throw(_("Enable developer mode to create a standard Web Template"))

		for field in self.fields:
			if not field.fieldname:
				field.fieldname = saashq.scrub(field.label)
	...
	def on_trash(self):
		if saashq.conf.developer_mode and self.standard:
	...
	def get_template_path(self):
		...
		file_name = saashq.scrub(self.name) + ".html"
	...
	def render(self, values=None):
		if not values:
			values = {}
		values = saashq.parse_json(values)
		values.update({"values": values})
		template = self.get_template(self.standard)

		return saashq.render_template(template, values)
