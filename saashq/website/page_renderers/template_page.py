...
import saashq
from saashq.website.page_renderers.base_template_page import BaseTemplatePage
from saashq.website.router import get_base_template, get_page_info
...
class TemplatePage(BaseTemplatePage):
	...
	def set_template_path(self):
		...
		for app in reversed(saashq.get_installed_apps()):
	...
	def get_index_path_options(search_path):
		return (
			saashq.as_unicode(f"{search_path}{d}") for d in ("", ".html", ".md", "/index.html", "/index.md")
		)
	...
	def update_context(self):
		...
		self.context.build_version = saashq.utils.get_build_version()

		if self.pymodule_name:
			self.pymodule = saashq.get_module(self.pymodule_name)
		...
		self.context.developer_mode = saashq.conf.developer_mode
	...
	def render_template(self):
		if self.template_path.endswith("min.js"):
			...
			html = saashq.render_template(self.source, self.context, safe_render=safe_render)
	...
	def get_raw_template(self):
		return saashq.get_jloader().get_source(saashq.get_jenv(), self.context.template)[0]
	...
	def convert_from_markdown(self):
		if self.template_path.endswith(".md"):
			self.source = saashq.utils.md_to_html(self.source)
	...
	def set_standard_path(self, path):
		self.app = "saashq"
		self.app_path = saashq.get_app_path("saashq")
		self.path = path
	...
	def set_user_info(self):
		from saashq.utils.user import get_fullname_and_avatar

		info = get_fullname_and_avatar(saashq.session.user)
