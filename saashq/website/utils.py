...
def delete_page_cache(path):
	...
	if path:
		saashq.cache.hdel_names(groups, path)
		saashq.cache.delete_value("full_index")
	else:
		groups.append("full_index")
		saashq.cache.delete_value(groups)
...
def can_cache(no_cache=False):
	if saashq.flags.force_website_cache:
		return True
	if saashq.conf.disable_website_cache or saashq.conf.developer_mode:
		return False
	if getattr(saashq.local, "no_cache", False):
		return False
	return not no_cache


def get_comment_list(doctype, name):
	comments = saashq.get_all(
		"Comment",
		...
		or_filters=[["owner", "=", saashq.session.user], ["published", "=", 1]],
	)

	communications = saashq.get_all(
		"Communication",
		fields=[
			"name",
			"creation",
		...
		or_filters=[
			["recipients", "like", f"%{saashq.session.user}%"],
			["cc", "like", f"%{saashq.session.user}%"],
			["bcc", "like", f"%{saashq.session.user}%"],
		],
	)
...
def get_home_page():
	if saashq.local.flags.home_page and not saashq.flags.in_test:
		return saashq.local.flags.home_page
	...
	def _get_home_page():
		...
		if saashq.session.user != "Guest":
			# by role
			for role in saashq.get_roles():
				home_page = saashq.db.get_value("Role", role, "home_page")
				if home_page:
					break

			# portal default
			if not home_page:
				home_page = saashq.db.get_single_value("Portal Settings", "default_portal_home")
		...
		# global
		if not home_page:
			home_page = saashq.db.get_single_value("Website Settings", "home_page")

		if not home_page:
			home_page = "login" if saashq.session.user == "Guest" else "me"
	...
	return saashq.cache.hget("home_page", saashq.session.user, _get_home_page)


def get_home_page_via_hooks():
	home_page = None

	home_page_method = saashq.get_hooks("get_website_user_home_page")
	if home_page_method:
		home_page = saashq.get_attr(home_page_method[-1])(saashq.session.user)
	elif saashq.get_hooks("website_user_home_page"):
		home_page = saashq.get_hooks("website_user_home_page")[-1]

	if not home_page:
		role_home_page = saashq.get_hooks("role_home_page")
		if role_home_page:
			for role in saashq.get_roles():
	...
	if not home_page:
		home_page = saashq.get_hooks("home_page")
		if home_page:
...
def get_boot_data():
	...
	return {
		...
		"sysdefaults": {
			"float_precision": cint(saashq.get_system_settings("float_precision")) or 3,
		...
		"time_zone": {
			...
			"user": saashq.db.get_value("User", saashq.session.user, "time_zone") or get_system_timezone(),
		},
		"assets_json": get_assets_json(),
		"sitename": saashq.local.site,
	}


def is_signup_disabled():
	return saashq.get_website_settings("disable_signup")
...
def get_toc(route, url_prefix=None, app=None):
	...
	return saashq.get_template("templates/includes/full_index.html").render(
...
def get_full_index(route=None, app=None):
	...
	from saashq.website.router import get_pages

	if not saashq.local.flags.children_map:
		...
		children_map = saashq.cache.get_value("website_full_index", _build)

		saashq.local.flags.children_map = children_map

	return saashq.local.flags.children_map
...
def clear_cache(path=None):
	...
	if path:
		saashq.cache.hdel("website_redirects", path)
		delete_page_cache(path)
	else:
		clear_sitemap()
		saashq.clear_cache("Guest")
		keys += [
			"portal_menu_items",
			"home_page",
			"website_route_rules",
	...
	saashq.cache.delete_value(keys)

	for method in saashq.get_hooks("website_clear_cache"):
		saashq.get_attr(method)(path)
...
def get_sidebar_items(parent_sidebar, basepath=None):
	...
	hooks = saashq.get_hooks("look_for_sidebar_json")
	look_for_sidebar_json = hooks[0] if hooks else saashq.flags.look_for_sidebar
...
def get_portal_sidebar_items():
	sidebar_items = saashq.cache.hget("portal_menu_items", saashq.session.user)
	if sidebar_items is None:
		sidebar_items = []
		roles = saashq.get_roles()
		portal_settings = saashq.get_doc("Portal Settings", "Portal Settings")
		...
		items_via_hooks = saashq.get_hooks("portal_menu_items")
		if items_via_hooks:
			for i in items_via_hooks:
				i["enabled"] = 1
			add_items(sidebar_items, items_via_hooks)

		saashq.cache.hset("portal_menu_items", saashq.session.user, sidebar_items)
...
def get_sidebar_items_from_sidebar_file(basepath, look_for_sidebar_json):
	...
	with open(sidebar_json_path) as sidebarfile:
		try:
			...
			saashq.throw("Invalid Sidebar JSON at " + sidebar_json_path)
...
def cache_html(func):
	...
	def cache_html_decorator(*args, **kwargs):
		if can_cache():
			html = None
			page_cache = saashq.cache.hget("website_page", args[0].path)
			if page_cache and saashq.local.lang in page_cache:
				html = page_cache[saashq.local.lang]
			if html:
				saashq.local.response.from_cache = True
				saashq.local.response.can_cache = True
				return html
		html = func(*args, **kwargs)
		context = args[0].context
		if can_cache(context.no_cache):
			page_cache = saashq.cache.hget("website_page", args[0].path) or {}
			page_cache[saashq.local.lang] = html
			saashq.cache.hset("website_page", args[0].path, page_cache)
			saashq.local.response.can_cache = True
...
def build_response(path, data, http_status_code, headers: dict | None = None):
	...
	response.headers["X-From-Cache"] = saashq.local.response.from_cache or False
...
def add_preload_for_bundled_assets(response):
	links = [f"<{css}>; rel=preload; as=style" for css in saashq.local.preload_assets["style"]]
	links.extend(f"<{js}>; rel=preload; as=script" for js in saashq.local.preload_assets["script"])

	version = get_build_version()
	links.extend(
		f"<{svg}?v={version}>; rel=preload; as=fetch; crossorigin"
		for svg in saashq.local.preload_assets["icons"]
	)
