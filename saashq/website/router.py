...
import saashq
from saashq.website.utils import extract_title, get_frontmatter
...
def get_page_info_from_web_page_with_dynamic_routes(path):
	...
	"""
	from saashq.website.doctype.web_page.web_page import get_dynamic_web_pages
...
def get_page_info_from_web_form(path):
	...
	from saashq.website.doctype.web_form.web_form import get_published_web_forms
	...
	for d in get_published_web_forms():
		...
		if end_point:
			if path.endswith("/list"):
				saashq.form_dict.is_list = True
			elif path.endswith("/new"):
				saashq.form_dict.is_new = True
			elif path.endswith("/edit"):
				saashq.form_dict.is_edit = True
			else:
				saashq.form_dict.is_read = True
			return d
...
def evaluate_dynamic_routes(rules, path):
	...
	if hasattr(saashq.local, "request") and saashq.local.request.environ:
		urls = route_map.bind_to_environ(saashq.local.request.environ)
		try:
			endpoint, args = urls.match("/" + path)
			if args:
				# don't cache when there's a query string!
				saashq.local.no_cache = 1
				saashq.local.form_dict.update(args)
		except NotFound:
			pass
...
def get_pages(app=None):
	...
	def _build(app):
		...
		if app:
			...
			apps = saashq.local.flags.web_pages_apps or saashq.get_installed_apps()

		for app in apps:
			app_path = saashq.get_app_path(app)
	...
	return saashq.cache.get_value("website_pages", lambda: _build(app))
...
def setup_source(page_info):
	...
	if page_info.template.endswith((".md", ".html")):
		...
		if page_info.template.endswith(".md"):
			source = saashq.utils.md_to_html(source)
...
def get_base_template(path=None):
	...
	"""
	if not path:
		path = saashq.local.request.path

	base_template_map = saashq.get_hooks("base_template_map") or {}
...
def load_properties_from_controller(page_info):
	...
	module = saashq.get_module(page_info.controller)
	if not module:
		return
...
def get_doctypes_with_web_view():
	...
	def _get():
		...
		doctypes = saashq.get_hooks("website_generators")
		doctypes_with_web_view = saashq.get_all(
			"DocType", fields=["name", "module"], filters=dict(has_web_view=1)
		)
		module_app_map = saashq.local.module_app
		doctypes += [
			d.name
			for d in doctypes_with_web_view
			if module_app_map.get(saashq.scrub(d.module)) in installed_apps
		]
		return doctypes

	return saashq.cache.get_value("doctypes_with_web_view", _get)


def get_start_folders():
	return saashq.local.flags.web_pages_folders or ("www", "templates/pages")


def clear_routing_cache():
	from saashq.website.doctype.web_form.web_form import get_published_web_forms
	from saashq.website.doctype.web_page.web_page import get_dynamic_web_pages
	from saashq.website.page_renderers.document_page import _find_matching_document_webview
	from saashq.www.sitemap import get_public_pages_from_doctypes
	...
	saashq.cache.delete_value("home_page")
