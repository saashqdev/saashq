...
def print_has_permission_check_logs(func):
	...
	def inner(*args, **kwargs):
		...
		self_perm_check = True if not kwargs.get("user") else kwargs.get("user") == saashq.session.user

		if print_logs:
			saashq.flags["has_permission_check_logs"] = []
		...
		if not result and self_perm_check and print_logs:
			msgprint(("<br>").join(saashq.flags.get("has_permission_check_logs", [])))

		if print_logs:
			saashq.flags.pop("has_permission_check_logs", None)
		return result
...
def _debug_log(log: str):
	if not hasattr(saashq.local, "permission_debug_log"):
		saashq.local.permission_debug_log = []
	saashq.local.permission_debug_log.append(log)


def _pop_debug_log() -> list[str]:
	if log := getattr(saashq.local, "permission_debug_log", None):
		del saashq.local.permission_debug_log
		return log
	return []
...
def has_permission(
	...
	*,
	...
	:param print_logs: If True, will display a message using saashq.msgprint
	...
	if ptype == "share" and saashq.get_system_settings("disable_document_sharing"):
	...
	if doc:
		if isinstance(doc, str | int):
			doc = saashq.get_doc(meta.name, doc)
		...
		if not perm:
			...
			msg = _("User {0} does not have access to this document").format(saashq.bold(user))
			if saashq.has_permission(doc.doctype):
		...
		debug and _debug_log(
			...
			+ saashq.as_json(role_permissions, indent=8)
		)
		...
		if not perm:
			push_perm_check_log(
				_("User {0} does not have doctype access via role permission for document {1}").format(
					saashq.bold(user), saashq.bold(_(doctype))
				),
				debug=debug,
			)
	...
	def false_if_not_shared():
		...
		if doc:
			...
			shared = saashq.share.get_shared(
				doctype,
				user,
				rights=rights,
		...
		elif saashq.share.get_shared(doctype, user, rights=rights, limit=1):
...
def get_doc_permissions(doc, user=None, ptype=None, debug=False):
	...
	meta = saashq.get_meta(doc.doctype)
	...
	debug and _debug_log(
		"User has following permissions using role permission system: "
		+ saashq.as_json(permissions, indent=8)
	)
	...
	if permissions.get("has_if_owner_enabled"):
		...
		debug and _debug_log(
			"User is owner of document, so permissions are updated to: " + saashq.as_json(permissions)
		)
...
def get_role_permissions(doctype_meta, user=None, is_owner=None, debug=False):
	...
	if isinstance(doctype_meta, str):
		doctype_meta = saashq.get_meta(doctype_meta)  # assuming doctype name was passed
	...
	if not saashq.local.role_permissions.get(cache_key) or debug:
		perms = saashq._dict(if_owner={})
		...
		saashq.local.role_permissions[cache_key] = perms

	return saashq.local.role_permissions[cache_key]


def get_user_permissions(user):
	from saashq.core.doctype.user_permission.user_permission import get_user_permissions
...
def has_user_permission(doc, user=None, debug=False):
	...
	apply_strict_user_permissions = (
		False if doc.meta.issingle else saashq.get_system_settings("apply_strict_user_permissions")
	)
	...
	def check_user_permission_on_link_fields(d):
		...
		meta = saashq.get_meta(d.get("doctype"))
...
def has_controller_permissions(doc, ptype, user=None, debug=False) -> bool:
	...
	hooks = saashq.get_hooks("has_permission")
	methods = hooks.get(doc.doctype, []) + hooks.get("*", [])

	for method in reversed(methods):
		controller_permission = saashq.call(method, doc=doc, ptype=ptype, user=user, debug=debug)
...
def get_all_perms(role):
	...
	perms = saashq.get_all("DocPerm", fields="*", filters=dict(role=role))
	custom_perms = saashq.get_all("Custom DocPerm", fields="*", filters=dict(role=role))
	doctypes_with_custom_perms = saashq.get_all("Custom DocPerm", pluck="parent", distinct=True)
...
def get_roles(user=None, with_standard=True):
	...
	def get():
		if user == "Administrator":
			return saashq.get_all("Role", pluck="name")  # return all available roles
		else:
	...
	roles = saashq.cache.hget("roles", user, get)
...
def get_perms_for(roles, perm_doctype="DocPerm"):
	...
	return saashq.get_all(perm_doctype, fields=["*"], filters=filters)


def get_doctypes_with_custom_docperms():
	"""Return all the doctypes with Custom Docperms."""

	doctypes = saashq.get_all("Custom DocPerm", fields=["parent"], distinct=1)
...
def add_user_permission(
	...
	"""Add user permission"""
	from saashq.core.doctype.user_permission.user_permission import user_permission_exists

	if not user_permission_exists(user, doctype, name, applicable_for):
		if not saashq.db.exists(doctype, name):
			saashq.throw(_("{0} {1} not found").format(_(doctype), name), saashq.DoesNotExistError)
...
def remove_user_permission(doctype, name, user):
	user_permission_name = saashq.db.get_value(
		"User Permission", dict(user=user, allow=doctype, for_value=name)
	)
	saashq.delete_doc("User Permission", user_permission_name)
...
def clear_user_permissions_for_doctype(doctype, user=None):
	...
	if user:
		filters["user"] = user
	user_permissions_for_doctype = saashq.get_all("User Permission", filters=filters)
	for d in user_permissions_for_doctype:
		saashq.delete_doc("User Permission", d.name)


def can_import(doctype, raise_exception=False):
	if not ("System Manager" in saashq.get_roles() or has_permission(doctype, "import")):
		if raise_exception:
			raise saashq.PermissionError(f"You are not allowed to import: {doctype}")
		else:
			return False
	return True
...
def can_export(doctype, raise_exception=False):
	if "System Manager" in saashq.get_roles():
		...
		role_permissions = saashq.permissions.get_role_permissions(doctype)
		has_access = role_permissions.get("export") or role_permissions.get("if_owner").get("export")
		if not has_access and raise_exception:
			raise saashq.PermissionError(_("You are not allowed to export {} doctype").format(doctype))
		return has_access
...
def update_permission_property(
	...
	from saashq.core.doctype.doctype.doctype import validate_permissions_for_doctype

	out = setup_custom_perms(doctype)

	custom_docperm = saashq.db.get_value(
...
def setup_custom_perms(parent):
	...
	if not saashq.db.exists("Custom DocPerm", dict(parent=parent)):
		copy_perms(parent)
		return True
...
def copy_perms(parent):
	...
	for d in saashq.get_all("DocPerm", fields="*", filters=dict(parent=parent)):
		custom_perm = saashq.new_doc("Custom DocPerm")
...
def reset_perms(doctype):
	...
	from saashq.desk.notifications import delete_notification_count_for

	delete_notification_count_for(doctype)
	for custom_docperm in saashq.get_all("Custom DocPerm", filters={"parent": doctype}, pluck="name"):
		saashq.delete_doc("Custom DocPerm", custom_docperm, ignore_permissions=True)


def get_linked_doctypes(dt: str) -> list:
	meta = saashq.get_meta(dt)
	linked_doctypes = [dt] + [
		d.options
		for d in meta.get(
			"fields",
...
def push_perm_check_log(log, debug=False):
	...
	if saashq.flags.get("has_permission_check_logs") is None:
		return

	saashq.flags.get("has_permission_check_logs").append(log)
...
def has_child_permission(
	...
	*,
	...
	if not parent_doctype:
		push_perm_check_log(
			_("Please specify a valid parent DocType for {0}").format(saashq.bold(child_doctype)),
			debug=debug,
		)
		return False

	parent_meta = saashq.get_meta(parent_doctype)
	...
	if parent_meta.istable or not (
		...
		push_perm_check_log(
			_("{0} is not a valid parent DocType for {1}").format(
				saashq.bold(parent_doctype), saashq.bold(child_doctype)
			),
			debug=debug,
		)
		return False
	...
	if child_doc:
		...
		if not parentfield:
			push_perm_check_log(
				_("Parentfield not specified in {0}: {1}").format(
					saashq.bold(child_doctype), saashq.bold(child_doc.name)
				),
				debug=debug,
			)
			return False
		...
		if parentfield not in valid_parentfields:
			push_perm_check_log(
				_("{0} is not a valid parentfield for {1}").format(
					saashq.bold(parentfield), saashq.bold(child_doctype)
				),
				debug=debug,
			)
			return False
		...
		if permlevel > 0 and permlevel not in accessible_permlevels:
			push_perm_check_log(
				_("Insufficient Permission Level for {0}").format(saashq.bold(parent_doctype)), debug=debug
			)
			debug and _debug_log(
...
def is_system_user(user: str | None = None) -> bool:
	return saashq.get_cached_value("User", user or saashq.session.user, "user_type") == "System User"
