...
from saashq.utils import cint, encode, get_backups_path, get_files_path, get_request_site_address
...
class DropboxSettings(Document):
	...
	def onload(self):
		if not self.app_access_key and saashq.conf.dropbox_access_key:
			self.set_onload("dropbox_setup_via_site_config", 1)

	def validate(self):
		if self.enabled and self.limit_no_of_backups and self.no_of_backups < 1:
			saashq.throw(_("Number of DB backups cannot be less than 1"))
...
def take_backup():
	...
	enqueue(
		"saashq.integrations.doctype.dropbox_settings.dropbox_settings.take_backup_to_dropbox",
		queue="long",
		timeout=1500,
	)
...
def take_backups_if(freq):
	if saashq.db.get_single_value("Dropbox Settings", "backup_frequency") == freq:
...
def take_backup_to_dropbox(retry_count=0, upload_db_backup=True):
	...
	try:
		if cint(saashq.db.get_single_value("Dropbox Settings", "enabled")):
			validate_file_size()
			...
			if cint(saashq.db.get_single_value("Dropbox Settings", "send_email_for_successful_backup")):
...
def upload_from_folder(path, is_private, dropbox_folder, dropbox_client, did_not_upload, error_log):
	...
	if is_fresh_upload():
		...
		response = saashq._dict({"entries": []})

	path = str(path)

	for f in saashq.get_all(
		"File",
		...
		for file_metadata in response.entries:
			try:
				...
				error_log.append(saashq.get_traceback())
...
def update_file_dropbox_status(file_name):
	saashq.db.set_value("File", file_name, "uploaded_to_dropbox", 1, update_modified=False)


def is_fresh_upload():
	file_name = saashq.db.get_value("File", {"uploaded_to_dropbox": 1}, "name")
...
def get_dropbox_settings(redirect_uri=False):
	...
	settings = saashq.get_doc("Dropbox Settings")
	app_details = {
		"app_key": settings.app_access_key or saashq.conf.dropbox_access_key,
		"app_secret": settings.get_password(fieldname="app_secret_key", raise_exception=False)
		if settings.app_secret_key
		else saashq.conf.dropbox_secret_key,
		...
		app_details.update(
			{
				...
				+ "/api/method/saashq.integrations.doctype.dropbox_settings.dropbox_settings.dropbox_auth_finish"
			}
		)
...
def dropbox_auth_finish():
	...
	if not callback.state or not callback.code:
		saashq.respond_as_web_page(
			...
			indicator_color="red",
			http_status_code=saashq.AuthenticationError.http_status_code,
		)
		return
	...
	saashq.local.response["location"] = "/app/dropbox-settings"
...
def set_dropbox_token(access_token, refresh_token=None):
	...
	dropbox_settings = saashq.get_single("Dropbox Settings")
