...
class GoogleContacts(Document):
	...
	def validate(self):
		if not saashq.db.get_single_value("Google Settings", "enable"):
			saashq.throw(_("Enable Google API in Google Settings."))

	def get_access_token(self):
		if not self.refresh_token:
			button_label = saashq.bold(_("Allow Google Contacts Access"))
...
def authorize_access(g_contact, reauthorize=False, code=None):
	...
	"""
	contact = saashq.get_doc("Google Contacts", g_contact)
...
def get_google_contacts_object(g_contact):
	...
	account = saashq.get_doc("Google Contacts", g_contact)
	oauth_obj = GoogleOAuth("contacts")
...
def sync(g_contact=None):
	...
	google_contacts = saashq.get_list("Google Contacts", filters=filters)
...
def sync_contacts_from_google_contacts(g_contact):
	...
	saashq.db.set_value("Google Contacts", account.name, "last_sync_on", saashq.utils.now_datetime())
...
def insert_contacts_to_google_contacts(doc, method=None):
	...
	"""
	if (
		not saashq.db.exists("Google Contacts", {"name": doc.google_contacts})
	...
	try:
		...
		saashq.db.set_value("Contact", doc.name, "google_contacts_id", contact.get("resourceName"))
	except HttpError as err:
		saashq.msgprint(
...
def update_contacts_to_google_contacts(doc, method=None):
	...
	try:
		...
		saashq.msgprint(_("Contact Synced with Google Contacts."))
	except HttpError as err:
		saashq.msgprint(
