...
class S3BackupSettings(Document):
	...
	def validate(self):
		...
		try:
			...
			bucket_name = saashq.bold(self.bucket)
			if error_code == "403":
...
def take_backup():
	...
	enqueue(
		"saashq.integrations.doctype.s3_backup_settings.s3_backup_settings.take_backups_s3",
		queue="long",
		timeout=1500,
	)
...
def take_backups_if(freq):
	if cint(saashq.db.get_single_value("S3 Backup Settings", "enabled")):
		if saashq.db.get_single_value("S3 Backup Settings", "frequency") == freq:
			take_backups_s3()
...
def notify():
	error_message = saashq.get_traceback()
	send_email(False, "Amazon S3", "S3 Backup Settings", "notify_email", error_message)


def backup_to_s3():
	from saashq.utils import get_backups_path
	from saashq.utils.backups import new_backup

	doc = saashq.get_single("S3 Backup Settings")
	bucket = doc.bucket
