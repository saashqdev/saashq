...
from saashq.permissions import SYSTEM_USER_ROLE
...
class OAuthClient(Document):
	...
	if TYPE_CHECKING:
		from saashq.integrations.doctype.oauth_client_role.oauth_client_role import OAuthClientRole
	...
	def validate(self):
		...
		if not self.client_secret:
			self.client_secret = saashq.generate_hash(length=10)
	...
	def user_has_allowed_role(self) -> bool:
		...
		return bool(allowed_roles & set(saashq.get_roles()))
