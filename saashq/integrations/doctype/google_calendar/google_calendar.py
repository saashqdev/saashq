...
from saashq.utils.password import set_encrypted_password
...
class GoogleCalendar(Document):
	...
	def validate(self):
		...
		if not google_settings.enable:
			saashq.throw(_("Enable Google API in Google Settings."))

		if not google_settings.client_id or not google_settings.client_secret:
			saashq.throw(_("Enter Client Id and Client Secret in Google Settings."))
	...
	def get_access_token(self):
		...
		if not self.refresh_token:
			button_label = saashq.bold(_("Allow Google Calendar Access"))
...
def authorize_access(g_calendar, reauthorize=None):
	...
	"""
	google_settings = saashq.get_doc("Google Settings")
	google_calendar = saashq.get_doc("Google Calendar", g_calendar)
	google_calendar.check_permission("write")

	redirect_uri = (
		get_request_site_address(True)
		+ "?cmd=saashq.integrations.doctype.google_calendar.google_calendar.google_callback"
	)

	if not google_calendar.authorization_code or reauthorize:
		saashq.cache.hset("google_calendar", "google_calendar", google_calendar.name)
		...
		try:
			...
			saashq.local.response["location"] = "/app/Form/{}/{}".format(
				quote("Google Calendar"), quote(google_calendar.name)
			)

			saashq.msgprint(_("Google Calendar has been configured."))
...
def google_callback(code=None):
	...
	"""
	google_calendar = saashq.cache.hget("google_calendar", "google_calendar")
	saashq.db.set_value("Google Calendar", google_calendar, "authorization_code", code)
	saashq.db.commit()
...
def sync(g_calendar=None):
	...
	google_calendars = saashq.get_list("Google Calendar", filters=filters)
...
def get_google_calendar_object(g_calendar):
	...
	google_settings = saashq.get_doc("Google Settings")
	account = saashq.get_doc("Google Calendar", g_calendar)
	...
	try:
		if account.google_calendar_id:
			...
			calendar = {
				...
				"timeZone": saashq.get_system_settings("time_zone"),
			}
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	while True:
		try:
			...
			if err.resp.status == 410:
				...
				saashq.msgprint(msg, title="Invalid Sync Token", indicator="blue")
			else:
				saashq.throw(msg)
	...
	for idx, event in enumerate(results):
		saashq.publish_realtime(
			"import_google_calendar", dict(progress=idx + 1, total=len(results)), user=saashq.session.user
		)
		...
		if event.get("status") == "confirmed":
			...
			if not saashq.db.exists("Event", {"google_calendar_event_id": event.get("id")}):
			...
			saashq.get_doc(
				{
					...
					"reference_name": saashq.db.get_value(
						"Event",
						{
							"google_calendar_id": account.google_calendar_id,
							"google_calendar_event_id": event.get("id"),
...
def insert_event_to_calendar(account, event, recurrence=None):
	"""
	Inserts event in Saashq Calendar during Sync
	"""
	calendar_event = {
		"doctype": "Event",
	...
	saashq.get_doc(calendar_event).insert(ignore_permissions=True)


def update_event_in_calendar(account, event, recurrence=None):
	"""
	Updates Event in Saashq Calendar if any existing Google Calendar Event is updated
	"""
	calendar_event = saashq.get_doc("Event", {"google_calendar_event_id": event.get("id")})
...
def insert_event_in_google_calendar(doc, method=None):
	...
	if (
		...
		or not saashq.db.exists("Google Calendar", {"name": doc.google_calendar})
	):
		return
	...
	try:
		...
		saashq.msgprint(_("Event Synced with Google Calendar."))
	except HttpError as err:
		saashq.throw(
...
def update_event_in_google_calendar(doc, method=None):
	"""
	Updates Events in Google Calendar if any existing event is modified in Saashq Calendar
	"""
...
def delete_event_from_google_calendar(doc, method=None):
	"""
	Delete Events from Google Calendar if Saashq Event is deleted.
	"""

	if not saashq.db.exists("Google Calendar", {"name": doc.google_calendar}):
		return
