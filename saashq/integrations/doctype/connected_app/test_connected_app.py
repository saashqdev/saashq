...
import saashq
from saashq.integrations.doctype.social_login_key.test_social_login_key import (
...
def get_connected_app():
	...
	connected_app = saashq.new_doc(doctype)
	connected_app.provider_name = "saashq"
...
def get_oauth_client():
	oauth_client = saashq.new_doc("OAuth Client")
...
class TestConnectedApp(IntegrationTestCase):
	def setUp(self):
		...
		Saashq comes with it's own oAuth2 provider that we can test against. The
		...
		Saashq's "Authorization URL" and "Access Token URL" (actually they're
	...
	def test_web_application_flow(self):
		...
		resp = oauth2_session.get(urljoin(self.base_url, "/api/method/saashq.auth.get_logged_user"))
	...
	def tearDown(self):
		...
		if getattr(self, "oauth_client", None):
			tokens = saashq.get_all("OAuth Bearer Token", filters={"client": self.oauth_client.name})
			for token in tokens:
				doc = saashq.get_doc("OAuth Bearer Token", token.name)
				doc.delete()

			codes = saashq.get_all("OAuth Authorization Code", filters={"client": self.oauth_client.name})
			for code in codes:
				doc = saashq.get_doc("OAuth Authorization Code", code.name)
				doc.delete()
