...
import saashq
from saashq import _, safe_encode
from saashq.model.document import Document
from saashq.twofactor import authenticate_for_2factor, confirm_otp_token, should_run_2fa
...
class LDAPSettings(Document):
	...
	if TYPE_CHECKING:
		from saashq.integrations.doctype.ldap_group_mapping.ldap_group_mapping import LDAPGroupMapping
	...
	def connect_to_ldap(self, base_dn, password, read_only=True) -> ldap3.Connection:
		try:
			...
			saashq.throw(msg, title=_("LDAP Not Installed"))
		except LDAPInvalidCredentialsResult:
			saashq.throw(_("Invalid username or password"))
		except Exception as ex:
			saashq.throw(_(str(ex)))

	@staticmethod
	def get_ldap_client_settings() -> dict:
		# return the settings to be used on the client side.
		result = {"enabled": False}
		ldap = saashq.get_cached_doc("LDAP Settings")
		if ldap.enabled:
			result["enabled"] = True
			result["method"] = "saashq.integrations.doctype.ldap_settings.ldap_settings.login"
		return result
	...
	def sync_roles(self, user: "User", additional_groups: list | None = None):
		...
		all_mapped_roles = {r.erpnexus_role for r in self.ldap_groups}
		matched_roles = {r.erpnexus_role for r in self.ldap_groups if r.ldap_group.lower() in lower_groups}
	...
	def create_or_update_user(self, user_data: dict, groups: list | None = None):
		...
		if saashq.db.exists("User", user_data["email"]):
			user = saashq.get_doc("User", user_data["email"])
		...
		if self.default_user_type == "System User":
			...
			role = saashq.db.get_value("User Type", user.user_type, "role")
	...
	def authenticate(self, username: str, password: str):
		if not self.enabled:
			saashq.throw(_("LDAP is not enabled."))
		...
		try:
			...
			saashq.throw(_("Please use a valid LDAP search filter"), title=_("Misconfigured"))
		...
		if conn.search(
			...
			if conn.entries and conn.entries[0]:
				...
				if conn.modify(entry_dn, changes=changes):
					...
					saashq.msgprint(_("Password changed successfully."))
				else:
					saashq.throw(_("Failed to change password."))
			else:
				saashq.throw(_("No Entry for the User {0} found within LDAP!").format(user))
		else:
			saashq.throw(_("No LDAP User found for email: {0}").format(user))
...
def login():
	...
	ldap: LDAPSettings = saashq.get_doc("LDAP Settings")

	user = ldap.authenticate(saashq.as_unicode(args.usr), saashq.as_unicode(args.pwd))

	saashq.local.login_manager.user = user.name
	if should_run_2fa(user.name):
		authenticate_for_2factor(user.name)
		if not confirm_otp_token(saashq.local.login_manager):
			return False
