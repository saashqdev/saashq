...
import saashq
from saashq.exceptions import MandatoryError, ValidationError
...
class LDAP_TestCase:
	...
	def mock_ldap_connection(f):
		...
		def wrapped(self, *args, **kwargs):
			with mock.patch(
				"saashq.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.connect_to_ldap",
	...
	def clean_test_users():
		with contextlib.suppress(Exception):
			saashq.get_doc("User", "posix.user1@unit.testing").delete()
		with contextlib.suppress(Exception):
			saashq.get_doc("User", "posix.user2@unit.testing").delete()
		with contextlib.suppress(Exception):
			saashq.get_doc("User", "website_ldap_user@test.com").delete()

	@classmethod
	def setUpClass(cls):
		cls.clean_test_users()
		# Save user data for restoration in tearDownClass()
		cls.user_ldap_settings = saashq.get_doc("LDAP Settings")
		...
		user = saashq.get_doc(cls.user1doc)
		...
		user = saashq.get_doc(cls.user2doc)
		user.insert(ignore_permissions=True)
	...
	def tearDownClass(cls):
		with contextlib.suppress(Exception):
			saashq.get_doc("LDAP Settings").delete()
	...
	def test_connect_to_ldap(self):
		...
		prevent_connection_parameters = {
			"mode": {
				"IP_V4_ONLY": "Locks the user to IPv4 without saashq providing a way to configure",
				"IP_V6_ONLY": "Locks the user to IPv6 without saashq providing a way to configure",
			},
			"auto_bind": {
	...
	def test_update_user_fields(self):
		...
		test_user = saashq.get_doc("User", test_user_data["email"])
		self.test_class.update_user_fields(test_user, test_user_data)
		updated_user = saashq.get_doc("User", test_user_data["email"])
		...
		self.assertIn(self.test_class.default_role, saashq.get_roles(updated_user.name))
	...
	def test_create_website_user(self):
		...
		new_user = saashq.get_doc("User", new_test_user_data["email"])
	...
	def test_sync_roles(self):
		if self.TEST_LDAP_SERVER.lower() == "openldap":
			test_user_data = {
				"posix.user1": [
					...
					"default_role",
					"saashq_default_all",
					"saashq_default_guest",
					"saashq_default_desk_user",
				],
				"posix.user2": [
					"Users",
					"Group3",
		...
		role_to_group_map = {
			self.doc["ldap_groups"][0]["erpnexus_role"]: self.doc["ldap_groups"][0]["ldap_group"],
			self.doc["ldap_groups"][1]["erpnexus_role"]: self.doc["ldap_groups"][1]["ldap_group"],
			self.doc["ldap_groups"][2]["erpnexus_role"]: self.doc["ldap_groups"][2]["ldap_group"],
			"Newsletter Manager": "default_role",
			"All": "saashq_default_all",
			"Guest": "saashq_default_guest",
			"Desk User": "saashq_default_desk_user",
		}

		# re-create user1 to ensure clean
		saashq.get_doc("User", "posix.user1@unit.testing").delete()
		user = saashq.get_doc(self.user1doc)
		user.insert(ignore_permissions=True)

		for test_user in test_user_data:
			test_user_doc = saashq.get_doc("User", f"{test_user}@unit.testing")
			test_user_roles = saashq.get_roles(f"{test_user}@unit.testing")

			self.assertTrue(
				len(test_user_roles) == 2, "User should only be a part of the All and Guest roles"
			)  # check default saashq roles

			self.test_class.sync_roles(test_user_doc, test_user_data[test_user])  # update user roles

			saashq.get_doc("User", f"{test_user}@unit.testing")
			updated_user_roles = saashq.get_roles(f"{test_user}@unit.testing")
	...
	def test_create_or_update_user(self):
		...
		saashq.get_doc("User", f"{test_user}@unit.testing").delete()

		with self.assertRaises(
			saashq.exceptions.DoesNotExistError
		):  # ensure user deleted so function can be tested
			saashq.get_doc("User", f"{test_user}@unit.testing")

		with mock.patch(
			"saashq.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.update_user_fields"
		) as update_user_fields_method:
			with mock.patch(
				"saashq.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.sync_roles"
	...
	def test_authenticate(self):
		with mock.patch(
			"saashq.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.fetch_ldap_groups"
		...
		for username, password in enumerate(invalid_users):
			with self.assertRaises(saashq.exceptions.ValidationError) as display_massage:
	...
	DOCUMENT_GROUP_MAPPINGS: typing.ClassVar[list] = [
		{
			...
			"erpnexus_role": "System Manager",
		},
		{"doctype": "LDAP Group Mapping", "ldap_group": "Users", "erpnexus_role": "Blogger"},
		{"doctype": "LDAP Group Mapping", "ldap_group": "Group3", "erpnexus_role": "Accounts User"},
	]
...
class Test_ActiveDirectory(LDAP_TestCase, TestCase):
	...
	DOCUMENT_GROUP_MAPPINGS: typing.ClassVar[list] = [
		...
		{"doctype": "LDAP Group Mapping", "ldap_group": "Domain Users", "erpnexus_role": "Blogger"},
		{
			"doctype": "LDAP Group Mapping",
			"ldap_group": "Enterprise Administrators",
			"erpnexus_role": "Accounts User",
		},
	]
