...
from saashq.automation.doctype.assignment_rule.assignment_rule import get_repeated
from saashq.contacts.doctype.contact.contact import (
...
from saashq.utils.background_jobs import get_jobs
...
class AutoRepeat(Document):
	...
	if TYPE_CHECKING:
		from saashq.automation.doctype.auto_repeat_day.auto_repeat_day import AutoRepeatDay
	...
	def on_update(self):
		saashq.get_doc(self.reference_doctype, self.reference_document).notify_update()

	def on_trash(self):
		saashq.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", "")
		saashq.get_doc(self.reference_doctype, self.reference_document).notify_update()
	...
	def validate_reference_doctype(self):
		if saashq.flags.in_test or saashq.flags.in_patch:
			return
		if not saashq.get_meta(self.reference_doctype).allow_auto_repeat:
			saashq.throw(
	...
	def validate_submit_on_creation(self):
		if self.submit_on_creation and not saashq.get_meta(self.reference_doctype).is_submittable:
			saashq.throw(
				_("Cannot enable {0} for a non-submittable doctype").format(saashq.bold("Submit on Creation"))
			)
	...
	def validate_dates(self):
		...
		if self.end_date == self.start_date:
			saashq.throw(
				_("{0} should not be same as {1}").format(saashq.bold("End Date"), saashq.bold("Start Date"))
			)
			...
			if self.recipients:
				...
				for email in email_list:
					if not validate_email_address(email):
						saashq.throw(_("{0} is an invalid email address in 'Recipients'").format(email))
			else:
				saashq.throw(_("'Recipients' not specified"))
		...
		if len(set(auto_repeat_days)) != len(auto_repeat_days):
			...
			saashq.throw(
				_("Auto Repeat Day{0} {1} has been repeated.").format(
					plural, saashq.bold(", ".join(repeated_days))
				)
			)

	def update_auto_repeat_id(self):
		# check if document is already on auto repeat
		auto_repeat = saashq.db.get_value(self.reference_doctype, self.reference_document, "auto_repeat")
		if auto_repeat and auto_repeat != self.name and not saashq.flags.in_patch:
			saashq.throw(
				_("The {0} is already on auto repeat {1}").format(self.reference_document, auto_repeat)
			)
		else:
			saashq.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", self.name)
	...
	def create_documents(self):
		try:
			...
			if self.reference_document and not saashq.flags.in_test:
				self.notify_error_to_user(error_log)

	def make_new_document(self):
		reference_doc = saashq.get_doc(self.reference_doctype, self.reference_document)
		new_doc = saashq.copy_doc(reference_doc, ignore_no_copy=False)
	...
	def update_doc(self, new_doc, reference_doc):
		...
		auto_repeat_doc = saashq.get_doc("Auto Repeat", self.name)
	...
	def send_notification(self, new_doc):
		...
		if not self.subject:
			...
			subject = saashq.render_template(self.subject, {"doc": new_doc})
		...
		try:
			...
			).format(saashq.bold(_("Note")), saashq.bold(_("Allow Print for Draft")))
			attachments = None
		...
		if error_string:
			...
			message = saashq.render_template(self.message, {"doc": new_doc})
	...
	def fetch_linked_contacts(self):
		if self.reference_doctype and self.reference_document:
			...
			if not email_ids:
				saashq.msgprint(_("No contacts linked to document"), alert=True)
			else:
				self.recipients = ", ".join(email_ids)

	def disable_auto_repeat(self):
		saashq.db.set_value("Auto Repeat", self.name, "disabled", 1)
	...
	def notify_error_to_user(self, error_log):
		...
		form_link = saashq.utils.get_link_to_form(self.reference_doctype, self.reference_document)
		auto_repeat_failed_for = _("Auto Repeat failed for {0}").format(form_link)

		error_log_link = saashq.utils.get_link_to_form("Error Log", error_log.name)
...
def make_auto_repeat_entry():
	enqueued_method = "saashq.automation.doctype.auto_repeat.auto_repeat.create_repeated_entries"
	jobs = get_jobs()

	if not jobs or enqueued_method not in jobs[saashq.local.site]:
		date = getdate(today())
		data = get_auto_repeat_entries(date)
		saashq.enqueue(enqueued_method, data=data)


def create_repeated_entries(data):
	for d in data:
		doc = saashq.get_doc("Auto Repeat", d.name)
		...
		if schedule_date == current_date and not doc.disabled:
			...
			if schedule_date and not doc.disabled:
				saashq.db.set_value("Auto Repeat", doc.name, "next_schedule_date", schedule_date)
...
def set_auto_repeat_as_completed():
	auto_repeat = saashq.get_all("Auto Repeat", filters={"status": ["!=", "Disabled"]})
...
def make_auto_repeat(doctype, docname, frequency="Daily", start_date=None, end_date=None):
	...
	doc = saashq.new_doc("Auto Repeat")
...
def update_reference(docname: str, reference: str):
	doc = saashq.get_doc("Auto Repeat", str(docname))
...
def generate_message_preview(reference_dt, reference_doc, message=None, subject=None):
	saashq.has_permission("Auto Repeat", "write", throw=True)
	doc = saashq.get_doc(reference_dt, reference_doc)
	doc.check_permission()
	subject_preview = _("Please add a subject to your email")
	msg_preview = saashq.render_template(message, {"doc": doc})
	if subject:
		subject_preview = saashq.render_template(subject, {"doc": doc})
