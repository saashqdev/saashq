...
import saashq
from saashq.automation.doctype.auto_repeat.auto_repeat import (
...
from saashq.utils import add_days, add_months, getdate, today

if TYPE_CHECKING:
	from saashq.custom.doctype.custom_field.custom_field import CustomField
...
def add_custom_fields() -> "CustomField":
	...
	return create_custom_field("ToDo", df) or saashq.get_doc(
...
class TestAutoRepeat(IntegrationTestCase):
	...
	def test_daily_auto_repeat(self):
		...
		todo = saashq.get_doc(doc.reference_doctype, doc.reference_document)
		self.assertEqual(todo.auto_repeat, doc.name)

		new_todo = saashq.db.get_value("ToDo", {"auto_repeat": doc.name, "name": ("!=", todo.name)}, "name")

		new_todo = saashq.get_doc("ToDo", new_todo)
	...
	def monthly_auto_repeat(self, doctype, docname, start_date, end_date=None):
		...
		data = get_auto_repeat_entries(getdate(today()))
		create_repeated_entries(data)
		docnames = saashq.get_all(doc.reference_doctype, {"auto_repeat": doc.name})
		self.assertEqual(len(docnames), 1)

		doc = saashq.get_doc("Auto Repeat", doc.name)
	...
	def test_email_notification(self):
		...
		email_queue = saashq.db.exists("Email Queue", dict(reference_doctype="ToDo", reference_name=new_todo))
	...
	def test_submit_on_creation(self):
		...
		submittable_doc = saashq.get_doc(doctype=doctype, test="test submit on creation").insert()
	...
	return saashq.get_doc(
		{
			...
			"reference_document": args.reference_document or saashq.db.get_value("ToDo", "name"),
...
def create_submittable_doctype(doctype, submit_perms=1):
	if saashq.db.exists("DocType", doctype):
		return
	else:
		doc = saashq.get_doc(
			{
