...
from saashq.cache_manager import clear_doctype_map, get_doctype_map
from saashq.desk.form import assign_to
from saashq.model import log_types
from saashq.model.document import Document
from saashq.utils.data import comma_and
...
class AssignmentRule(Document):
	...
	if TYPE_CHECKING:
		from saashq.automation.doctype.assignment_rule_day.assignment_rule_day import AssignmentRuleDay
		from saashq.automation.doctype.assignment_rule_user.assignment_rule_user import (
			AssignmentRuleUser,
		)
	...
	def validate_document_types(self):
		if self.document_type == "ToDo":
			saashq.throw(_("Assignment Rule is not allowed on {0} document type").format(saashq.bold("ToDo")))
		...
		if user:
			assign_to.add(
				dict(
					...
					name=doc.get("name"),
					description=saashq.render_template(self.description, doc),
	...
	def get_user_based_on_field(self, doc):
		...
		if saashq.db.exists("User", val):
			return val

	def safe_eval(self, fieldname, doc):
		try:
			if self.get(fieldname):
				return saashq.safe_eval(self.get(fieldname), None, doc)
		except Exception as e:
			# when assignment fails, don't block the document as it may be
			# a part of the email pulling
			saashq.msgprint(saashq._("Auto assignment failed: {0}").format(str(e)), indicator="orange")
	...
	def is_rule_not_applicable_today(self):
		today = saashq.flags.assignment_day or saashq.utils.get_weekday()
		assignment_days = self.get_assignment_days()
...
def reopen_closed_assignment(doc):
	...
	for todo in todo_list:
		todo_doc = saashq.get_doc("ToDo", todo)
...
def apply(doc=None, method=None, doctype=None, name=None):
	...
	assignment_rule_docs: list[AssignmentRule] = [
		saashq.get_cached_doc("Assignment Rule", d.get("name")) for d in assignment_rules
	]
			...
			if not new_apply:
				...
				if to_close_todos:
					...
					for todo in todos_to_close:
						_todo = saashq.get_doc("ToDo", todo)
						_todo.status = "Closed"
...
def update_due_date(doc, state=None):
	...
	for rule in assignment_rules:
		rule_doc = saashq.get_cached_doc("Assignment Rule", rule.get("name"))
		...
		if field_updated:
			assignment_todos = saashq.get_all(
				"ToDo",
				filters={
...
def get_assignment_rules() -> list[str]:
	return saashq.get_all("Assignment Rule", filters={"disabled": 0}, pluck="document_type")
