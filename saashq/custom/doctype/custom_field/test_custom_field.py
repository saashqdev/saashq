...
import saashq
from saashq.custom.doctype.custom_field.custom_field import (
	create_custom_field,
...
class TestCustomField(IntegrationTestCase):
	def test_create_custom_fields(self):
		...
		self.assertTrue(saashq.db.exists("Custom Field", "Address-_test_custom_field_1"))
		self.assertTrue(saashq.db.exists("Custom Field", "Address-_test_custom_field_2"))
		self.assertTrue(saashq.db.exists("Custom Field", "Contact-_test_custom_field_2"))
	...
	def test_custom_field_sorting(self):
		try:
			...
			fields = saashq.get_meta("ToDo", cached=False).fields
	...
	def test_custom_section_and_column_breaks_ordering(self):
		...
		updated_meta = saashq.get_meta("Test Custom Breaks Ordering", cached=False)
	...
	def test_custom_field_renaming(self):
		def gen_fieldname():
			return "test_" + saashq.generate_hash()

		field = create_custom_field("ToDo", {"label": gen_fieldname()}, is_system_generated=False)
		old = field.fieldname
		new = gen_fieldname()
		data = saashq.generate_hash()
		doc = saashq.get_doc({"doctype": "ToDo", old: data, "description": "Something"}).insert()

		rename_fieldname(field.name, new)
		field.reload()
		self.assertEqual(field.fieldname, new)

		doc = saashq.get_doc("ToDo", doc.name)  # doc.reload doesn't clear old fields.
