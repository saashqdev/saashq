...
from saashq.utils import cstr, random_string
...
class CustomField(Document):
		...
		if not self.fieldname:
			...
			if not label:
				if self.fieldtype in ["Section Break", "Column Break", "Tab Break"]:
					...
					saashq.throw(_("Label is mandatory"))
	...
	def validate(self):
		...
		from saashq.core.doctype.doctype.doctype import check_fieldname_conflicts
		...
		if not self.fieldname:
			saashq.throw(_("Fieldname not set for Custom Field"))
	...
	def on_trash(self):
		...
		doctype_layouts = saashq.get_all("DocType Layout", filters={"document_type": self.dt}, pluck="name")
	...
	def validate_insert_after(self, meta):
		if not meta.get_field(self.insert_after):
			saashq.throw(
				...
				saashq.DoesNotExistError,
			)

		if self.fieldname == self.insert_after:
			saashq.throw(_("Insert After cannot be set as {0}").format(meta.get_label(self.insert_after)))
...
def get_fields_label(doctype=None):
	...
	if doctype in core_doctypes_list:
		return saashq.msgprint(_("Custom Fields cannot be added to core DocTypes."))

	if meta.custom:
		return saashq.msgprint(_("Custom Fields can only be added to a standard DocType."))

	return [
		{"value": df.fieldname or "", "label": _(df.label, context=df.parent) if df.label else ""}
		for df in saashq.get_meta(doctype).get("fields")
	]


def create_custom_field_if_values_exist(doctype, df):
	df = saashq._dict(df)
	if df.fieldname in saashq.db.get_table_columns(doctype) and saashq.db.count(
...
def create_custom_field(doctype, df, ignore_validate=False, is_system_generated=True):
	...
	if not df.fieldname and df.label:
		df.fieldname = saashq.scrub(df.label)
	if not saashq.db.get_value("Custom Field", {"dt": doctype, "fieldname": df.fieldname}):
...
def create_custom_fields(custom_fields: dict, ignore_validate=False, update=True):
	...
	try:
		saashq.flags.in_create_custom_fields = True
		doctypes_to_update = set()

		if saashq.flags.in_setup_wizard:
		...
		for doctypes, fields in custom_fields.items():
			...
			for doctype in doctypes:
				...
				for df in fields:
					field = saashq.db.get_value("Custom Field", {"dt": doctype, "fieldname": df["fieldname"]})
					if not field:
						try:
							df = df.copy()
						...
						except saashq.exceptions.DuplicateEntryError:
							pass
		...
		saashq.flags.in_create_custom_fields = False
...
def rename_fieldname(custom_field: str, fieldname: str):
	...
	field: CustomField = saashq.get_doc("Custom Field", custom_field)
	...
	if field.is_system_generated:
		saashq.throw(_("System Generated Fields can not be renamed"))
	if saashq.db.has_column(parent_doctype, fieldname):
		saashq.throw(_("Can not rename as column {0} is already present on DocType.").format(fieldname))
	if old_fieldname == new_fieldname:
		saashq.msgprint(_("Old and new fieldnames are same."), alert=True)
		return

	if saashq.db.has_column(field.dt, old_fieldname):
		saashq.db.rename_column(parent_doctype, old_fieldname, new_fieldname)
	...
	saashq.msgprint(_("Custom field renamed to {0} successfully.").format(fieldname), alert=True)
	saashq.db.commit()
	saashq.clear_cache()
