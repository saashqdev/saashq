...
import saashq
from saashq.core.doctype.doctype.doctype import InvalidFieldNameError
...
class TestCustomizeForm(IntegrationTestCase):
	def insert_custom_field(self):
		saashq.delete_doc_if_exists("Custom Field", "Event-custom_test_field")
		self.field = saashq.get_doc(
			{
				...
				"options": "\nCustom 1\nCustom 2\nCustom 3",
				"default": "Custom 3",
				"insert_after": saashq.get_meta("Event").fields[-1].fieldname,
			}
		).insert()

	def setUp(self):
		self.insert_custom_field()
		saashq.db.delete("Property Setter", dict(doc_type="Event"))
		saashq.db.commit()
		saashq.clear_cache(doctype="Event")

	def tearDown(self):
		saashq.delete_doc("Custom Field", self.field.name)
		saashq.db.commit()
		saashq.clear_cache(doctype="Event")

	def get_customize_form(self, doctype=None):
		d = saashq.get_doc("Customize Form")
		if doctype:
			d.doc_type = doctype
	...
	def test_fetch_to_customize(self):
		...
		self.assertEqual(len(d.get("fields")), len(saashq.get_doc("DocType", d.doc_type).fields) + 1)
	...
	def test_save_customization_property(self):
		d = self.get_customize_form("Event")
		self.assertEqual(
			saashq.db.get_value("Property Setter", {"doc_type": "Event", "property": "allow_copy"}, "value"),
			None,
		)
	...
	def test_save_customization_custom_field_property(self):
		d = self.get_customize_form("Event")
		self.assertEqual(saashq.db.get_value("Custom Field", self.field.name, "reqd"), 0)

		custom_field = d.get("fields", {"fieldname": self.field.fieldname})[0]
		custom_field.reqd = 1
		custom_field.no_copy = 1
		d.run_method("save_customization")
		self.assertEqual(saashq.db.get_value("Custom Field", self.field.name, "reqd"), 1)
		self.assertEqual(saashq.db.get_value("Custom Field", self.field.name, "no_copy"), 1)
		...
		d.run_method("save_customization")
		self.assertEqual(saashq.db.get_value("Custom Field", self.field.name, "reqd"), 0)
		self.assertEqual(saashq.db.get_value("Custom Field", self.field.name, "no_copy"), 0)
	...
	def test_save_customization_new_field(self):
		...
		self.assertEqual(
			saashq.db.get_value("Custom Field", custom_field_name, "fieldtype"),
			"Data",
		)

		self.assertEqual(
			saashq.db.get_value("Custom Field", custom_field_name, "insert_after"),
			last_fieldname,
		)

		saashq.delete_doc("Custom Field", custom_field_name)
		self.assertEqual(saashq.db.get_value("Custom Field", custom_field_name), None)
	...
	def test_save_customization_remove_field(self):
		...
		self.assertEqual(saashq.db.get_value("Custom Field", custom_field.name), None)
	...
	def test_core_doctype_customization(self):
		self.assertRaises(saashq.ValidationError, self.get_customize_form, "User")
	...
	def test_custom_internal_links(self):
		...
		saashq.clear_cache()
		user_group = saashq.get_meta("User Group")
		...
		saashq.clear_cache()
		user_group = saashq.get_meta("Event")
	...
	def test_system_generated_fields(self):
		...
		custom_field = saashq.get_doc("Custom Field", {"dt": doctype, "fieldname": custom_field_name})
		...
		self.assertEqual(
			saashq.db.get_value("Property Setter", property_setter_filters, "value"), "Test Description"
		)
	...
	def test_custom_field_order(self):
		...
		field_order_property = json.loads(
			saashq.db.get_value("Property Setter", {"doc_type": "ToDo", "property": "field_order"}, "value")
		)

		self.assertEqual(field_order_property, [df.fieldname for df in saashq.get_meta("ToDo").fields])
