...
import saashq
from saashq import sbool
from saashq.utils.response import Response

from .saashqclient import SaashqClient
...
class PushNotification:
	...
	def send_notification_to_user(
		...
		if strip_html:
			body = saashq.utils.strip_html(body)
	...
	def is_enabled(self) -> bool:
		...
		"""
		return sbool(
			saashq.db.get_single_value("Push Notification Settings", "enable_push_notification_relay")
		)
	...
	def _get_credential(self) -> tuple[str, str]:
		...
		"""
		notification_settings = saashq.get_doc("Push Notification Settings")
		...
		token = saashq.generate_hash(length=48)
		...
		body = {
			...
			"token": token,
			"webhook_route": "/api/method/saashq.push_notification.auth_webhook",
		}
	...
	def _send_post_request(self, method: str, params: dict, use_authentication: bool = True):
		...
		relay_server_endpoint = saashq.conf.get("push_relay_server_url")
		if use_authentication:
			api_key, api_secret = self._get_credential()
			client = SaashqClient(relay_server_endpoint, api_key=api_key, api_secret=api_secret)
		else:
			client = SaashqClient(relay_server_endpoint)
	...
	def _site_name(self) -> str:
		return urlparse(saashq.utils.get_url()).hostname

	@property
	def _site_protocol(self) -> str:
		return urlparse(saashq.utils.get_url()).scheme

	@property
	def _site_port(self) -> str:
		site_uri = urlparse(saashq.utils.get_url())
...
def auth_webhook():
	url = urlparse(saashq.utils.get_url()).hostname
	token = saashq.cache().get_value(f"{url}:push_relay_registration_token")
	response = Response()
...
def subscribe(fcm_token: str, project_name: str) -> dict:
	success, message = PushNotification(project_name).add_token(saashq.session.user, fcm_token)
	return {"success": success, "message": message}


@saashq.whitelist(methods=["GET"])
def unsubscribe(fcm_token: str, project_name: str) -> dict:
	success, message = PushNotification(project_name).remove_token(saashq.session.user, fcm_token)
	return {"success": success, "message": message}
