...
import saashq
from saashq.desk.search import get_names_for_mentions, search_link, search_widget
...
class TestSearch(IntegrationTestCase):
	...
	def test_validate_and_sanitize_search_inputs(self):
		...
		# should not fail if function is called via saashq.call with extra arguments
		args = ("Random DocType", "Random", "email", "2", "10", dict())
		kwargs = {"as_dict": False}
		self.assertListEqual(saashq.call("saashq.tests.test_search.get_data", *args, **kwargs), [])
	...
	def test_reference_doctype(self):
		...
		results = search_link(
			...
			query="saashq.tests.test_search.query_with_reference_doctype",
		)
		self.assertListEqual(results, [])

	def test_search_relevance(self):
		saashq.db.set_value("Language", {"name": ("like", "e%")}, "enabled", 1)
		...
		saashq.db.set_value("Language", "es", "idx", 10)
...
def setup_test_link_field_order(TestCase):
	...
	# Create Tree doctype
	if not saashq.db.exists("DocType", TestCase.tree_doctype_name):
		TestCase.tree_doc = saashq.get_doc(
			{
				"doctype": "DocType",
		...
		TestCase.tree_doc = saashq.get_doc("DocType", TestCase.tree_doctype_name)

	# Create root for the tree doctype
	if not saashq.db.exists(TestCase.tree_doctype_name, {"random": TestCase.parent_doctype_name}):
		saashq.get_doc(
