...
class VirtualDoctypeTest(Document):
	...
	def get_list():
		...
		return [saashq._dict(doc) for name, doc in data.items()]
...
class TestVirtualDoctypes(IntegrationTestCase):
	...
	def setUpClass(cls):
		...
		cls.addClassCleanup(saashq.flags.pop, "allow_doctype_export", None)
		...
		patch_virtual_doc = patch(
			"saashq.controllers", new={saashq.local.site: {TEST_DOCTYPE_NAME: VirtualDoctypeTest}}
		)
	...
	def test_insert_update_and_load_from_desk(self):
		...
		docname = saashq.response.docs[0]["name"]

		doc = saashq.get_doc(TEST_DOCTYPE_NAME, docname)
	...
	def test_multiple_doc_insert_and_get_list(self):
		doc1 = saashq.new_doc(doctype=TEST_DOCTYPE_NAME)
		doc1.append("child_table", {"name": "first", "some_fieldname": "first-value"})
		doc1.insert()
		doc2 = saashq.new_doc(doctype=TEST_DOCTYPE_NAME)
	...
	def test_delete_doc(self):
		doc = saashq.get_doc(doctype=TEST_DOCTYPE_NAME).insert()

		saashq.delete_doc(doc.doctype, doc.name)
