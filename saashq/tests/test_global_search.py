...
from saashq.utils import global_search, now_datetime
...
class TestGlobalSearch(IntegrationTestCase):
	def setUp(self):
		...
		self.assertTrue("__global_search" in saashq.db.get_tables())
		doctype = "Event"
		global_search.reset()
	...
	def tearDown(self):
		saashq.db.delete("Property Setter", {"doc_type": "Event"})
	...
	def test_update_doc(self):
		...
		event = saashq.get_doc("Event", saashq.get_all("Event")[0].name)
	...
	def test_delete_doc(self):
		self.insert_test_events()
		event_name = saashq.get_all("Event")[0].name
		event = saashq.get_doc("Event", event_name)
		...
		saashq.delete_doc("Event", event_name)
		global_search.sync_global_search()
		saashq.db.commit()
	...
	def test_insert_child_table(self):
		...
		for text in phrases:
			doc = saashq.get_doc({"doctype": "Event", "subject": text, "starts_on": now_datetime()})
			doc.insert()
