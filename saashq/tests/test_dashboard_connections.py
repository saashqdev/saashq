...
from saashq.custom.doctype.customize_form.test_customize_form import TestCustomizeForm
from saashq.desk.notifications import get_open_count
...
class TestDashboardConnections(IntegrationTestCase):
	...
	def test_external_doctype_link_with_dashboard_override(self):
		...
		# create a test doc
		todo_doc = saashq.get_doc(doctype="ToDo", description="test").insert()
		saashq.get_doc(doctype="Test Doctype D", title="d-001", doclink=todo_doc.name).insert()
		saashq.get_doc(doctype="Test Doctype E", title="e-001", todo=todo_doc.name).insert()
		...
		with self.patch_hooks(
			{
				"override_doctype_dashboards": {
					"ToDo": ["saashq.tests.test_dashboard_connections.get_dashboard_for_todo"]
				}
			}
		):
			connections = get_open_count("ToDo", todo_doc.name)["count"]
...
def add_links_in_child_tables():
	test_child_table_with_link_to_doctype_a = saashq.get_doc(
	...
	test_child_table_with_link_to_doctype_b = saashq.get_doc(
