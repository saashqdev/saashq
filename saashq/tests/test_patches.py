...
import saashq
from saashq.modules import patch_handler
...
execute:saashq.db.updatedb("Item")
execute:saashq.function(arg="1")
...
class TestPatches(IntegrationTestCase):
	def test_patch_module_names(self):
		saashq.flags.final_patches = []
		saashq.flags.in_install = True
		for patchmodule in patch_handler.get_all_patches():
			if patchmodule.startswith("execute:"):
				...
				self.assertTrue(saashq.get_attr(patchmodule.split(maxsplit=1)[0] + ".execute"))

		saashq.flags.in_install = False

	def test_get_patch_list(self):
		pre = patch_handler.get_patches_from_app("saashq", patch_handler.PatchType.pre_model_sync)
		post = patch_handler.get_patches_from_app("saashq", patch_handler.PatchType.post_model_sync)
		all_patches = patch_handler.get_patches_from_app("saashq")
	...
	def test_all_patches_are_marked_completed(self):
		all_patches = patch_handler.get_patches_from_app("saashq")
		finished_patches = saashq.db.count("Patch Log")
...
class TestPatchReader(IntegrationTestCase):
	def get_patches(self):
		return (
			patch_handler.get_patches_from_app("saashq"),
			patch_handler.get_patches_from_app("saashq", patch_handler.PatchType.pre_model_sync),
			patch_handler.get_patches_from_app("saashq", patch_handler.PatchType.post_model_sync),
		)
...
def check_patch_files(app):
	...
	patch_dir = Path(saashq.get_app_path(app)) / "patches"
	...
	for file in patch_dir.glob("**/*.py"):
		...
		try:
			patch_module = saashq.get_module(module)
...
def _get_dotted_path(file: Path, app) -> str:
	app_path = Path(saashq.get_app_path(app))
