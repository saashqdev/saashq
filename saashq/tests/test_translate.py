...
first_lang, second_lang, third_lang, fourth_lang, fifth_lang = choices(
	...
	saashq.get_all("Language", pluck="name", filters=[["name", "not like", "en%"], ["enabled", "=", 1]]),
	k=5,
)
...
class TestTranslate(IntegrationTestCase):
	...
	def tearDown(self):
		saashq.form_dict.pop("_lang", None)
		if self._testMethodName in self.guest_sessions_required:
	...
	def test_clear_cache(self):
		...
		self.assertIsNotNone(saashq.cache.hget(USER_TRANSLATION_KEY, saashq.local.lang))
		self.assertIsNotNone(saashq.cache.hget(MERGED_TRANSLATION_KEY, saashq.local.lang))

		clear_cache()

		self.assertIsNone(saashq.cache.hget(USER_TRANSLATION_KEY, saashq.local.lang))
		self.assertIsNone(saashq.cache.hget(MERGED_TRANSLATION_KEY, saashq.local.lang))
	...
	def test_request_language_resolution_with_form_dict(self):
		"""Test for saashq.translate.get_language

		Case 1: saashq.form_dict._lang is set
		"""

		saashq.form_dict._lang = first_lang

		with patch.object(saashq.translate, "get_preferred_language_cookie", return_value=second_lang):
	...
	def test_request_language_resolution_with_cookie(self):
		...
		Case 2: saashq.form_dict._lang is not set, but preferred_language cookie is
		"""

		with patch.object(saashq.translate, "get_preferred_language_cookie", return_value="fr"):
	...
	def test_guest_request_language_resolution_with_cookie(self):
		...
		Case 3: saashq.form_dict._lang is not set, but preferred_language cookie is [Guest User]
		"""
	...
	def test_guest_request_language_resolution_with_request_header(self):
		...
		Case 4: saashq.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is [Guest User]
		"""
	...
	def test_request_language_resolution_with_request_header(self):
		...
		Case 5: saashq.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is
		"""
	...
	def test_load_all_translate_files(self):
		...
		verify_translation_files("saashq")
	...
	def test_python_extractor(self):
		code = textwrap.dedent(
			...
			saashq._("attr")
			_("name")
			saashq._("attr with", context="attr context")
...
def verify_translation_files(app):
	...
	translations_dir = Path(saashq.get_app_path(app)) / "translations"
