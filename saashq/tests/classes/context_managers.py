...
if TYPE_CHECKING:
	from saashq.model import Document
...
def freeze_time(time_to_freeze: Any, is_utc: bool = False, *args: Any, **kwargs: Any) -> None:
	...
	from saashq.utils.data import get_datetime, get_system_timezone
...
def patch_hooks(overridden_hooks: dict) -> None:
	...
	with patch.object(saashq, "get_hooks", patched_hooks):
		yield
...
def change_settings(doctype, settings_dict=None, /, commit=False, **settings) -> None:
	...
	settings = saashq.get_doc(doctype)
	...
	saashq.db.value_cache[settings] = {}
	if commit:
		saashq.db.commit()
	yield
	settings = saashq.get_doc(doctype)
...
def switch_site(site: str) -> None:
	...
	old_site = saashq.local.site
	saashq.init(site, force=True)
	saashq.connect()
	yield
	saashq.init(old_site, force=True)
	saashq.connect()
...
def enable_safe_exec() -> None:
	...
	from saashq.utils.safe_exec import SAFE_EXEC_CONFIG_KEY

	conf = os.path.join(saashq.local.sites_path, "common_site_config.json")
...
def trace_fields(
	...
	**field_configs: dict[str, list | Callable | None],
	...
	Yields:
	    Document class
	"""
	from saashq.model.trace import traced_field
...
# NOTE: declare those who should also be made available directly saashq.tests.* namespace
