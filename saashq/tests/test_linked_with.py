...
from saashq.desk.form import linked_with
...
class TestLinkedWith(IntegrationTestCase):
	...
	def test_get_doctype_references_by_dlink_field(self):
		...
		parent_record = saashq.get_doc({"doctype": "Parent DocType"}).insert()
	...
	def test_check_delete_integrity(self):
		...
		doc = saashq.get_doc({"doctype": "Parent DocType"}).insert()
		doc.submit()
		doc.cancel()

		amendment = saashq.copy_doc(doc)
		amendment.amended_from = doc.name
		amendment.docstatus = 0
		amendment.insert()
		amendment.submit()

		self.assertRaises(saashq.LinkExistsError, doc.delete)
	...
	def test_reserved_keywords(self):
		...
		linked_doc = saashq.new_doc(dt_name).insert().submit()

		second_doc = (
			saashq.new_doc(dt_name, **{"from": linked_doc.doctype, "order": linked_doc.name})
			.insert()
			.submit()
		)

		with savepoint(saashq.LinkExistsError):
