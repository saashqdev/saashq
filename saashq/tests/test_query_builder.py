...
import saashq
from saashq.query_builder import Case
from saashq.query_builder.builder import Function
from saashq.query_builder.custom import ConstantColumn
from saashq.query_builder.functions import (
	Cast_,
	Coalesce,
	CombineDatetime,
	Date,
...
def run_only_if(dbtype: db_type_is) -> Callable:
	return unittest.skipIf(db_type_is(saashq.conf.db_type) != dbtype, f"Only runs for {dbtype.value}")
...
class TestCustomFunctionsMariaDB(IntegrationTestCase):
	...
	def test_constant_column(self):
		query = saashq.qb.from_("DocType").select("name", ConstantColumn("John").as_("User"))
		self.assertEqual(query.get_sql(), "SELECT `name`,'John' `User` FROM `tabDocType`")

	def test_timestamp(self):
		note = saashq.qb.DocType("Note")
		self.assertEqual(
	...
	def test_time(self):
		...
		select_query = saashq.qb.from_(note).select(CombineDatetime(note.posting_date, note.posting_time))
	...
	def test_cast(self):
		...
		self.assertEqual(
			saashq.qb.from_("red").from_(note).select("other", Cast_(note.name, "varchar")).get_sql(),
	...
	def test_round(self):
		...
		query = saashq.qb.from_(note).select(Round(note.price))
		self.assertEqual("select round(`price`,0) from `tabnote`", str(query).lower())

		query = saashq.qb.from_(note).select(Round(note.price, 3))
		self.assertEqual("select round(`price`,3) from `tabnote`", str(query).lower())

	def test_truncate(self):
		note = saashq.qb.DocType("Note")
		query = saashq.qb.from_(note).select(Truncate(note.price, 3))
...
class TestBuilderBase:
	def test_adding_tabs(self):
		self.assertEqual("tabNotes", saashq.qb.DocType("Notes").get_sql())
		self.assertEqual("__Auth", saashq.qb.DocType("__Auth").get_sql())
		self.assertEqual("Notes", saashq.qb.Table("Notes").get_sql())

	def test_run_patcher(self):
		query = saashq.qb.from_("ToDo").select("*").limit(1)
	...
	def test_agg_funcs(self):
		saashq.db.truncate("Communication")
		sample_data = {
		...
		saashq.get_doc(sample_data).insert()
		sample_data["rating"] = 3
		saashq.get_doc(sample_data).insert()
		sample_data["rating"] = 4
		saashq.get_doc(sample_data).insert()
		self.assertEqual(saashq.qb.max("Communication", "rating"), 4)
		self.assertEqual(saashq.qb.min("Communication", "rating"), 1)
		self.assertAlmostEqual(saashq.qb.avg("Communication", "rating"), 2.666, places=2)
		self.assertEqual(saashq.qb.sum("Communication", "rating"), 8.0)
		saashq.db.rollback()
...
class TestParameterization(IntegrationTestCase):
	def test_where_conditions(self):
		...
		query = saashq.qb.from_(DocType).select(DocType.name).where(DocType.owner == "Administrator' --")
	...
	def test_set_conditions(self):
		DocType = saashq.qb.DocType("DocType")
		query = saashq.qb.update(DocType).set(DocType.value, "some_value")
	...
	def test_where_conditions_functions(self):
		DocType = saashq.qb.DocType("DocType")
		query = (
			saashq.qb.from_(DocType).select(DocType.name).where(Coalesce(DocType.search_fields == "subject"))
		)
	...
	def test_case(self):
		DocType = saashq.qb.DocType("DocType")
		query = saashq.qb.from_(DocType).select(
			Case()
	...
	def test_case_in_update(self):
		DocType = saashq.qb.DocType("DocType")
		query = saashq.qb.update(DocType).set(
			"parent",
			Case()
			.when(DocType.search_fields == "value", "other_value")
			.when(Coalesce(DocType.search_fields == "subject_in_function"), "true_value")
	...
	def test_named_parameter_wrapper(self):
		from saashq.query_builder.terms import NamedParameterWrapper
...
class TestBuilderMaria(IntegrationTestCase, TestBuilderBase):
	def test_adding_tabs_in_from(self):
		self.assertEqual("SELECT * FROM `tabNotes`", saashq.qb.from_("Notes").select("*").get_sql())
		self.assertEqual("SELECT * FROM `__Auth`", saashq.qb.from_("__Auth").select("*").get_sql())

	def test_get_qb_type(self):
		from saashq.query_builder import get_query_builder

		qb = get_query_builder(saashq.db.db_type)
...
class TestBuilderPostgres(IntegrationTestCase, TestBuilderBase):
	...
	def test_replace_tables(self):
		info_schema = saashq.qb.Schema("information_schema")
		self.assertEqual(
			'SELECT * FROM "pg_stat_all_tables"',
			saashq.qb.from_(info_schema.tables).select("*").get_sql(),
		)

	def test_replace_fields_post(self):
		self.assertEqual("relname", saashq.qb.Field("table_name").get_sql())
...
class TestMisc(IntegrationTestCase):
	...
	def test_union(self):
		...
		role = saashq.qb.DocType("Role")
		users = saashq.qb.from_(user).select(user.name)
		roles = saashq.qb.from_(role).select(role.name)
