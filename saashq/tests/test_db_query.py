...
from saashq.database.utils import DefaultOrderBy
from saashq.desk.reportview import get_filters_cond
from saashq.handler import execute_cmd
from saashq.model.db_query import DatabaseQuery, get_between_date_filter
from saashq.permissions import add_user_permission, clear_user_permissions_for_doctype
from saashq.query_builder import Column
...
class TestDBQuery(IntegrationTestCase):
	...
	def test_child_table_field_syntax(self):
		note = saashq.get_doc(
			doctype="Note",
			title=f"Test {saashq.utils.random_string(8)}",
			content="test",
	...
	def test_child_table_join(self):
		saashq.delete_doc_if_exists("DocType", "Parent DocType 1")
		saashq.delete_doc_if_exists("DocType", "Parent DocType 2")
		saashq.delete_doc_if_exists("DocType", "Child DocType")
		# child table
		saashq.get_doc(
			{
				"doctype": "DocType",
		...
		# clear records
		saashq.db.delete("Parent DocType 1")
		saashq.db.delete("Parent DocType 2")
		saashq.db.delete("Child DocType")
		...
		# test query
		results1 = saashq.get_all("Parent DocType 1", fields=["name", "child.title as child_title"])
		results2 = saashq.get_all("Parent DocType 2", fields=["name", "child.title as child_title"])
	...
	def test_link_field_syntax(self):
		todo = saashq.get_doc(doctype="ToDo", description="Test ToDo", allocated_to="Administrator").insert()
		result = saashq.get_all(
			"ToDo",
	...
	def test_build_match_conditions(self):
		...
		test2user = saashq.get_doc("User", "test2@example.com")
	...
	def test_string_as_field(self):
		self.assertEqual(
			saashq.get_all("DocType", as_list=True), saashq.get_all("DocType", fields="name", as_list=True)
		)

	def test_none_filter(self):
		query = saashq.qb.get_query("DocType", fields="name", filters={"restrict_to_domain": None})
	...
	def test_between_filters_date_bounds(self):
		date_df = saashq._dict(fieldtype="Date")
		datetime_df = saashq._dict(fieldtype="Datetime")
		today = saashq.utils.nowdate()
	...
	def test_ignore_permissions_for_get_filters_cond(self):
		saashq.set_user("test2@example.com")
		self.assertRaises(saashq.PermissionError, get_filters_cond, "DocType", dict(istable=1), [])
	...
	def test_query_fields_sanitizer(self):
		...
		with self.assertRaises(saashq.DataError):
			DatabaseQuery("DocType").execute(
				fields=[
					"name",
					"issingle",
	...
	def test_nested_permission(self):
		...
		from saashq.core.page.permission_manager.permission_manager import update
	...
	def test_order_by_group_by_sanitizer(self):
		...
		with self.assertRaises(saashq.ValidationError):
			DatabaseQuery("DocType").execute(
				fields=["name"],
	...
	def test_of_not_of_descendant_ancestors(self):
		...
		data = saashq.get_all("Nested DocType", {"name": ("descendants of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)

		data = saashq.get_all("Nested DocType", {"name": ("descendants of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = saashq.get_all("Nested DocType", {"name": ("ancestors of", "Level 2 A")})
		...
		data = saashq.get_all("Nested DocType", {"name": ("ancestors of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = saashq.get_all("Nested DocType", {"name": ("not descendants of", "Level 2 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		...
		data = saashq.get_all("Nested DocType", {"name": ("not descendants of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		...
		data = saashq.get_all("Nested DocType", {"name": ("not ancestors of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = saashq.get_all("Nested DocType", {"name": ("not ancestors of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = saashq.get_all("Nested DocType", {"name": ("ancestors of", "Root")})
		self.assertTrue(len(data) == 0)
		self.assertTrue(
			len(saashq.get_all("Nested DocType", {"name": ("not ancestors of", "Root")}))
			== len(saashq.get_all("Nested DocType"))
		)
	...
	def test_is_set_is_not_set(self):
		...
		saashq.db.set_value("DocType", "Property Setter", "autoname", None, update_modified=False)
	...
	def test_virtual_field_get_list(self):
		try:
			saashq.get_list("Prepared Report", ["*"])
			saashq.get_list("Scheduled Job Type", ["*"])
		except Exception:
	...
	def test_prepare_select_args(self):
		# saashq.get_all inserts modified field into order_by clause
	...
	def test_column_comparison(self):
		...
		users_unedited = saashq.get_all(
			"User",
			filters={"creation": Column("modified")},
			fields=["name", "creation", "modified"],
			limit=1,
		)
		users_edited = saashq.get_all(
			"User",
			filters={"creation": ("!=", Column("modified"))},
			fields=["name", "creation", "modified"],
			limit=1,
	...
	def test_cast_name(self):
		...
		saashq.delete_doc_if_exists("DocType", "autoinc_dt_test")
	...
	def test_fieldname_starting_with_int(self):
		...
		saashq.delete_doc_if_exists("DocType", "dt_with_int_named_fieldname")
		saashq.delete_doc_if_exists("DocType", "table_dt")
		...
		dt_data = saashq.get_doc({"doctype": "dt_with_int_named_fieldname", "1field": "10"}).insert(
			ignore_permissions=True
		)
	...
	def test_permission_query_condition(self):
		from saashq.desk.doctype.dashboard_settings.dashboard_settings import (
		...
		dashboard_settings = saashq.db.sql(
			f"""
				SELECT name
	...
	def test_virtual_doctype(self):
		...
		with patch(
			"saashq.controllers",
			new={saashq.local.site: {"Virtual DocType": VirtualDocType}},
		):
			saashq.get_all("Virtual DocType", filters={"name": "test"}, fields=["name"], limit=1)

	def test_coalesce_with_in_ops(self):
		self.assertNotIn("ifnull", saashq.get_all("User", {"first_name": ("in", ["a", "b"])}, run=0))
		self.assertIn("ifnull", saashq.get_all("User", {"first_name": ("in", ["a", None])}, run=0))
		self.assertIn("ifnull", saashq.get_all("User", {"first_name": ("in", ["a", ""])}, run=0))
		self.assertIn("ifnull", saashq.get_all("User", {"first_name": ("in", [])}, run=0))
		self.assertIn("ifnull", saashq.get_all("User", {"first_name": ("not in", ["a"])}, run=0))
		self.assertIn("ifnull", saashq.get_all("User", {"first_name": ("not in", [])}, run=0))
		self.assertIn("ifnull", saashq.get_all("User", {"first_name": ("not in", [""])}, run=0))

		# primary key is never nullable
		self.assertNotIn("ifnull", saashq.get_all("User", {"name": ("in", ["a", None])}, run=0))
		self.assertNotIn("ifnull", saashq.get_all("User", {"name": ("in", ["a", ""])}, run=0))
		self.assertNotIn("ifnull", saashq.get_all("User", {"name": ("in", (""))}, run=0))
		self.assertNotIn("ifnull", saashq.get_all("User", {"name": ("in", ())}, run=0))

	def test_coalesce_with_datetime_ops(self):
		self.assertNotIn("ifnull", saashq.get_all("User", {"last_active": (">", "2022-01-01")}, run=0))
		self.assertNotIn("ifnull", saashq.get_all("User", {"creation": ("<", "2022-01-01")}, run=0))
		self.assertNotIn(
			"ifnull",
			saashq.get_all("User", {"last_active": ("between", ("2022-01-01", "2023-01-01"))}, run=0),
		)
		self.assertIn("ifnull", saashq.get_all("User", {"last_active": ("<", "2022-01-01")}, run=0))

	def test_ambiguous_linked_tables(self):
		from saashq.desk.reportview import get

		if not saashq.db.exists("DocType", "Related Todos"):
			saashq.get_doc(
				{
					"doctype": "DocType",
					"custom": 1,
			...
			saashq.db.delete("Related Todos")

		todo_one = saashq.get_doc(
			{
				"doctype": "ToDo",
		...
		saashq.form_dict.doctype = "Related Todos"
	...
	def test_select_star_expansion(self):
		count = saashq.get_list("Language", ["SUM(1)", "COUNT(*)"], as_list=1, order_by=None)[0]
		self.assertEqual(count[0], saashq.db.count("Language"))
		self.assertEqual(count[1], saashq.db.count("Language"))


class TestReportView(IntegrationTestCase):
	@run_only_if(db_type_is.MARIADB)  # TODO: postgres name casting is messed up
	def test_get_count(self):
		saashq.local.request = saashq._dict()
		saashq.local.request.method = "GET"
		...
		count = execute_cmd("saashq.desk.reportview.get_count")
		saashq.local.form_dict = saashq._dict(
			{
				"doctype": "DocType",
		...
		dict_filter_response = execute_cmd("saashq.desk.reportview.get_count")
		...
		child_filter_response = execute_cmd("saashq.desk.reportview.get_count")
		current_value = saashq.db.sql(
	...
	def test_reportview_get(self):
		...
		saashq.local.request = saashq._dict()
		saashq.local.request.method = "POST"
		...
		response = execute_cmd("saashq.desk.reportview.get")
	...
	def test_reportview_get_permlevel_system_users(self):
		with setup_patched_blog_post(), setup_test_user(set_user=True):
			...
			self.assertEqual(saashq.get_list("Blog Post", "published"), [])
	...
	def test_db_filter_not_set(self):
		...
		"""
		saashq.get_doc({"doctype": "ToDo", "description": "filter test"}).insert()
		saashq.get_doc({"doctype": "ToDo", "description": "filter test", "reference_name": ""}).insert()
		...
		self.assertEqual(
			len(saashq.get_all("ToDo", filters={"reference_name": ["is", "not set"]})),
			saashq.db.count("ToDo", {"reference_name": ["is", "not set"]}),
		)


def add_child_table_to_blog_post():
	child_table = saashq.get_doc(
		{
			"doctype": "DocType",
			"istable": 1,
			"custom": 1,
...
def create_nested_doctype():
	if saashq.db.exists("DocType", "Nested DocType"):
		return
...
def create_nested_doctype_records():
	...
	for r in records:
		d = saashq.new_doc("Nested DocType")
		d.update(r)
