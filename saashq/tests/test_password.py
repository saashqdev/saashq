...
from saashq.utils.password import check_password, decrypt, encrypt, passlibctx, update_password


class TestPassword(IntegrationTestCase):
	def setUp(self):
		saashq.delete_doc("Email Account", "Test Email Account Password")
		saashq.delete_doc("Email Account", "Test Email Account Password-new")
	...
	def make_email_account(self, name="Test Email Account Password"):
		if not saashq.db.exists("Email Account", name):
			...
			return saashq.get_doc("Email Account", name)
	...
	def test_hashed_password(self, user="test@example.com"):
		...
		self.assertRaises(saashq.AuthenticationError, check_password, user, new_password)
	...
	def test_password_on_rename_user(self):
		...
		saashq.rename_doc(doc.doctype, old_name, new_name)

		new_doc = saashq.get_doc(doc.doctype, new_name)
		self.assertEqual(new_doc.get_password(), password)
		self.assertTrue(not get_password_list(doc))

		saashq.rename_doc(doc.doctype, new_name, old_name)
