...
from saashq.modules import get_doctype_module, get_module_path, load_doctype_module

if TYPE_CHECKING:
	from saashq.model.document import Document

logger = logging.getLogger(__name__)
testing_logger = logging.getLogger("saashq.testing.generators")
...
def get_modules(doctype) -> tuple[str, ModuleType]:
	...
	module = saashq.db.get_value("DocType", doctype, "module")
	try:
...
def load_test_records_for(index_doctype) -> dict[str, Any] | list:
	module_path = get_module_path(get_doctype_module(index_doctype), "doctype", saashq.scrub(index_doctype))

	json_path = os.path.join(module_path, "test_records.json")
	if os.path.exists(json_path):
		if not saashq.flags.deprecation_dumpster_invoked:
			from saashq.deprecation_dumpster import deprecation_warning
...
def _generate_all_records_towards(
	...
	visited = set(saashq.local.test_objects.keys())
...
def _generate_records_for(
	...
	if hasattr(test_module, "_make_test_records"):
		...
		if not test_records:
			...
			saashq.local.test_objects[index_doctype] = []  # avoid noisy retries on multiple invocations
...
def _sync_records(
	...
	def _load(do_create=True):
		...
		for _sub_doctype, records in test_records.items():
			for record in records:
				...
				saashq.local.test_objects[index_doctype].append(MappingProxyType(record))
	...
	if index_doctype in test_record_manager_instance.get():
		...
		if reset:
			...
			saashq.local.test_objects[index_doctype] = []
			...
			if index_doctype not in saashq.local.test_objects:
...
def _try_create(record, reset=False, commit=False) -> tuple["Document", bool]:
	...
	if not reset:
		saashq.db.savepoint("creating_test_record")

	d = saashq.copy_doc(record)
	...
	if saashq.db.exists(d.doctype, d.name) and not reset:
		saashq.db.rollback(save_point="creating_test_record")
		# do not create test records, if already exists
		return saashq.get_doc(d.doctype, d.name), False
...
class TestRecordManager:
	def __init__(self):
		...
		self.log_file = Path(saashq.get_site_path(PERSISTENT_TEST_LOG_FILE))
		self._log = None
	...
	def _append_to_log(self, index_doctype, records: list["Document"]):
		...
		with self.log_file.open("a") as f:
			f.write(saashq.as_json(entry, indent=None) + "\n")
		...
		if self.log_file.exists():
			with self.log_file.open() as f:
				for line in f:
					...
					try:
						for r in records:
							log.setdefault(index_doctype, []).append(saashq.get_doc(r["doctype"], r["name"]))
					except saashq.DoesNotExistError as e:
						raise ValueError(
...
def _after_install_clear_test_log():
	log_file_path = saashq.get_site_path(PERSISTENT_TEST_LOG_FILE)
