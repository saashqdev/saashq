...
class TestNonNullableDocfield(IntegrationTestCase):
	...
	def test_non_nullable_field(self):
		doc = saashq.new_doc(doctype=self.doctype_name)
		doc.insert()
		inserted_doc = saashq.db.get(self.doctype_name, {"name": doc.name})
		self.assertEqual(inserted_doc.test_field, "")

	def test_edit_field_nullable_status(self):
		doc = saashq.new_doc(doctype=self.nullable_doctype_name)
		doc.insert()
		inserted_doc = saashq.db.get(self.nullable_doctype_name, {"name": doc.name})
		...
		for column in saashq.db.sql(query, table.table_name, as_dict=True):
			if column.name == "test_field":
				self.assertFalse(column.not_nullable)

		doctype_doc = saashq.get_doc("DocType", self.nullable_doctype_name)
