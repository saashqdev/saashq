...
import saashq
from saashq.desk.reportview import export_query, extract_fieldnames
...
class TestReportview(IntegrationTestCase):
	def test_csv(self):
		...
		for delimiter in (",", ";", "\t", "|"):
			saashq.local.form_dict.csv_delimiter = delimiter
			for quoting in (QUOTE_ALL, QUOTE_MINIMAL, QUOTE_NONE, QUOTE_NONNUMERIC):
				saashq.local.form_dict.csv_quoting = quoting
