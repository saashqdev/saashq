import saashq
from saashq.model.document import Document, get_doc
...
class TestDocRef(IntegrationTestCase):
	...
	def test_doc_ref_in_query(self):
		...
		result = saashq.db.get_value("ToDo", {"reference_name": user}, ["name", "description"])
		self.assertEqual(result[0], test_doc.name)
		self.assertEqual(result[1], "Test ToDo")
		# Setter using Document as DocRef
		saashq.db.set_value("ToDo", test_doc, "description", "Revised Test ToDo")
		test_doc.reload()
