...
from saashq.model.naming import (
...
from saashq.utils import now_datetime, nowdate, nowtime
...
class TestNaming(IntegrationTestCase):
	...
	def test_append_number_if_name_exists(self):
		...
		note = saashq.get_doc({"doctype": DOCTYPE, "title": TITLE}).insert()
	...
	def test_field_autoname_name_sync(self):
		country = saashq.get_last_doc("Country")
	...
	def test_child_table_naming(self):
		...
		name = saashq.generate_hash(length=10)

		doc = saashq.new_doc(dt_with_child_autoname.name)
	...
	def test_format_autoname_for_consecutive_week_number(self):
		...
		todo_doctype = saashq.get_doc("DocType", doctype)
		todo_doctype.autoname = "format:TODO-{WW}-{##}"
		todo_doctype.save()

		description = "Format"

		todo = saashq.new_doc(doctype)
	...
	def test_revert_series(self):
		...
		saashq.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 1)""", (series,))
		revert_series_if_last(key, name)
		current_index = saashq.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]

		self.assertEqual(current_index.get("current"), 0)
		saashq.db.delete("Series", {"name": series})

		series = f"TEST-{year}-"
		key = "TEST-.YYYY.-.#####"
		name = f"TEST-{year}-00002"
		saashq.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 2)""", (series,))
		revert_series_if_last(key, name)
		current_index = saashq.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]
		...
		name = "TEST-00003"
		saashq.db.delete("Series", {"name": series})
		saashq.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 3)""", (series,))
		revert_series_if_last(key, name)
		current_index = saashq.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]
	...
	def test_naming_for_cancelled_and_amended_doc(self):
		submittable_doctype = saashq.get_doc(
			{
				"doctype": "DocType",
				"module": "Core",
				"custom": 1,
		...
		doc = saashq.new_doc("Submittable Doctype")
		doc.save()
		...
		amended_doc = saashq.copy_doc(doc)
	...
	def test_naming_validations(self):
		...
		# set name via prompt
		tag = saashq.get_doc({"doctype": "Tag", "__newname": "Tag"})
		self.assertRaises(saashq.NameError, tag.insert)

		# set by passing set_name as ToDo
		self.assertRaises(saashq.NameError, make_invalid_todo)

		# name (via title field) cannot be the same as the doctype
		note = saashq.get_doc({"doctype": "Currency", "currency_name": "Currency"})
		self.assertRaises(saashq.NameError, note.insert)

		# case 2: set name with "New ---"
		tag = saashq.get_doc({"doctype": "Tag", "__newname": "New Tag"})
		self.assertRaises(saashq.NameError, tag.insert)
		...
		tag = saashq.get_doc({"doctype": "Tag", "__newname": ""})
		self.assertRaises(saashq.ValidationError, tag.insert)

	def test_autoincremented_naming(self):
		from saashq.core.doctype.doctype.test_doctype import new_doctype

		doctype = "autoinc_doctype" + saashq.generate_hash(length=5)
		dt = new_doctype(doctype, autoname="autoincrement").insert(ignore_permissions=True)

		for i in range(1, 20):
			self.assertEqual(saashq.new_doc(doctype).save(ignore_permissions=True).name, i)
	...
	def test_naming_using_fields(self):
		webhook = saashq.new_doc("Webhook")
	...
	def test_hash_collision(self):
		...
		for _ in range(10):
			...
			saashq.new_doc(doctype).update({"name": name}).insert()
		saashq.flags.pop("in_import", None)
		...
		with self.patch_hooks(
			{
				"naming_series_variables": {
					"PM": ["saashq.tests.test_naming.parse_naming_series_variable"],
				},
			},
		):
	...
	def test_uuid_naming(self):
		...
		self.assertEqual("uuid", saashq.db.get_column_type(uuid_doctype, "name"))

		# Auto set names
		document = saashq.new_doc(uuid_doctype).insert()
		...
		for uid in (uuid_utils.uuid4(), uuid_utils.uuid7()):
			doc = saashq.new_doc(uuid_doctype, name=uid).insert()
			self.assertEqual(doc.name, str(uid))

		# Can specify valid UUID strings too
		for uid in (uuid_utils.uuid4(), uuid_utils.uuid7()):
			doc = saashq.new_doc(uuid_doctype, name=str(uid)).insert()
			self.assertEqual(doc.name, str(uid))

		self.assertRaises(InvalidUUIDValue, saashq.new_doc(uuid_doctype, name="XYZ").insert)
...
def make_invalid_todo():
	saashq.get_doc({"doctype": "ToDo", "description": "Test"}).insert(set_name="ToDo")
