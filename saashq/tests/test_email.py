...
from saashq.email.doctype.email_account.test_email_account import TestEmailAccount
...
class TestEmail(IntegrationTestCase):
	...
	def test_send_after(self):
		...
		flush()
		email_queue = saashq.db.sql("""select name from `tabEmail Queue` where status='Sent'""", as_dict=1)
	...
	def test_flush(self):
		...
		self.assertTrue("test1@example.com" in queue_recipients)
		self.assertEqual(len(queue_recipients), 2)
		self.assertTrue("Unsubscribe" in saashq.safe_decode(saashq.flags.sent_mail))
	...
	def test_cc_footer(self):
		...
		self.assertTrue(
			...
			in saashq.safe_decode(saashq.flags.sent_mail)
		)
	...
	def test_expose(self):
		...
		email_obj = email.message_from_string(saashq.safe_decode(saashq.flags.sent_mail))
		for part in email_obj.walk():
			...
			if content:
				...
				query_string = re.search(
					r"(?<=/api/method/saashq.email.queue.unsubscribe\?).*(?=" + eol + ")", content.decode()
				).group(0)
	...
	def test_sender(self):
		def _patched_assertion(email_account, assertion):
			with patch.object(QueueBuilder, "get_outgoing_email_account", return_value=email_account):
				...
				email_queue_sender = saashq.db.get_value("Email Queue", {"status": "Sent"}, "sender")
	...
	def test_unsubscribe(self):
		from saashq.email.queue import unsubscribe
		...
		email_queue = saashq.db.get_value("Email Queue", {"status": "Not Sent"})
		queue_recipients = [
			r.recipient
			for r in saashq.db.sql(
				"""select recipient from `tabEmail Queue Recipient`
		...
		saashq.get_doc("Email Queue", email_queue).send()
		self.assertTrue("Unsubscribe" in saashq.safe_decode(saashq.flags.sent_mail))
	...
	def test_image_parsing(self):
		...
		saashq.db.delete("Communication", {"sender": "sukh@yyy.com"})

		with open(saashq.get_app_path("saashq", "tests", "data", "email_with_image.txt")) as raw:
			messages = {
...
class TestEmailIntegrationTest(IntegrationTestCase):
	...
	def setUp(self) -> None:
		# Saashq code is configured to not attempting sending emails during test.
		saashq.flags.testing_email = True
		requests.delete(f"{self.SMTP4DEV_WEB}/api/Messages/*")
		return super().setUp()

	def tearDown(self) -> None:
		saashq.flags.testing_email = False
	...
	def test_store_attachments(self):
		...
		communication = saashq.get_doc("Communication", name)

		attachments = get_attachments(communication.doctype, communication.name)
		self.assertEqual(len(attachments), 1)

		file = saashq.get_doc("File", attachments[0].name)
