...
from saashq.desk.treeview import get_children
...
class TestNestedSet(IntegrationTestCase):
	...
	def test_basic_tree(self):
		...
		min_lft = 1
		max_rgt = saashq.qb.from_(TEST_DOCTYPE).select(Max(Field("rgt"))).run(pluck=True)[0]

		for record in records:
			lft, rgt, parent_test_tree_doctype = saashq.db.get_value(
				TEST_DOCTYPE,
			...
			if parent_test_tree_doctype:
				parent_lft, parent_rgt = saashq.db.get_value(
	...
	def test_recursion(self):
		leaf_node = saashq.get_doc(TEST_DOCTYPE, {"some_fieldname": "Parent 2"})
	...
	def test_move_group_into_another(self):
		old_lft, old_rgt = saashq.db.get_value(TEST_DOCTYPE, "Parent 2", ["lft", "rgt"])
		...
		# after move
		new_lft, new_rgt = saashq.db.get_value(TEST_DOCTYPE, "Parent 2", ["lft", "rgt"])
	...
	def test_move_leaf_into_another_group(self):
		child_2 = saashq.get_doc(TEST_DOCTYPE, "Child 2")

		# assert that child 2 is not already under parent 1
		parent_lft_old, parent_rgt_old = saashq.db.get_value(TEST_DOCTYPE, "Parent 2", ["lft", "rgt"])
		...
		parent_lft_new, parent_rgt_new = saashq.db.get_value(TEST_DOCTYPE, "Parent 2", ["lft", "rgt"])
	...
	def test_delete_leaf(self):
		...
		child_1 = saashq.get_doc(TEST_DOCTYPE, "Child 1")
		child_1.delete()
		records.remove(el)

		self.test_basic_tree()

		n = saashq.new_doc(TEST_DOCTYPE)
		n.update(el)
		n.insert()
		records.append(el)
	...
	def test_delete_group(self):
		...
		with self.assertRaises(NestedSetChildExistsError):
			saashq.delete_doc(TEST_DOCTYPE, "Parent 1")
	...
	def test_rename_nestedset(self):
		...
		# Rename doctype
		saashq.rename_doc("DocType", doctype.name, "Test " + random_string(10), force=True)

	def test_merge_groups(self):
		global records
		el = {"some_fieldname": "Parent 2", "parent_test_tree_doctype": "Root Node", "is_group": 1}
		saashq.rename_doc(TEST_DOCTYPE, "Parent 2", "Parent 1", merge=True)
		records.remove(el)
		self.test_basic_tree()
	...
	def test_merge_leaf_into_group(self):
		with self.assertRaises(NestedSetInvalidMergeError):
			saashq.rename_doc(TEST_DOCTYPE, "Child 1", "Parent 1", merge=True)

	def test_merge_group_into_leaf(self):
		with self.assertRaises(NestedSetInvalidMergeError):
			saashq.rename_doc(TEST_DOCTYPE, "Parent 1", "Child 1", merge=True)

	def test_root_deletion(self):
		for doc in ["Child 3", "Child 2", "Child 1", "Parent 2", "Parent 1"]:
			saashq.delete_doc(TEST_DOCTYPE, doc)

		root_node = saashq.get_doc(TEST_DOCTYPE, "Root Node")
		...
		with patch("saashq.get_doc", return_value=root_node):
			with self.assertRaises(saashq.ValidationError):
				root_node.delete()

		# root deletion without allow_root_deletion
		root_node.delete()
		self.assertFalse(saashq.db.exists(TEST_DOCTYPE, "Root Node"))
	...
	def test_desc_filters(self):
		...
		# db_query
		self.assertNotIn(record, saashq.get_all(TEST_DOCTYPE, exclusive_filter, run=0))
		self.assertIn(record, saashq.get_all(TEST_DOCTYPE, inclusive_filter, run=0))
		self.assertNotIn(record, saashq.get_all(linked_doctype, exclusive_link, run=0))
		self.assertIn(record, saashq.get_all(linked_doctype, inclusive_link, run=0))

		# QB
		self.assertNotIn(record, str(saashq.qb.get_query(TEST_DOCTYPE, filters=exclusive_filter)))
		self.assertIn(record, str(saashq.qb.get_query(TEST_DOCTYPE, filters=inclusive_filter)))

		self.assertNotIn(record, str(saashq.qb.get_query(table=linked_doctype, filters=exclusive_link)))
		self.assertIn(record, str(saashq.qb.get_query(table=linked_doctype, filters=inclusive_link)))
