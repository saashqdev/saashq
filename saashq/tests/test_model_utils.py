...
import saashq
from saashq.model import core_doctypes_list, get_permitted_fields, is_default_field
from saashq.model.utils import get_fetch_values
...
class TestModelUtils(IntegrationTestCase):
	def test_get_fetch_values(self):
		...
		full_name = saashq.db.get_value("User", user, "full_name")
	...
	def test_get_permitted_fields(self):
		...
		todo_all_columns = saashq.get_meta("ToDo").get_valid_columns()
		...
		with set_user("Guest"):
			...
			picked_doctype_all_columns = saashq.get_meta(picked_doctype).get_valid_columns()
		...
		with set_user("Administrator"):
			...
			child_all_fields = saashq.get_meta("Installed Application").get_valid_columns()
...
def set_user(user: str):
	past_user = saashq.session.user or "Administrator"
	saashq.set_user(user)
	yield
	saashq.set_user(past_user)
