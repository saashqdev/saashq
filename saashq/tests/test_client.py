...
class TestClient(IntegrationTestCase):
	def test_set_value(self):
		...
		saashq.set_value("ToDo", todo.name, "description", "test 1")
		self.assertEqual(saashq.get_value("ToDo", todo.name, "description"), "test 1")

		saashq.set_value("ToDo", todo.name, {"description": "test 2"})
		self.assertEqual(saashq.get_value("ToDo", todo.name, "description"), "test 2")

	def test_delete(self):
		from saashq.client import delete
		from saashq.desk.doctype.note.note import Note

		note = saashq.get_doc(
			doctype="Note",
			title=saashq.generate_hash(length=8),
			content="test",
		...
		self.assertFalse(saashq.db.exists("Note", note.name))
		self.assertRaises(saashq.DoesNotExistError, delete, "Note", note.name)
		self.assertRaises(saashq.DoesNotExistError, delete, "Note Seen By", child_row_name)
	...
	def test_http_valid_method_access(self):
		...
		saashq.local.form_dict = saashq._dict(
			{"doc": dict(doctype="ToDo", description="Valid http method"), "cmd": "saashq.client.save"}
		)
		todo = execute_cmd("saashq.client.save")
	...
	def test_http_invalid_method_access(self):
		...
		saashq.local.form_dict = saashq._dict(
			{"doc": dict(doctype="ToDo", description="Invalid http method"), "cmd": "saashq.client.save"}
		)

		self.assertRaises(saashq.PermissionError, execute_cmd, "saashq.client.save")

	def test_run_doc_method(self):
		from saashq.handler import execute_cmd

		if not saashq.db.exists("Report", "Test Run Doc Method"):
			...
			report = saashq.get_doc("Report", "Test Run Doc Method")
		...
		execute_cmd(saashq.local.form_dict.cmd)
		...
		self.assertRaises(saashq.PermissionError, execute_cmd, saashq.local.form_dict.cmd)
	...
	def test_array_values_in_request_args(self):
		...
		params = {
			...
			"sid": saashq.session.sid,
		}
		headers = {
			"accept": "application/json",
			"content-type": "application/json",
		}
		url = get_site_url(saashq.local.site)
		url += "/api/method/saashq.client.get_list"
	...
	def test_client_get(self):
		from saashq.client import get

		todo = saashq.get_doc(doctype="ToDo", description="test").insert()
		filters = {"name": todo.name}
		filters_json = saashq.as_json(filters)
	...
	def test_client_insert(self):
		from saashq.client import insert

		def get_random_title():
			return f"test-{saashq.generate_hash()}"
		...
		# cleanup
		saashq.delete_doc("Note", note1.name)
		saashq.delete_doc("Note", note2.name)

	def test_client_insert_many(self):
		from saashq.client import insert, insert_many

		def get_random_title():
			return f"test-{saashq.generate_hash(length=5)}"
		...
		self.assertEqual(saashq.db.get_value("Note", docs[3], "title"), "not-a-random-title")
		self.assertEqual(saashq.db.get_value("Note", docs[6], "title"), "another-note-title")
		self.assertIn(note1.name, docs)

		# cleanup
		for doc in docs:
			saashq.delete_doc("Note", doc)
