...
class TestDBUpdate(IntegrationTestCase):
	def test_db_update(self):
		doctype = "User"
		saashq.reload_doctype("User", force=True)
		saashq.model.meta.trim_tables("User")
		...
		table_columns = saashq.db.get_table_columns_description(f"tab{doctype}")
		...
		for field_def in field_defs:
			...
			fallback_default = (
				"0" if field_def.get("fieldtype") in saashq.model.numeric_fieldtypes else "NULL"
			)
	...
	def test_index_and_unique_constraints(self):
		...
		saashq.db.add_index(doctype, ["email_signature(200)"])
		saashq.db.updatedb(doctype)
	...
	def test_bigint_conversion(self):
		...
		with self.assertRaises(saashq.CharacterLengthExceededError):
			saashq.get_doc(doctype=doctype.name, int_field=2**62 - 1).insert()
	...
	def test_unique_index_on_install(self):
		...
		for dt in saashq.get_all("DocType", {"is_virtual": 0, "issingle": 0}, pluck="name"):
			doctype = saashq.get_meta(dt)
	...
	def test_unique_index_on_alter(self):
		...
		new_field = saashq.copy_doc(doctype.fields[0])
	...
	def test_uuid_varchar_migration(self):
		...
		doctype.save()
		self.assertEqual(saashq.db.get_column_type(doctype.name, "name"), "uuid")

		doc = saashq.new_doc(doctype.name).insert()

		doctype.autoname = "hash"
		doctype.save()
		varchar = "varchar" if saashq.db.db_type == "mariadb" else "character varying"
		self.assertIn(varchar, saashq.db.get_column_type(doctype.name, "name"))
	...
	def test_uuid_link_field(self):
		...
		self.assertEqual(saashq.db.get_column_type(uuid_doctype.name, "name"), "uuid")
		...
		self.assertEqual(saashq.db.get_column_type(referring_doctype.name, link), "uuid")


def get_fieldtype_from_def(field_def):
	fieldtuple = saashq.db.type_map.get(field_def.fieldtype, ("", 0))
...
def get_other_fields_meta(meta):
	...
	optional_fields = saashq.db.OPTIONAL_COLUMNS
	if meta.track_seen:
		optional_fields.append("_seen")

	child_table_fields_map = {}
	if meta.istable:
		child_table_fields_map.update({field: ("Data", 0) for field in saashq.db.CHILD_TABLE_COLUMNS})
	...
	return [
		saashq._dict({"fieldname": field, "fieldtype": _type, "length": _length})
