...
import saashq
from saashq.integrations.oauth2 import encode_params
from saashq.tests import IntegrationTestCase
from saashq.tests.test_api import get_test_client, make_request, suppress_stdout
from saashq.tests.utils import make_test_records

if TYPE_CHECKING:
	from saashq.integrations.doctype.social_login_key.social_login_key import SocialLoginKey


class SaashqRequestTestCase(IntegrationTestCase):
	@property
	def sid(self) -> str:
		if not getattr(self, "_sid", None):
			...
			saashq.local.login_manager.login_as("test@example.com")
			self._sid = saashq.session.sid
...
class TestOAuth20(SaashqRequestTestCase):
	...
	def setUpClass(cls):
		...
		# Set Saashq server URL reqired for id_token generation
		saashq_login_key: "SocialLoginKey" = saashq.new_doc("Social Login Key")
		saashq_login_key.get_social_login_provider("Saashq", initialize=True)
		saashq_login_key.base_url = saashq.utils.get_url()
		saashq_login_key.enable_social_login = 0
		saashq_login_key.insert(ignore_if_duplicate=True)
		saashq.db.commit()

	def setUp(self):
		self.TEST_CLIENT = get_test_client()
		self.oauth_client = saashq.new_doc("OAuth Client")
	...
	def test_login_using_authorization_code(self):
		...
		resp = self.get(
			"/api/method/saashq.integrations.oauth2.authorize",
			{
		...
		token_response = self.post(
			"/api/method/saashq.integrations.oauth2.get_token",
	...
	def test_revoke_token(self):
		client = saashq.get_doc("OAuth Client", self.client_id)
		...
		revoke_token_response = self.post(
			"/api/method/saashq.integrations.oauth2.revoke_token",
			headers=self.form_header,
	...
	def test_login_using_implicit_token(self):
		oauth_client = saashq.get_doc("OAuth Client", self.client_id)
		...
		try:
			session.get(
				get_full_url("/api/method/saashq.integrations.oauth2.authorize"),
...
def check_valid_openid_response(access_token=None, client: "SaashqRequestTestCase" = None):
	"""Return True for valid response."""
	# Use token in header
	headers = {}
	URL = "/api/method/saashq.integrations.oauth2.openid_profile"
...
def get_full_url(endpoint):
	...
	return urljoin(saashq.utils.get_url(), endpoint)
