...
import saashq
import saashq.model
from saashq.modules import get_module_path, scrub, scrub_dt_dn
...
def export_to_files(record_list=None, record_module=None, verbose=0, create_init=None):
	...
	if record_list:
		for record in record_list:
			...
			write_document_file(
				saashq.get_doc(record[0], record[1]),
				record_module,
...
def write_document_file(doc, record_module=None, create_init=True, folder_name=None):
	...
	with open(path, "w+") as txtfile:
		txtfile.write(saashq.as_json(doc_export))
...
def strip_default_fields(doc, doc_export):
	...
	for df in doc.meta.get_table_fields():
		for d in doc_export.get(df.fieldname):
			for fieldname in saashq.model.default_fields + saashq.model.child_table_fields:
				if fieldname in d:
					del d[fieldname]
...
def get_module_name(doc):
	if doc.doctype == "Module Def":
		...
		module = saashq.db.get_value("DocType", doc.document_type, "module")
	elif hasattr(doc, "module"):
		module = doc.module
	else:
		module = saashq.db.get_value("DocType", doc.doctype, "module")

	return module


def delete_folder(module, dt, dn):
	if saashq.db.get_value("Module Def", module, "custom"):
...
def get_custom_module_path(module):
	package = saashq.db.get_value("Module Def", module, "package")
	if not package:
		saashq.throw(f"Package must be set for custom Module <b>{module}</b>")
...
def get_package_path(package):
	path = os.path.join(
		saashq.get_site_path("packages"), saashq.db.get_value("Package", package, "package_name")
	)
	if not os.path.exists(path):
		os.makedirs(path)
	return path
