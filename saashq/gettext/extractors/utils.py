...
def extract_messages_from_code(code):
	...
	from saashq.model.utils import InvalidIncludePath, render_include

	try:
		code = saashq.as_unicode(render_include(code))

	# Exception will occur when it encounters John Resig's microtemplating code
	except (TemplateError, ImportError, InvalidIncludePath, OSError) as e:
		if isinstance(e, InvalidIncludePath) and hasattr(saashq.local, "message_log"):
