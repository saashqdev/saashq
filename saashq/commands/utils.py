...
@click.option(
	...
	envvar="SAASHQ_HARD_LINK_ASSETS",
)
...
def build(
	...
	from saashq.build import bundle, download_saashq_assets
	from saashq.gettext.translate import compile_translations
	...
	with filelock("wrench_build", is_global=True, timeout=10):
		...
		if not (force or apps or os.environ.get("CI")):
			...
			skip_saashq = download_saashq_assets(verbose=verbose)
		else:
			skip_saashq = False

		# don't minify in developer_mode for faster builds
		development = saashq.local.conf.developer_mode or saashq.local.dev_server
...
def watch(apps=None):
	...
	from saashq.build import watch
...
def destroy_all_sessions(context: CliCtxObj, reason=None):
	...
	for site in context.sites:
		try:
			saashq.init(site)
			saashq.connect()
			saashq.sessions.clear_all_sessions(reason)
			saashq.db.commit()
		finally:
			saashq.destroy()
	if not context.sites:
...
def show_config(context: CliCtxObj, format):
	...
	for site in context.sites:
		...
		configuration = saashq.get_site_config(sites_path=sites_path, site_path=site)
	...
	if format == "json":
		click.echo(saashq.as_json(sites_config))
...
def reset_perms(context: CliCtxObj):
	...
	from saashq.permissions import reset_perms

	for site in context.sites:
		try:
			saashq.init(site)
			saashq.connect()
			for d in saashq.db.sql_list(
				"""select name from `tabDocType`
				where istable=0 and custom=0"""
			):
				saashq.clear_cache(doctype=d)
				reset_perms(d)
		finally:
			saashq.destroy()
	if not context.sites:
...
def export_doc(context: CliCtxObj, doctype, docname):
	...
	for site in context.sites:
		try:
			saashq.init(site)
			saashq.connect()
			saashq.modules.export_doc(doctype, docname)
		finally:
			saashq.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def export_json(context: CliCtxObj, doctype, path, name=None):
	...
	from saashq.core.doctype.data_import.data_import import export_json
...
def data_import(
	...
	from saashq.core.doctype.data_import.data_import import import_file
...
def postgres(context: CliCtxObj, extra_args):
	...
	site = get_site(context)
	saashq.init(site)
	saashq.conf.db_type = "postgres"
	_enter_console(extra_args=extra_args)


def _enter_console(extra_args=None):
	from saashq.database import get_command
	from saashq.utils import get_site_path
	...
	bin, args, bin_name = get_command(
		...
		user=saashq.conf.db_user,
		password=saashq.conf.db_password,
...
def jupyter(context: CliCtxObj):
	...
	jupyter_notebooks_path = os.path.abspath(saashq.get_site_path("jupyter_notebooks"))
	sites_path = os.path.abspath(saashq.get_site_path(".."))
...
Run the following in your first cell to connect notebook to saashq
```
import saashq
...
def console(context: CliCtxObj, autoreload=False):
	...
	all_apps = saashq.get_installed_apps()
	failed_to_import = []
...
def transform_database(context: CliCtxObj, table, engine, row_format, failfast):
	...
	if table == "all":
		...
		queried_tables = (
			...
			.where(
				...
				& (information_schema.tables.table_schema == saashq.conf.db_name)
			)
			.run()
		)
	...
	for current, table in enumerate(tables):
		...
		try:
			saashq.db.sql(f"ALTER TABLE `{table}`{values_to_set}")
...
def serve(
	...
	with CodeCoverage(with_coverage, "saashq"):
		if with_coverage:
		...
		try:
			...
			if args:
				...
				if args.startswith("/api/method"):
					saashq.local.form_dict.cmd = args.split("?", 1)[0].split("/")[-1]
			elif path:
			...
			saashq.handler.execute_cmd(saashq.form_dict.cmd)

			print(saashq.response)
		finally:
			saashq.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def make_app(destination, app_name, no_git=False):
	...
	from saashq.utils.boilerplate import make_boilerplate
...
def create_patch():
	...
	from saashq.utils.boilerplate import PatchCreator
...
def get_version(output):
	...
	from saashq.utils.change_log import get_app_branch
	from saashq.utils.commands import render_table

	saashq.init("")
	data = []

	for app in sorted(saashq.get_all_apps()):
		module = saashq.get_module(app)
		app_hooks = saashq.get_module(app + ".hooks")

		app_info = saashq._dict()

		try:
			app_info.commit = Repo(saashq.get_app_source_path(app)).head.object.hexsha[:7]
...
def rebuild_global_search(context: CliCtxObj, static_pages=False):
	...
	from saashq.utils.global_search import (
