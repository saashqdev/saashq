...
def pass_context(f):
	...
	def _func(ctx, *args, **kwargs):
		...
		try:
			...
			saashq.exceptions.SiteNotSpecifiedError,
			saashq.exceptions.IncorrectSitePath,
			saashq.exceptions.CommandFailedError,
		) as e:
...
def get_site(context, raise_err=True):
	try:
		...
		if raise_err:
			raise saashq.SiteNotSpecifiedError
		return None
...
def get_commands():
	...
	clickable_link = "https://saashqframework.com/docs"
	all_commands = (
		scheduler_commands
		+ site_commands
		+ testing_commands
