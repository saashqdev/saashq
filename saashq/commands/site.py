...
def new_site(
	...
	"Create a new site"
	from saashq.installer import _new_site

	saashq.init(site, new_site=True)
...
def _restore(
	*,
	...
	from saashq.installer import extract_files
	from saashq.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key

	err, out = saashq.utils.execute_in_shell(f"file {sql_file_path}", check_exit_code=True)
	if err:
...
def restore_backup(
	...
	if is_partial(sql_file_path):
		click.secho(
			"Partial Backup file detected. You cannot use a partial file to restore a Saashq site.",
			fg="red",
		)
		click.secho(
	...
	# Check if the backup is of an older version of saashq and the user hasn't specified force
...
def partial_restore(context: CliCtxObj, sql_file_path, verbose, encryption_key=None):
	from saashq.installer import is_partial, partial_restore
	from saashq.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key
...
def _reinstall(
	...
	try:
		...
		saashq.connect()
		saashq.clear_cache()
		installed = saashq.get_installed_apps()
		saashq.clear_cache()
	except Exception:
		installed = []
	finally:
...
def install_app(context: CliCtxObj, apps, force=False):
	...
	from saashq.installer import install_app as _install_app
	from saashq.utils.synchronization import filelock
	...
	for site in context.sites:
		...
		with filelock("install_app", timeout=1):
			for app in apps:
				...
				except saashq.IncompatibleApp as err:
					err_msg = f":\n{err}" if str(err) else ""
					print(f"App {app} is Incompatible with Site {site}{err_msg}")
					exit_code = 1
				except Exception as err:
					err_msg = f": {err!s}\n{saashq.get_traceback(with_context=True)}"
...
def list_apps(context: CliCtxObj, format):
	...
	for site in context.sites:
		...
		apps = saashq.get_single("Installed Applications").installed_applications
		if apps:
			installed_apps_info.extend(format_app(app) for app in apps)
		else:
			installed_apps_info.extend(saashq.get_installed_apps())
	...
	if format == "json":
		click.echo(saashq.as_json(summary_dict))
...
def add_db_index(context: CliCtxObj, doctype, column):
	...
	for site in context.sites:
		...
		saashq.connect()
		try:
			saashq.db.add_index(doctype, columns)
			if len(columns) == 1:
...
def describe_database_table(context, doctype, column):
	...
	from saashq.core.doctype.recorder.recorder import _fetch_table_stats

	for site in context.sites:
		saashq.init(site=site)
		saashq.connect()
		try:
...
def add_system_manager(context: CliCtxObj, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		...
		saashq.connect()
		try:
			saashq.utils.user.add_system_manager(email, first_name, last_name, send_welcome_email, password)
			saashq.db.commit()
		finally:
			saashq.destroy()
	if not context.sites:
...
def disable_user(context: CliCtxObj, email):
	...
	with saashq.init_site(site):
		saashq.connect()
		user = saashq.get_doc("User", email)
		user.enabled = 0
...
def migrate(context: CliCtxObj, skip_failing=False, skip_search_index=False):
	...
	from saashq.migrate import SiteMigration
...
def migrate_to():
	...
	from saashq.integrations.saashq_providers import migrate_to
...
def run_patch(context: CliCtxObj, module, force):
	...
	for site in context.sites:
		...
		try:
			saashq.connect()
			saashq.modules.patch_handler.run_single(module, force=force or context.force)
		finally:
			saashq.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def reload_doc(context: CliCtxObj, module, doctype, docname):
	...
	for site in context.sites:
		try:
			saashq.init(site)
			saashq.connect()
			saashq.reload_doc(module, doctype, docname, force=context.force)
			saashq.db.commit()
		finally:
			saashq.destroy()
	if not context.sites:
...
def reload_doctype(context: CliCtxObj, doctype):
	...
	for site in context.sites:
		try:
			saashq.init(site)
			saashq.connect()
			saashq.reload_doctype(doctype, force=context.force)
			saashq.db.commit()
		finally:
			saashq.destroy()
	if not context.sites:
	...
	for site in context.sites:
		try:
			...
			if rollback_callback:
				...
				rollback_callback = None
			if verbose:
				print(saashq.get_traceback(with_context=True))
			exit_code = 1
			continue
		if saashq.get_system_settings("encrypt_backup") and saashq.get_site_config().encryption_key:
			click.secho(
...
def remove_from_installed_apps(context: CliCtxObj, app):
	...
	ensure_app_not_saashq(app)
	from saashq.installer import remove_from_installed_apps
...
def uninstall(context: CliCtxObj, app, dry_run, yes, no_backup, force):
	...
	ensure_app_not_saashq(app)
	from saashq.installer import remove_app
	from saashq.utils.synchronization import filelock
...
def _drop_site(
	...
	from saashq.database import drop_user_and_database
	from saashq.utils.backups import scheduled_backup
	...
	saashq.flags.root_login = db_root_username
	saashq.flags.root_password = db_root_password
...
def move(dest_dir, site):
	...
	saashq.init(site)
	old_path = saashq.utils.get_site_path()
...
def set_last_active_for_user(context: CliCtxObj, user=None):
	...
	with saashq.init_site(site):
		...
		saashq.db.set_value("User", user, "last_active", now_datetime())
		saashq.db.commit()
...
def browse(context: CliCtxObj, site, user=None):
	...
	if site not in saashq.utils.get_sites():
...
def start_ngrok(context: CliCtxObj, bind_tls, use_default_authtoken):
	...
	ngrok_authtoken = saashq.conf.ngrok_authtoken
...
def clear_log_table(context: CliCtxObj, doctype, days, no_backup):
	...
	"""
	from saashq.core.doctype.log_settings.log_settings import LOG_DOCTYPES
	from saashq.core.doctype.log_settings.log_settings import clear_log_table as clear_logs
	from saashq.utils.backups import scheduled_backup
...
def trim_database(context: CliCtxObj, dry_run, format, no_backup, yes=False):
	...
	for site in context.sites:
		...
		table_name = saashq.qb.Field("table_name").as_("name")

		database_tables: list[str] = (
			saashq.qb.from_(information_schema.tables)
			.select(table_name)
			.where(information_schema.tables.table_schema == saashq.conf.db_name)
			.where(information_schema.tables.table_type == "BASE TABLE")
			.run(pluck=True)
		)
		doctype_tables = saashq.get_all("DocType", pluck="name")
		...
		if not TABLES_TO_DROP:
			...
			for table in TABLES_TO_DROP:
				...
				saashq.db.sql_ddl(f"drop table `{table}`")

			ALL_DATA[saashq.local.site] = TABLES_TO_DROP
		saashq.destroy()
...
def get_standard_tables():
	...
	sql_file = os.path.join(
		...
		"database",
		saashq.conf.db_type,
		f"framework_{saashq.conf.db_type}.sql",
	)
...
def trim_tables(context: CliCtxObj, dry_run, format, no_backup):
	...
	from saashq.model.meta import trim_tables
	from saashq.utils.backups import scheduled_backup
	...
	for site in context.sites:
		...
		if not (no_backup or dry_run):
			click.secho(f"Taking backup for {saashq.local.site}", fg="green")
		...
		try:
			...
			if format == "table" and not dry_run:
				click.secho(f"The following data have been removed from {saashq.local.site}", fg="green")
...
def handle_data(data: dict, format="json"):
	if format == "json":
		...
		print(json.dumps({saashq.local.site: data}, indent=1, sort_keys=True))
	else:
		from saashq.utils.commands import render_table
...
def ensure_app_not_saashq(app: str) -> None:
	"""
	Ensure that the app name passed is not 'saashq'

	:param app: Name of the app
	:return: Nothing
	"""
	if app == "saashq":
		click.secho("You cannot remove or uninstall the app `saashq`", fg="red")
		sys.exit(1)
...
def bypass_patch(context: CliCtxObj, patch_name: str, yes: bool):
	...
	from saashq.modules.patch_handler import update_patch_log
