...
from saashq.desk.reportview import validate_args
from saashq.model.db_query import check_parent_permission
from saashq.model.utils import is_virtual_doctype
from saashq.utils import get_safe_filters
...
Requests via SaashqClient are also handled here.
"""
...
def get_list(
	...
	validate_args(args)
	return saashq.get_list(**args)


@saashq.whitelist()
def get_count(doctype, filters=None, debug=False, cache=False):
	return saashq.db.count(doctype, get_safe_filters(filters), debug, cache)
...
def get(doctype, name=None, filters=None, parent=None):
	...
	if name:
		...
		doc = saashq.get_doc(doctype, saashq.parse_json(filters))
	else:
		doc = saashq.get_doc(doctype)  # single
...
def get_value(doctype, fieldname, filters=None, as_dict=True, debug=False, parent=None):
	...
	if not saashq.has_permission(doctype, parent_doctype=parent):
		saashq.throw(_("No permission for {0}").format(_(doctype)), saashq.PermissionError)
	...
	try:
		fields = saashq.parse_json(fieldname)
	...
	if saashq.get_meta(doctype).issingle:
		value = saashq.db.get_values_from_single(fields, filters, doctype, as_dict=as_dict, debug=debug)
	else:
		value = get_list(
			doctype,
			filters=filters,
...
def get_single_value(doctype, field):
	...
	return saashq.db.get_single_value(doctype, field)
...
def set_value(doctype, name, fieldname, value=None):
	...
	if fieldname in (saashq.model.default_fields + saashq.model.child_table_fields):
		saashq.throw(_("Cannot edit standard fields"))
	...
	if not saashq.get_meta(doctype).istable:
		doc = saashq.get_doc(doctype, name)
		doc.update(values)
	else:
		doc = saashq.db.get_value(doctype, name, ["parenttype", "parent"], as_dict=True)
		doc = saashq.get_doc(doc.parenttype, doc.parent)
...
def insert_many(docs=None):
	...
	if len(docs) > 200:
		saashq.throw(_("Only 200 inserts allowed in one request"))
...
def rename_doc(doctype, old_name, new_name, merge=False):
	...
	new_name = saashq.rename_doc(doctype, old_name, new_name, merge=merge)
	return new_name
...
def cancel(doctype, name):
	...
	wrapper = saashq.get_doc(doctype, name)
	wrapper.cancel()
...
def bulk_update(docs):
	...
	for doc in docs:
		doc.pop("flags", None)
		try:
			existing_doc = saashq.get_doc(doc["doctype"], doc["docname"])
			existing_doc.update(doc)
			existing_doc.save()
		except Exception:
			failed_docs.append({"doc": doc, "exc": saashq.utils.get_traceback()})
...
def has_permission(doctype, docname, perm_type="read"):
	...
	return {"has_permission": saashq.has_permission(doctype, perm_type.lower(), docname)}
...
def get_doc_permissions(doctype, docname):
	...
	return {"permissions": saashq.permissions.get_doc_permissions(doc)}
...
def get_password(doctype, name, fieldname):
	...
	return saashq.get_doc(doctype, name).get_password(fieldname)


from saashq.deprecation_dumpster import get_js as _get_js

get_js = saashq.whitelist()(_get_js)


@saashq.whitelist(allow_guest=True)
def get_time_zone():
	"""Return the default time zone."""
	return {"time_zone": saashq.defaults.get_defaults().get("time_zone")}
...
def is_document_amended(doctype, docname):
	if saashq.permissions.has_permission(doctype):
		try:
			return saashq.db.exists(doctype, {"amended_from": docname})
...
def validate_link(doctype: str, docname: str, fields=None):
	if not isinstance(doctype, str):
		saashq.throw(_("DocType must be a string"))

	if not isinstance(docname, str):
		saashq.throw(_("Document Name must be a string"))

	if doctype != "DocType" and not (
		saashq.has_permission(doctype, "select") or saashq.has_permission(doctype, "read")
	):
		saashq.throw(
			_("You do not have Read or Select Permissions for {}").format(saashq.bold(doctype)),
	...
	if is_virtual_doctype(doctype):
		try:
			saashq.get_doc(doctype, docname)
			values.name = docname
		except saashq.DoesNotExistError:
			saashq.clear_last_message()
			saashq.msgprint(
				_("Document {0} {1} does not exist").format(saashq.bold(doctype), saashq.bold(docname)),
			)
		return values

	values.name = saashq.db.get_value(doctype, docname, cache=True)
	...
	try:
		...
		saashq.msgprint(
			_("You need {0} permission to fetch values from {1} {2}").format(
				saashq.bold(_("Read")), saashq.bold(doctype), saashq.bold(docname)
			),
...
def insert_doc(doc) -> "Document":
	...
	if saashq.is_table(doc.doctype):
		if not (doc.parenttype and doc.parent and doc.parentfield):
			saashq.throw(_("Parenttype, Parent and Parentfield are required to insert a child record"))

		# inserting a child record
		parent = saashq.get_doc(doc.parenttype, doc.parent)
		parent.append(doc.parentfield, doc)
		parent.save()
		return parent

	return saashq.get_doc(doc).insert()
...
def delete_doc(doctype, name):
	...
	if saashq.is_table(doctype):
		values = saashq.db.get_value(doctype, name, ["parenttype", "parent", "parentfield"])
		if not values:
			raise saashq.DoesNotExistError
		parenttype, parent, parentfield = values
		parent = saashq.get_doc(parenttype, parent)
