...
from saashq.utils import cint, cstr, flt
...
class DBTable:
	def __init__(self, doctype, meta=None):
		...
		self.meta = meta or saashq.get_meta(doctype, False)
	...
	def sync(self):
		...
		if self.is_new():
			...
			saashq.cache.hdel("table_columns", self.table_name)
			self.alter()
	...
	def get_column_definitions(self):
		column_list = [*saashq.db.DEFAULT_COLUMNS]
		ret = []
	...
	def get_columns_from_docfields(self):
		...
		if not self.meta.get("istable"):
			for fieldname in saashq.db.OPTIONAL_COLUMNS:
	...
	def validate(self):
		...
		columns = [
			saashq._dict({"fieldname": f, "fieldtype": "Data"}) for f in saashq.db.STANDARD_VARCHAR_COLUMNS
		]
		if self.meta.get("istable"):
			columns += [
				saashq._dict({"fieldname": f, "fieldtype": "Data"}) for f in saashq.db.CHILD_TABLE_COLUMNS
			]
		...
		for col in columns:
			if len(col.fieldname) >= 64:
				saashq.throw(
					_("Fieldname is limited to 64 characters ({0})").format(saashq.bold(col.fieldname))
				)

			if "varchar" in saashq.db.type_map.get(col.fieldtype, ()):
				# validate length range
				new_length = cint(col.length) or cint(saashq.db.VARCHAR_LEN)
				if not (1 <= new_length <= 1000):
					saashq.throw(_("Length of {0} should be between 1 and 1000").format(col.fieldname))
				...
				if cint(current_length) != cint(new_length):
					...
					if max_length and max_length[0][0] and max_length[0][0] > new_length:
						...
						saashq.msgprint(info_message)

	def is_new(self):
		return self.table_name not in saashq.db.get_tables()

	def setup_table_columns(self):
		# TODO: figure out a way to get key data
		for c in saashq.db.get_table_columns_description(self.table_name):
...
class DbColumn:
	...
	def get_definition(self, for_modification=False):
		...
		if self.fieldtype in ("Check", "Int"):
			...
			self.default
			and (self.default not in saashq.db.DEFAULT_SHORTCUTS)
			and not cstr(self.default).startswith(":")
		):
			default = saashq.db.escape(self.default)
	...
	if special_characters := SPECIAL_CHAR_PATTERN.findall(n):
		...
		saashq.throw(
			_("Fieldname {0} cannot have special characters like {1}").format(
				saashq.bold(cstr(n)), special_characters
			),
			saashq.db.InvalidColumnName,
		)
	return n


def validate_column_length(fieldname):
	if len(fieldname) > saashq.db.MAX_COLUMN_LENGTH:
		saashq.throw(_("Fieldname is limited to 64 characters ({0})").format(fieldname))


def get_definition(fieldtype, precision=None, length=None, *, options=None):
	d = saashq.db.type_map.get(fieldtype)
	...
	if (
		...
		and saashq.db.exists("DocType", options)
		and saashq.get_meta(options).autoname == "UUID"
	):
		d = ("uuid", None)
	...
	if fieldtype == "Int" and length and length > 11:
		...
		d = saashq.db.type_map.get("Long Int")
	...
	if size:
		...
		if length:
			if coltype == "varchar":
				...
				# NOTE: this will only be applicable for mariadb as saashq implements int
