...
from saashq.database.mariadb.schema import MariaDBTable
from saashq.utils import UnicodeWithAttrs, cstr, get_datetime, get_table_name
...
class MariaDBConnectionUtil:
	...
	def get_connection_settings(self) -> dict:
		...
		if saashq.conf.local_infile:
			conn_settings["local_infile"] = saashq.conf.local_infile

		if saashq.conf.db_ssl_ca and saashq.conf.db_ssl_cert and saashq.conf.db_ssl_key:
			conn_settings["ssl"] = {
				"ca": saashq.conf.db_ssl_ca,
				"cert": saashq.conf.db_ssl_cert,
				"key": saashq.conf.db_ssl_key,
			}
...
class MariaDBDatabase(MariaDBConnectionUtil, MariaDBExceptionUtil, Database):
	...
	def escape(s, percent=True):
		...
		# Also, given we're promoting use of the PyPika builder via saashq.qb, the use
		# of this method should be limited.

		# pymysql expects unicode argument to escape_string with Python 3
		s = saashq.as_unicode(escape_string(saashq.as_unicode(s)), "utf-8").replace("`", "\\`")
	...
	def get_column_index(self, table_name: str, fieldname: str, unique: bool = False) -> saashq._dict | None:
	...
	def get_tables(self, cached=True):
		...
		if cached:
			tables = saashq.cache.get_value("db_tables")
			to_query = not tables
		...
		if to_query:
			...
			saashq.cache.set_value("db_tables", tables)
