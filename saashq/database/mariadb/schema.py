...
from saashq.database.schema import DBTable
...
class MariaDBTable(DBTable):
	...
	def alter(self):
		...
		add_index_query = [
			...
			if not saashq.db.get_column_index(self.table_name, col.fieldname, unique=False)
		]

		if self.meta.sort_field == "modified" and not saashq.db.get_column_index(
		...
		for col in {*self.drop_index, *self.drop_unique}:
			...
			if unique_constraint_changed and not col.unique:
				if unique_index := saashq.db.get_column_index(self.table_name, col.fieldname, unique=True):
			...
			if index_constraint_changed and not col.set_index:
				if index_record := saashq.db.get_column_index(self.table_name, col.fieldname, unique=False):
	...
	def alter_primary_key(self) -> str | None:
		...
		if autoname != "UUID" and saashq.db.get_column_type(self.doctype, "name") == "uuid":
			return f"modify name varchar({saashq.db.VARCHAR_LEN})"
