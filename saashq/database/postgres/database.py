...
from saashq.database.postgres.schema import PostgresTable
from saashq.database.utils import EmptyQueryValues, LazyDecode
from saashq.utils import cstr, get_table_name
...
class PostgresExceptionUtil:
	...
	def is_statement_timeout(e):
		return PostgresDatabase.is_timedout(e) or isinstance(e, saashq.QueryTimeoutError)
...
class PostgresDatabase(PostgresExceptionUtil, Database):
	...
	@property
	def db_schema(self):
		return saashq.conf.get("db_schema", "public").replace("'", "").replace('"', "")
	...
	def get_db_table_columns(self, table) -> list[str]:
		...
		if (columns := saashq.cache.hget("table_columns", table)) is not None:
			return columns
	...
	def describe(self, doctype: str) -> list | tuple:
		...
		return self.sql(
			f"SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_NAME = '{table_name}' and table_schema='{saashq.conf.get('db_schema', 'public')}'"
		)
