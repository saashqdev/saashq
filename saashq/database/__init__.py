...
def setup_database(force, verbose=None, mariadb_user_host_login_scope=None):
	...
	if saashq.conf.db_type == "postgres":
		import saashq.database.postgres.setup_db

		return saashq.database.postgres.setup_db.setup_database()
	else:
		import saashq.database.mariadb.setup_db

		return saashq.database.mariadb.setup_db.setup_database(force, verbose, mariadb_user_host_login_scope)
...
def bootstrap_database(verbose=None, source_sql=None):
	...
	if saashq.conf.db_type == "postgres":
		...
		return saashq.database.postgres.setup_db.bootstrap_database(verbose, source_sql)
	else:
		import saashq.database.mariadb.setup_db

		return saashq.database.mariadb.setup_db.bootstrap_database(verbose, source_sql)
...
def drop_user_and_database(db_name, db_user):
	...
	if saashq.conf.db_type == "postgres":
		...
		return saashq.database.postgres.setup_db.drop_user_and_database(db_name, db_user)
	else:
		import saashq.database.mariadb.setup_db

		return saashq.database.mariadb.setup_db.drop_user_and_database(db_name, db_user)
...
def get_db(socket=None, host=None, user=None, password=None, port=None, cur_db_name=None):
	...
	if saashq.conf.db_type == "postgres":
		import saashq.database.postgres.database

		return saashq.database.postgres.database.PostgresDatabase(
			socket, host, user, password, port, cur_db_name
		)
	else:
		import saashq.database.mariadb.database

		return saashq.database.mariadb.database.MariaDBDatabase(
			socket, host, user, password, port, cur_db_name
		)
