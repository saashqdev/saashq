...
from saashq.database.utils import (
	DefaultOrderBy,
	EmptyQueryValues,
	FallBackDateTimeStr,
	FilterValue,
...
from saashq.exceptions import DoesNotExistError, ImplicitCommitError
from saashq.monitor import get_trace_id
from saashq.query_builder.functions import Count
from saashq.utils import CallbackManager, cint, get_datetime, get_table_name, getdate, now, sbool
from saashq.utils import cast as cast_fieldtype
...
class Database:
	...
	class InvalidColumnName(saashq.ValidationError):
		pass
	...
	def __init__(
		...
		self.value_cache = {}
		self.logger = saashq.logger("database")
	...
	def sql(
		...
		*,
		...
		Examples:
		        ...
		        saashq.db.sql("select name from tabCustomer", as_dict=True)

		        # return names beginning with a
		        saashq.db.sql("select name from tabCustomer where name like %s", "a%")

		        # values as dict
		        saashq.db.sql("select name from tabCustomer where name like %(name)s and owner=%(owner)s",
		                {"name": "a%", "owner":"test@example.com"})

		"""
		if isinstance(query, MySQLQueryBuilder | PostgreSQLQueryBuilder):
			saashq.log("Use run method to execute SQL queries generated by Query Builder")
		...
		if trace_id := get_trace_id():
			query += f" /* SAASHQ_TRACE_ID: {trace_id} */"

		try:
			self._cursor.execute(query, values)
		except Exception as e:
			if self.is_syntax_error(e):
				saashq.log(f"Syntax error in query:\n{query} {values or ''}")

			elif self.is_deadlocked(e):
				raise saashq.QueryDeadlockError(e) from e

			elif self.is_timedout(e):
				raise saashq.QueryTimeoutError(e) from e
				...
				saashq.throw(
					...
					exc=saashq.InReadOnlyMode,
				)

			# TODO: added temporarily
			elif self.db_type == "postgres":
				traceback.print_stack()
				saashq.log(f"Error in query:\n{e}")
				raise
		...
		if debug:
			time_end = time()
			saashq.log(f"Execution time: {time_end - time_start:.2f} sec")
	...
	def _return_as_iterator(self, *, pluck, as_dict, as_list, update):
		while result := self._transform_result(self._cursor.fetchmany(SQL_ITERATOR_BATCH_SIZE)):
			if pluck:
				...
				for row in result:
					row = saashq._dict(zip(keys, row, strict=False))
					if update:
						row.update(update)
					yield row
				...
				saashq.throw(_("`as_iterator` only works with `as_list=True` or `as_dict=True`"))
	...
	def _log_query(
		...
		if saashq.conf.allow_tests and saashq.cache.get_value("flag_print_sql"):
	...
	def mogrify(self, query: Query, values: QueryValues):
		...
		try:
			...
			if isinstance(values, dict):
				return query % {
					k: saashq.db.escape(v) if isinstance(v, str) else v for k, v in values.items()
				}
			elif isinstance(values, list | tuple):
				return query % tuple(saashq.db.escape(v) if isinstance(v, str) else v for v in values)
			return query, values
	...
	def explain_query(self, query, values=EmptyQueryValues):
		...
		saashq.log("--- query explain ---")
		try:
			results = self.sql(f"EXPLAIN {query}", values, as_dict=1)
		except Exception as e:
			saashq.log(f"error in query explain: {e}")
		else:
			saashq.log(json.dumps(results, indent=1))
			saashq.log("--- query explain end ---")
	...
	def sql_list(self, query, values=(), debug=False, **kwargs):
		...
		Example:
		        ...
		        doctypes = saashq.db.sql_list("select name from DocType")
		"""
		...
		if query.lstrip()[:6].lower() in ("update", "insert", "delete"):
			...
			if self.transaction_writes > self.MAX_WRITES_PER_TRANSACTION:
				if self.auto_commit_on_many_writes:
					...
					raise saashq.TooManyWritesError(msg)
	...
	def fetch_as_dict(self, result) -> list[saashq._dict]:
		"""Internal. Convert results to dict."""
		if result:
			keys = [column[0] for column in self._cursor.description]

		return [saashq._dict(zip(keys, row, strict=False)) for row in result]

	@staticmethod
	def clear_db_table_cache(query):
		if query and is_query_type(query, ("drop", "create")):
			saashq.cache.delete_key("db_tables")
	...
	def get_value(
		...
		*,
		...
		Example:
		        ...
		        saashq.db.get_value("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        saashq.db.get_value("User", "test@example.com", "last_login")

		        last_login, last_ip = saashq.db.get_value("User", "test@example.com",
		                ["last_login", "last_ip"])

		        # returns default date_format
		        saashq.db.get_value("System Settings", None, "date_format")
		"""
	...
	def get_values(
		...
		*,
		...
		Example:
		        ...
		        # return first customer starting with a
		        customers = saashq.db.get_values("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        user = saashq.db.get_values("User", "test@example.com", "*")[0]
		"""
		out = None
		cache_key = None
		...
		if isinstance(filters, list):
			...
			if (filters is not None) and (filters != doctype or doctype == "DocType"):
				try:
					...
					if ignore and (
						saashq.db.is_missing_column(e)
						or saashq.db.is_table_missing(e)
						or str(e).startswith("Invalid DocType")
					):
						out = None
					elif (not ignore) and saashq.db.is_table_missing(e):
	...
	def get_singles_dict(self, doctype, debug=False, *, for_update=False, cast=False):
		...
		Example:
		        ...
		        account_settings = saashq.db.get_singles_dict("Accounts Settings")
		"""
		queried_result = saashq.qb.get_query(
			"Singles",
		...
		if not cast:
			return saashq._dict(queried_result)
	...
	def get_all(*args, **kwargs):
		return saashq.get_all(*args, **kwargs)

	@staticmethod
	def get_list(*args, **kwargs):
		return saashq.get_list(*args, **kwargs)
	...
	def _get_update_dict(
		...
		if update_modified:
			...
			modified_by = modified_by or saashq.session.user
	...
	def set_single_value(
		...
		*,
		...
		Example:
		        ...
		        saashq.db.set_single_value("System Settings", "deny_multiple_sessions", True)
		"""
		...
		saashq.qb.into("Singles").columns("doctype", "field", "value").insert(*singles_data).run(debug=debug)
		saashq.clear_document_cache(doctype, doctype)
	...
	def get_single_value(self, doctype: str, fieldname: str, cache: bool = True):
		...
		Example:
		        ...
		        company = saashq.db.get_single_value('Global Defaults', 'default_company')
		"""
		...
		df = saashq.get_meta(doctype).get_field(fieldname)

		if not df:
			saashq.throw(
				_("Field {0} does not exist on {1}").format(
					saashq.bold(fieldname), saashq.bold(doctype), self.InvalidColumnName
				)
			)
	...
	def set_value(
		...
		**Warning:** this function will not call Document events and should be avoided in normal cases.
		...
		"""
		from saashq.model.utils import is_single_doctype
		...
		if isinstance(dn, FilterValue):
			saashq.clear_document_cache(dt, convert_to_value(dn))
		else:
			# No way to guess which documents are modified, clear all of them
			saashq.clear_document_cache(dt)
	...
	def set_default(key, val, parent="__default", parenttype=None):
		...
		saashq.defaults.set_default(key, val, parent, parenttype)
	...
	def add_default(key, val, parent="__default", parenttype=None):
		...
		saashq.defaults.add_default(key, val, parent, parenttype)

	@staticmethod
	def get_defaults(key=None, parent="__default"):
		"""Get all defaults"""
		defaults = saashq.defaults.get_defaults_for(parent)
		if not key:
			return defaults
		...
		return defaults.get(saashq.scrub(key))

	def begin(self, *, read_only=False):
		read_only = read_only or saashq.flags.read_only
	...
	def savepoint(self, save_point):
		...
		Changes can be undone to a save point by doing saashq.db.rollback(save_point)
	...
	def a_row_exists(self, doctype):
		...
		return saashq.get_all(doctype, limit=1, order_by=None, as_list=True)
	...
	def count(self, dt, filters=None, debug=False, cache=False, distinct: bool = True):
		...
		if cache and not filters:
			cache_count = saashq.cache.get_value(f"doctype:count:{dt}")
			if cache_count is not None:
				return cache_count
		count = saashq.qb.get_query(
			table=dt,
			filters=filters,
			fields=Count("*"),
			distinct=distinct,
		...
		if not filters and cache:
			saashq.cache.set_value(f"doctype:count:{dt}", count, expires_in_sec=86400)
		return count
	...
	def get_db_table_columns(self, table) -> list[str]:
		...
		columns = saashq.cache.hget("table_columns", table)
		if columns is None:
			information_schema = saashq.qb.Schema("information_schema")

			columns = (
				saashq.qb.from_(information_schema.columns)
			...
			if columns:
				saashq.cache.hset("table_columns", table, columns)
	...
	def get_system_setting(self, key):
		return saashq.get_system_settings(key)
	...
	from saashq.deprecation_dumpster import is_column_missing as _is_column_missing
	...
	def log_touched_tables(self, query):
		if is_query_type(query, ("insert", "delete", "update", "alter", "drop", "rename")):
			...
			if saashq.flags.touched_tables is None:
				saashq.flags.touched_tables = set()
			saashq.flags.touched_tables.update(tables)
	...
	def create_sequence(self, *args, **kwargs):
		from saashq.database.sequence import create_sequence

		return create_sequence(*args, **kwargs)

	def set_next_sequence_val(self, *args, **kwargs):
		from saashq.database.sequence import set_next_val

		set_next_val(*args, **kwargs)

	def get_next_sequence_val(self, *args, **kwargs):
		from saashq.database.sequence import get_next_val
	...
	def unbuffered_cursor(self):
		...
		Usage:
		        with saashq.db.unbuffered_cursor():
		                for row in saashq.db.sql("query with huge result", as_iterator=True):
...
def get_query_execution_timeout() -> int:
	...
	if not saashq.conf.get("enable_db_statement_timeout"):
		return 0

	# Zero means no timeout, which is the default value in db.
	timeout = 0
	with suppress(Exception):
		if getattr(saashq.local, "request", None):
			timeout = saashq.conf.http_timeout or 300
