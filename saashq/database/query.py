...
from saashq.database.operator_map import OPERATOR_MAP
from saashq.database.schema import SPECIAL_CHAR_PATTERN
from saashq.database.utils import DefaultOrderBy, FilterValue, convert_to_value, get_doctype_name
from saashq.query_builder import Criterion, Field, Order, functions
from saashq.query_builder.functions import Function, SqlFunctions
from saashq.query_builder.utils import PseudoColumnMapper
from saashq.utils.data import MARIADB_SPECIFIC_COMMENT
...
# less restrictive version of saashq.core.doctype.doctype.doctype.START_WITH_LETTERS_PATTERN
...
class Engine:
	def get_query(
		...
		*,
		...
		self.is_mariadb = saashq.db.db_type == "mariadb"
		self.is_postgres = saashq.db.db_type == "postgres"
		...
		if isinstance(table, Table):
			...
			self.table = saashq.qb.DocType(table)

		if update:
			self.query = saashq.qb.update(self.table)
		elif into:
			self.query = saashq.qb.into(self.table)
		elif delete:
			self.query = saashq.qb.from_(self.table).delete()
		else:
			self.query = saashq.qb.from_(self.table)
	...
	def validate_doctype(self):
		if not TABLE_NAME_PATTERN.match(self.doctype):
			saashq.throw(_("Invalid DocType: {0}").format(self.doctype))
	...
	def _apply_filter(
		...
		if not isinstance(_field, str):
			...
			saashq.throw(_("Invalid filter: {0}").format(_field))
		elif not doctype or doctype == self.doctype:
			_field = self.table[field]
		elif doctype:
			_field = saashq.qb.DocType(doctype)[field]
		...
		if _operator in OPERATOR_MAP["nested_set"]:
			...
			_df = saashq.get_meta(self.doctype).get_field(field)
...
class Permission:
	...
	def check_permissions(cls, query, **kwargs):
		...
		for dt in doctype:
			...
			) and not saashq.has_permission(
				dt,
				"read",
				user=kwargs.get("user"),
				parent_doctype=kwargs.get("parent_doctype"),
			):
				saashq.throw(_("Insufficient Permission for {0}").format(saashq.bold(dt)))
	...
	def parse(field: str, doctype: str):
		if "." in field:
			...
			if field.startswith("`tab") or field.startswith('"tab'):
				...
				linked_field = saashq.get_meta(doctype).get_field(linked_fieldname)
				linked_doctype = linked_field.options
				if linked_field.fieldtype == "Link":
					return LinkTableField(linked_doctype, fieldname, doctype, linked_fieldname, alias=alias)
				elif linked_field.fieldtype in saashq.model.table_fields:
...
class ChildTableField(DynamicTableField):
	def __init__(
		...
		self.alias = alias
		self.parent_doctype = parent_doctype
		self.table = saashq.qb.DocType(self.doctype)
	...
	def apply_join(self, query: QueryBuilder) -> QueryBuilder:
		table = saashq.qb.DocType(self.doctype)
		main_table = saashq.qb.DocType(self.parent_doctype)
...
class ChildQuery:
	def __init__(
		...
		field = saashq.get_meta(parent_doctype).get_field(fieldname)
		if field.fieldtype not in saashq.model.table_fields:
			return
		self.fieldname = fieldname
		self.fields = fields
		self.parent_doctype = parent_doctype
...
def get_nested_set_hierarchy_result(doctype: str, name: str, hierarchy: str) -> list[str]:
	...
	table = saashq.qb.DocType(doctype)
	try:
		lft, rgt = saashq.qb.from_(table).select("lft", "rgt").where(table.name == name).run()[0]
	except IndexError:
		lft, rgt = None, None
