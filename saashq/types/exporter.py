...
class TypeExporter:
	def __init__(self, doc):
		...
		self.imports = {"from saashq.types import DF"}
		self.indent = "\t"
		self.controller_path = (
			Path(saashq.get_module_path(doc.module))
			/ "doctype"
			/ scrub(self.doctype)
	...
	def _validate_code(code) -> bool:
		"""Make sure whatever code Saashq adds dynamically is valid python."""
		try:
			ast.parse(code)
			return True
		except Exception:
			saashq.msgprint(saashq._("Failed to export python type hints"), alert=True)
			return False
