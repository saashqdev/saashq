...
login.bind_events = function () {
	...
	$(".form-login").on("submit", function (event) {
		...
		var args = {};
		args.cmd = "login";
		args.usr = saashq.utils.xss_sanitise(($("#login_email").val() || "").trim());
		args.pwd = $("#login_password").val();
		if (!args.usr || !args.pwd) {
			{# striptags is used to remove newlines, e is used for escaping #}
			saashq.msgprint("{{ _('Both login and password required') | striptags | e }}");
			return false;
		}
	...
	$(".form-signup").on("submit", function (event) {
		event.preventDefault();
		var args = {};
		args.cmd = "saashq.core.doctype.user.user.sign_up";
		args.email = ($("#signup_email").val() || "").trim();
		args.redirect_to = saashq.utils.sanitise_redirect(saashq.utils.get_url_arg("redirect-to"));
		args.full_name = saashq.utils.xss_sanitise(($("#signup_fullname").val() || "").trim());
	...
	$(".form-forgot").on("submit", function (event) {
		event.preventDefault();
		var args = {};
		args.cmd = "saashq.core.doctype.user.user.reset_password";
	...
	$(".form-login-with-email-link").on("submit", function (event) {
		event.preventDefault();
		var args = {};
		args.cmd = "saashq.www.login.send_login_link";
	...
	var login_handlers = {
		200: function (data) {
			if (data.message == 'Logged In') {
				...
				window.location.href = saashq.utils.sanitise_redirect(saashq.utils.get_url_arg("redirect-to")) || data.home_page;
			} else if (data.message == 'Password Reset') {
				window.location.href = saashq.utils.sanitise_redirect(data.redirect_to);
			} else if (data.message == "No App") {
				login.set_status({{ _("Success") | tojson }}, 'green');
				if (localStorage) {
					var last_visited =
						localStorage.getItem("last_visited")
						|| saashq.utils.sanitise_redirect(saashq.utils.get_url_arg("redirect-to"));
				...
				if (cint(data.message[0]) == 0) {
					...
					login.set_status({{ _("Success") | tojson }}, 'green');
					saashq.msgprint(data.message[1])
				}
...
var verify_token = function (event) {
	$(".form-verify").on("submit", function (eventx) {
		...
		args.tmp_id = saashq.get_cookie('tmp_id');
		if (!args.otp) {
			{# striptags is used to remove newlines, e is used for escaping #}
			saashq.msgprint("{{ _('Login token required') | striptags | e }}");
			return false;
		}
		login.call(args);
		return false;
