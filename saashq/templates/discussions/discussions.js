saashq.ready(() => {
	...
	saashq.require("controls.bundle.js", () => {
...
const setup_socket_io = () => {
	saashq.realtime.init(window.socketio_port || "9000");

	saashq.realtime.on("publish_message", (data) => {
		publish_message(data);
	});

	saashq.realtime.on("update_message", (data) => {
		update_message(data);
	});

	saashq.realtime.socket.on("delete_message", (data) => {
		delete_message(data);
	});
};
...
const insert_message = (data) => {
	...
	if ($(`.discussion-on-page[data-topic=${topic.name}]`).length) {
		...
		if (topic.owner == saashq.session.user) {
	...
	} else if (topic.owner == saashq.session.user && document_match_found) {
...
const submit_discussion = (e) => {
	...
	if (strip_html(reply).trim() != "" || reply.includes("img")) {
		...
		saashq.call({
			method: "saashq.website.doctype.discussion_topic.discussion_topic.submit_discussion",
			args: {
...
const get_color_from_palette = (element) => {
	const palette = saashq.get_palette(element.attr("title"));
...
const delete_reply = (e) => {
	saashq.call({
		method: "saashq.website.doctype.discussion_reply.discussion_reply.delete_message",
		args: {
...
const hide_actions_on_conditions = (template, owner) => {
	...
	saashq.session.user != owner && $template.find(".dropdown").remove();
	return $template.prop("outerHTML");
};
...
const make_comment_editor = (element) => {
	this.comment_editor = new saashq.ui.FieldGroup({
		fields: [
			{
	...
	element.find(".saashq-control").removeClass("hide-control");
