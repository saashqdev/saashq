...
from saashq.utils import cint, get_datetime, get_url, time_diff_in_seconds
from saashq.utils.background_jobs import enqueue
from saashq.utils.password import decrypt, encrypt

PARENT_FOR_DEFAULTS = "__2fa"


def get_default(key):
	return saashq.db.get_default(key, parent=PARENT_FOR_DEFAULTS)


def set_default(key, value):
	saashq.db.set_default(key, value, parent=PARENT_FOR_DEFAULTS)


def clear_default(key):
	saashq.defaults.clear_default(key, parent=PARENT_FOR_DEFAULTS)
...
def toggle_two_factor_auth(state, roles=None):
	...
	for role in roles or []:
		role = saashq.get_doc("Role", {"role_name": role})
...
def two_factor_is_enabled(user=None):
	...
	enabled = cint(saashq.get_system_settings("enable_two_factor_auth"))
	if enabled:
		bypass_two_factor_auth = cint(saashq.get_system_settings("bypass_2fa_for_retricted_ip_users"))
		if bypass_two_factor_auth and user:
			...
			if restrict_ip_list and saashq.local.request_ip:
				for ip in restrict_ip_list:
					if saashq.local.request_ip.startswith(ip):
						enabled = False
						break
...
def get_cached_user_pass():
	...
	user = pwd = None
	tmp_id = saashq.form_dict.get("tmp_id")
	if tmp_id:
		user = saashq.safe_decode(saashq.cache.get(tmp_id + "_usr"))
		pwd = saashq.safe_decode(saashq.cache.get(tmp_id + "_pwd"))
	return (user, pwd)


def authenticate_for_2factor(user):
	"""Authenticate two factor for enabled user before login."""
	if saashq.form_dict.get("otp"):
		return
	otp_secret = get_otpsecret_for_(user)
	token = int(pyotp.TOTP(otp_secret).now())
	tmp_id = saashq.generate_hash(length=8)
	cache_2fa_data(user, token, otp_secret, tmp_id)
	verification_obj = get_verification_obj(user, token, otp_secret)
	# Save data in local
	saashq.local.response["verification"] = verification_obj
	saashq.local.response["tmp_id"] = tmp_id


def cache_2fa_data(user, token, otp_secret, tmp_id):
	"""Cache and set expiry for data."""
	pwd = saashq.form_dict.get("pwd")
	...
	if verification_method in ["SMS", "Email"]:
		expiry_time = saashq.flags.token_expiry or 300
		pipeline.set(tmp_id + "_token", token, expiry_time)
	else:
		expiry_time = saashq.flags.otp_expiry or 180
...
def two_factor_is_enabled_for_(user):
	...
	role_doctype = saashq.qb.DocType("Role")
	no_of_users = saashq.db.count(
		role_doctype,
...
def get_verification_method():
	return saashq.get_system_settings("two_factor_method")


def confirm_otp_token(login_manager, otp=None, tmp_id=None):
	"""Confirm otp matches."""
	from saashq.auth import get_login_attempt_tracker

	if not otp:
		otp = saashq.form_dict.get("otp")
	if not otp:
	...
	if not tmp_id:
		tmp_id = saashq.form_dict.get("tmp_id")
	hotp_token = saashq.cache.get(tmp_id + "_token")
	otp_secret = saashq.cache.get(tmp_id + "_otp_secret")
	if not otp_secret:
	...
	if hotp_token:
		if hotp.verify(otp, int(hotp_token)):
			saashq.cache.delete(tmp_id + "_token")
...
def get_verification_obj(user, token, otp_secret):
	otp_issuer = saashq.get_system_settings("otp_issuer_name")
	verification_method = get_verification_method()
...
def process_2fa_for_sms(user, token, otp_secret):
	...
	phone = saashq.db.get_value("User", user, ["phone", "mobile_no"], as_dict=1)
...
def get_email_subject_for_2fa(kwargs_dict):
	...
	subject_template = _("Login Verification Code from {}").format(
		saashq.get_system_settings("otp_issuer_name")
	)
	return saashq.render_template(subject_template, kwargs_dict)
...
def get_email_body_for_2fa(kwargs_dict):
	...
	"""
	return saashq.render_template(body_template, kwargs_dict)
...
def get_link_for_qrcode(user, totp_uri):
	...
	key = saashq.generate_hash(length=20)
	key_user = f"{key}_user"
	key_uri = f"{key}_uri"
	lifespan = int(saashq.get_system_settings("lifespan_qrcode_image")) or 240
	saashq.cache.set_value(key_uri, totp_uri, expires_in_sec=lifespan)
	saashq.cache.set_value(key_user, user, expires_in_sec=lifespan)
...
def send_token_via_sms(otpsecret, token=None, phone_no=None):
	...
	try:
		from saashq.core.doctype.sms_settings.sms_settings import send_request
	except Exception:
		return False
	...
	ss = saashq.get_doc("SMS Settings", "SMS Settings")
...
def send_token_via_email(user, token, otp_secret, otp_issuer, subject=None, message=None):
	...
	user_email = saashq.db.get_value("User", user, "email")
	if not user_email:
		return False
	hotp = pyotp.HOTP(otp_secret)
...
def create_barcode_folder():
	...
	folder = saashq.db.exists("File", {"file_name": folder_name})
	if folder:
		return folder
	folder = saashq.get_doc({"doctype": "File", "file_name": folder_name, "is_folder": 1, "folder": "Home"})
...
def delete_qrimage(user, check_expiry=False):
	...
	for barcode in user_barcodes:
		if check_expiry and not should_remove_barcode_image(barcode):
			continue
		barcode = saashq.get_doc("File", barcode.name)
		saashq.delete_doc("File", barcode.name, ignore_permissions=True)


def delete_all_barcodes_for_users():
	"""Task to delete all barcodes for user."""

	users = saashq.get_all("User", {"enabled": 1})
	for user in users:
...
def should_remove_barcode_image(barcode):
	...
	if isinstance(barcode, str):
		barcode = saashq.get_doc("File", barcode)
	lifespan = saashq.get_system_settings("lifespan_qrcode_image") or 240
...
def disable():
	saashq.db.set_single_value("System Settings", "enable_two_factor_auth", 0)


@saashq.whitelist()
def reset_otp_secret(user: str):
	if saashq.session.user != user:
		saashq.only_for("System Manager", message=True)

	settings = saashq.get_cached_doc("System Settings")
	...
	otp_issuer = settings.otp_issuer_name or "Saashq Framework"
	user_email = saashq.get_cached_value("User", user, "email")
	...
	saashq.msgprint(_("OTP Secret has been reset. Re-registration will be required on next login."))
