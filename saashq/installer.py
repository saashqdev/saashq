...
import saashq
from saashq.defaults import _clear_cache
from saashq.utils import cint, is_git_url
...
def _is_scheduler_enabled(site) -> bool:
	...
	try:
		saashq.init(site)
		saashq.connect()
		enable_scheduler = cint(saashq.db.get_single_value("System Settings", "enable_scheduler"))
	except Exception:
		pass
	finally:
		saashq.db.close()
...
def _new_site(
	...
	"""Install a new Saashq site"""

	from saashq.utils import scheduler
	...
	if not db_name:
		db_name = f"_{saashq.generate_hash(length=16)}"
	...
	with filelock("wrench_new_site", timeout=1):
		...
		apps_to_install = ["saashq"] + (saashq.conf.get("install_apps") or []) + (list(install_apps or []))
	...
	scheduler_status = "disabled" if saashq.utils.scheduler.is_scheduler_disabled() else "enabled"
...
def install_db(
	...
	from saashq.database import bootstrap_database, drop_user_and_database, setup_database
	...
	saashq.flags.in_install_db = True

	saashq.flags.root_login = root_login
	saashq.flags.root_password = root_password
	...
	saashq.conf.admin_password = saashq.conf.admin_password or admin_password

	remove_missing_apps()

	saashq.db.create_auth_table()
	saashq.db.create_global_search_table()
	saashq.db.create_user_settings_table()

	saashq.flags.in_install_db = False
...
def find_org(org_repo: str) -> tuple[str, str]:
	...
	from saashq.exceptions import InvalidRemoteException

	for org in ["saashq", "erpnexus"]:
...
def install_app(name, verbose=False, set_as_patched=True, force=False):
	...
	from saashq.model.sync import sync_for
	from saashq.modules.utils import sync_customizations
	from saashq.utils.fixtures import sync_fixtures

	saashq.flags.in_install = name
	saashq.flags.ignore_in_install = False

	saashq.clear_cache()
	app_hooks = saashq.get_hooks(app_name=name)
	...
	if name not in saashq.get_all_apps():
	...
	for before_install in app_hooks.before_install or []:
		out = saashq.get_attr(before_install)()
		if out is False:
			return

	for fn in saashq.get_hooks("before_app_install"):
		saashq.get_attr(fn)(name)
	...
	saashq.get_doc("Portal Settings", "Portal Settings").sync_menu()

	if set_as_patched:
		set_all_patches_as_completed(name)

	for after_install in app_hooks.after_install or []:
		saashq.get_attr(after_install)()

	for fn in saashq.get_hooks("after_app_install"):
		saashq.get_attr(fn)(name)
	...
	for after_sync in app_hooks.after_sync or []:
		saashq.get_attr(after_sync)()  #

	saashq.flags.in_install = False
...
def add_to_installed_apps(app_name, rebuild_website=True):
	...
	if app_name not in installed_apps:
		...
		saashq.db.set_global("installed_apps", json.dumps(installed_apps))
		saashq.db.commit()
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	app_hooks = saashq.get_hooks(app_name=app_name)

	# dont allow uninstall app if not installed unless forced
	if not force:
		if app_name not in saashq.get_installed_apps():
	...
	for app in saashq.get_installed_apps():
		if app != app_name:
			hooks = saashq.get_hooks(app_name=app)
	...
	if not (dry_run or no_backup):
		from saashq.utils.backups import scheduled_backup

		print("Backing up...")
		scheduled_backup(ignore_files=True)

	saashq.flags.in_uninstall = True

	for before_uninstall in app_hooks.before_uninstall or []:
		saashq.get_attr(before_uninstall)()

	for fn in saashq.get_hooks("before_app_uninstall"):
		saashq.get_attr(fn)(app_name)

	modules = saashq.get_all("Module Def", filters={"app_name": app_name}, pluck="name")
	...
	for after_uninstall in app_hooks.after_uninstall or []:
		saashq.get_attr(after_uninstall)()

	for fn in saashq.get_hooks("after_app_uninstall"):
		saashq.get_attr(fn)(app_name)

	click.secho(f"Uninstalled App {app_name} from Site {site}", fg="green")
	saashq.flags.in_uninstall = False
...
def _delete_modules(modules: list[str], dry_run: bool) -> list[str]:
	...
	for module_name in modules:
		...
		for doctype in saashq.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					saashq.delete_doc("DocType", doctype.name, ignore_on_trash=True, force=True)
				else:
		...
		if not dry_run:
			saashq.delete_doc("Module Def", module_name, ignore_on_trash=True, force=True)
...
def _delete_linked_documents(module_name: str, doctype_linkfield_map: dict[str, str], dry_run: bool) -> None:
	...
	for doctype, fieldname in doctype_linkfield_map.items():
		for record in saashq.get_all(doctype, filters={fieldname: module_name}, pluck="name"):
			print(f"* removing {doctype} '{record}'...")
			if not dry_run:
				saashq.delete_doc(doctype, record, ignore_on_trash=True, force=True)
...
def _get_module_linked_doctype_field_map() -> dict[str, str]:
	...
	linked_doctypes = saashq.get_all(
		"DocField",
		filters={"fieldtype": "Link", "options": "Module Def"},
		fields=["parent", "fieldname"],
	)
	existing_linked_doctypes = [d for d in linked_doctypes if saashq.db.exists("DocType", d.parent)]
...
def _delete_doctypes(doctypes: list[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			saashq.delete_doc("DocType", doctype, ignore_on_trash=True, force=True)
			saashq.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{doctype}`")
...
def set_all_patches_as_completed(app):
	from saashq.modules.patch_handler import get_patches_from_app

	patches = get_patches_from_app(app)
	for patch in patches:
		saashq.get_doc({"doctype": "Patch Log", "patch": patch}).insert(ignore_permissions=True)
	saashq.db.commit()


def init_singles():
	singles = saashq.get_all("DocType", filters={"issingle": True}, pluck="name")
	for single in singles:
		if saashq.db.get_singles_dict(single):
			continue

		try:
			doc = saashq.new_doc(single)
			doc.flags.ignore_mandatory = True
			doc.flags.ignore_validate = True
			doc.save()
		except (ImportError, saashq.DoesNotExistError):
...
def make_conf(
	...
	sites_path = saashq.local.sites_path
	saashq.destroy()
	saashq.init(site, sites_path=sites_path)
...
def make_site_config(
	...
	saashq.create_folder(os.path.join(saashq.local.site_path))
...
def _update_config_file(key: str, value, config_file: str):
	...
	if hasattr(saashq.local, "conf"):
		saashq.local.conf[key] = value


def get_site_config_path():
	return os.path.join(saashq.local.site_path, "site_config.json")
...
def make_site_dirs():
	for dir_path in [
		...
		path = saashq.get_site_path(dir_path)
		os.makedirs(path, exist_ok=True)


def add_module_defs(app, ignore_if_duplicate=False):
	modules = saashq.get_module_list(app)
	for module in modules:
		d = saashq.new_doc("Module Def")
		d.app_name = app
...
def remove_missing_apps():
	...
	apps = ("saashq_subscription", "shopping_cart")
	installed_apps = json.loads(saashq.db.get_global("installed_apps") or "[]")
	for app in apps:
...
def extract_files(site_name, file_path):
	...
	abs_site_path = os.path.abspath(saashq.get_site_path())
...
def is_downgrade(sql_file_path, verbose=False):
	...
	current_version = Version(saashq.__version__)
	...
	if verbose and is_downgrade:
		print(f"Your site is currently on Saashq {current_version} and your backup is {backup_version}.")

	return is_downgrade


def get_old_backup_version(sql_file_path: str) -> Version | None:
	"""Return the saashq version used to create the specified database dump.

	This methods supports older versions of Saashq wich used a different format.
	"""
	header = get_db_dump_header(sql_file_path).split("\n")
	if match := re.search(r"Saashq (\d+\.\d+\.\d+)", header[0]):
...
def get_backup_version(sql_file_path: str) -> Version | None:
	...
	header = get_db_dump_header(sql_file_path).split("\n")
	metadata = ""
	if "begin saashq metadata" in header[0]:
		for line in header[1:]:
			if "end saashq metadata" in line:
				break
			metadata += line.replace("--", "").strip() + "\n"
		parser = configparser.ConfigParser()
		parser.read_string(metadata)
		return Version(parser["saashq"]["version"])
...
def partial_restore(sql_file_path, verbose=False):
	if saashq.conf.db_type == "mariadb":
		from saashq.database.mariadb.setup_db import import_db_from_sql
	elif saashq.conf.db_type == "postgres":
		import warnings

		from saashq.database.postgres.setup_db import import_db_from_sql
...
def validate_database_sql(path: str, _raise: bool = True) -> None:
	...
	if os.path.getsize(path):
		if (executable := which(executable_name)) is None:
			saashq.throw(
				...
				exc=saashq.ExecutableNotFound,
			)
		try:
			saashq.utils.execute_in_shell(f"{executable} -m1 __Auth {path}", check_exit_code=True)
			return
		except Exception:
	...
	if _raise:
		raise saashq.InvalidDatabaseFile
