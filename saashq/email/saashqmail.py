...
from saashq.saashqclient import SaashqClient, SaashqOAuth2Client
from saashq.utils import convert_utc_to_system_timezone, get_datetime, get_datetime_str, get_system_timezone


class SaashqMail:
	"""Class to interact with the Saashq Mail API."""
	...
	def get_client(
		...
		api_secret: str | None = None,
		access_token: str | None = None,
	) -> SaashqClient | SaashqOAuth2Client:
		"""Returns a SaashqClient or SaashqOAuth2Client instance."""

		if hasattr(saashq.local, "saashq_mail_clients"):
			if client := saashq.local.saashq_mail_clients.get(mailbox):
				return client
		else:
			saashq.local.saashq_mail_clients = {}

		client = (
			SaashqOAuth2Client(url=site, access_token=access_token)
			if access_token
			else SaashqClient(url=site, api_key=api_key, api_secret=api_secret)
		)
		saashq.local.saashq_mail_clients[mailbox] = client
	...
	def request(
		...
		"""Makes a request to the Saashq Mail API."""
	...
	def send_raw(self, sender: str, recipients: str | list, message: str) -> None:
		"""Sends an email using the Saashq Mail API."""
	...
	def send_newsletter(self, sender: str, recipients: str | list, message: str) -> None:
		"""Sends an newsletter using the Saashq Mail API."""
	...
	def pull_raw(self, limit: int = 50, last_synced_at: str | None = None) -> dict[str, list[str] | str]:
		"""Pulls emails from the mailbox using the Saashq Mail API."""
		...
		headers = {"X-Site": saashq.utils.get_url()}
