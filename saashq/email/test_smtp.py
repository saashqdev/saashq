...
class TestSMTP(IntegrationTestCase):
	...
	def test_get_email_account(self):
		existing_email_accounts = saashq.get_all(
		...
		for email_account in existing_email_accounts:
			saashq.db.set_value("Email Account", email_account["name"], unset_details)

		# remove mail_server config so that test@example.com is not created
		mail_server = saashq.conf.get("mail_server")
		del saashq.conf["mail_server"]

		saashq.local.outgoing_email_account = {}
		...
		saashq.conf["mail_server"] = mail_server
		for email_account in existing_email_accounts:
			set_details = {
			...
			saashq.db.set_value("Email Account", email_account["name"], set_details)
...
def create_email_account(email_id, password, enable_outgoing, default_outgoing=0, append_to=None):
	...
	email_account = saashq.new_doc("Email Account")
