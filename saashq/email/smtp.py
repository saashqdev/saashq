...
from saashq.utils import cint, cstr, get_traceback


class InvalidEmailCredentials(saashq.ValidationError):
	pass
...
class SMTPServer:
	def __init__(
		...
		if not self.server:
			saashq.msgprint(
				...
				raise_exception=saashq.OutgoingEmailError,
			)
	...
	def session(self):
		...
		try:
			...
			if not _session:
				saashq.msgprint(
					_("Could not connect to outgoing email server"), raise_exception=saashq.OutgoingEmailError
				)
			...
			if self.use_oauth:
				...
				if res[0] != 235:
					saashq.msgprint(res[1], raise_exception=saashq.OutgoingEmailError)
	...
	def _enqueue_connection_closure(self):
		if saashq.request and hasattr(saashq.request, "after_response"):
			saashq.request.after_response.add(self.quit)
		elif saashq.job:
			saashq.job.after_job.add(self.quit)
		elif not saashq.flags.in_test:
			# Console?
			import atexit
