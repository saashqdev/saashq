...
import saashq
from saashq import _, safe_encode, task
from saashq.core.utils import html2text
from saashq.database.database import savepoint
from saashq.email.doctype.email_account.email_account import EmailAccount
from saashq.email.email_body import add_attachment, get_email, get_formatted_html
from saashq.email.saashqmail import SaashqMail
from saashq.email.queue import get_unsubcribed_url, get_unsubscribe_message
...
class EmailQueue(Document):
	...
	if TYPE_CHECKING:
		from saashq.email.doctype.email_queue_recipient.email_queue_recipient import EmailQueueRecipient
	...
	def prevent_email_queue_delete(self):
		if saashq.session.user != "Administrator":
			saashq.throw(_("Only Administrator can delete Email Queue"))

	def get_duplicate(self, recipients):
		values = self.as_dict()
		del values["name"]
		duplicate = saashq.get_doc(values)
	...
	def new(cls, doc_data, ignore_permissions=False) -> "EmailQueue":
		...
		doc = saashq.new_doc(cls.DOCTYPE)
		doc.update(data)
	...
	def update_status(self, status, commit=False, **kwargs):
		...
		if self.communication:
			communication_doc = saashq.get_doc("Communication", self.communication)
	...
	def get_email_account(self, raise_error=False):
		if self.email_account:
			return saashq.get_cached_doc("Email Account", self.email_account)
	...
	def can_send_now(self):
		if (
			saashq.are_emails_muted()
			or not self.is_to_be_sent()
			or cint(saashq.db.get_default("suspend_email_queue")) == 1
		):
			return False

		return True

	def send(self, smtp_server_instance: SMTPServer = None, saashq_mail_client: SaashqMail = None):
		"""Send emails to recipients."""
		if not self.can_send_now():
			return

		with SendMailContext(self, smtp_server_instance, saashq_mail_client) as ctx:
			...
			for recipient in self.recipients:
				...
				elif not saashq.flags.in_test or saashq.flags.testing_email:
					if ctx.email_account_doc.service == "Saashq Mail":
						if self.reference_doctype == "Newsletter":
							ctx.saashq_mail_client.send_newsletter(
								sender=self.sender,
								recipients=recipient.recipient,
								message=message.decode("utf-8"),
							)
						else:
							ctx.saashq_mail_client.send_raw(
								sender=self.sender,
								recipients=recipient.recipient,
								message=message.decode("utf-8"),
							)
			...
			if saashq.flags.in_test and not saashq.flags.testing_email:
				saashq.flags.sent_mail = message
				return
	...
	def clear_old_logs(days=30):
		...
		"""
		days = days or 31
		email_queue = saashq.qb.DocType("Email Queue")
		email_recipient = saashq.qb.DocType("Email Queue Recipient")

		# Delete queue table
		(
			saashq.qb.from_(email_queue).delete().where(email_queue.creation < (Now() - Interval(days=days)))
		).run()
		...
		(
			saashq.qb.from_(email_recipient)
			.delete()
			.where(email_recipient.creation < (Now() - Interval(days=days)))
		).run()


from saashq.deprecation_dumpster import send_mail as _send_mail
...
class SendMailContext:
	def __init__(
		...
		saashq_mail_client: SaashqMail = None,
	):
		self.queue_doc: EmailQueue = queue_doc
		self.smtp_server: SMTPServer = smtp_server_instance
		self.saashq_mail_client: SaashqMail = saashq_mail_client
	...
	def fetch_outgoing_server(self):
		...
		if self.email_account_doc.service == "Saashq Mail":
			if not self.saashq_mail_client:
				self.saashq_mail_client = self.email_account_doc.get_saashq_mail_client()
	...
	def __exit__(self, exc_type, exc_val, exc_tb):
		if exc_type:
			update_fields = {"error": saashq.get_traceback()}
	...
	def get_tracker_str(self, recipient_email) -> str:
		...
		if self.queue_doc.get("email_read_tracker_url"):
			...
			tracker_url = f"{get_url()}/api/method/saashq.core.doctype.communication.email.mark_email_as_seen?name={self.queue_doc.communication}"
	...
	def include_attachments(self, message):
		...
		for attachment in attachments:
			...
			if file_filters:
				_file = saashq.get_doc("File", file_filters)
				...
				print_format_file = saashq.attach_print(**attachment)
	...
	def _store_file(self, file_name, content):
		if not saashq.get_system_settings("store_attached_pdf_document"):
			return

		file_data = saashq._dict(file_name=file_name, is_private=1)
		...
		if saashq.db.exists("File", file_data):
			return

		file = saashq.new_doc("File", **file_data)
...
def retry_sending(queues: str | list[str]):
	if not saashq.has_permission("Email Queue", throw=True):
		return
	...
	email_queue = saashq.qb.DocType("Email Queue")
	saashq.qb.update(email_queue).set(email_queue.status, "Not Sent").set(email_queue.modified, now()).set(
		email_queue.modified_by, saashq.session.user
...
def toggle_sending(enable):
	...
	saashq.db.set_default("suspend_email_queue", 0 if sbool(enable) else 1)


def on_doctype_update():
	"""Add index in `tabCommunication` for `(reference_doctype, reference_name)`"""
	saashq.db.add_index("Email Queue", ("status", "send_after", "priority", "creation"), "index_bulk_flush")

	saashq.db.add_index("Email Queue", ["message_id(140)"])


def get_email_retry_limit():
	return cint(saashq.db.get_system_setting("email_retry_limit")) or 3
...
class QueueBuilder:
	...
	def __init__(
		...
		:param unsubscribe_method: URL method for unsubscribe. Default is `/api/method/saashq.email.queue.unsubscribe`.
	...
	def unsubscribe_method(self):
		return self._unsubscribe_method or "/api/method/saashq.email.queue.unsubscribe"
	...
	def get_unsubscribed_user_emails(self):
		...
		if len(all_ids) > 0:
			unsubscribed = (
				saashq.qb.from_(EmailUnsubscribe)
	...
	def process(self, send_now=False) -> EmailQueue | None:
		...
		if not queue_separately:
			...
			if send_now and len(final_recipients) >= 1000:
				...
				send_now = False
			for recipients in saashq.utils.create_batch(final_recipients, 1000):
				saashq.enqueue(
					self.send_emails,
					queue_data=queue_data,
					final_recipients=recipients,
					job_name=saashq.utils.get_job_name(
						"send_bulk_emails_for", self.reference_doctype, self.reference_name
					),
					now=saashq.flags.in_test or send_now,
					queue="long",
				)
	...
	def send_emails(self, queue_data, final_recipients):
		...
		saashq_mail_client = None
		smtp_server_instance = None
		for r in final_recipients:
			recipients = list(set([r, *self.final_cc(), *self.bcc]))
			q = EmailQueue.new({**queue_data, **{"recipients": recipients}}, ignore_permissions=True)
			if not saashq_mail_client and not smtp_server_instance:
				email_account = q.get_email_account(raise_error=True)

				if email_account.service == "Saashq Mail":
					saashq_mail_client = email_account.get_saashq_mail_client()
				else:
					smtp_server_instance = email_account.get_smtp_server()

			with suppress(Exception):
				q.send(smtp_server_instance=smtp_server_instance, saashq_mail_client=saashq_mail_client)
	...
	def as_dict(self, include_recipients=True):
		...
		except saashq.InvalidEmailAddressError:
