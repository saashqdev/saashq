...
from saashq.core.doctype.role.role import get_info_based_on_role, get_user_info
from saashq.core.doctype.sms_settings.sms_settings import send_sms
from saashq.desk.doctype.notification_log.notification_log import enqueue_create_notification
from saashq.integrations.doctype.slack_webhook_url.slack_webhook_url import send_slack_message
from saashq.model.document import Document
from saashq.modules.utils import export_module_json, get_doc_module
from saashq.utils import add_to_date, cast, now_datetime, nowdate, validate_email_address
...
class Notification(Document):
	...
	if TYPE_CHECKING:
		from saashq.email.doctype.notification_recipient.notification_recipient import NotificationRecipient
	...
	def preview_meets_condition(self, preview_document):
		if not self.condition:
			return _("Yes")
		try:
			doc = saashq.get_cached_doc(self.document_type, preview_document)
			context = get_context(doc)
			return _("Yes") if saashq.safe_eval(self.condition, eval_locals=context) else _("No")
	...
	def preview_message(self, preview_document):
		try:
			...
			msg = saashq.render_template(self.message, context)
			if self.channel == "SMS":
				return saashq.utils.strip_html_tags(msg)
			return msg
		except Exception as e:
	...
	def preview_subject(self, preview_document):
		try:
			...
			if not self.subject:
				return _("No subject")
			if "{" in self.subject:
				return saashq.render_template(self.subject, context)
			return self.subject
		except Exception as e:
	...
	def validate(self):
		...
		if self.event in ("Days Before", "Days After") and not self.date_changed:
			saashq.throw(_("Please specify which date field must be checked"))

		if self.event in ("Minutes Before", "Minutes After"):
			if not self.datetime_changed:
				saashq.throw(_("Please specify which datetime field must be checked"))
			if not self.minutes_offset:
				saashq.throw(_("Please specify the minutes offset"))
		...
		if self.event == "Value Change" and not self.value_changed:
			saashq.throw(_("Please specify which value field must be checked"))

		self.validate_forbidden_document_types()
		self.validate_condition()
		self.validate_standard()
		saashq.cache.hdel("notifications", self.document_type)
...
"""
	...
	def validate_standard(self):
		if self.is_standard and self.enabled and not saashq.conf.developer_mode:
			saashq.throw(
				_("Cannot edit Standard Notification. To edit, please disable this and duplicate it")
			)

	def validate_condition(self):
		temp_doc = saashq.new_doc(self.document_type)
		if self.condition:
			try:
				saashq.safe_eval(self.condition, None, get_context(temp_doc.as_dict()))
			except Exception:
				saashq.throw(_("The Condition '{0}' is invalid").format(self.condition))

	def validate_forbidden_document_types(self):
		if self.document_type in FORBIDDEN_DOCUMENT_TYPES or (
			saashq.get_meta(self.document_type).istable and self.event not in DATE_BASED_EVENTS
		):
	...
	def get_documents_for_today(self):
		...
		for d in doc_list:
			doc = saashq.get_doc(self.document_type, d.name)

			if self.condition and not saashq.safe_eval(self.condition, None, get_context(doc)):
				continue
	...
	def queue_send(self, doc, enqueue_after_commit=True):
		...
		Example:
		              ...
		              ```python
		              notification = saashq.get_doc("Notification", "My Notification", ignore_permissions=True)
		...
		return enqueue(
			"saashq.email.doctype.notification.notification.evaluate_alert",
			doc=doc,
			alert=self,
		...
		if self.set_property_after_alert:
			...
			try:
				if allow_update and not doc.flags.in_notification_update:
					...
					if doc.meta.get_field(fieldname).fieldtype in saashq.model.numeric_fieldtypes:
						value = saashq.utils.cint(value)
	...
	def create_system_notification(self, doc, context):
		...
		if "{" in subject:
			subject = saashq.render_template(self.subject, context)
		...
		notification_doc = {
			...
			"email_content": saashq.render_template(self.message, context),
	...
	def send_an_email(self, doc, context):
		...
		from saashq.core.doctype.communication.email import _make as make_communication
		...
		sender = None
		message = saashq.render_template(self.message, context)
		...
		if doc.doctype != "Communication":
			...
			comm = saashq.get_doc("Communication", communication)
	...
	def send_sms(self, doc, context):
		send_sms(
			...
			msg=saashq.utils.strip_html_tags(saashq.render_template(self.message, context)),
		)
	...
	def get_list_of_recipients(self, doc, context):
		...
		for recipient in self.recipients:
			if recipient.condition:
				if not saashq.safe_eval(recipient.condition, None, context):
					continue
	...
	def get_attachment(self, doc):
		...
		print_settings = saashq.get_doc("Print Settings", "Print Settings")
			...
			return [
				{
					...
					"lang": saashq.db.get_value("Print Format", self.print_format, "default_print_language")
	...
	def get_template(self, md_as_html=False):
		...
		path = os.path.join(os.path.dirname(module.__file__), saashq.scrub(self.name))
		extension = FORMATS.get(self.message_type, ".md")
		file_path = path + extension
		...
		if extension == ".md":
			return saashq.utils.md_to_html(template)
...
def get_documents_for_today(notification):
	notification = saashq.get_doc("Notification", notification)
...
def trigger_notifications(doc, method=None):
	if saashq.flags.in_import or saashq.flags.in_patch:
	...
	if method == "daily":
		...
		for d in doc_list:
			alert = saashq.get_doc("Notification", d.name)
...
def evaluate_alert(doc: Document, alert, event=None):
	...
	try:
		if isinstance(alert, str):
			alert = saashq.get_doc("Notification", alert)

		context = get_context(doc)

		if alert.condition:
			if not saashq.safe_eval(alert.condition, None, context):
				return

		if event == "Value Change" and not doc.is_new():
			if not saashq.db.has_column(doc.doctype, alert.value_changed):
		...
		message = _("Error while evaluating Notification {0}. Please fix your template.").format(
			saashq.utils.get_link_to_form("Notification", alert.name)
		)
		saashq.throw(message, title=_("Error in Notification"))
	except Exception as e:
		title = str(e)
		message = saashq.get_traceback(with_context=True)
		saashq.log_error(title=title, message=message)
		msg = f"<details><summary>{title}</summary>{message}</details>"
		saashq.throw(msg, title=_("Error in Notification"))


def get_context(doc):
	Saashq = namedtuple("Saashq", ["saashq"])
	saashq = Saashq(saashq=get_safe_globals().get("saashq"))
	return {
		"doc": doc,
		"nowdate": nowdate,
		"saashq": saashq.saashq,
	}


def get_assignees(doc):
	assignees = []
	assignees = saashq.get_all(
		"ToDo",
...
def get_emails_from_template(template, context):
	...
	emails = saashq.render_template(template, context) if "{" in template else template
