...
saashq.notification = {
	setup_fieldname_select: function (frm) {
		...
		saashq.model.with_doctype(frm.doc.document_type, function () {
			...
			let get_receiver_fields = function (
				...
				let extract_receiver_field = function (df) {
					...
					if (saashq.model.table_fields.includes(df.fieldtype)) {
						let child_fields = saashq.get_doc("DocType", df.options).fields;
			...
			let fields = saashq.get_doc("DocType", frm.doc.document_type).fields;
			let options = $.map(fields, function (d) {
				return saashq.model.no_value_type.includes(d.fieldtype)
					? null
...
saashq.ui.form.on("Notification", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		saashq.notification.setup_fieldname_select(frm);
		saashq.notification.setup_example_message(frm);
		...
		frm.get_field("is_standard").toggle(saashq.boot.developer_mode);
		frm.trigger("event");
		if (frm.doc.document_type) {
	...
	view_properties: function (frm) {
		saashq.route_options = { doc_type: frm.doc.document_type };
	...
	event: function (frm) {
		...
		frm.add_custom_button(__("Get Alerts for Today"), function () {
			saashq.call({
				method: "saashq.email.doctype.notification.notification.get_documents_for_today",
				args: {
					notification: frm.doc.name,
				},
				callback: function (r) {
					if (r.message && r.message.length > 0) {
						saashq.msgprint(r.message.toString());
					} else {
						saashq.msgprint(__("No alerts for today"));
					}
				},
			});
		});
