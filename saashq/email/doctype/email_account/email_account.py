...
import saashq
from saashq import _, are_emails_muted, safe_encode
from saashq.desk.form import assign_to
from saashq.email.doctype.email_domain.email_domain import EMAIL_DOMAIN_FIELDS
from saashq.email.saashqmail import SaashqMail
from saashq.email.receive import EmailServer, InboundMail, SentEmailInInboxError
from saashq.email.smtp import SMTPServer
from saashq.email.utils import get_port
from saashq.model.document import Document
from saashq.utils import cint, comma_or, cstr, parse_addr, validate_email_address
...
def cache_email_account(cache_name):
	def decorator_cache_email_account(func):
		...
		def wrapper_cache_email_account(*args, **kwargs):
			if not hasattr(saashq.local, cache_name):
				setattr(saashq.local, cache_name, {})

			cached_accounts = getattr(saashq.local, cache_name)
...
class EmailAccount(Document):
	...
	if TYPE_CHECKING:
		from saashq.email.doctype.imap_folder.imap_folder import IMAPFolder
		...
		service: DF.Literal[
			"", "Saashq Mail", "GMail", "Sendgrid", "SparkPost", "Yahoo Mail", "Outlook.com", "Yandex.Mail"
		]
	...
	def validate(self):
		...
		if self.login_id_is_different:
			if not self.login_id:
				saashq.throw(_("Login Id is required"))
		else:
			self.login_id = None

		if self.service == "Saashq Mail":
			self.use_imap = 0
			self.always_use_account_email_id_as_sender = 1

			if self.auth_method == "Basic" or self.get_oauth_token():
				self.validate_saashq_mail_settings()

		# validate the imap settings
		if self.enable_incoming and self.use_imap and len(self.imap_folder) <= 0:
			saashq.throw(_("You need to set one IMAP folder for {0}").format(saashq.bold(self.email_id)))

		if saashq.local.flags.in_patch or saashq.local.flags.in_test:
			return
		...
		if (
			not saashq.local.flags.in_install
			and not self.awaiting_password
			and not self.service == "Saashq Mail"
		):
			if validate_oauth or self.password or self.smtp_server in ("127.0.0.1", "localhost"):
				...
				if self.enable_incoming or (self.enable_outgoing and not self.no_smtp_authentication):
					if not use_oauth:
						saashq.throw(_("Password is required or select Awaiting Password"))

		if self.notify_if_unreplied:
			if not self.send_notification_to:
				saashq.throw(_("{0} is mandatory").format(self.meta.get_label("send_notification_to")))
			...
			for folder in self.imap_folder:
				if folder.append_to:
					...
					if folder.append_to not in valid_doctypes:
						saashq.throw(_("Append To can be one of {0}").format(comma_or(valid_doctypes)))

	@saashq.whitelist()
	def validate_saashq_mail_settings(self):
		if self.service == "Saashq Mail":
			saashq_mail_client = self.get_saashq_mail_client()
			saashq_mail_client.validate(for_inbound=self.enable_incoming, for_outbound=self.enable_outgoing)

	def validate_smtp_conn(self):
		if not self.smtp_server:
			saashq.throw(_("SMTP Server is required"))
	...
	def before_save(self):
		...
		if not self.enable_incoming and self.default_incoming:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					saashq.bold(_("Default Incoming")),
					saashq.bold(_("Enable Incoming")),
				)
			)
		if not self.enable_outgoing and self.default_outgoing:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					saashq.bold(_("Default Outgoing")),
					saashq.bold(_("Enable Outgoing")),
				)
			)
		if messages:
			if len(messages) == 1:
	...
	def there_must_be_only_one_default(self):
		...
		for field in ("default_incoming", "default_outgoing"):
			...
			for email_account in saashq.get_all("Email Account", filters={field: 1}):
				if email_account.name == self.name:
					continue

				email_account = saashq.get_doc("Email Account", email_account.name)
				email_account.set(field, 0)
				email_account.save()

	@saashq.whitelist()
	def get_domain_values(self, domain: str):
		return saashq.db.get_value("Email Domain", domain, EMAIL_DOMAIN_FIELDS, as_dict=True)
	...
	def get_incoming_server(self, in_receive=False, email_sync_rule="UNSEEN"):
		...
		if not args.get("host"):
			saashq.throw(_("{0} is required").format("Email Server"))

		email_server = EmailServer(saashq._dict(args))
	...
	def check_email_server_connection(self, email_server, in_receive):
		...
		try:
			...
			if in_receive:
				...
				description = saashq.message_log.pop() if saashq.message_log else "Socket Error"
	...
	def _password(self):
		raise_exception = not (
			self.auth_method == "OAuth" or self.no_smtp_authentication or saashq.flags.in_test
		)
	...
	def from_record(cls, record):
		email_account = saashq.new_doc(cls.DOCTYPE)
		email_account.update(record)
		return email_account

	@classmethod
	def find(cls, name):
		return saashq.get_doc(cls.DOCTYPE, name)

	@classmethod
	def find_one_by_filters(cls, **kwargs) -> "EmailAccount":
		name = saashq.db.get_value(cls.DOCTYPE, kwargs)
	...
	def get_account_details_from_site_config(cls):
		if not saashq.conf.get("mail_server"):
			return {}
		...
		field_to_conf_name_map = {
			...
			"name": {"conf_names": ("email_sender_name",), "default": "Saashq"},
		...
		for doc_field_name, d in field_to_conf_name_map.items():
			...
			value = [saashq.conf.get(k) for k in conf_names if saashq.conf.get(k)]
	...
	def get_saashq_mail_client(self):
		return self._saashq_mail_client

	@functools.cached_property
	def _saashq_mail_client(self):
		if self.auth_method == "OAuth":
			if access_token := self.get_access_token():
				return SaashqMail(self.saashq_mail_site, self.email_id, access_token=access_token)

			saashq.throw(
				_("Please Authorize OAuth for Email Account {0}").format(
					saashq.bold(self.email_account_name)
				),
				title=_("Saashq Mail OAuth Error"),
			)
		else:
			return SaashqMail(
				self.saashq_mail_site, self.email_id, self.api_key, self.get_password("api_secret")
			)
	...
	def handle_incoming_connect_error(self, description):
		if self.get_failed_attempts_count() > 5:
			...
			saashq.enqueue(self._disable_broken_incoming_account, description=description)
		else:
	...
	def set_failed_attempts_count(self, value):
		saashq.cache.set_value(f"{self.name}:email-account-failed-attempts", value)

	def get_failed_attempts_count(self):
		return cint(saashq.cache.get_value(f"{self.name}:email-account-failed-attempts"))
	...
	def receive(self):
		...
		for mail in inbound_mails:
			try:
				...
				try:
					...
					if self.use_imap:
						self.handle_bad_emails(mail.uid, mail.raw_message, saashq.get_traceback())
					exceptions.append(saashq.get_traceback())
				except Exception:
					saashq.db.rollback()
				else:
					saashq.db.commit()
		...
		if exceptions:
			raise Exception(saashq.as_json(exceptions))
	...
	def get_inbound_mails(self) -> list[InboundMail]:
		...
		try:
			if self.service == "Saashq Mail":
				saashq_mail_client = self.get_saashq_mail_client()
				messages = saashq_mail_client.pull_raw(last_synced_at=self.last_synced_at)
	...
	def handle_bad_emails(self, uid, raw, reason):
		...
		if cint(self.use_imap):
			...
			unhandled_email = saashq.get_doc(
				{
					"raw": raw_str,
					"uid": uid,
					"reason": reason,
	...
	def send_auto_reply(self, communication, email):
		...
		from saashq.core.doctype.communication.email import (
		...
		if self.enable_auto_reply:
			...
			saashq.sendmail(
				...
				or saashq.get_template("templates/emails/auto_reply.html").render(communication.as_dict()),
	...
	def on_trash(self):
		...
		Communication = saashq.qb.DocType("Communication")
		saashq.qb.update(Communication).set(Communication.email_account, "").where(
	...
	def after_rename(self, old, new, merge=False):
		saashq.db.set_value("Email Account", new, "email_account_name", new)
	...
	def check_automatic_linking_email_account(self):
		if self.enable_automatic_linking:
			if not self.enable_incoming:
				saashq.throw(_("Automatic Linking can be activated only if Incoming is enabled."))

			if saashq.db.exists(
				"Email Account",
				{"enable_automatic_linking": 1, "name": ("!=", self.name)},
			):
				saashq.throw(_("Automatic Linking can be activated only for one Email Account."))
	...
	def get_oauth_token(self):
		if self.auth_method == "OAuth":
			connected_app = saashq.get_doc("Connected App", self.connected_app)
...
def get_append_to(doctype=None, txt=None, searchfield=None, start=None, page_len=None, filters=None):
	...
	email_append_to_list = [
		dt.name for dt in saashq.get_all("DocType", filters=filters, fields=["name", "email_append_to"])
	]
	# Set Email Append To DocTypes set via Customize Form
	email_append_to_list.extend(
		dt.doc_type
		for dt in saashq.get_list(
			"Property Setter",
...
def notify_unreplied():
	...
	for email_account in saashq.get_all(
		"Email Account",
		"name",
		...
		if append_to:
			...
			for comm in saashq.get_all(
				...
				comm = saashq.get_doc("Communication", comm.name)

				if saashq.db.get_value(comm.reference_doctype, comm.reference_name, "status") == "Open":
...
def pull(now=False):
	...
	from saashq.integrations.doctype.connected_app.connected_app import has_token

	doctype = saashq.qb.DocType("Email Account")
	email_accounts = (
	...
	for email_account in email_accounts:
		...
		if now:
			...
			queued_jobs = get_jobs(site=saashq.local.site, key="job_name")[saashq.local.site]
...
def pull_emails(email_account: str) -> None:
	...
	saashq.has_permission("Email Account", "read", throw=True)
	...
	if job_name not in queued_jobs:
		...
		saashq.msgprint(_("Emails are already being pulled from this account."))


def pull_from_email_account(email_account):
	"""Runs within a worker process"""
	email_account = saashq.get_doc("Email Account", email_account)
...
def setup_user_email_inbox(email_account, awaiting_password, email_id, enable_outgoing, used_oauth):
	...
	from saashq.core.doctype.user.user import ask_pass_update
	...
	user_names = saashq.db.get_values("User", {"email": email_id}, as_dict=True)
	if not user_names:
		return
	...
	if update_user_email_settings:
		UserEmail = saashq.qb.DocType("User Email")
		saashq.qb.update(UserEmail).set(UserEmail.awaiting_password, (awaiting_password or 0)).set(
		...
		users = " and ".join([saashq.bold(user.get("name")) for user in user_names])
		saashq.msgprint(_("Enabled email inbox for user {0}").format(users))
	ask_pass_update()
...
def remove_user_email_inbox(email_account):
	...
	for user in users:
		doc = saashq.get_doc("User", user.get("name"))
...
def set_email_password(email_account, password):
	account = saashq.get_doc("Email Account", email_account)
