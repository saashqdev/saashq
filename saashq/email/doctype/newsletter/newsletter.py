...
from saashq.email.doctype.email_group.email_group import add_subscribers
from saashq.rate_limiter import rate_limit
from saashq.utils.safe_exec import is_job_queued
from saashq.utils.verified_command import get_signed_params, verify_request
...
class Newsletter(WebsiteGenerator):
	...
	if TYPE_CHECKING:
		from saashq.email.doctype.newsletter_attachment.newsletter_attachment import NewsletterAttachment
		from saashq.email.doctype.newsletter_email_group.newsletter_email_group import NewsletterEmailGroup
	...
	def get_sending_status(self):
		count_by_status = saashq.get_all(
			"Email Queue",
		...
		emails_queued = is_job_queued(
			job_name=saashq.utils.get_job_name("send_bulk_emails_for", self.doctype, self.name),
			queue="long",
		)
	...
	def send_test_email(self, email):
		test_emails = saashq.utils.validate_email_address(email, throw=True)
		self.send_newsletter(emails=test_emails, test_email=True)
		saashq.msgprint(_("Test email sent to {0}").format(email), alert=True)
	...
	def validate_newsletter_status(self):
		if self.email_sent:
			saashq.throw(_("Newsletter has already been sent"), exc=NewsletterAlreadySentError)

		if self.get("__islocal"):
			saashq.throw(_("Please save the Newsletter before sending"), exc=NewsletterNotSavedError)

	def validate_newsletter_recipients(self):
		if not self.newsletter_recipients:
			saashq.throw(_("Newsletter should have atleast one recipient"), exc=NoRecipientFoundError)

	def validate_sender_address(self):
		"""Validate self.send_from is a valid email address or not."""
		if self.sender_email:
			saashq.utils.validate_email_address(self.sender_email, throw=True)
			self.send_from = (
	...
	def validate_publishing(self):
		if self.send_webview_link and not self.published:
			saashq.throw(_("Newsletter must be published to send webview link in email"))

	def validate_scheduling_date(self):
		if (
			self.schedule_sending
			and saashq.utils.get_datetime(self.schedule_send) < saashq.utils.now_datetime()
		):
			saashq.throw(_("Past dates are not allowed for Scheduling."))
	...
	def queue_all(self):
		...
		self.email_sent_at = saashq.utils.now()
	...
	def send_newsletter(self, emails: list[str], test_email: bool = False):
		...
		sender = self.send_from or saashq.utils.get_formatted_email(self.owner)
		args = self.as_dict()
		args["message"] = self.get_message(medium="email")

		is_auto_commit_set = bool(saashq.db.auto_commit_on_many_writes)
		saashq.db.auto_commit_on_many_writes = not saashq.flags.in_test
		...
		saashq.sendmail(
			...
			if test_email
			else "/api/method/saashq.email.doctype.newsletter.newsletter.newsletter_email_read",
		)

		saashq.db.auto_commit_on_many_writes = is_auto_commit_set

	def get_message(self, medium=None) -> str:
		message = self.message
		if self.content_type == "Markdown":
			message = saashq.utils.md_to_html(self.message_md)
		if self.content_type == "HTML":
			message = self.message_html

		html = saashq.render_template(message, {"doc": self.as_dict()})
	...
	def add_source(self, html: str, medium="None") -> str:
		...
		for link in links:
			...
			if href and not href.startswith("#"):
				if not saashq.utils.is_site_link(href):
					continue
				new_href = saashq.utils.add_trackers_to_url(
	...
	def get_email_groups(self) -> list[str]:
		...
		return [x.email_group for x in self.email_group] or saashq.get_all(
...
def confirmed_unsubscribe(email, group):
	...
	doc = saashq.get_doc("Email Group Member", {"email": email, "email_group": group})
...
def subscribe(email, email_group=None):
	...
	api_endpoint = saashq.utils.get_url(
		"/api/method/saashq.email.doctype.newsletter.newsletter.confirm_subscription"
	)
	...
	email_confirmation_template = saashq.db.get_value(
	...
	if email_confirmation_template:
		...
		email_template = saashq.get_doc("Email Template", email_confirmation_template)
		email_subject = email_template.subject
		content = saashq.render_template(email_template.response, args)
	else:
...
def confirm_subscription(email, email_group=None):
	...
	try:
		group = saashq.get_doc("Email Group", email_group)
	except saashq.DoesNotExistError:
		group = saashq.get_doc({"doctype": "Email Group", "title": email_group}).insert(
	...
	if welcome_url:
		...
		saashq.local.response["location"] = welcome_url
	else:
...
def send_scheduled_email():
	...
	scheduled_newsletter = saashq.get_all(
		"Newsletter",
		filters={
			"schedule_send": ("<=", saashq.utils.now_datetime()),
			"email_sent": False,
	...
	for newsletter_name in scheduled_newsletter:
		try:
			newsletter = saashq.get_doc("Newsletter", newsletter_name)
			...
			saashq.db.set_value("Newsletter", newsletter_name, "email_sent", 0)
...
def newsletter_email_read(recipient_email=None, reference_doctype=None, reference_name=None):
	...
	verify_request()
	try:
		doc = saashq.get_cached_doc("Newsletter", reference_name)
		if doc.add_viewed(recipient_email, force=True, unique_views=True):
			newsletter = saashq.qb.DocType("Newsletter")
			(
				saashq.qb.update(newsletter)
		...
		saashq.response.update(saashq.utils.get_imaginary_pixel_response())


def get_default_email_group():
	return _("Website", lang=saashq.db.get_default("language"))
