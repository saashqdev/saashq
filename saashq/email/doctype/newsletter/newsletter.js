...
saashq.ui.form.on("Newsletter", {
	refresh(frm) {
		let doc = frm.doc;
		let can_write = saashq.boot.user.can_write.includes(doc.doctype);
		...
		if (frm.is_new() && !doc.sender_email) {
			let { fullname, email } = saashq.user_info(doc.owner);
	...
	send_emails(frm) {
		saashq.dom.freeze(__("Queuing emails..."));
		frm.call("send_emails").then(() => {
			frm.refresh();
			saashq.dom.unfreeze();
			saashq.show_alert(
				__("Queued {0} emails", [saashq.utils.shorten_number(frm.doc.total_recipients)])
			);
		});
	},

	schedule_send_dialog(frm) {
		let hours = saashq.utils.range(24);
	...
	update_schedule_message(frm) {
		if (!frm.doc.email_sent && frm.doc.schedule_send) {
			let datetime = saashq.datetime.global_date_format(frm.doc.schedule_send);
