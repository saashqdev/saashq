...
class EmailDomain(Document):
	...
	def validate(self):
		...
		if saashq.local.flags.in_patch or saashq.local.flags.in_test or saashq.local.flags.in_install:
			return
	...
	def on_update(self):
		...
		for email_account in saashq.get_all("Email Account", filters={"domain": self.name}):
			try:
