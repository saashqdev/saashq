	...
	refresh: function (frm) {
		if (!frm.is_new()) {
			frm.add_custom_button(
				...
				function () {
					saashq.prompt(
						...
						function (data) {
							saashq.call({
								method: "saashq.email.doctype.email_group.email_group.import_from",
								args: {
									name: frm.doc.name,
			...
			frm.add_custom_button(
				...
				function () {
					saashq.prompt(
						...
						function (data) {
							saashq.call({
								method: "saashq.email.doctype.email_group.email_group.add_subscribers",
								args: {
									name: frm.doc.name,
			...
			frm.add_custom_button(
				__("New Newsletter"),
				function () {
					saashq.route_options = { email_group: frm.doc.name };
					saashq.new_doc("Newsletter");
				},
				__("Action")
			);
		}
