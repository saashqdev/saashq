...
class EMail:
	...
	def attach_file(self, n):
		...
		_file = saashq.get_doc("File", {"file_name": n})
	...
	def validate(self):
		...
		self.recipients = [strip(r) for r in self.recipients if r not in saashq.STANDARD_USERS]
		self.cc = [strip(r) for r in self.cc if r not in saashq.STANDARD_USERS]
		self.bcc = [strip(r) for r in self.bcc if r not in saashq.STANDARD_USERS]
	...
	def make(self):
		...
		for hook in saashq.get_hooks("make_email_body_message"):
			saashq.get_attr(hook)(self)
...
def get_formatted_html(
	...
	email_account=None,
	header=None,
	unsubscribe_link: saashq._dict | None = None,
	sender=None,
	with_container=False,
):
	email_account = email_account or EmailAccount.find_outgoing(match_by_email=sender)

	rendered_email = saashq.get_template("templates/emails/standard.html").render(
		{
...
def get_email_html(template, args, subject, header=None, with_container=False):
	...
	email = saashq.utils.jinja.get_email_from_template(template, args)
...
def inline_style_in_html(html):
	...
	css_files = saashq.get_hooks("email_css")
...
def get_message_id():
	...
	return email.utils.make_msgid(domain=saashq.local.site)
...
def get_footer(email_account, footer=None):
	...
	sender_address = saashq.db.get_default("email_footer_address")

	if sender_address:
		args.update({"sender_address": sender_address})

	if not cint(saashq.db.get_default("disable_standard_email_footer")):
		args.update({"default_mail_footer": saashq.get_hooks("default_mail_footer")})

	footer += saashq.utils.jinja.get_email_from_template("email_footer", args)[0]

	return footer


def replace_filename_with_cid(message):
	"""Replaces <img embed="assets/saashq/images/filename.jpg" ...> with
	...
	while True:
		...
		# found match
		img_path = groups[0]
		img_path_escaped = saashq.utils.html_utils.unescape_html(img_path)
...
def get_filecontent_from_path(path):
	...
	if path.startswith("assets/"):
		...
		# public file
		full_path = saashq.get_site_path("public", path)
	elif path.startswith("private/files/"):
		# private file
		full_path = saashq.get_site_path(path)
	else:
		full_path = path
...
def get_header(header=None):
	...
	from saashq.utils.jinja import get_email_from_template
	...
	if not title:
		title = saashq.get_hooks("app_title")[-1]
