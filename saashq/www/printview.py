...
import saashq
from saashq import _, cstr, get_module_path
from saashq.core.doctype.access_log.access_log import make_access_log
from saashq.core.doctype.document_share_key.document_share_key import is_expired
from saashq.utils import cint, escape_html, strip_html
...
if TYPE_CHECKING:
	...
	from saashq.printing.doctype.print_settings.print_settings import PrintSettings
...
def get_context(context) -> PrintContext:
	...
	if not ((saashq.form_dict.doctype and saashq.form_dict.name) or saashq.form_dict.doc):
		return PrintContext(
			print_style="",
			comment="",
			title="Error",
...
<pre>{escape_html(saashq.as_json(saashq.form_dict, indent=2))}</pre>
""",
		)

	if saashq.form_dict.doc:
		doc = saashq.form_dict.doc
	else:
		doc = saashq.get_doc(saashq.form_dict.doctype, saashq.form_dict.name)

	set_link_titles(doc)

	settings = saashq.parse_json(saashq.form_dict.settings)

	letterhead = saashq.form_dict.letterhead or None
	...
	make_access_log(
		doctype=saashq.form_dict.doctype, document=saashq.form_dict.name, file_type="PDF", method="Print"
	)
	...
	body = get_rendered_template(
		...
		meta=meta,
		trigger_print=saashq.form_dict.trigger_print,
		no_letterhead=saashq.form_dict.no_letterhead,
		letterhead=letterhead,
		settings=settings,
	)
	print_style = get_print_style(saashq.form_dict.style, print_format)

	return {
		"body": body,
		"print_style": print_style,
		"comment": saashq.session.user,
		"title": saashq.utils.strip_html(cstr(doc.get_title() or doc.name)),
		"lang": saashq.local.lang,
		"layout_direction": "rtl" if is_rtl() else "ltr",
		"doctype": saashq.form_dict.doctype,
		"name": saashq.form_dict.name,
		"key": saashq.form_dict.get("key"),
	}
...
def get_print_format_doc(print_format_name: str, meta: "Meta") -> Optional["PrintFormat"]:
	...
	if not print_format_name:
		print_format_name = saashq.form_dict.format or meta.default_print_format or "Standard"

	if print_format_name == "Standard":
		return None
	else:
		try:
			return saashq.get_doc("Print Format", print_format_name)
...
def get_rendered_template(
	...
	print_settings = saashq.get_single("Print Settings").as_dict()
	...
	if not saashq.flags.ignore_print_permissions:
		validate_print_permission(doc)

	if doc.meta.is_submittable:
		if doc.docstatus.is_draft() and not cint(print_settings.allow_print_for_draft):
			saashq.throw(_("Not allowed to print draft documents"), saashq.PermissionError)

		if doc.docstatus.is_cancelled() and not cint(print_settings.allow_print_for_cancelled):
			saashq.throw(_("Not allowed to print cancelled documents"), saashq.PermissionError)
	...
	if print_format:
		...
		template = None
		if hook_func := saashq.get_hooks("get_print_format_template"):
			template = saashq.get_attr(hook_func[-1])(jenv=jenv, print_format=print_format)
	...
	letter_head = saashq._dict(get_letter_head(doc, no_letterhead, letterhead) or {})

	if letter_head.content:
		letter_head.content = saashq.utils.jinja.render_template(letter_head.content, {"doc": doc.as_dict()})
	...
	if letter_head.footer:
		letter_head.footer = saashq.utils.jinja.render_template(letter_head.footer, {"doc": doc.as_dict()})
	...
	hook_func = saashq.get_hooks("pdf_body_html")
	html = saashq.get_attr(hook_func[-1])(jenv=jenv, template=template, print_format=print_format, args=args)
...
def set_title_values_for_link_and_dynamic_link_fields(
	...
	for field in meta.get_link_fields() + meta.get_dynamic_link_fields():
		...
		link_title = saashq.get_cached_value(doctype, doc.get(field.fieldname), meta.title_field)
		if parent_doc:
...
def set_title_values_for_table_and_multiselect_fields(meta: "Meta", doc: "Document") -> None:
	for field in meta.get_table_fields():
		...
		_meta = saashq.get_meta(field.options)
	...
	for field in doc.meta.fields:
		if field.fieldtype == "Text Editor":
			...
			if value and "<!-- markdown -->" in value:
				doc.set(field.fieldname, saashq.utils.md_to_html(value))
...
def get_html_and_style(
	...
	if isinstance(name, str):
		document = saashq.get_doc(doc, name)
	else:
		document = saashq.get_doc(json.loads(doc))
	...
	try:
		html = get_rendered_template(
			...
			settings=saashq.parse_json(settings),
		)
	except saashq.TemplateNotFoundError:
...
def get_rendered_raw_commands(doc: str, name: str | None = None, print_format: str | None = None) -> dict:
	...
	if not print_format or (print_format and not print_format.raw_printing):
		saashq.throw(
			_("{0} is not a raw printing format.").format(print_format), saashq.TemplateNotFoundError
		)
...
def validate_print_permission(doc: "Document") -> None:
	for ptype in ("read", "print"):
		if saashq.has_permission(doc.doctype, ptype, doc) or saashq.has_website_permission(doc):
			return

	key = saashq.form_dict.key
	if key and isinstance(key, str):
		validate_key(key, doc)
	else:
		raise saashq.PermissionError(_("You do not have permission to view this document"))


def validate_key(key: str, doc: "Document") -> None:
	document_key_expiry = saashq.get_cached_value(
		"Document Share Key",
	...
	if document_key_expiry is not None:
		if is_expired(document_key_expiry[0]):
			raise saashq.exceptions.LinkExpired
		else:
			return

	# TODO: Deprecate this! kept it for backward compatibility
	if saashq.get_system_settings("allow_older_web_view_links") and key == doc.get_signature():
		return

	raise saashq.exceptions.InvalidKeyError
...
def get_print_format(doctype: str, print_format: "PrintFormat") -> str:
	if print_format.disabled:
		saashq.throw(_("Print Format {0} is disabled").format(print_format.name), saashq.DoesNotExistError)

	# server, find template
	module = print_format.module or saashq.db.get_value("DocType", doctype, "module")
	path = os.path.join(
		get_module_path(module, "Print Format", print_format.name),
		saashq.scrub(print_format.name) + ".html",
	)
	...
	if os.path.exists(path):
		...
		saashq.throw(_("No template found at path: {0}").format(path), saashq.TemplateNotFoundError)
...
def make_layout(doc: "Document", meta: "Meta", format_data=None) -> list:
	...
	for df in format_data or meta.fields:
		...
		if df.fieldtype == "Signature" and not doc.get(df.fieldname):
			placeholder_image = "/assets/saashq/images/signature-placeholder.png"
...
def get_print_style(
	...
	css = saashq.get_template("templates/styles/standard.css").render(context)

	if style and saashq.db.exists("Print Style", style):
		css = css + "\n" + saashq.db.get_value("Print Style", style, "css")
...
def get_visible_columns(data: list, table_meta: "Meta", df: "DocField") -> list["DocField"]:
	...
	columns = []
	doc = data[0] or saashq.new_doc(df.options)
