...
{% if enable_view_tracking %}
	if (navigator.doNotTrack != 1 && !window.is_404) {
		saashq.ready(() => {
			let browser = saashq.utils.get_browser();
			let query_params = saashq.utils.get_query_params();

			// Get visitor ID based on browser uniqueness
			import('/assets/saashq/js/lib/fingerprintjs.js')
				.then(fingerprint_js => fingerprint_js.load())
				.then(fp => fp.get())
				.then(result => {
					saashq.call("saashq.website.doctype.web_page_view.web_page_view.make_view_log", {
