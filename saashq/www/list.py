...
from saashq.model.document import Document, get_controller
from saashq.utils import cint, quoted
from saashq.website.path_resolver import resolve_path
...
def get_context(context, **dict_params):
	...
	saashq.local.form_dict.update(dict_params)
	doctype = saashq.local.form_dict.doctype
	context.parents = [{"route": "me", "title": _("My Account")}]
	context.meta = saashq.get_meta(doctype)
	context.update(get_list_context(context, doctype) or {})
	context.doctype = doctype
	context.txt = saashq.local.form_dict.txt
	context.update(get(**saashq.local.form_dict))
...
def get(doctype, txt=None, limit_start=0, limit=20, pathname=None, **kwargs):
	...
	list_context = saashq.flags.list_context
	...
	for doc in raw_result:
		doc.doctype = doctype
		new_context = saashq._dict(doc=doc, meta=meta, list_view_fields=list_view_fields)

		if not list_context.get_list and not isinstance(new_context.doc, Document):
			new_context.doc = saashq.get_doc(doc.doctype, doc.name)
			new_context.update(new_context.doc.as_dict())

		if not saashq.flags.in_test:
			pathname = pathname or saashq.local.request.path
			new_context["pathname"] = pathname.strip("/ ")
		new_context.update(list_context)
		set_route(new_context)
		rendered_row = saashq.render_template(row_template, new_context, is_path=True)
...
def get_list_data(
	...
	if saashq.is_table(doctype):
		saashq.throw(_("Child DocTypes are not allowed"), title=_("Invalid DocType"))

	if not txt and saashq.form_dict.search:
		txt = saashq.form_dict.search
		del saashq.form_dict["search"]

	controller = get_controller(doctype)
	meta = saashq.get_meta(doctype)

	filters = prepare_filters(doctype, controller, kwargs)
	list_context = get_list_context(saashq._dict(), doctype, web_form_name)
	...
	saashq.flags.list_context = list_context
...
def prepare_filters(doctype, controller, kwargs):
	...
	if filters.pathname:
		...
		for key, val in saashq.local.form_dict.items():
...
def get_list_context(context, doctype, web_form_name=None):
	...
	from saashq.website.doctype.web_form.web_form import get_web_form_module

	list_context = context or saashq._dict()
	meta = saashq.get_meta(doctype)
	...
	def update_context_from_module(module, list_context):
		...
		if hasattr(module, "get_list_context"):
			out = saashq._dict(module.get_list_context(list_context) or {})
			if out:
				list_context = out
		return list_context
	...
	if meta.custom and web_form_name:
		webform_list_contexts = saashq.get_hooks("webform_list_context")
		if webform_list_contexts and not saashq.get_doc("Module Def", meta.module).custom:
			out = saashq._dict(saashq.get_attr(webform_list_contexts[0])(meta.module) or {})
			if out:
				list_context = out
