...
from saashq.permissions import is_system_user


def get_context(context):
	if not is_system_user():
		saashq.throw(_("You need to be a system user to access this page."), saashq.PermissionError)
...
def get_js_deps(app: str) -> list[dict]:
	package_json = Path(saashq.get_app_path(app, "..", "package.json"))
...
def get_pyproject_info(app: str) -> dict:
	pyproject_toml = Path(saashq.get_app_path(app, "..", "pyproject.toml"))
