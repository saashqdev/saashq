...
def execute():
	...
	for name, content in saashq.get_all(
		...
		saashq.db.set_value("Comment", name, "content", content, update_modified=False)
	...
	for name, content in saashq.get_all(
		...
		if content and content.startswith("Removed "):
			saashq.db.set_value("Comment", name, "content", content[8:], update_modified=False)
