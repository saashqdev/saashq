import saashq
from saashq.model.naming import is_autoincremented
...
def execute():
	...
	for doctype in possible_log_types:
		if (
			saashq.db.exists("DocType", doctype)
			and _is_implicit_int_pk(doctype)
			and not is_autoincremented(doctype)
		):
			saashq.db.change_column_type(
				doctype,
				"name",
				type=f"varchar({saashq.db.VARCHAR_LEN})",
				nullable=True,
			)
...
def _is_implicit_int_pk(doctype: str) -> bool:
	...
	if saashq.db.db_type == "mariadb":
		...
		values = (saashq.db.cur_db_name,)

	col_type = saashq.db.sql(query, values)
