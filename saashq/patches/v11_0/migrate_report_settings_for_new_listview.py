...
def execute():
	...
	reports = saashq.get_all("Report", {"report_type": "Report Builder"})

	for report_name in reports:
		settings = saashq.db.get_value("Report", report_name, "json")
		if not settings:
			continue

		settings = saashq._dict(json.loads(settings))
		...
		saashq.db.set_value("Report", report_name, "json", json.dumps(settings))
