...
import saashq
from saashq.model.utils.user_settings import sync_user_settings, update_user_settings


def execute():
	"""Update list_view's order by property from __UserSettings"""

	users = saashq.db.sql("select distinct(user) from `__UserSettings`", as_dict=True)

	for user in users:
		# get user_settings for each user
		settings = saashq.db.sql(
			f"select * from `__UserSettings` \
			where user={saashq.db.escape(user.user)}",
			as_dict=True,
		)
