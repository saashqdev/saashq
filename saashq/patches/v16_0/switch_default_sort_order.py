...
import saashq
from saashq.model.utils.user_settings import sync_user_settings
...
def execute():
	...
	for doctype in doctypes:
		...
		if table not in db_tables:
			continue
		saashq.db.add_index(doctype, ["creation"], index_name="creation")
		...
		if saashq.db.get_value("DocType", doctype, "sort_field") != "modified":
...
def update_sort_order_in_user_settings():
	...
	user_settings = saashq.db.sql("select user, doctype, data from `__UserSettings`", as_dict=1)

	for setting in user_settings:
		doctype = setting.get("doctype")
		doctype_sort_order = saashq.db.get_value("DocType", doctype, "sort_field") or "creation"
		data = setting.data
