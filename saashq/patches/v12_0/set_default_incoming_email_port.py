...
def execute():
	...
	"""
	saashq.reload_doc("email", "doctype", "email_domain", force=True)
	saashq.reload_doc("email", "doctype", "email_account", force=True)
...
def setup_incoming_email_port_in_email_domains():
	email_domains = saashq.get_all("Email Domain", ["incoming_port", "use_imap", "use_ssl", "name"])
