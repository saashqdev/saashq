...
def execute():
	...
	all_tables = saashq.db.get_tables()
	final_deletion_map = saashq._dict()

	for table in all_tables:
		indexes_to_keep_map = saashq._dict()
		indexes_to_delete = []
		index_info = saashq.db.sql(
		...
		for index in index_list:
			try:
				if is_clustered_index(table_name, index):
					continue
				saashq.db.sql_ddl(f"ALTER TABLE `{table_name}` DROP INDEX `{index}`")
