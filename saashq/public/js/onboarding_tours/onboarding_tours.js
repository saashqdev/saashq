saashq.ui.OnboardingTour = class OnboardingTour {
	constructor() {
		this.driver_steps = [];
	...
	init_driver() {
		this.driver = new saashq.Driver({
			className: "saashq-driver",
			allowClose: false,
			padding: 10,
			...
			opacity: 0.5,
			onHighlighted: (step) => {
				saashq.ui.next_form_tour = step.options.step_info?.next_form_tour;
				const wait_for_node = setInterval(() => {
					...
					if (step.popover.closeBtnNode) {
						step.popover.closeBtnNode.onclick = () => {
							...
							!saashq.boot.user.onboarding_status[this.tour.name] &&
								(saashq.boot.user.onboarding_status[this.tour.name] = {});
							saashq.boot.user.onboarding_status[this.tour.name].is_complete = true;
							if (!this.driver.hasNextStep()) {
								saashq.boot.user.onboarding_status[
									this.tour.name
								].all_steps_completed = true;
							}

							saashq.call({
								method: "saashq.desk.doctype.form_tour.form_tour.update_user_status",
								args: {
									value: JSON.stringify(saashq.boot.user.onboarding_status),
				...
				if ($input) saashq.utils.sleep(200).then(() => $input.focus());
			},
		});
	}

	async init({ tour_name, start_step }) {
		this.tour = await saashq.db.get_doc("Form Tour", tour_name);
		this.init_driver();
		this.build_steps();
	...
	get_step(step_info, on_next) {
		...
		return {
			...
			popover: {
				title,
				description,
				position: saashq.router.slug(position || "Bottom"),
			},
			onNext: on_next,
			step_info: step_info,
			showButtons,
...
saashq.ui.init_onboarding_tour = () => {
	...
	typeof saashq.boot.onboarding_tours == "undefined" && saashq.boot.onboarding_tours == [];
	typeof saashq.boot.user.onboarding_status == "undefined" &&
		saashq.boot.user.onboarding_status == {};
	let route = saashq.router.current_route;
	if (route?.[0] === "") return;
	...
	if (route[0] != "dashboard-view") {
		saashq.boot.onboarding_tours &&
			saashq.boot.onboarding_tours.forEach((tour) => {
	...
	matching_tours = matching_tours.filter((tour) => {
		if (saashq.boot.user.onboarding_status[tour[0]]?.is_complete == true) return false;
		return true;
	});
	matching_tours = matching_tours.map((tour) => {
		if (saashq.boot.user.onboarding_status[tour[0]]?.steps_complete != undefined) {
			tour.push(saashq.boot.user.onboarding_status[tour[0]].steps_complete);
		}
		return tour;
	});
	if (matching_tours.length == 0) return;
	let current_tour = matching_tours.find(
		(tour) => tour[0] == saashq.ui.currentTourInstance?.tour?.name
	);
	let next_tour = matching_tours.find((tour) => tour[0] == saashq.ui.next_form_tour);
	if (current_tour) {
		...
		saashq.ui.next_form_tour = undefined;
	} else {
	...
	if (!tour_name) return;
	if (saashq.ui.currentTourInstance?.driver) {
		saashq.ui.currentTourInstance.driver_steps = [];
		saashq.ui.currentTourInstance.driver.reset(true);
		saashq.ui.currentTourInstance.update_driver_steps();
	}
	const tour = (saashq.ui.currentTourInstance = new saashq.ui.OnboardingTour());
