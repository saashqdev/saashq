...
function edit_html() {
	let d = new saashq.ui.Dialog({
		...
		primary_action: ({ html }) => {
			html = saashq.dom.remove_script_and_style(html);
			props.df["html"] = html;
			d.hide();
		},
	});
...
function get_all_columns() {
	let meta = saashq.get_meta(props.df.options);
	let more_columns = [
		{
			label: __("Sr No."),
			value: "idx",
	...
	return more_columns.concat(
		meta.fields
			.map((tf) => {
				if (saashq.model.no_value_type.includes(tf.fieldtype)) {
					return;
				}
				return {
					label: tf.label,
...
function get_column_to_add(fieldname) {
	...
	return {
		...saashq.meta.get_docfield(props.df.options, fieldname),
		width: 10,
	};
}
