...
function clone_field(df) {
	...
	if (cloned.custom) {
		...
		cloned.fieldname += "_" + saashq.utils.get_random(8);
	}
	return cloned;
}
...
let print_templates = computed(() => {
	...
	for (let template of templates) {
		let df;
		if (template.field) {
			df = saashq.meta.get_docfield(meta.value.name, template.field);
		} else {
			df = {
				label: template.name,
				fieldname: saashq.scrub(template.name),
			};
		}
		out.push({
...
onMounted(() => {
	let method =
		"saashq.printing.page.print_format_builder_beta.print_format_builder_beta.get_google_fonts";
...
.control-font :deep(.saashq-control[data-fieldname="font"] label) {
	display: none;
}
</style>
