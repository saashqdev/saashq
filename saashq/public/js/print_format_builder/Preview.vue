...
function get_default_docname() {
	return saashq.db.get_list(doctype.value, { limit: 1 }).then((doc) => {
...
let url = computed(() => {
	...
	let _url =
		type.value == "PDF" ? `/api/method/saashq.utils.weasyprint.download_pdf` : "/printpreview";
	return `${_url}?${params.toString()}`;
});

// mounted
onMounted(() => {
	doc_select.value = saashq.ui.form.make_control({
	...
	preview_type.value = saashq.ui.form.make_control({
