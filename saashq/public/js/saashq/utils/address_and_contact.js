saashq.provide("saashq.contacts");

$.extend(saashq.contacts, {
	...
	render_address_and_contact: function (frm) {
		...
		if (frm.fields_dict["address_html"] && "addr_list" in frm.doc.__onload) {
			$(frm.fields_dict["address_html"].wrapper)
				.html(saashq.render_template("address_list", frm.doc.__onload))
				.find(".btn-address")
		...
		if (frm.fields_dict["contact_html"] && "contact_list" in frm.doc.__onload) {
			$(frm.fields_dict["contact_html"].wrapper)
				.html(saashq.render_template("contact_list", frm.doc.__onload))
				.find(".btn-contact")
				.on("click", () => new_record("Contact", frm));
		}
	},
	get_last_doc: function (frm) {
		const reverse_routes = saashq.route_history.slice().reverse();
	...
	get_address_display: function (frm, address_field, display_field) {
		...
		saashq
			.xcall("saashq.contacts.doctype.address.address.get_address_display", {
...
function new_record(doctype, frm) {
	...
	if (saashq.boot.enable_address_autocompletion === 1 && doctype === "Address") {
		new saashq.ui.AddressAutocompleteDialog({
