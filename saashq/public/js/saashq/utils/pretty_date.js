function prettyDate(date, mini) {
	...
	if (typeof date == "string") {
		date = saashq.datetime.convert_to_user_tz(date);
		date = new Date(
			(date || "")
				.replace(/-/g, "/")
				.replace(/[TZ]/g, " ")
	...
	let diff =
		(new Date(saashq.datetime.now_datetime().replace(/-/g, "/")).getTime() - date.getTime()) /
		1000;
...
window.comment_when = function (datetime, mini) {
	var timestamp = saashq.datetime.str_to_user ? saashq.datetime.str_to_user(datetime) : datetime;
	return (
		'<span class="saashq-timestamp ' +
		(mini ? " mini" : "") +
		'" data-timestamp="' +
		datetime +
		'" title="' +
...
saashq.datetime.comment_when = comment_when;
saashq.datetime.prettyDate = prettyDate;

saashq.datetime.refresh_when = function () {
	if (jQuery) {
		$(".saashq-timestamp").each(function () {
