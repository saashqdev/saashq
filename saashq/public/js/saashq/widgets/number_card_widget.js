...
export default class NumberCardWidget extends Widget {
	...
	make_card() {
		saashq.model.with_doc("Number Card", this.number_card_name || this.name).then((card) => {
			if (!card) {
	...
	create_number_card() {
		this.set_doc_args();
		return saashq
			.xcall("saashq.desk.doctype.number_card.number_card.create_number_card", {
				args: this.card_doc,
			})
			.then((doc) => {
				this.name = doc.name;
	...
	set_route() {
		...
		const route = saashq.utils.generate_route({
			name: name,
		...
		if (is_document_type) {
			...
			saashq.route_options = filters.reduce((acc, filter) => {
	...
	set_route_for_custom_card() {
		...
		if (this.data.route_options) {
			saashq.route_options = this.data.route_options;
		}

		saashq.set_route(this.data.route);
	}
	...
	get_settings(type) {
		...
		const settings_map = {
			...
			"Document Type": {
				method: "saashq.desk.doctype.number_card.number_card.get_result",
				args: {
					doc: this.card_doc,
					filters: this.filters,
				},
	...
	get_filters() {
		return saashq.dashboard_utils.get_all_filters(this.card_doc);
	}
	...
	async get_data() {
		this.data = await saashq.xcall(this.settings.method, this.settings.args);
	...
	get_number_for_doctype_card(res) {
		...
		if (this.card_doc.function !== "Count") {
			return saashq.model.with_doctype(this.card_doc.document_type, () => {
				const based_on_df = saashq.meta.get_docfield(
	...
	get_number_for_report_card(res) {
		...
		this.number = saashq.report_utils.get_result_of_fn(this.card_doc.report_function, vals);
	...
	set_formatted_number(df, doc) {
		...
		const shortened_number = saashq.utils.shorten_number(this.number, default_country, 5);
		let number_parts = shortened_number.split(" ");

		const symbol = number_parts[1] || "";
		number_parts[0] = window.convert_old_to_new_number_format(number_parts[0]);
		const formatted_number = saashq.format(number_parts[0], df, null, doc);
		...
		return this.get_percentage_stats().then(() => {
			if (this.percentage_stat == 0 || this.percentage_stat == undefined) {
				...
				caret_html = `<span class="indicator-pill-round green">
						${saashq.utils.icon("es-line-arrow-up-right", "xs")}
					</span>`;
				color_class = "green-stat";
			} else {
				caret_html = `<span class="indicator-pill-round red">
						${saashq.utils.icon("arrow-down-right", "xs")}
					</span>`;
	...
	get_percentage_stats() {
		return saashq
			.xcall("saashq.desk.doctype.number_card.number_card.get_percentage_difference", {
				doc: this.card_doc,
				filters: this.filters,
				result: this.number,
			})
			.then((res) => {
				if (res !== undefined) {
					this.percentage_stat = saashq.utils.shorten_number(res);
				}
			});
	}
		...
		let actions = [
			...
			},
			{
				...
				handler: () => {
					...
					saashq.set_route("Form", "Number Card", number_card);
				},
			},
		];
