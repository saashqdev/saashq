...
export default class OnboardingWidget extends Widget {
	async refresh() {
		saashq.utils.load_video_player();
	...
	add_step(step, index) {
		...
		let $step = $(`<a class="onboarding-step ${status}">
				<div class="step-title">
					<div class="step-index step-pending">${saashq.utils.icon(
						"es-line-success",
						"md",
						"",
						"",
					...
					<div class="step-index step-skipped">${saashq.utils.icon(
					...
					<div class="step-index step-complete">${saashq.utils.icon(
						"es-solid-success",
						"md",
						"",
						"",
	...
	show_step(step) {
		...
		let actions = {
			...
			"Create Entry": (step) => {
				if (step.is_complete) {
					saashq.set_route(`/app/List/${step.reference_document}`);
				} else {
		...
		const set_description = () => {
			let content = __(step.description)
				? saashq.markdown(__(step.description))
				: `<h1>${__(step.title)}</h1>`;

			if (step.action === "Create Entry") {
				// add a secondary action to view list
				content += `<p>
					<a href='/app/${saashq.router.slug(step.reference_document)}'>
		...
		const toggle_video = () => {
			...
			let plyr = new saashq.Plyr(video[0], {
				hideControls: true,
				resetOnEnd: true,
			});
	...
	go_to_page(step) {
		...
		saashq.set_route(step.path).then(() => {
			let message =
			...
			let msg_dialog = saashq.msgprint({
				message: message,
				title: title,
				primary_action: {
					action: () => {
	...
	open_report(step) {
		...
		let current_route = saashq.get_route();

		saashq.set_route(route).then(() => {
			let msg_dialog = saashq.msgprint({
				message: __(step.report_description),
				title: __(step.reference_report),
				primary_action: {
					action: () => {
						saashq.set_route(current_route).then(() => {
							this.mark_complete(step);
						});
						msg_dialog.hide();
					},
			...
			saashq.msg_dialog.custom_onhide = () => this.mark_complete(step);
		});
	}
	...
	show_form_tour(step) {
		let route;
		if (step.is_single) {
			route = saashq.router.slug(step.reference_document);
		} else {
			route = `${saashq.router.slug(step.reference_document)}/new`;
		}

		let current_route = saashq.get_route();

		saashq.route_hooks = {};
		saashq.route_hooks.after_load = (frm) => {
			const on_finish = () => {
				let msg_dialog = saashq.msgprint({
	...
	update_settings(step) {
		...
		saashq.route_hooks.after_save = (frm) => {
			let success = false;
			let args = {};
			...
			if (success) {
				...
				args.secondary_action = {
					action: () => saashq.set_route(current_route),
					label: __("Go Back"),
				};
			...
			saashq.msgprint(args);
			saashq.msg_dialog.custom_onhide = () => custom_onhide();
		};

		saashq.set_route("Form", step.reference_document);
	}
	...
	async create_entry(step) {
		...
		if (step.is_submittable) {
			saashq.route_hooks.after_save = () => {
				saashq.msgprint({
					message: __("Submit this document to complete this step."),
					title: __("Document Saved"),
				});
			};
			saashq.route_hooks.after_submit = callback;
		} else {
			saashq.route_hooks.after_save = callback;
		}

		saashq.set_route("Form", step.reference_document, docname);
	}

	show_quick_entry(step) {
		let current_route = saashq.get_route_str();
		saashq.ui.form.make_quick_entry(
			step.reference_document,
			() => {
				if (saashq.get_route_str() != current_route) {
					let success_dialog = saashq.msgprint({
						message: __("Let's take you back to onboarding"),
						title: __("Document Saved"),
						primary_action: {
							action: () => {
	...
	show_video(step) {
		saashq.help.show_video(step.video_url, step.title);
		this.mark_complete(step);
	}
	...
	update_step_status(step, status, value, callback) {
		...
		saashq
			.call("saashq.desk.desktop.update_onboarding_step", {
				name: step.name,
				field: status,
				value: value,
			})
	...
	show_success() {
		...
		let success_state_image =
			this.success_state_image || "/assets/saashq/images/ui-states/success-color.png";
		let documentation = "";
		if (this.docs_url) {
			documentation = __(
	...
	is_dismissed() {
		...
		if (Object.keys(dismissed).includes(this.title)) {
			...
			let today = new Date();
			let diff = saashq.datetime.get_hour_diff(today, last_hidden);
			return diff < 24;
		}
		return false;
	}
	...
	set_actions() {
		...
		dismiss.on("click", () => {
			let dismissed = JSON.parse(localStorage.getItem("dismissed-onboarding") || "{}");
			dismissed[this.title] = saashq.datetime.now_datetime();

			localStorage.setItem("dismissed-onboarding", JSON.stringify(dismissed));
			this.delete(true, true);
			this.widget.closest(".ce-block").hide();
			saashq.telemetry.capture("dismissed_" + saashq.scrub(this.title), "saashq_onboarding");
		});
		...
		return saashq.model
			...
			.then((onboarding_doc) => {
				if (onboarding_doc) {
					...
					const method =
						"saashq.desk.doctype.onboarding_step.onboarding_step.get_onboarding_steps";
					return saashq
	...
	async get_first_document(doctype) {
		...
		if (message.first_document) {
			await saashq.db.get_list(doctype, { order_by: "creation" }).then((res) => {
