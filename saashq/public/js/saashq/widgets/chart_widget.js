...
export default class ChartWidget extends Widget {
	...
	render_time_series_filters() {
		...
		saashq.dashboard_utils.render_chart_filters(filters, "chart-actions", this.action_area, 0);
	}
			...
			filters = [
				{
					...
					options: saashq.dashboard_utils.get_years_since_creation(
						saashq.boot.user.creation
					),
	...
	render_date_range_field() {
		if (!this.date_field_wrapper || !this.date_field_wrapper.is(":visible")) {
			...
			this.date_range_field = saashq.ui.form.make_control({
				df: {
					fieldtype: "DateRange",
					fieldname: "from_date",
	...
	get_report_chart_data(result) {
		if (result.chart && this.chart_doc.use_report_chart) {
			...
			let columns = result.columns.map((col) => {
				return saashq.report_utils.prepare_field_from_column(col);
			});

			return saashq.report_utils.make_chart_options(columns, result, chart_fields).data;
		}
	}
	...
	prepare_chart_actions() {
		let actions = [
			...
			},
			{
				...
				action: "action-edit",
				handler: () => {
					saashq.set_route("Form", "Dashboard Chart", this.chart_doc.name);
				},
			},
			{
		...
		if (this.chart_doc.document_type) {
			actions.push({
				...
				action: "action-list",
				handler: () => {
					saashq.set_route("List", this.chart_doc.document_type);
				},
			});
			...
			actions.push({
				...
				action: "action-list",
				handler: () => {
					saashq.set_route("query-report", this.chart_doc.report_name, this.filters);
				},
			});
		}
	...
	setup_filter_dialog(fields) {
		...
		if (this.chart_doc.chart_type == "Report") {
			...
			saashq.query_report = new saashq.views.QueryReport({ filters: dialog.fields_list });
			saashq.query_reports[this.chart_doc.report_name].onload &&
				saashq.query_reports[this.chart_doc.report_name].onload(saashq.query_report);
		}
	...
	save_chart_config_for_user(config, reset = 0) {
		...
		saashq.xcall(
			"saashq.desk.doctype.dashboard_settings.dashboard_settings.save_chart_config",
			{
				reset: reset,
	...
	create_filter_group_and_add_filters() {
		...
		this.filters &&
			saashq.model.with_doctype(this.chart_doc.document_type, () => {
	...
	fetch(filters, refresh = false, args) {
		...
		return saashq.xcall(method, args);
	}
	...
	async render() {
		let setup_dashboard_chart = () => {
			...
			if (!this.dashboard_chart) {
				this.dashboard_chart = saashq.utils.make_chart(this.chart_wrapper[0], chart_args);
			} else {
		...
		if (!this.data || !this.data.labels || !Object.keys(this.data).length) {
			...
			if (this.chart_doc.document_type) {
				saashq.model.with_doctype(this.chart_doc.document_type, setup_dashboard_chart);
			} else {
	...
	get_chart_args() {
		...
		if (this.chart_doc.document_type) {
			let doctype_meta = saashq.get_meta(this.chart_doc.document_type);
	...
	update_chart_object() {
		saashq.db.get_doc("Dashboard Chart", this.chart_doc.name).then((doc) => {
			this.chart_doc = doc;
	...
	prepare_chart_object() {
		if (this.chart_doc.type == "Heatmap" && !this.chart_doc.heatmap_year) {
			this.chart_doc.heatmap_year = saashq.dashboard_utils.get_year(
				saashq.datetime.now_date()
			);
		}
	...
	set_chart_filters() {
		...
		let chart_saved_filters = saashq.dashboard_utils.get_all_filters(this.chart_doc);

		if (this.chart_doc.chart_type == "Report") {
			return saashq.dashboard_utils
				.get_filters_for_chart_type(this.chart_doc)
				.then((filters) => {
					...
					this.filters =
						saashq.utils.parse_array(user_saved_filters) ||
						saashq.utils.parse_array(this.filters) ||
						saashq.utils.parse_array(chart_saved_filters);
				});
		} else {
			this.filters =
				saashq.utils.parse_array(user_saved_filters) ||
	...
	get_settings() {
		return saashq.model.with_doc("Dashboard Chart", this.chart_name).then((chart_doc) => {
			if (chart_doc) {
				this.chart_doc = chart_doc;
				if (this.chart_doc.chart_type == "Custom") {
					// custom source
					if (saashq.dashboards.chart_sources[this.chart_doc.source]) {
						this.settings = saashq.dashboards.chart_sources[this.chart_doc.source];
						return Promise.resolve();
					} else {
						const method =
							"saashq.desk.doctype.dashboard_chart_source.dashboard_chart_source.get_config";
						return saashq
							.xcall(method, { name: this.chart_doc.source })
							.then((config) => {
								saashq.dom.eval(config);
								this.settings =
									saashq.dashboards.chart_sources[this.chart_doc.source];
							});
					}
				} else if (this.chart_doc.chart_type == "Report") {
					this.settings = {
					...
					this.settings = {
						method: "saashq.desk.doctype.dashboard_chart.dashboard_chart.get",
					};
					return Promise.resolve();
				}
			}
