...
saashq.ui.form.Review = class Review {
	constructor({ parent, frm }) {
		this.parent = parent;
		this.frm = frm;
		this.points = saashq.boot.points;
	...
	update_points() {
		return saashq
			.xcall("saashq.social.doctype.energy_point_log.energy_point_log.get_energy_points", {
				user: saashq.session.user,
			})
			.then((data) => {
				saashq.boot.points = data;
				this.points = data;
			});
	}
	...
	show_review_dialog() {
		...
		const review_dialog = new saashq.ui.Dialog({
			title: __("Add Review"),
			fields: [
				{
					fieldname: "to_user",
			...
			primary_action: (values) => {
				...
				if (values.points > this.points.review_points) {
					return saashq.msgprint(__("You do not have enough points"));
				}
				saashq
					.xcall("saashq.social.doctype.energy_point_log.energy_point_log.review", {
						doc: {
	...
	setup_detail_popover(el, data) {
		let subject = "";
		let fullname = saashq.user.full_name(data.user);
		let timestamp = `<span class="text-muted">${saashq.datetime.comment_when(
			data.creation
		)}</span>`;
			...
			content: () => {
				return `
					<div class="text-medium">
						...
						<div class="subject">
							${saashq.utils.icon("review")}
							${subject}

							<p class="mt-1">
								<!-- ${saashq.avatar("shivam@example.com", "avatar-xs")} -->
								<span>${saashq.user.full_name(data.owner)}</span> - ${timestamp}
							</p>
						</div>
					</div>
				`;
