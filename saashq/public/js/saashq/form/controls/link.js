...
saashq.ui.form.recent_link_validations = {};

saashq.ui.form.ControlLink = class ControlLink extends saashq.ui.form.ControlData {
		...
		$(`<div class="link-field ui-front" style="position: relative;">
			...
			<span class="link-btn">
				<a class="btn-clear no-decoration">
					${saashq.utils.icon("close", "xs", "es-icon")}
				</a>
				<a class="btn-open no-decoration" title="${__("Open Link")}">
					${saashq.utils.icon("arrow-right", "xs")}
				</a>
			</span>
		...
		this.$input.on("focus", function () {
			setTimeout(function () {
				if (me.$input.val() && me.get_options()) {
					...
					me.$link.toggle(true);
					me.$link_open.attr("href", saashq.utils.get_form_link(doctype, name));
	...
	get_reference_doctype() {
		...
		if (this.doctype) return this.doctype;
			...
			return saashq.get_route && saashq.get_route()[0] === "List"
				? saashq.get_route()[1]
				: null;
		}
	}
	...
	is_translatable() {
		return (saashq.boot?.translated_doctypes || []).includes(this.get_options());
	}
	is_title_link() {
		return (saashq.boot?.link_title_doctypes || []).includes(this.get_options());
	}
	async set_link_title(value) {
		...
		const link_title =
			saashq.utils.get_link_title(doctype, value) ||
			(await saashq.utils.fetch_link_title(doctype, value));
	...
	parse_validate_and_set_in_model(value, e, label) {
		...
		if (label) {
			...
			saashq.utils.add_link_title(this.df.options, value, label);
		}
	...
	new_doc() {
		...
		if (df && df.get_route_options_for_new_doc) {
			saashq.route_options = df.get_route_options_for_new_doc(this);
		} else {
			saashq.route_options = {};
		}

		// partially entered name field
		saashq.route_options.name_field = this.get_label_value();

		// reference to calling link
		saashq._from_link = saashq.utils.deep_clone(this);
		saashq._from_link_scrollY = $(document).scrollTop();

		saashq.ui.form.make_quick_entry(doctype, (doc) => {
	...
	setup_awesomeplete() {
		...
		this.awesomplete = new Awesomplete(me.input, {
			...
			item: function (item) {
				...
				return $(`<div role="option">`)
					...
					.html(`<p title="${saashq.utils.escape_html(_label)}">${html}</p>`)
					.get(0);
			},
			sort: function () {
				return 0;
		...
		this.$input.on(
			...
			saashq.utils.debounce(function (e) {
				...
				var args = {
					...
					page_length: cint(saashq.boot.sysdefaults?.link_field_results_limit) || 10,
				};
					...
					callback: function (r) {
						...
						if (!me.df.only_select) {
							...
							let custom__link_options =
								saashq.ui.form.ControlLink.link_options &&
								saashq.ui.form.ControlLink.link_options(me);
						...
						r.message.forEach((item) => {
							saashq.utils.add_link_title(doctype, item.value, item.label);
						});
					},
				});
			}, 500)
		...
		this.$input.on("awesomplete-select", function (e) {
			...
			if (me.df.remember_last_selected_value) {
				saashq.boot.user.last_selected_values[me.df.options] = item.value;
			}
	...
	toggle_href(doctype) {
		if (saashq.model.can_select(doctype) && !saashq.model.can_read(doctype)) {
	...
	get_filter_description(filters) {
		...
		function get_filter_description(filter) {
			...
			let docfield = saashq.meta.get_docfield(doctype, fieldname);
			let label = docfield ? docfield.label : saashq.model.unscrub(fieldname);
	...
	set_custom_query(args) {
		const is_valid_value = (value, key) => {
			...
			if (this.frm) {
				let field = saashq.meta.get_docfield(this.frm.doctype, key);
	...
	parse_filters(link_filters) {
		...
		link_filters.forEach((filter) => {
			...
			if (value?.startsWith?.("eval:")) {
				...
				value = saashq.utils.eval(value, context);
			}
	...
	validate_link_and_fetch(value) {
		...
		if (value) {
			return saashq
				.xcall("saashq.client.validate_link", {
					doctype: options,
					docname: value,
