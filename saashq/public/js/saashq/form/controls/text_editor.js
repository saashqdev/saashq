...
saashq.ui.form.ControlTextEditor = class ControlTextEditor extends saashq.ui.form.ControlCode {
	make_wrapper() {
		super.make_wrapper();
	}
	...
	bind_events() {
		this.quill.on(
			"text-change",
			saashq.utils.debounce((delta, oldDelta, source) => {
		...
		$(this.quill.root).on("keydown", (e) => {
			const key = saashq.ui.keys && saashq.ui.keys.get_key(e);
	...
	get_mention_options() {
		...
		return {
			...
			source: saashq.utils.debounce(async function (search_term, renderList) {
				let method =
					me.mention_search_method || "saashq.desk.search.get_names_for_mentions";
				let values = await saashq.xcall(method, {
					search_term,
				});
			...
			renderItem(item) {
				...
				return `${value} ${item.is_group ? saashq.utils.icon("users") : ""}`;
			},
		};
	}
	...
	parse(value) {
		...
		return saashq.dom.remove_script_and_style(value);
	}
