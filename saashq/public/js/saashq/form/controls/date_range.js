saashq.ui.form.ControlDateRange = class ControlDateRange extends saashq.ui.form.ControlData {
	make_input() {
		super.make_input();
	...
	set_date_options() {
		...
		this.datepicker_options.dateFormat = saashq.boot.sysdefaults.date_format || "yyyy-mm-dd";
	...
	parse(value) {
		...
		if (value && value.includes(",")) {
			...
			var from_date = moment(saashq.datetime.user_to_obj(vals[0])).format("YYYY-MM-DD");
			var to_date = moment(saashq.datetime.user_to_obj(vals[vals.length - 1])).format(
				"YYYY-MM-DD"
			);
	...
	format_for_input(value1, value2) {
		if (value1 && value2) {
			value1 = saashq.datetime.str_to_user(value1, false, true);
			value2 = saashq.datetime.str_to_user(value2, false, true);
