saashq.ui.form.ControlPassword = class ControlPassword extends saashq.ui.form.ControlData {
	...
	make_input() {
		...
		this.toggle_password.on("click", () => {
			if (this.$input.attr("type") === "password") {
				...
				this.toggle_password.html(saashq.utils.icon("hide", "sm"));
			} else {
				this.$input.attr("type", "password");
				this.toggle_password.html(saashq.utils.icon("unhide", "sm"));
			}
		});
	...
	get_password_strength(value) {
		...
		saashq.call({
			type: "POST",
			method: "saashq.core.doctype.user.user.test_password_strength",
			args: {
				new_password: value || "",
			},
			callback: function (r) {
