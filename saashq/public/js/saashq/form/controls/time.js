saashq.ui.form.ControlTime = class ControlTime extends saashq.ui.form.ControlDate {
	...
	set_time_options() {
		...
		this.time_format = saashq.defaultTimeFormat;
		this.datepicker_options = {
			language: "en",
			timepicker: true,
			onlyTimepicker: true,
			timeFormat: time_format.toLowerCase().replace("mm", "ii"),
			startDate: saashq.datetime.now_time(true),
			onSelect: () => {
	...
	set_input(value) {
		...
		let time_format = saashq.sys_defaults.time_format || "HH:mm:ss";
		if (!should_refresh) {
		...
		if (should_refresh) {
			this.datepicker.selectDate(
				saashq.datetime.moment_to_date_obj(moment(value, time_format))
			);
		}
	}
	set_datepicker() {
		...
		this.datepicker.$datepicker.find('[data-action="today"]').click(() => {
			this.datepicker.selectDate(saashq.datetime.now_time(true));
	...
	set_description() {
		...
		const { time_zone } = saashq.sys_defaults;
	...
	format_for_input(value) {
		if (value) {
			return saashq.datetime.str_to_user(value, true);
		}
		return "";
	}
	validate(value) {
		if (value && !saashq.datetime.validate(value)) {
			...
			let time_format =
				sysdefaults && sysdefaults.time_format ? sysdefaults.time_format : "HH:mm:ss";
			saashq.msgprint(__("Time {0} must be in format: {1}", [value, time_format]));
			return "";
		}
		return value;
	}
