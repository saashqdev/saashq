...
saashq.ui.form.ControlPhone = class ControlPhone extends saashq.ui.form.ControlData {
	async make_input() {
	...
	async setup_country_codes() {
		...
		if (data) {
			...
			const data = await saashq.xcall("saashq.geo.country_info.get_country_timezone_info");
	...
	input_events() {
		this.$input.keydown((e) => {
			...
			if ([saashq.ui.keyCode.BACKSPACE].includes(key_code)) {
	...
	setup_country_code_picker() {
		...
		if (!this.selected_icon.length) {
			this.selected_icon = $(
				`<div class="selected-phone">${saashq.utils.icon("down", "sm")}</div>`
			);
