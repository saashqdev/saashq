...
saashq.ui.form.ControlGeolocation = class ControlGeolocation extends saashq.ui.form.ControlData {
	...
	set_disp_area(value) {
		...
		if (this.frm) {
			...
			$(document).on("saashq.ui.Dialog:shown", () => {
				this.make_map();
				if (value) {
					this.bind_leaflet_data(value);
				}
	...
	bind_leaflet_map() {
		...
		this.map.setView(saashq.utils.map_defaults.center, saashq.utils.map_defaults.zoom);
	...
	get_leaflet_controls() {
		return new L.Control.Draw({
			...
			draw: {
				polyline: {
					shapeOptions: {
						color: saashq.ui.color.get("blue"),
						weight: 10,
					},
				},
				polygon: {
					...
					drawError: {
						color: saashq.ui.color.get("orange"), // Color the shape will turn when intersects
