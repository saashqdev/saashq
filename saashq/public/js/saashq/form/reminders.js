export class ReminderManager {
	...
	_update_datetime_selector() {
		...
		this.dialog.fields_dict.remind_at.datepicker?.update({
			minDate: saashq.datetime.str_to_obj(saashq.datetime.now_datetime()),
		});
	}
	...
	_convert_period_to_absolute_time() {
		...
		const now_time = saashq.datetime.str_to_obj(saashq.datetime.now_datetime());
		let [magnitude, unit] = period.split("_");

		let time_to_set = moment(now_time)
			.add(magnitude, unit)
			.format(saashq.defaultDatetimeFormat);
		this.dialog.set_value("remind_at", time_to_set);
	}

	create_reminder() {
		saashq
			.xcall("saashq.automation.doctype.reminder.reminder.create_new_reminder", {
			...
			.then((reminder) => {
				saashq.show_alert(
					__("Reminder set at {0}", [saashq.datetime.str_to_user(reminder.remind_at)])
				);
			});
	}
}
