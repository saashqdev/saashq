...
saashq.ui.form.get_open_grid_form = function () {
	return $(".grid-row-open").data("grid_row");
};

saashq.ui.form.close_grid_form = function () {
	var open_form = saashq.ui.form.get_open_grid_form();
	open_form && open_form.hide_form();

	// hide editable row too
	if (saashq.ui.form.editable_row) {
...
export default class Grid {
	constructor(opts) {
		...
		this.debounced_refresh = saashq.utils.debounce(this.debounced_refresh, 100);
	}
	...
	make() {
		...
		saashq.utils.bind_actions_with_object(this.wrapper, this);
	...
	delete_all_rows() {
		saashq.confirm(__("Are you sure you want to delete all rows?"), () => {
	...
	scroll_to_top() {
		saashq.utils.scroll_to(this.wrapper);
	}
	...
	debounced_refresh_remove_rows_button = saashq.utils.debounce(
		this.refresh_remove_rows_button,
		100
	);
	...
	refresh() {
		...
		if (this.frm) {
			this.display_status = saashq.perm.get_field_display_status(
				this.df,
				this.frm.doc,
				this.perm
			);
	...
	setup_fields() {
		...
		if (this.frm && this.frm.docname) {
			...
			this.df = saashq.meta.get_docfield(
				this.frm.doctype,
				this.df.fieldname,
				this.frm.docname
			);
			...
			if (this.df.options) {
				this.df =
					saashq.meta.get_docfield(this.df.options, this.df.fieldname) ||
					this.df ||
					null;
			}
		}
		...
		if (this.doctype && this.frm) {
			this.docfields = saashq.meta.get_docfields(this.doctype, this.frm.docname);
		} else {
	...
	get_data_based_on_fieldtype(df, data, value) {
		...
		if (fieldtype === "Check") {
			value = saashq.utils.string_to_boolean(value);
			...
			let formatted_duration = saashq.utils.get_formatted_duration(fieldvalue);
			...
			let user_formatted_date = saashq.datetime.str_to_user(fieldvalue);
				...
				row.columns_list.forEach((column) => {
					...
					if (column.df.fieldname == fieldname) {
						if (show) {
							...
							if (row != saashq.ui.form.editable_row) {
	...
	get_docfield(fieldname) {
		return saashq.meta.get_docfield(
			this.doctype,
			fieldname,
	...
	setup_visible_columns() {
		...
		for (var ci in fields) {
			...
			if (
				...
				if (
					...
					const docfield = saashq.meta.docfield_map[df.parent][df.fieldname];
		...
		while (total_colsize < 11 && passes < 12) {
			for (var i in this.visible_columns) {
				...
				if (colsize > 1 && colsize < 11 && saashq.model.is_non_std_field(df.fieldname)) {
					if (
						passes < 3 &&
	...
	setup_user_defined_columns() {
		...
		let user_settings = saashq.get_user_settings(this.frm.doctype, "GridView");
		if (user_settings && user_settings[this.doctype] && user_settings[this.doctype].length) {
			this.user_defined_columns = user_settings[this.doctype]
				.map((row) => {
					let column = saashq.meta.get_docfield(this.doctype, row.fieldname);
	...
	set_multiple_add(link, qty) {
		...
		var link_field = saashq.meta.get_docfield(this.df.options, link);
			...
			$(this.wrapper)
				...
				.on("click", () => {
					new saashq.ui.FileUploader({
						...
						on_success(file) {
							var data = saashq.utils.csv_to_array(
								saashq.utils.get_decoded_string(file.dataurl)
							);
	...
	setup_download() {
		let title = this.df.label || saashq.model.unscrub(this.df.fieldname);
		$(this.wrapper)
			.find(".grid-download")
			.removeClass("hidden")
			.on("click", () => {
				...
				data.push(["------"]);
				$.each(saashq.get_meta(this.df.options).fields, (i, df) => {
					// don't include the read-only field in the template
					if (saashq.model.is_value_type(df.fieldtype)) {
						data[1].push(df.label);
						data[2].push(df.fieldname);
						let description = (df.description || "") + " ";
						if (df.fieldtype === "Date") {
							description += saashq.boot.sysdefaults.date_format;
						}
				...
				saashq.tools.downloadify(data, null, title);
				return false;
			});
	}
