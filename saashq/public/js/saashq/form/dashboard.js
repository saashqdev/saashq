...
saashq.ui.form.Dashboard = class FormDashboard {
	...
	setup_dashboard_sections() {
		...
		this.heatmap_area = this.make_section({
			...
			is_dashboard_section: 1,
			body_html: `
				<div id="heatmap-${saashq.model.scrub(this.frm.doctype)}" class="heatmap"></div>
	...
	refresh() {
		this.reset();
		if (this.frm.doc.__islocal || !saashq.boot.desk_settings.dashboard) {
			return;
		}
	...
	render_links() {
		...
		$(saashq.render_template("form_links", this.data)).appendTo(transactions_area_body);
	...
	render_report_links() {
		...
		if (this.data.reports && this.data.reports.length) {
			$(saashq.render_template("report_links", this.data)).appendTo(parent);
			// bind reports
	...
	open_report($link) {
		...
		saashq.provide("saashq.route_options");
		saashq.route_options[fieldname] = this.frm.doc.name;
		saashq.set_route("query-report", report);
	}
	...
	open_document_list($link, show_open) {
		...
		if (
			...
			if (names.length) {
				saashq.route_options = { name: ["in", names] };
			} else {
				return false;
			}
		} else if (this.data.fieldname) {
			saashq.route_options = this.get_document_filter(doctype);
			if (show_open && saashq.ui.notifications) {
				saashq.ui.notifications.show_open_count_list(doctype);
			}
		}

		saashq.set_route("List", doctype, "List");
	}
	...
	set_open_count() {
		...
		let method = this.data.method || "saashq.desk.notifications.get_open_count";
		saashq.call({
			type: "GET",
			method: method,
			args: {
	...
	// heatmap
	render_heatmap() {
		this.heatmap = new saashq.Chart("#heatmap-" + saashq.model.scrub(this.frm.doctype), {
			type: "heatmap",
	...
	render_graph(args) {
		...
		this.chart = new saashq.Chart(".form-graph", args);
		if (!this.chart) {
			this.hide();
		}
	}
