...
saashq.provide("saashq.success_action");

saashq.ui.form.SuccessAction = class SuccessAction {
	constructor(form) {
		this.form = form;
		this.load_setting();
	}
	...
	load_setting() {
		this.setting = saashq.boot.success_action.find(
	...
	show_alert() {
		saashq.db.get_list(this.form.doctype, { limit: 2 }).then((result) => {
	...
	get default_actions() {
		return {
			new: {
				label: __("New"),
				action: (frm) => saashq.new_doc(frm.doctype),
			},
			print: {
				label: __("Print"),
			...
			list: {
				label: __("View All"),
				action: (frm) => {
					saashq.set_route("List", frm.doctype);
				},
			},
		};
	}
