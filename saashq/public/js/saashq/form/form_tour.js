saashq.ui.form.FormTour = class FormTour {
	constructor({ frm }) {
		this.frm = frm;
		this.driver_steps = [];
	}
	...
	init_driver() {
		...
		saashq.router.on("change", () => this.driver.reset());
	...
	async init({ tour_name, on_finish }) {
		if (tour_name) {
			...
			const doctype_tour_exists = await saashq.db.exists("Form Tour", this.frm.doctype);
			if (doctype_tour_exists) {
				this.tour = await saashq.db.get_doc("Form Tour", this.frm.doctype);
			} else {
				this.tour = { steps: saashq.tour[this.frm.doctype] };
			}
		}
	...
	get_step(step_info, on_next, on_prev) {
		...
		let element = `.saashq-control[data-fieldname='${fieldname}']`;
		...
		if (is_table_field) {
			...
			element = `.grid-row-open .saashq-control[data-fieldname='${fieldname}']`;
		}
	...
	add_new_row_step(step) {
		const $add_row = `.saashq-control[data-fieldname='${step.fieldname}'] .grid-add-row`;
		const add_row_step = {
			element: $add_row,
	...
	expand_row_and_proceed(step, start_from) {
		...
		saashq.utils.sleep(300).then(() => this.driver.start(start_from));
	}
	...
	open_first_row_of(fieldname) {
		...
		$($close_row).one("click", () => {
			...
			saashq.utils.scroll_to(next_element, true, 150, null, () => {
				this.driver.moveNext();
				saashq.flags.disable_auto_scroll = false;
			});
			saashq.flags.disable_auto_scroll = true;
		});
	}
