saashq.RoleEditor = class {
	constructor(wrapper, frm, disable) {
		...
		this.multicheck = saashq.ui.form.make_control({
			...
			df: {
				...
				get_data: () => {
					return saashq
						.xcall("saashq.core.doctype.user.user.get_all_roles")
						.then((roles) => {
	...
	show_permissions(role) {
		...
		return saashq
			.xcall("saashq.core.doctype.user.user.get_perm_info", { role })
			.then((permissions) => {
						...
						<table class="user-perm">
							<thead>
								<tr>
									...
									<th> ${__("Level")} </th>
									<th> ${__("If Owner")} </th>
									${saashq.perm.rights.map((p) => `<th> ${__(saashq.unscrub(p))}</th>`).join("")}
								</tr>
							</thead>
							<tbody></tbody>
						</table>
					...
					permissions.forEach((perm) => {
						$body.find("tbody").append(`
							<tr>
								...
								<td>${perm.if_owner ? saashq.utils.icon("check", "xs") : "-"}</td>
								${saashq.perm.rights
									.map(
										(p) =>
											`<td class="text-muted bold">${
												perm[p] ? saashq.utils.icon("check", "xs") : "-"
											}</td>`
									)
									.join("")}
							</tr>
	...
	make_perm_dialog() {
		this.perm_dialog = new saashq.ui.Dialog({
	...
	set_roles_in_table() {
		...
		roles.map((role_doc) => {
			if (!checked_options.includes(role_doc.role)) {
				saashq.model.clear_doc(role_doc.doctype, role_doc.name);
			}
		});
		checked_options.map((role) => {
			if (!roles.find((d) => d.role === role)) {
				let role_doc = saashq.model.add_child(this.frm.doc, "Has Role", "roles");
				role_doc.role = role;
			}
		});
	}
