...
*/
saashq.model.DocTypeController = class DocTypeController extends saashq.ui.form.Controller {
	setup() {
		// setup formatters for fieldtype
		saashq.meta.docfield_map[
	...
	show_db_utilization() {
		...
		saashq
			.xcall("saashq.core.doctype.doctype.doctype.get_row_size_utilization", {
				doctype,
			})
			.then((r) => {
				if (r < 50.0) return;
	...
	setup_fetch_from_fields(doc, doctype, docname) {
		...
		let row = saashq.get_doc(doctype, docname);
		...
		function update_fieldname_options() {
			...
			saashq.model.with_doctype(link_doctype, () => {
				let fields = saashq.meta
