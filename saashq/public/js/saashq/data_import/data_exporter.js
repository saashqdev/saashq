saashq.provide("saashq.data_import");

saashq.data_import.DataExporter = class DataExporter {
	...
	make_dialog(filetype = "CSV") {
		this.dialog = new saashq.ui.Dialog({
			...
			fields: [
				...
				...saashq.meta.get_table_fields(this.doctype).map((df) => {
	...
	export_records() {
		let method = "/api/method/saashq.core.doctype.data_import.data_import.download_template";
	...
	make_select_all_buttons() {
		...
		`);
		saashq.utils.bind_actions_with_object($select_all_buttons, this);
	...
	select_mandatory() {
		let mandatory_table_fields = saashq.meta
	...
	update_record_count_message() {
		...
		let count_method = {
			all: () => saashq.db.count(this.doctype),
			by_filter: () =>
				saashq.db.count(this.doctype, {
					filters: this.get_filters(),
				}),
	...
	get_multicheck_options(doctype, child_fieldname = null) {
		...
		if (meta.autoname && meta.autoname.startsWith("field:")) {
			...
			autoname_field = saashq.meta.get_field(doctype, fieldname);
		}
...
export function get_columns_for_picker(doctype) {
	...
	const exportable_fields = (df) => {
		let keep = true;
		if (saashq.model.no_value_type.includes(df.fieldtype)) {
			keep = false;
		}
	...
	// parent
	let doctype_fields = saashq.meta.get_docfields(doctype).filter(exportable_fields);
	...
	// children
	const table_fields = saashq.meta.get_table_fields(doctype);
	table_fields.forEach((df) => {
		const cdt = df.options;
		const child_table_fields = saashq.meta.get_docfields(cdt).filter(exportable_fields);
