...
saashq.data_import.ImportPreview = class ImportPreview {
	...
	setup_styles() {
		...
		this.datatable.style.setStyle(`svg.import-success`, {
			width: "16px",
			fill: saashq.ui.color.get_color_shade("green", "dark"),
		});
		...
		this.datatable.style.setStyle(row_classes, {
			pointerEvents: "none",
			backgroundColor: saashq.ui.color.get_color_shade("gray", "extra-light"),
			color: saashq.ui.color.get_color_shade("gray", "dark"),
		});
	}
	...
	show_column_warning(_, $target) {
		...
		saashq.utils.scroll_to($warning, true, 30);
	}
...
function get_fields_as_options(doctype, column_map) {
	let keys = [doctype];
	saashq.meta.get_table_fields(doctype).forEach((df) => {
	...
	return [].concat(
		...keys.map((key) => {
			return column_map[key].map((df) => {
				...
				if (doctype !== key) {
					let table_field = saashq.meta.get_docfield(doctype, key);
