...
$.extend(saashq.perm, {
	...
	has_perm: (doctype, permlevel = 0, ptype = "read", doc) => {
		const perms = saashq.perm.get_perm(doctype, doc);
	...
	get_perm: (doctype, doc) => {
		...
		if (doc && !doc.__islocal) {
			return saashq.perm._get_perm(doctype, doc);
		}

		return (saashq.perm.doctype_perm[doctype] ??= saashq.perm._get_perm(doctype));
	},
	...
	_get_perm: (doctype, doc) => {
		...
		if (user === "Administrator" || saashq.user_roles.includes("Administrator")) {
			perm[0].read = 1;
		}

		if (!meta) return perm;

		perm = saashq.perm.get_role_permissions(meta);
		const base_perm = perm[0];

		if (doc) {
			// apply user permissions via docinfo (which is processed server-side)
			let docinfo = saashq.model.get_docinfo(doctype, doc.name);
			...
			// if owner
			if (doc.owner !== user) {
				for (const right of saashq.perm.rights) {
			...
			if (docinfo && docinfo.shared) {
				for (const s of docinfo.shared) {
					...
					if (s.read) {
						...
						base_perm.email =
							saashq.boot.user.can_email.indexOf(doctype) !== -1 ? 1 : 0;
						base_perm.print =
							saashq.boot.user.can_print.indexOf(doctype) !== -1 ? 1 : 0;
					}
				}
			}
		}

		if (!base_perm.read && saashq.model.can_read(doctype)) {
			// read via sharing
			base_perm.read = 1;
		}
	...
	get_role_permissions: (meta) => {
		...
		*/
		...
		(meta.permissions || []).forEach((p) => {
			...
			if (saashq.user_roles.includes(p.role)) {
				saashq.perm.rights.forEach((right) => {
					if (!p[right]) return;
	...
	get_match_rules: (doctype, ptype) => {
		...
		let perm = saashq.perm.get_perm(doctype);

		let user_permissions = saashq.defaults.get_user_permissions();

		if (user_permissions && !$.isEmptyObject(user_permissions)) {
			let rules = {};
			let fields_to_check = saashq.meta.get_fields_to_check_permissions(doctype);
			$.each(fields_to_check, (i, df) => {
				const user_permissions_for_doctype = user_permissions[df.options] || [];
				const allowed_records = saashq.perm.get_allowed_docs_for_doctype(
		...
		if (base_perm.read && !base_perm.rights_without_if_owner.has("read")) {
			match_rules.push({ Owner: saashq.session.user });
		}
		return match_rules;
	},
	...
	get_field_display_status: (df, doc, perm, explain) => {
		...
		if (!perm && doc) {
			perm = saashq.perm.get_perm(doc.doctype, doc);
		}
	...
	is_visible: (df, doc, perm) => {
		if (typeof df === "string") {
			// df is fieldname
			df = saashq.meta.get_docfield(doc.doctype, df, doc.parent || doc.name);
		}

		let status = saashq.perm.get_field_display_status(df, doc, perm);
	...
	get_allowed_docs_for_doctype: (user_permissions, doctype) => {
		...
		return saashq.perm.filter_allowed_docs_for_doctype(user_permissions, doctype, false);
	},
