...
saashq.provide("saashq.realtime");
...
class RealTimeClient {
	...
	get_host(port = 9000) {
		...
		if (window.dev_server) {
			...
			port = saashq.boot.socketio_port || port.toString() || "9000";
			if (parts.length > 2) {
				host = parts[0] + ":" + parts[1];
			}
			host = host + ":" + port;
		}
		return host + `/${saashq.boot.sitename}`;
	}
	...
	doc_subscribe(doctype, docname) {
		...
		saashq.flags.doc_subscribe = true;

		// throttle to 1 per sec
		setTimeout(function () {
			saashq.flags.doc_subscribe = false;
		}, 1000);
...
saashq.realtime = new RealTimeClient();

// backward compatibility
saashq.socketio = saashq.realtime;
