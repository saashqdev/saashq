export default class ListSettings {
	constructor({ listview, doctype, meta, settings }) {
		...
		saashq.model.with_doctype("List View Settings", () => {
			this.make();
	...
	make() {
		...
		let list_view_settings = saashq.get_meta("List View Settings");
		...
		me.dialog.set_primary_action(__("Save"), () => {
			...
			saashq.call({
				method: "saashq.desk.doctype.list_view_settings.list_view_settings.save_listview_settings",
				args: {
					doctype: me.doctype,
		...
		for (let idx in me.fields) {
			...
			fields += `
				<div class="control-input flex align-center form-control fields_order ${is_sortable}"
					...
					<div class="row">
						<div class="col-1">
							${saashq.utils.icon("drag", "xs", "", "", "sortable-handle " + show_sortable_handle)}
						</div>
		...
		d.set_primary_action(__("Save"), () => {
			...
			for (let idx in values) {
				...
				if (me.fields.length === parseInt(me.dialog.get_values().total_fields)) {
					...
					let field = saashq.meta.get_docfield(me.doctype, value);
					if (field) {
						me.fields.push({
	...
	reset_listview_fields(dialog) {
		...
		saashq
			.xcall(
				"saashq.desk.doctype.list_view_settings.list_view_settings.get_default_listview_fields",
				{
					doctype: me.doctype,
				}
			)
	...
	set_list_view_fields(meta) {
		...
		meta.fields.forEach((field) => {
			if (
				field.in_list_view &&
				!saashq.model.no_value_type.includes(field.fieldtype) &&
	...
	set_subject_field(meta) {
		...
		if (meta.title_field) {
			let field = saashq.meta.get_docfield(me.doctype, meta.title_field.trim());
	...
	set_status_field() {
		...
		if (saashq.has_indicator(me.doctype)) {
			me.fields.push({
				type: "Status",
				label: __("Status"),
