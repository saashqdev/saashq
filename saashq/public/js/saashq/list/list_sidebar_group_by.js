...
saashq.views.ListGroupBy = class ListGroupBy {
	constructor(opts) {
		$.extend(this, opts);
		this.make_wrapper();
	...
	make_group_by_fields_modal() {
		...
		d.set_primary_action(__("Save"), ({ group_by_fields }) => {
			saashq.model.user_settings.save(
				this.doctype,
				"group_by_fields",
	...
	render_group_by_items() {
		let get_item_html = (fieldname) => {
			...
			if (fieldname === "assigned_to") {
				...
				label = saashq.meta.get_label(this.doctype, fieldname);
				let docfield = saashq.meta.get_docfield(this.doctype, fieldname);
				if (!docfield) {
					return;
				}
			...
			return `<div class="group-by-field list-link">
						...
						href="#" onclick="return false;">
							...
							<span>${saashq.utils.icon("select", "xs")}</span>
						</a>
	...
	setup_search($dropdown) {
		saashq.utils.setup_search($dropdown, ".group-by-item", ".group-by-value", "data-name");
	}
	...
	get_group_by_count(field) {
		...
		return saashq.call("saashq.desk.listview.get_group_by_count", args).then((r) => {
			let field_counts = r.message || [];
			field_counts = field_counts.filter((f) => f.count !== 0);
			let current_user = field_counts.find((f) => f.name === saashq.session.user);
			field_counts = field_counts.filter(
				(f) => !["Guest", "Administrator", saashq.session.user].includes(f.name)
			);
			// Set saashq.session.user on top of the list
	...
	get_dropdown_html(field, fieldtype, applied = false) {
		...
		if (field.name == null) {
			label = __("Not Set");
		} else if (field.name === saashq.session.user) {
			label = __("Me");
		...
		let applied_html = applied
			? `<span class="applied"> ${saashq.utils.icon("tick", "xs")} </span>`
			: "";
