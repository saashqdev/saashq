saashq.ui.Filter = class {
	constructor(opts) {
		...
		this.utils = saashq.ui.filter_utils;
		this.set_conditions();
	...
	set_conditions_from_config() {
		if (saashq.boot.additional_filters_config) {
			this.filters_config = saashq.boot.additional_filters_config;
	...
	make() {
		this.filter_edit_area = $(
			saashq.render_template("edit_filter", {
	...
	make_select() {
		this.fieldselect = new saashq.ui.FieldSelect({
	...
	set_field(doctype, fieldname, fieldtype, condition) {
		...
		if (
			...
			if (this.filters_config[condition].data) {
				...
				saashq.xcall(this.filters_config[condition].get_field, args).then((field) => {
	...
	make_field(df, old_fieldtype) {
		...
		let f = saashq.ui.form.make_control({
			df: df,
			parent: field_area,
			only_input: true,
		});
	...
	toggle_nested_set_conditions(df) {
		let show_condition =
			df.fieldtype === "Link" && saashq.boot.nested_set_doctypes.includes(df.options);
...
saashq.ui.filter_utils = {
	get_formatted_value(field, value) {
		...
		return saashq.format(value, field.df, { only_value: 1 });
	},
	...
	get_selected_value(field, condition) {
		...
		} else if (saashq.boot.additional_filters_config[condition]) {
			val = field.value || val;
		}
		if (val === "%") {
			val = "";
