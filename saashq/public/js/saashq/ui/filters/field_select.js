...
saashq.ui.FieldSelect = class FieldSelect {
	...
	build_options() {
		var me = this;
		me.table_fields = [];
		var std_filters = $.map(saashq.model.std_fields, function (d) {
		...
		var doctype_obj = saashq.get_meta(me.doctype);
		...
		// main table
		var main_table_fields = std_filters.concat(saashq.meta.docfield_list[me.doctype]);
		$.each(saashq.utils.sort(main_table_fields, "label", "string"), function (i, df) {
			let doctype =
				saashq.get_meta(me.doctype).istable && me.parent_doctype
					? me.parent_doctype
					: me.doctype;

			// show fields where user has read access and if report hide flag is not set
			if (saashq.perm.has_perm(doctype, df.permlevel, "read")) me.add_field_option(df);
		});

		// child tables
		$.each(me.table_fields, function (i, table_df) {
			if (table_df.options) {
				let child_table_fields = [].concat(saashq.meta.docfield_list[table_df.options]);

				if (table_df.fieldtype === "Table MultiSelect") {
					const link_field = saashq.meta
						.get_docfields(table_df.options)
						.find((df) => df.fieldtype === "Link");
					child_table_fields = link_field ? [link_field] : [];
				}

				$.each(saashq.utils.sort(child_table_fields, "label", "string"), function (i, df) {
					let doctype =
						saashq.get_meta(me.doctype).istable && me.parent_doctype
							? me.parent_doctype
							: me.doctype;
					...
					// show fields where user has read access and if report hide flag is not set
					if (saashq.perm.has_perm(doctype, df.permlevel, "read"))
	...
	add_field_option(df) {
		...
		if (df.fieldname == "docstatus" && !saashq.model.is_submittable(me.doctype)) return;
		...
		if (
			saashq.model.no_value_type.indexOf(df.fieldtype) == -1 &&
