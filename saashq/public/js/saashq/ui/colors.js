...
saashq.ui.color = {
	get: function (color_name, shade) {
		...
		return saashq.ui.color_map;
	},
	get_color: function (color_name) {
		const color_names = Object.keys(saashq.ui.color_map);
		if (color_names.includes(color_name)) {
			return saashq.ui.color_map[color_name];
		} else {
	...
	all: function () {
		return Object.values(saashq.ui.color_map).reduce((acc, curr) => acc.concat(curr), []);
	},
	names: function () {
		return Object.keys(saashq.ui.color_map);
	},
	...
	get_color_name: function (hex) {
		for (const key in saashq.ui.color_map) {
			const colors = saashq.ui.color_map[key];
...
saashq.ui.color_map = saashq.ui.color.get_color_map();
