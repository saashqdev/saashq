saashq.ui.TagEditor = class TagEditor {
	constructor(opts) {
		/* docs:
		Arguments
	...
	setup_tags() {
		...
		this.tags = new saashq.ui.Tags({
			...
			onTagRemove: (tag) => {
				if (!me.refreshing) {
					return saashq.call({
						method: "saashq.desk.doctype.tag.tag.remove_tag",
						args: me.get_args(tag),
						callback: function (r) {
	...
	setup_awesomplete() {
		...
		$input.on("input", function (e) {
			...
			saashq.call({
				method: "saashq.desk.doctype.tag.tag.get_tags",
				args: {
