...
saashq.ui.open_dialogs = [];

saashq.ui.Dialog = class Dialog extends saashq.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		...
		this.$wrapper
			.on("hide.bs.modal", function () {
				...
				saashq.ui.form.get_open_grid_form?.()?.hide_form();

				if (saashq.ui.open_dialogs[saashq.ui.open_dialogs.length - 1] === me) {
					saashq.ui.open_dialogs.pop();
					if (saashq.ui.open_dialogs.length) {
						window.cur_dialog =
							saashq.ui.open_dialogs[saashq.ui.open_dialogs.length - 1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function () {
				...
				saashq.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.hide_scrollbar(true);
				me.on_page_show && me.on_page_show();
				$(document).trigger("saashq.ui.Dialog:shown");
	...
	toggle_minimize() {
		...
		this.get_minimize_btn().html(saashq.utils.icon(icon));
