saashq.provide("saashq.ui.keys");

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
saashq.ui.keys.shortcut_groups = shortcut_groups;

saashq.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new saashq.ui.keys.AltShortcutGroup());
	}
...
saashq.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on("keydown", (e) => {
		let key = (saashq.ui.keys.key_map[e.which] || "").toLowerCase();
...
saashq.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = saashq.ui.keys.standard_shortcuts
			.filter((s) => !s.page)
