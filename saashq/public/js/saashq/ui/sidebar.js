...
saashq.ui.Sidebar = class Sidebar {
	constructor() {
		this.items = {};
	...
	make_dom() {
		...
		this.wrapper = $(
			saashq.render_template("sidebar", {
				app_logo_url: saashq.boot.app_data[0].app_logo_url,
				app_title: __(saashq.boot.app_data[0].app_title),
			})
		).prependTo("body");
		...
		if (this.has_access) {
			this.wrapper
				...
				.removeClass("hidden")
				.on("click", () => {
					saashq.quick_edit("Workspace Settings");
				});
		}
	...
	set_all_pages() {
		this.sidebar_pages = saashq.boot.sidebar_pages;
	...
	set_default_app() {
		...
		saashq.boot.app_data.sort((a, b) => (a.workspaces.length < b.workspaces.length ? 1 : -1));
		saashq.current_app = saashq.boot.app_data[0].app_name;
	}
	...
	setup_app_switcher() {
		...
		saashq.boot.app_data_map = {};
		this.add_private_app(app_switcher_menu);

		for (var app of saashq.boot.app_data) {
			saashq.boot.app_data_map[app.app_name] = app;
	...
	add_private_app(app_switcher_menu) {
		...
		saashq.boot.app_data_map["private"] = app;
	...
	setup_select_app(app_switcher_menu) {
		app_switcher_menu.find(".app-item").on("click", (e) => {
			...
			if (route.startsWith("/app/private")) {
				...
				let ws = Object.values(saashq.workspace_map).find((ws) => ws.public === 0);
				route += "/" + saashq.router.slug(ws.title);
	...
	set_current_app(app) {
		...
		let app_data = saashq.boot.app_data_map[app] || saashq.boot.app_data_map["saashq"];
		...
		if (saashq.current_app === app) return;
		saashq.current_app = app;
		...
		this.add_app_item(
			{
				...
				app_route: "/",
				app_logo_url: "/assets/saashq/images/web.svg",
			},
			app_switcher_menu
		);
	}
	...
	setup_pages() {
		...
		if (this.all_pages) {
			saashq.workspaces = {};
			saashq.workspace_list = [];
			saashq.workspace_map = {};
			for (let page of this.all_pages) {
				saashq.workspaces[saashq.router.slug(page.name)] = {
					name: page.name,
					public: page.public,
				};
				if (!page.app && page.module) {
					page.app = saashq.boot.module_app[saashq.slug(page.module)];
				}
				saashq.workspace_map[page.name] = page;
				saashq.workspace_list.push(page);
			}
			this.make_sidebar();
		}
	}
	...
	make_sidebar() {
		...
		let app_workspaces = saashq.boot.app_data_map[saashq.current_app || "saashq"].workspaces;

		let parent_pages = this.all_pages.filter((p) => !p.parent_page).uniqBy((p) => p.name);
		if (saashq.current_app === "private") {
		...
		this.wrapper.find(".selected").length &&
			!saashq.dom.is_element_in_viewport(this.wrapper.find(".selected")) &&
	...
	sidebar_item_container(item) {
		...
		if (item.type === "Link") {
			if (item.link_type === "Report") {
				path = saashq.utils.generate_route({
					type: item.link_type,
					name: item.link_to,
				...
				path = saashq.utils.generate_route({ type: item.link_type, name: item.link_to });
			}
		} else if (item.type === "URL") {
			path = item.external_link;
		} else {
			if (item.public) {
				path = "/app/" + saashq.router.slug(item.name);
			} else {
				path = "/app/private/" + saashq.router.slug(item.name.split("-")[0]);
			}
		}
						...
						<span class="sidebar-item-icon" item-icon=${item.icon || "folder-normal"}>
							${
								item.public || item.icon
									? saashq.utils.icon(item.icon || "folder-normal", "md")
	...
	add_toggle_children(item, sidebar_control, item_container) {
		...
		let $drop_icon = $(`<button class="btn-reset drop-icon hidden">`)
			.html(saashq.utils.icon(drop_icon, "sm"))
		...
		for (let container of this.$sidebar.find(".nested-container")) {
			Sortable.create(container, {
				...
				onEnd: () => {
					...
					saashq.xcall(
						"saashq.desk.doctype.workspace_settings.workspace_settings.set_sequence",
						{
	...
	reload() {
		return saashq.workspace.get_pages().then((r) => {
