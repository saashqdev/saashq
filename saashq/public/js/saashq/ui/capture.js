 ...
 *
 * @example
 * saashq._.get_data_uri(video)
 * // returns "data:image/pngbase64,..."
 */
saashq._.get_data_uri = (element) => {
...
/**
 * @description Saashq's Capture object.
 *
 * @example
 * const capture = saashq.ui.Capture()
 * capture.show()
 *
 ...
 */
saashq.ui.Capture = class {
	constructor(options = {}) {
		this.options = saashq.ui.Capture.OPTIONS;
	...
	set_options(options) {
		this.options = { ...saashq.ui.Capture.OPTIONS, ...options };
	...
	show() {
		...
		if (cint(saashq.boot.sysdefaults.force_web_capture_mode_for_uploads)) {
			this.show_for_desktop();
		} else if (saashq.is_mobile()) {
	...
	build_dialog() {
		...
		me.$template = $(saashq.ui.Capture.TEMPLATE);
	...
	show_for_desktop() {
		...
		this.render_stream()
			...
			.catch((err) => {
				if (me.options.error) {
					saashq.show_alert(saashq.ui.Capture.ERR_MESSAGE, 3);
				}
	...
	setup_take_photo_action() {
		...
		this.dialog.set_primary_action(__("Take Photo"), () => {
			const data_url = saashq._.get_data_uri(me.video);
...
saashq.ui.Capture.OPTIONS = {
	title: __("Camera"),
	animate: false,
	error: false,
};
saashq.ui.Capture.ERR_MESSAGE = __("Unable to load camera.");
saashq.ui.Capture.TEMPLATE = `
<div class="saashq-capture">
	<div class="embed-responsive embed-responsive-16by9 fc-stream-container">
		<video class="fc-stream embed-responsive-item">${saashq.ui.Capture.ERR_MESSAGE}</video>
	</div>
