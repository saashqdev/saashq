...
saashq.ui.AddressAutocompleteDialog = class AddressAutocompleteDialog {
	constructor(opts) {
	...
	_get_dialog() {
		...
		const dialog = new saashq.ui.Dialog({
			...
			fields: [
				{
					...
					reqd: 1,
					get_query:
						"saashq.integrations.doctype.geolocation_settings.geolocation_settings.autocomplete",
					onchange: () => {
						// Disable "Create Address" button if mandatory fields are missing
						saashq.model.with_doctype("Address", () => {
							const address = this.parse_selected_value();
							const mandatory_fields = saashq
								.get_meta("Address")
								.fields.filter(
									(field) =>
										field.reqd &&
			...
			primary_action: () => {
				...
				saashq.db.insert(address).then((doc) => {
			...
			secondary_action: () => {
				...
				const address = this.parse_selected_value();
				saashq.new_doc("Address", address);
			},
		});
