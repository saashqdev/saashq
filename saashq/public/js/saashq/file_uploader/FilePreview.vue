<template>
	<div class="file-preview">
		<div class="file-icon">
			...
			<div class="fallback" v-else v-html="saashq.utils.icon('file', 'md')"></div>
		</div>
		<div>
			<div>
			...
			<div class="flex config-area">
				<label v-if="allow_toggle_optimize" class="saashq-checkbox"
					><input
						type="checkbox"
						:checked="optimize"
					...
					/>{{ __("Optimize") }}</label
				>
				<label v-if="allow_toggle_private" class="saashq-checkbox"
					><input
						type="checkbox"
		...
		<div class="file-actions">
			<ProgressRing
				...
				:stroke="3"
			/>
			<div v-if="uploaded" v-html="saashq.utils.icon('solid-success', 'lg')"></div>
			<div v-if="file.failed" v-html="saashq.utils.icon('solid-error', 'lg')"></div>
			<div class="file-action-buttons">
				<button
					v-if="is_cropable"
					class="btn btn-crop muted"
					@click="emit('toggle_image_cropper')"
					v-html="saashq.utils.icon('crop', 'md')"
				></button>
				<button
					v-if="!uploaded && !file.uploading && !file.failed"
					class="btn muted"
					@click="emit('remove')"
					v-html="saashq.utils.icon('delete', 'md')"
				></button>
			</div>
		</div>
	</div>
...
let file_size = computed(() => {
	return saashq.form.formatters.FileSize(props.file.file_obj.size);
});
