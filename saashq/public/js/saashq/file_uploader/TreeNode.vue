	...
	<div class="tree-node" :class="{ opened: node.open }">
		...
		>
			...
			<a
				...
				v-html="saashq.utils.icon('external-link', 'sm')"
				@click.stop
			/>
		</span>
		<div v-if="node.file_url && saashq.utils.is_image_file(node.file_url)">
...
let icon = computed(() => {
	let icons = {
		open: saashq.utils.icon("folder-open", "md"),
		closed: saashq.utils.icon("folder-normal", "md"),
		leaf: saashq.utils.icon("primitive-dot", "xs"),
		search: saashq.utils.icon("search"),
	};
...
let open_file = (filename) => {
	return saashq.utils.get_form_link("File", filename);
};
