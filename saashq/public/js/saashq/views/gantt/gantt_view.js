...
saashq.views.GanttView = class GanttView extends saashq.views.ListView {
	get view_name() {
		return "Gantt";
	}
	...
	prepare_tasks() {
		...
		this.tasks = this.data.map(function (item) {
			...
			if (item.color && saashq.ui.color.validate_hex(item.color)) {
	...
	render_gantt() {
		...
		this.gantt = new Gantt(this.$result[0], this.tasks, {
			...
			on_click: (task) => {
				saashq.set_route("Form", task.doctype, task.id);
			},
			on_date_change: (task, start, end) => {
				if (!me.can_write) return;
				saashq.db.set_value(task.doctype, task.id, {
	...
	set_colors() {
		...
		let style = classes
			.map((c) => {
				...
				const progress_color = saashq.ui.color.get_contrast_color(bar_color);
				return `
	...
	get required_libs() {
		return [
			"assets/saashq/node_modules/saashq-gantt/dist/saashq-gantt.css",
			"assets/saashq/node_modules/saashq-gantt/dist/saashq-gantt.min.js",
		];
	}
};
