export default class KanbanSettings {
	constructor({ kanbanview, doctype, meta, settings }) {
		if (!doctype) {
			saashq.throw(__("DocType required"));
		}
	...
	make() {
		...
		this.dialog.set_primary_action(__("Save"), () => {
			...
			saashq.call({
				method: "saashq.desk.doctype.kanban_board.kanban_board.save_settings",
				args: {
		...
		for (let fieldname of this.fields) {
			...
			fields += `
				<div class="control-input flex align-center form-control fields_order sortable"
					...
					<div class="row">
						<div class="col-md-1">
							${saashq.utils.icon("drag", "xs", "", "", "sortable-handle")}
						</div>
	...
	get_docfield(field_name) {
		return (
			saashq.meta.get_docfield(this.doctype, field_name) ||
	...
	get_multiselect_fields() {
		...
		const ignore_fieldtypes = [
			...
			"Code",
			"Color",
			...saashq.model.no_value_type,
		];

		return saashq.model.std_fields
