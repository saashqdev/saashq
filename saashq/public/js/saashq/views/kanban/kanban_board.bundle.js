...
(function () {
	var method_prefix = "saashq.desk.doctype.kanban_board.kanban_board.";
	...
	var store = createStore({
		...
		actions: {
			...
			add_column: function (context, col) {
				if (saashq.model.can_create("Custom Field")) {
			...
			add_card: function (context, { card_title, column_title }) {
				var state = context.state;
				var doc = saashq.model.get_new_doc(state.doctype);
				...
				if (field && !quick_entry) {
					...
					saashq.new_doc(state.doctype, doc);
				}
			},
	...
	saashq.views.KanbanBoard = function (opts) {
		var self = {};
		...
		function prepare() {
			...
			if (self.$kanban_board.length === 0) {
				self.$kanban_board = $(saashq.render_template("kanban_board"));
		...
		function make_columns() {
			...
			columns.filter(is_active_column).map(function (col) {
				saashq.views.KanbanBoardColumn(col, self.$kanban_board, self.board_perms);
			});
		}
		...
		function bind_add_column() {
			...
			const is_custom_field = saashq.meta.get_docfield(doctype, fieldname)?.is_custom_field;
	...
	saashq.views.KanbanBoardColumn = function (column, wrapper, board_perms) {
		var self = {};
		var filtered_cards = [];
		...
		function make_dom() {
			self.$kanban_column = $(
				saashq.render_template("kanban_column", {
					title: column.title,
					doctype: store.state.doctype,
					indicator: saashq.scrub(column.indicator, "-"),
				})
			).appendTo(wrapper);
			// add task, archive
			...
			if (order) {
				...
				filtered_cards.forEach(function (card) {
					if (order.indexOf(card.name) === -1) {
						saashq.views.KanbanBoardCard(card, self.$kanban_cards);
					}
				});
				order.forEach(function (name) {
					if (!filtered_cards_names.includes(name)) return;
					saashq.views.KanbanBoardCard(get_card(name), self.$kanban_cards);
				});
			} else {
		...
		function setup_sortable() {
			...
			if (!saashq.model.can_write(store.state.doctype)) return;
		...
		function bind_add_card() {
			...
			if (!saashq.model.can_create(store.state.doctype)) {
		...
		function bind_options() {
			...
			get_column_indicators(function (indicators) {
				let html = `<li class="button-group">${indicators
					.map((indicator) => {
						let classname = saashq.scrub(indicator, "-");
	...
	saashq.views.KanbanBoardCard = function (card, wrapper) {
		var self = {};
		...
		function make_dom() {
			var opts = {
				name: card.name,
				title: saashq.utils.html2text(card.title),
				disable_click: card._disable_click ? "disable-click" : "",
				creation: card.creation,
				doc_content: get_doc_content(card),
				image_url: cur_list.get_image_url(card),
				form_link: saashq.utils.get_form_link(card.doctype, card.name),
			};

			self.$card = $(saashq.render_template("kanban_card", opts)).appendTo(wrapper);

			if (!saashq.model.can_write(card.doctype)) {
		...
		function get_doc_content(card) {
			...
			for (let field_name of cur_list.board.fields) {
				let field =
					saashq.meta.docfield_map[card.doctype]?.[field_name] ||
					saashq.model.get_std_field(field_name);
				let label = cur_list.board.show_labels
					? `<span>${__(field.label, null, field.parent)}: </span>`
					: "";
				let value = saashq.format(card.doc[field_name], field);
				fields.push(`
		...
		function render_card_meta() {
			...
			if (card.color && saashq.ui.color.validate_hex(card.color)) {
				const $div = $("<div>");
				$("<div></div>")
					.css({
						width: "30px",
		...
		function get_assignees_group() {
			return saashq.avatar_group(card.assigned_list, 3, {
		...
		function show_assign_to_dialog(e) {
			e.preventDefault();
			e.stopPropagation();
			self.assign_to = new saashq.ui.form.AssignToDialog({
				obj: self,
	...
	function fetch_customization(doctype) {
		return new Promise(function (resolve) {
			saashq.model.with_doc("Customize Form", "Customize Form", function () {
				var doc = saashq.get_doc("Customize Form");
	...
	function insert_doc(doc) {
		return saashq.call({
			method: "saashq.client.insert",
			args: {
				doc: doc,
			},
			callback: function () {
				saashq.model.clear_doc(doc.doctype, doc.name);
				saashq.show_alert({ message: __("Saved"), indicator: "green" }, 1);
			},
		});
	}
	...
	function get_column_indicators(callback) {
		saashq.model.with_doctype("Kanban Board Column", function () {
			var meta = saashq.get_meta("Kanban Board Column");
			var indicators;
