...
saashq.provide("saashq.views.calendar");
saashq.provide("saashq.views.calendars");

saashq.views.CalendarView = class CalendarView extends saashq.views.ListView {
	static load_last_view() {
		const route = saashq.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = saashq.get_user_settings(doctype)["Calendar"] || {};
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.calendar_settings = saashq.views.Calendar[this.doctype] || {};
			this.calendar_name = saashq.get_route()[3];
		});
	}
	...
	render() {
		...
		this.load_lib
			...
			.then((options) => {
				this.calendar = new saashq.views.Calendar(options);
			});
	}
	...
	get_calendar_preferences() {
		...
		return new Promise((resolve) => {
			if (calendar_name === "default") {
				Object.assign(options, saashq.views.calendar[this.doctype]);
				resolve(options);
			} else {
				saashq.model.with_doc("Calendar View", calendar_name, () => {
					const doc = saashq.get_doc("Calendar View", calendar_name);
					if (!doc) {
						saashq.show_alert(
						...
						saashq.set_route("List", this.doctype, "Calendar", "default");
						return;
					}
...
saashq.views.Calendar = class Calendar {
	constructor(options) {
		$.extend(this, options);
	...
	make_page() {
		...
		$.each(saashq.boot.calendars, function (i, doctype) {
			if (saashq.model.can_read(doctype)) {
				me.page.add_menu_item(__(doctype), function () {
					saashq.set_route("List", doctype, "Calendar");
				});
			}
		});
	...
	make() {
		...
		this.footnote_area = saashq.utils.set_footnote(
			this.footnote_area,
			this.$wrapper,
		...
		this.fullCalendar = new saashq.FullCalendar(this.$cal[0], this.cal_options);
	...
	set_css() {
		...
		this.$wrapper
			...
			.attr("class", "")
			.html(saashq.utils.icon("left"));
		this.$wrapper
			.find(`.fc-next-button span`)
			.attr("class", "")
			.html(saashq.utils.icon("right"));
		if (this.$wrapper.find(".fc-today-button svg").length == 0)
			this.$wrapper.find(".fc-today-button").prepend(saashq.utils.icon("today"));
	...
	get_system_datetime(date) {
		return saashq.datetime.convert_to_system_tz(date, true);
	}
	setup_options(defaults) {
		var me = this;
		defaults.meridiem = "false";
		this.cal_options = {
			plugins: saashq.FullCalendar.Plugins,
			initialView: defaults.initialView || "dayGridMonth",
			locale: saashq.boot.lang,
			firstDay: 1,
			headerToolbar: {
			...
			events: function (info, successCallback, failureCallback) {
				return saashq.call({
					method: me.get_events_method || "saashq.desk.calendar.get_events",
					type: "GET",
			...
			eventClick: function (info) {
				...
				if (saashq.model.can_read(doctype)) {
					saashq.set_route("Form", doctype, info.event.id);
				}
			},
			...
			select: function (info) {
				...
				var event = saashq.model.get_new_doc(me.doctype);
				...
				saashq.set_route("Form", me.doctype, event.name);
			},
	...
	prepare_events(events) {
		...
		return (events || []).map((d) => {
			d.id = d.name;
			d.editable = saashq.model.can_write(d.doctype || me.doctype);
			...
			if (d.convertToUserTz) {
				d.start = saashq.datetime.convert_to_user_tz(d.start);
				d.end = saashq.datetime.convert_to_user_tz(d.end);
			}

			// show event on single day if start or end date is invalid
			if (!saashq.datetime.validate(d.start) && d.end) {
				d.start = saashq.datetime.add_days(d.end, -1);
			}

			if (d.start && !saashq.datetime.validate(d.end)) {
				d.end = saashq.datetime.add_days(d.start, 1);
			}

			me.prepare_colors(d);

			d.title = saashq.utils.html2text(d.title);
	...
	prepare_colors(d) {
		...
		if (this.get_css_class) {
			...
			if (color_name.startsWith("#")) {
				color_name = saashq.ui.color.validate_hex(color_name) ? color_name : "blue";
			}

			d.backgroundColor = saashq.ui.color.get(color_name, "extra-light");
			d.textColor = saashq.ui.color.get(color_name, "dark");
		} else {
			color = d.color;
			if (!saashq.ui.color.validate_hex(color) || !color) {
				color = saashq.ui.color.get("blue", "extra-light");
			}
			d.backgroundColor = color;
			d.textColor = saashq.ui.color.get_contrast_color(color);
		}
		return d;
	}
	update_event(event, revertFunc) {
		var me = this;
		saashq.model.remove_from_locals(me.doctype, event.id);
		return saashq.call({
			method: me.update_event_method || "saashq.desk.calendar.update_event",
			args: me.get_update_args(event),
			callback: function (r) {
				if (r.exc) {
					saashq.show_alert(__("Unable to update event"));
					revertFunc();
				}
			},
			error: function () {
