...
saashq.standard_pages["Workspaces"] = function () {
	var wrapper = saashq.container.add_page("Workspaces");
	...
	saashq.workspace = new saashq.views.Workspace(wrapper);
	$(wrapper).bind("show", function () {
		saashq.workspace.show();
	});
};

saashq.views.Workspace = class Workspace {
	constructor(wrapper) {
		this.wrapper = $(wrapper);
		this.page = wrapper.page;
		this.blocks = saashq.workspace_block.blocks;
		...
		this.sidebar = saashq.app.sidebar;
		this.sidebar.setup_pages();
		this.cached_pages = $.extend(true, {}, saashq.boot.sidebar_pages);
		this.has_access = saashq.boot.sidebar_pages.has_access;
		this.has_create_access = saashq.boot.sidebar_pages.has_create_access;
		this.show();
	...
	get_pages() {
		return saashq.xcall("saashq.desk.desktop.get_workspace_sidebar_items", null, "GET");
	}
	...
	show() {
		...
		if (!saashq.router.current_route[0]) {
			saashq.route_flags.replace_route = true;
			saashq.set_route(saashq.router.slug(page.public ? page.name : "private/" + page.name));
			return;
		}
	...
	get_data(page) {
		return saashq
			.call({
				method: "saashq.desk.desktop.get_desktop_page",
				args: {
			...
			.then((data) => {
				...
				return saashq.dashboard_utils.get_dashboard_settings().then((settings) => {
					if (settings) {
	...
	get_page_to_show() {
		...
		if (saashq.boot.user.default_workspace) {
			default_page = {
				name: saashq.boot.user.default_workspace.name,
				public: saashq.boot.user.default_workspace.public,
			};
		} else if (
		...
		if (this.sidebar.all_pages.length) {
			...
			if (!this._page.public) {
				...
				if (!app && this._page.module) {
					app = saashq.boot.module_app[saashq.router.slug(this._page.module)];
				}
				if (!app) app = "saashq";
			}
			...
			if (this.pages && this.pages[current_page.name]) {
				...
				await saashq.after_ajax(() => this.get_data(current_page));
			}
			...
			saashq.app.sidebar.set_current_app(app);
			this.wrapper.find(".workspace-title").html(__(this._page.title));
			this.wrapper
				.find(".workspace-icon")
				.html(saashq.utils.icon(this._page.icon || "folder-normal", "md"));
	...
	setup_customization_buttons(page) {
		...
		this.page.set_secondary_action(__("Discard"), async () => {
			...
			await this.editor.readOnly.toggle();
			this.is_read_only = true;
			saashq.boot.sidebar_pages = this.cached_pages;
			this.reload();
			saashq.show_alert({ message: __("Customizations Discarded"), indicator: "info" });
		});

		if (page.name && this.has_access) {
			this.page.add_inner_button(__("Settings"), () => {
				saashq.set_route(`workspace/${page.name}`);
			});
		}
	}
	...
	initialize_new_page() {
		...
		const d = new saashq.ui.Dialog({
			...
			primary_action: (values) => {
				...
				let name = values.title + (values.is_public ? "" : "-" + saashq.session.user);
				let blocks = [
					{
						type: "header",
				...
				let new_page = {
					...
					for_user: values.is_public ? "" : saashq.session.user,
					icon: values.icon,
					indicator_color: values.indicator_color,
					parent_page: values.parent || "",
					is_editable: true,
					selected: true,
					app: saashq.current_app,
					type: values.type,
					...
					this.editor
						...
						.then(async () => {
							...
							this.create_page(new_page).then(() => {
								...
								let route = pre_url + saashq.router.slug(new_page.title);
	...
	create_page(new_page) {
		return new Promise((resolve) => {
			saashq.call({
				method: "saashq.desk.doctype.workspace.workspace.new_page",
				args: {
					new_page: new_page,
				},
				callback: (r) => {
					if (r.message) {
						...
						saashq.boot.sidebar_pages = r.message;
						this.sidebar.setup_pages();

						if (!saashq.boot.app_data_map["private"] && new_page.public === 0) {
	...
	initialize_editorjs(blocks) {
		this.tools = {
			...
			HeaderSize: saashq.workspace_block.tunes["header_size"],
		};
	...
	save_page(page) {
		...
		return this.editor
			...
			.then((outputData) => {
				...
				page.content = JSON.stringify(blocks);
				saashq.call({
					method: "saashq.desk.doctype.workspace.workspace.save_page",
					args: {
						name: page.name,
	...
	create_page_skeleton() {
		...
		this.body.prepend(saashq.render_template("workspace_loading_skeleton"));
	...
	register_awesomebar_shortcut() {
		"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter) => {
			...
			saashq.ui.keys.add_shortcut({ shortcut: letter, ...default_shortcut });
			saashq.ui.keys.add_shortcut({ shortcut: `shift+${letter}`, ...default_shortcut });
		});
	}
};
