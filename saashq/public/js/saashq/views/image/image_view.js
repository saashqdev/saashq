/**
 * saashq.views.ImageView
 */
saashq.provide("saashq.views");

saashq.views.ImageView = class ImageView extends saashq.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = saashq.utils.escape_html(title);
	...
	get_attached_images() {
		return saashq
			.call({
				method: "saashq.core.api.file.get_attached_images",
				args: {
	...
	setup_gallery() {
		var me = this;
		this.gallery = new saashq.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
	...
	get required_libs() {
		return [
			"assets/saashq/node_modules/photoswipe/src/photoswipe.css",
			"photoswipe.bundle.js",
		];
	}
};

saashq.views.GalleryView = class GalleryView {
	constructor(opts) {
		$.extend(this, opts);
		var me = this;
		me.prepare();
	...
	prepare() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = saashq.render_template("photoswipe_dom");
	...
	_show(docname) {
		...
		// init
		this.pswp = new saashq.PhotoSwipe(options);
		this.pswp.init();
	}
};
