...
saashq.views.DashboardView = class DashboardView extends saashq.views.ListView {
	get view_name() {
		return "Dashboard";
	}
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.dashboard_settings =
				saashq.get_user_settings(this.doctype)["dashboard_settings"] || null;
		});
	}
	...
	reset_dashboard_customization() {
		saashq.confirm(__("Are you sure you want to reset all customizations?"), () => {
			this.dashboard_settings = null;
			saashq.model.user_settings
		...
		const dialog = new saashq.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					get_query: () => {
						return {
							query: "saashq.desk.doctype.dashboard_chart.dashboard_chart.get_charts_for_user",
							filters: {
								document_type: this.doctype,
							},
						};
			...
			primary_action: (values) => {
				...
				if (chart.new_or_existing == "New Chart") {
					...
					saashq
						.xcall(
							"saashq.desk.doctype.dashboard_chart.dashboard_chart.create_dashboard_chart",
							{ args: chart }
						)
						.then((doc) => {
	...
	get_field_options() {
		...
		saashq.get_meta(this.doctype).fields.map((df) => {
