...
saashq.provide("saashq.interaction_settings");

saashq.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		saashq.model.with_doctype("Event", () => {
			let categories = saashq.meta
	...
	setup_attach() {
		...
		attach.find(".add-more-attachments a").on("click", () => new saashq.ui.FileUploader(args));
		this.render_attach();
	}
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)) {
			interaction_values["owner"] = saashq.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = saashq.session.user;
		}
		return saashq.call({
	...
	add_attachments(doc, attachments) {
		saashq.call({
			method: "saashq.utils.file_manager.add_attachments",
			args: {
				doctype: doc.doctype,
				name: doc.name,
