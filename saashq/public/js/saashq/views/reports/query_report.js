...
saashq.provide("saashq.widget.utils");
saashq.provide("saashq.views");
saashq.provide("saashq.query_reports");

saashq.standard_pages["query-report"] = function () {
	var wrapper = saashq.container.add_page("query-report");
	...
	saashq.query_report = new saashq.views.QueryReport({
		parent: wrapper,
	});

	$(wrapper).bind("show", function () {
		saashq.query_report.show();
	});
};

saashq.views.QueryReport = class QueryReport extends saashq.views.BaseList {
	show() {
	...
	setup_defaults() {
		...
		this.refresh = saashq.utils.throttle(this.refresh, 300);
	...
	setup_events() {
		saashq.realtime.on("report_generated", (data) => {
			...
			if (data.report_name) {
				...
				if (data.name == this.prepared_report_doc_name) {
					...
					saashq.show_alert({ message: alert_message, indicator: "orange" });
				}
			}
		});
	...
	load() {
		if (saashq.get_route().length < 2) {
			this.toggle_nothing_to_show(true);
			return;
		}

		let route_options = {};
		route_options = Object.assign(route_options, saashq.route_options);

		if (this.report_name !== saashq.get_route()[1]) {
			// different report
			this.load_report(route_options);
		} else if (saashq.has_route_options()) {
	...
	add_card_button_to_toolbar() {
		if (!saashq.model.can_create("Number Card")) return;
	...
	add_chart_buttons_to_toolbar(show) {
		if (!saashq.model.can_create("Dashboard Chart")) return;
		if (show) {
	...
	add_card_to_dashboard() {
		let field_options = saashq.report_utils.get_field_options_from_report(
			this.columns,
			this.raw_data
		);
		const dashboard_field = saashq.dashboard_utils.get_dashboard_link_field();
		const set_standard = saashq.boot.developer_mode;
	...
	add_chart_to_dashboard() {
		if (this.chart_fields || this.chart_options) {
			...
			saashq.msgprint(__("Please Set Chart"));
		}
	}
	...
	create_number_card(values, dashboard_name, card_name, set_standard) {
		...
		this.add_to_dashboard(
			"saashq.desk.doctype.number_card.number_card.create_report_number_card",
			args,
			dashboard_name,
			card_name,
			"Number Card"
	...
	create_dashboard_chart(chart_args, dashboard_name, chart_name, set_standard) {
		let args = {
			...
			chart_type: "Report",
			report_name: this.report_name,
			type: chart_args.chart_type || saashq.model.unscrub(chart_args.type),
		...
		this.add_to_dashboard(
			"saashq.desk.doctype.dashboard_chart.dashboard_chart.create_report_chart",
			args,
			dashboard_name,
			chart_name,
			"Dashboard Chart"
	...
	add_to_dashboard(method, args, dashboard_name, name, doctype) {
		saashq.xcall(method, { args: args }).then(() => {
			let message;
			if (dashboard_name) {
			...
			saashq.msgprint(message, __("New {0} Created", [__(doctype)]));
		});
	}
	...
	get_report_doc() {
		return saashq.model
			...
			.then(() => saashq.model.with_doctype(this.report_doc?.ref_doctype));
	}

	get_report_settings() {
		return new Promise((resolve, reject) => {
			if (saashq.query_reports[this.report_name]) {
				this.report_settings = saashq.query_reports[this.report_name];
				resolve();
			} else {
				saashq
					.xcall("saashq.desk.query_report.get_script", {
						report_name: this.report_name,
					})
					.then((settings) => {
						saashq.dom.eval(settings.script || "");
						saashq.after_ajax(() => {
							...
							saashq.query_reports[this.report_name] = this.report_settings;
	...
	get_local_report_settings(custom_report_name) {
		let report_script_name =
			this.report_doc.report_type === "Custom Report"
				...
				: this.report_name;
		return saashq.query_reports[report_script_name] || {};
	}
	...
	setup_progress_bar() {
		...
		this.interval = setInterval(function () {
			seconds_elapsed += 1;
			saashq.show_progress(__("Preparing Report"), seconds_elapsed, execution_time);
		}, 1000);
	}
	...
	evaluate_depends_on_value(expression, filter_label) {
		...
		if (doc) {
			if (typeof expression === "boolean") {
				...
				try {
					out = saashq.utils.eval(expression.substr(5), { doc });
				} catch (e) {
					saashq.throw(
	...
	set_route_filters(route_options) {
		if (!route_options) route_options = saashq.route_options;
		...
		if (route_options) {
			...
			return saashq.run_serially(promises);
		}
	}
	...
	refresh(have_filters_changed) {
		...
		return new Promise((resolve) => {
			this.last_ajax = saashq.call({
				method: "saashq.desk.query_report.run",
				type: "GET",
				args: {
					report_name: this.report_name,
					filters: filters,
			...
			.then((r) => {
				...
				if (data.result && data.result.length) {
					...
					if (this.chart_options) {
						...
						this.$chart.empty();
						if (this.chart_fields) {
							this.chart_options = saashq.report_utils.make_chart_options(
								this.columns,
								this.raw_data,
								this.chart_fields
							);
	...
	render_summary(data) {
		data.forEach((summary) => {
			saashq.utils.build_summary_item(summary).appendTo(this.$summary);
		});
	...
	get_query_params() {
		const query_string = saashq.utils.get_query_string(saashq.get_route_str());
		return saashq.utils.get_query_params(query_string);
	}
	...
	add_prepared_report_buttons(doc) {
		if (doc) {
				...
				function () {
					window.open(
						saashq.urllib.get_full_url(
							"/api/method/saashq.core.doctype.prepared_report.prepared_report.download_attachment?" +
								"dn=" +
			...
			let pretty_diff = saashq.datetime.comment_when(doc.report_end_time);
			const days_old = saashq.datetime.get_day_diff(
				saashq.datetime.now_datetime(),
				doc.report_end_time
			);
			if (days_old > 1) {
		...
		this.primary_action_map = {
			...
			Edit: {
				label: __("Edit"),
				click: () => {
					saashq.set_route(saashq.get_route());
				},
			},
			Rebuild: {
				label: __("Rebuild"),
	...
	show_warning_or_generate_report() {
		saashq
			.xcall(
				"saashq.core.doctype.prepared_report.prepared_report.get_reports_in_queued_state",
				{
			...
			.then((reports) => {
				...
				if (reports.length) {
					...
					this.prepared_report_dialog = saashq.warn(
					...
					saashq.utils.bind_actions_with_object(
	...
	delete_old_queued_reports() {
		...
		saashq
			.xcall("saashq.core.doctype.prepared_report.prepared_report.delete_prepared_reports", {
	...
	generate_background_report() {
		...
		if (!missing_mandatory.length) {
			...
			return new Promise((resolve) =>
				saashq.call({
					method: "saashq.core.doctype.prepared_report.prepared_report.make_prepared_report",
					args: {
						report_name: this.report_name,
						filters: filters,
					},
				...
				let alert_message =
					...
					`</a>`;
				saashq.show_alert({ message: alert_message, indicator: "orange" }, 10);
				this.toggle_nothing_to_show(true);
			});
		}
	}
	...
	render_datatable() {
		...
		if (data.length > 1000000) {
			...
			this.toggle_message(true, `${saashq.utils.icon("solid-warning")} ${msg}`);
			return;
		}
		...
		if (
			...
			let datatable_options = {
				...
				data: data,
				inlineFilters: true,
				language: saashq.boot.lang,
				translations: saashq.utils.datatable.get_translations(),
				treeView: this.tree_report,
				layout: "fixed",
				cellHeight: 33,
				showTotalRow: this.raw_data.add_total_row && !this.report_settings.tree,
				direction: saashq.utils.is_rtl() ? "rtl" : "ltr",
				hooks: {
					columnTotal: saashq.utils.report_column_total,
				},
			};
	...
	render_chart(options) {
		this.$chart.empty();
		this.$chart.show();
		this.chart = new saashq.Chart(this.$chart[0], options);
	}
	...
	open_create_chart_dialog() {
		...
		function preview_chart() {
			...
			if (values.x_field && values.y_fields.length) {
				let options = saashq.report_utils.make_chart_options(
					me.columns,
					me.raw_data,
					values
				);
				...
				wrapper.empty();
				new saashq.Chart(wrapper[0], options);
	...
	prepare_columns(columns) {
		return columns.map((column) => {
			column = saashq.report_utils.prepare_field_from_column(column);
			...
			if (column.fieldtype === "Date") {
				compareFn = (cell, keyword) => {
					...
					const keywordValue = saashq.datetime.user_to_obj(keyword);
					const cellValue = saashq.datetime.str_to_obj(cell.content);
	...
	set_breadcrumbs() {
		...
		const ref_doctype = saashq.get_meta(this.report_doc.ref_doctype);
		saashq.breadcrumbs.add(ref_doctype.module);
	}

	make_access_log(method, file_format) {
		saashq.call("saashq.core.doctype.access_log.access_log.make_access_log", {
	...
	pdf_report(print_settings) {
		const base_url = saashq.urllib.get_base_url();
		const print_css = saashq.boot.print_css;
		const landscape = print_settings.orientation == "Landscape";
		...
		const content = saashq.render_template(template, {
			title: __(this.report_name),
			subtitle: filters_html,
		...
		const html = saashq.render_template("print_template", {
			title: __(this.report_name),
			content: content,
			base_url: base_url,
			print_css: print_css,
			...
			landscape: landscape,
			columns: columns,
			lang: saashq.boot.lang,
			layout_direction: saashq.utils.is_rtl() ? "rtl" : "ltr",
			can_use_smaller_font: this.report_doc.is_standard === "Yes" && custom_format ? 0 : 1,
		});
		...
		saashq.render_pdf(html, print_settings);
	}

	get_filters_html_for_print() {
		const applied_filters = this.get_filter_values();
		return Object.keys(applied_filters)
			.filter((fieldname) => !saashq.utils.is_empty(applied_filters[fieldname]))
			.map((fieldname) => {
				const docfield = saashq.query_report.get_filter(fieldname).df;
				const value = applied_filters[fieldname];
				return `<div class="filter-row">
					<b>${__(docfield.label, null, docfield.parent)}:</b> ${saashq.format(value, docfield)}
				</div>`;
			})
			.join("");
	}
	...
	export_report() {
		...
		this.export_dialog = saashq.report_utils.get_export_dialog(
			__(this.report_name),
			extra_fields,
			({
				file_format,
				...
				let applied_filters = Object.fromEntries(
					Object.entries(filters).map(([key, value]) => [
						saashq.query_report.get_filter(key).df.label,
						saashq.query_report.get_filter(key).df.fieldtype == "Check"
				...
				const args = {
					cmd: "saashq.desk.query_report.export_query",
					report_name: this.report_name,
				...
				open_url_post(saashq.request.url, args);
		...
		return rows.map((row) => {
			...
			return row.slice(standard_column_count).map((cell, i) => {
				if (cell.column.fieldtype === "Duration") {
					cell.content = saashq.utils.get_formatted_duration(cell.content);
				}
	...
	get_menu_items() {
		let items = [
			...
			{
				label: __("Edit"),
				action: () => saashq.set_route("Form", "Report", this.report_name),
				condition: () => saashq.user.is_report_manager(),
				standard: true,
			},
			{
				label: __("Print"),
				action: () => {
					let dialog = saashq.ui.get_print_settings(
						false,
				...
				condition: () => saashq.model.can_print(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("PDF"),
			...
			},
			{
				...
				condition: () => saashq.model.can_export(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("Setup Auto Email"),
				action: () =>
					saashq.set_route("List", "Auto Email Report", { report: this.report_name }),
				standard: true,
			},
			{
				label: __("Add Column"),
				action: () => {
					let d = new saashq.ui.Dialog({
						...
						fields: [
							{
								...
								change: () => {
									...
									saashq.model.with_doctype(doctype, () => {
										let options = saashq.meta
											.get_docfields(doctype)
											.filter(saashq.model.is_value_type)
											.map((df) => ({
												label: df.label,
												value: df.fieldname,
											}));
						...
						primary_action: (values) => {
							...
							let df = saashq.meta.get_docfield(values.doctype, values.field);
							...
							custom_columns.push({
								fieldname: this.columns
									...
									? df.fieldname + "-" + saashq.scrub(values.doctype)
									: df.fieldname,
							...
							saashq.call({
								method: "saashq.desk.query_report.get_data_for_custom_field",
								args: {
									field: values.field,
									doctype: values.doctype,
									names: Array.from(
			...
			},
			{
				...
				condition: () => saashq.user.has_role("System Manager"),
				standard: true,
			},
		];

		if (saashq.user.is_report_manager()) {
			items.push({
				label: __("Save"),
				action: () => {
					let d = new saashq.ui.Dialog({
						...
						primary_action: (values) => {
							saashq.call({
								method: "saashq.desk.query_report.save_report",
								args: {
								...
								callback: function (r) {
									this.show_save = false;
									d.hide();
									saashq.set_route("query-report", r.message);
								},
							});
						},
					});
	...
	add_custom_column(
		...
		this.data.forEach((row) => {
			if (column[0].fieldname.includes("-")) {
				row[column_field + "-" + saashq.scrub(new_column_data.doctype)] =
