...
saashq.views.ReportView = class ReportView extends saashq.views.ListView {
	get view_name() {
		return "Report";
	}
	...
	setup_events() {
		...
		saashq.realtime.on("list_update", (data) => this.on_update(data));
	}
	...
	setup_sort_selector() {
		...
		this.group_by_control = new saashq.ui.GroupBy(this);
	...
	before_refresh() {
		if (this.report_doc) {
			// don't parse saashq.route_options if this is a Custom Report
	...
	set_dirty_state_for_custom_report() {
		...
		if (!saashq.utils.deep_equal(current_settings, report_settings)) {
	...
	prepare_data(r) {
		let data = r.message || {};
		data = saashq.utils.dict(data.keys, data.values);
	...
	on_update(data) {
		if (this.doctype === data.doctype && data.name) {
			...
			const flash_row = data.user !== saashq.session.user;
	...
	setup_charts() {
		...
		for (let col of this.columns) {
			...
			// numeric values in y
			if (col.docfield && saashq.model.numeric_fieldtypes.includes(col.docfield.fieldtype)) {
				y_fields.push({
					label: col.content,
					fieldname: col.id,
					value: col.id,
			...
			fields: [
				...
				},
				{
					...
					default: defaults.chart_type
						? saashq.utils.to_title_case(defaults.chart_type)
						: "Bar",
				},
			],
			primary_action: (data) => {
	...
	make_chart() {
		...
		this.chart = new saashq.Chart(this.$charts_wrapper.find(".charts-inner-wrapper")[0], {
			...
			tooltipOptions: {
				formatTooltipY: (value) => {
					return saashq.format(value, get_df(this.chart_args.y_axes[0]), {
	...
	render_editing_input(colIndex, value, parent) {
		...
		if (col.docfield.fieldtype === "Text Editor") {
			...
			// make control
			control = saashq.ui.form.make_control({
				df: col.docfield,
				parent: parent,
				render_input: true,
			});
	...
	evaluate_read_only_depends_on(expression, data) {
		...
		if (typeof expression === "boolean") {
			...
			try {
				out = saashq.utils.eval(expression.substr(5), { doc: data });
	...
	is_editable(df, data) {
		return (
			df &&
			saashq.model.can_write(this.doctype) &&
			...
			saashq.model.is_non_std_field(df.fieldname) &&
	...
	get_fields() {
		let fields = this.fields.map((f) => {
			let column_name = saashq.model.get_full_column_name(f[0], f[1]);
			if (f[1] !== this.doctype) {
				// child table field
		...
		const cdt_name_fields = this.get_unique_cdt_in_view().map(
			(cdt) => saashq.model.get_full_column_name("name", cdt) + " as " + `'${cdt}:name'`
		);
	...
	add_currency_column(fieldname, doctype, col_index) {
		...
		const df = saashq.meta.get_docfield(doctype, fieldname);
		if (
			df &&
			df.fieldtype === "Currency" &&
			df.options &&
			!df.options.includes(":") &&
			saashq.meta.has_field(doctype, df.options)
		) {
			...
			const field_label = saashq.meta.get_label(doctype, field[0]);
			saashq.show_alert(
	...
	remove_column_from_datatable(column) {
		...
		if (field[0] === "name") {
			this.refresh();
			saashq.throw(__("Cannot remove ID field"));
		}
	...
	get_columns_for_picker() {
		...
		const standard_fields_filter = (df) => !saashq.model.no_value_type.includes(df.fieldtype);

		let doctype_fields = saashq.meta
			.get_docfields(this.doctype)
			.filter(standard_fields_filter);

		// filter out docstatus field from picker
		let std_fields = saashq.model.std_fields.filter(
		...
		if (this.data) {
			has_status_values = saashq.get_indicator(this.data[0], this.doctype);
		}

		if (!saashq.meta.has_field(this.doctype, "status") && has_status_values) {
			doctype_fields = [
				{
					label: __("Status"),
					fieldname: "docstatus",
		...
		const table_fields = saashq.meta.get_table_fields(this.doctype);

		table_fields.forEach((df) => {
			const cdt = df.options;
			const child_table_fields = saashq.meta
				.get_docfields(cdt)
				.filter(standard_fields_filter);
	...
	get_dialog_fields() {
		...
		const table_fields = saashq.meta.get_table_fields(this.doctype).filter((df) => !df.hidden);
		...
		for (let f of this.fields) {
			...
			if (f[0] !== "docstatus") {
				...
				if (
					...
					!saashq.meta.has_field(this.doctype, "status")
				) {
	...
	build_column(c) {
		...
		let docfield = saashq.meta.docfield_map[doctype || this.doctype][fieldname];
		...
		if (!docfield) {
			docfield = saashq.model.get_std_field(fieldname, true);
			...
			if (docfield) {
				...
				if (fieldname == "docstatus" && !saashq.meta.has_field(this.doctype, "status")) {
		...
		const editable =
			saashq.model.is_non_std_field(fieldname) &&
			!docfield.read_only &&
			!docfield.is_virtual;

		const align = (() => {
			const is_numeric = saashq.model.is_numeric_field(docfield);
			if (is_numeric) {
				return "right";
			}
		...
		return {
			...
			format: (value, row, column, data) => {
				...
				return saashq.format(value, column.docfield, { always_show_decimals: true }, doc);
			},
		};
	}
		...
		if (this.add_totals_row) {
			...
			const totals_row = this.columns.map((col, i) => {
				return {
					...
					format: (value) => {
						let formatted_value = saashq.format(
							value,
							col.docfield,
							{
								always_show_decimals: true,
	...
	format_total_cell(formatted_value, df) {
		...
		if (saashq.model.is_numeric_field(df.docfield)) {
	...
	build_row(d) {
		return this.columns.map((col) => {
			...
			if (col.field === "docstatus" && !saashq.meta.has_field(this.doctype, "status")) {
				// get status from docstatus
				let status = saashq.get_indicator(d, this.doctype);
				if (status) {
	...
	save_report(save_type) {
		const _save_report = (name) => {
			...
			return saashq.call({
				method: "saashq.desk.reportview.save_report",
				args: {
					name: name,
					doctype: this.doctype,
				...
				callback: (r) => {
					if (r.exc) {
						saashq.msgprint(__("Report was not saved (there were errors)"));
						return;
					}
					if (r.message != this.report_name) {
						// Rerender the reports dropdown,
						// so that this report is included in the dropdown as well.
						saashq.boot.user.all_reports[r.message] = {
							ref_doctype: this.doctype,
							report_type: "Report Builder",
							title: r.message,
						};

						saashq.set_route("List", this.doctype, "Report", r.message);
						return;
					}
	...
	delete_report() {
		return saashq.call({
			method: "saashq.desk.reportview.delete_report",
	...
	get_report_doc() {
		return new Promise((resolve) => {
			saashq.model.with_doc("Report", this.report_name, () => {
				resolve(saashq.get_doc("Report", this.report_name));
			});
		});
	}
		...
		return (
			...
			filters
				.map((f) => {
					...
					const docfield = saashq.meta.get_docfield(doctype, fieldname);
					const label = `<b>${__(saashq.meta.get_label(doctype, fieldname))}</b>`;
					switch (condition) {
						case "=":
							return __("{0} is equal to {1}", [
								label,
								saashq.format(value, docfield),
							]);
						case "!=":
						...
						case "Between":
							return __("{0} is between {1} and {2}", [
								__(label),
								saashq.format(value[0], docfield),
								saashq.format(value[1], docfield),
							]);
						case "Timespan":
						...
						case "in":
							return __("{0} is one of {1}", [
								__(label),
								saashq.utils.comma_or(
									value.map((v) => saashq.format(v, docfield))
								),
							]);
						case "not in":
	...
	get_columns_totals(data) {
		...
		this.columns.forEach((col, i) => {
			const totals = data.reduce((totals, d) => {
				if (col.id in d && saashq.model.is_numeric_field(col.docfield)) {
					totals += flt(d[col.id]);
					return totals;
				}
			}, 0);
	...
	report_menu_items() {
		let items = [
			...
			},
			{
				...
				action: () => {
					...
					saashq.ui.get_print_settings(false, (print_settings) => {
			...
			},
			{
				...
				action: () => {
					...
					saashq.utils.setup_search(d.$body, ".unit-checkbox", ".label-area");
					d.show();
				},
			},
		];

		if (saashq.model.can_export(this.doctype)) {
			items.push({
				label: __("Export"),
				action: () => {
					...
					const d = saashq.report_utils.get_export_dialog(
						__(this.doctype),
						extra_fields,
						(data) => {
							args.cmd = "saashq.desk.reportview.export_query";
		...
		items.push({
			...
			action: () => {
				if (this.report_name) {
					...
					saashq.msgprint(__("Please save the report first"));
				}
			},
		});
		...
		const can_edit_or_delete = (action) => {
			...
			return (
				...
				(saashq.model[method]("Report") || this.report_doc.owner === saashq.session.user)
			);
		};
		...
		if (can_edit_or_delete("delete")) {
			items.push({
				label: __("Delete"),
				action: () =>
					saashq.confirm("Are you sure you want to delete this report?", () =>
						this.delete_report()
					),
		...
		// user permissions
		if (this.report_name && saashq.user.has_role("System Manager")) {
			items.push({
				...
				action: () => {
					...
					saashq.set_route("List", "User Permission", args);
				},
			});
		}
	...
	parse_filters_from_route_options() {
		if (saashq.route_options?._group_by) {
			try {
				let config = JSON.parse(saashq.route_options._group_by);
				this.group_by_control.apply_settings({
					group_by: config[0],
					aggregate_on: config[1],
					aggregate_function: config[2],
				});
				delete saashq.route_options["_group_by"];
			} catch (e) {
