saashq.provide("saashq.report_utils");
...
saashq.report_utils = {
	make_chart_options: function (
		...
		let datasets = y_fields.map((y_field) => ({
			name: saashq.model.unscrub(y_field),
		...
		function get_options(fields) {
			return fields.map((field) => {
				if (field.fieldname) {
					...
					field = saashq.report_utils.prepare_field_from_column(field);
					return { label: field.label, value: field.fieldname };
				}
			});
		}
	...
	get_report_filters: function (report_name) {
		if (saashq.query_reports[report_name]) {
			let filters = saashq.query_reports[report_name].filters;
		...
		return saashq
			...
			.then((r) => {
				saashq.dom.eval(r.script || "");
				return saashq.after_ajax(() => {
					if (
						saashq.query_reports[report_name] &&
						!saashq.query_reports[report_name].filters &&
						r.filters
					) {
						return (saashq.query_reports[report_name].filters = r.filters);
					}
					return (
						saashq.query_reports[report_name] &&
						saashq.query_reports[report_name].filters
	...
	get_export_dialog(report_name, extra_fields, callback) {
		...
		function update_csv_preview(dialog) {
			const is_query_report = saashq.get_route()[0] === "query-report";
			const report = is_query_report ? saashq.query_report : cur_list;
			...
			dialog.set_value(
				"csv_preview",
				saashq.report_utils.get_csv_preview(
					PREVIEW_DATA,
	...
	get_csv_preview(data, quoting, delimiter, decimal_sep) {
		...
		if (delimiter.length > 1) {
			saashq.throw(__("Delimiter must be a single character"));
		}

		if (decimal_sep.length > 1) {
			saashq.throw(__("Decimal Separator must be a single character"));
		}

		if (0 > quoting || quoting > 3) {
			saashq.throw(__("Quoting must be between 0 and 3"));
		}

		if (decimal_sep !== "." && quoting === QUOTING.NonNumeric) {
			saashq.throw(__("Decimal Separator must be '.' when Quoting is set to Non-numeric"));
		}
