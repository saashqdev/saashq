...
saashq.realtime.on("build_event", (data) => {
	if (data.success) {
			...
			for (let file of changed_files) {
				if (file.includes(".bundle.")) {
					...
					if (parts.length === 2) {
						...
						saashq.assets._executed = saashq.assets._executed.filter(
		...
		// update assets json
		saashq.call("saashq.sessions.get_boot_assets_json").then((r) => {
			if (r.message) {
				saashq.boot.assets_json = r.message;

				if (saashq.hot_update) {
					saashq.hot_update.forEach((callback) => {
						callback();
					});
				}
			}
