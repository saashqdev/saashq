...
saashq.ready(function () {
	let web_form_doc = saashq.web_form_doc;
	let reference_doc = saashq.reference_doc;
	...
	function show_login_prompt() {
		if (saashq.session.user != "Guest" || !web_form_doc.login_required) return;
		const login_required = new saashq.ui.Dialog({
	...
	function show_form() {
		...
		$(".file-size").each(function () {
			$(this).text(saashq.form.formatters.FileSize($(this).text()));
		});
	}
