...
saashq.provide("saashq.web_form_list");

export default class WebFormList {
	constructor(opts) {
		Object.assign(this, opts);
		saashq.web_form_list = this;
	...
	make_filters() {
		...
		saashq
			.call("saashq.website.doctype.web_form.web_form.get_web_form_filters", {
			...
			.then((response) => {
				...
				fields.forEach((field) => {
					...
					let input = saashq.ui.form.make_control({
						df: {
	...
	fetch_data() {
		if (this.condition_json && JSON.parse(this.condition_json)) {
			let filter = saashq.utils.get_filter_from_json(this.condition_json, this.doctype);
			filter = saashq.utils.get_filter_as_json(filter);
			this.filters = Object.assign(this.filters, JSON.parse(filter));
		}

		let args = {
			method: "saashq.www.list.get_list_data",
			args: {
	...
	more() {
		...
		this.fetch_data().then((res) => {
			if (res.message.length === 0) {
				saashq.msgprint(__("No more items to display"));
			}
	...
	make_table_body() {
		if (this.data.length) {
			...
			this.wrapper.empty();
			saashq.has_permission(this.doctype, "", "create", () => {
	...
	append_rows(row_data) {
		...
		row_data.forEach((data_item) => {
			...
			let row = new saashq.ui.WebFormListRow({
				row: $row_element,
				doc: data_item,
	...
	make_actions() {
		...
		saashq.has_permission(this.doctype, "", "delete", () => {
	...
	delete_rows() {
		...
		saashq
			.call({
				type: "POST",
				method: "saashq.website.doctype.web_form.web_form.delete_multiple",
				args: {
					web_form_name: this.web_form_name,
...
saashq.ui.WebFormListRow = class WebFormListRow {
	...
	make_row() {
		...
		this.columns.forEach((field) => {
			let formatter = saashq.form.get_formatter(field.fieldtype);
			let value =
