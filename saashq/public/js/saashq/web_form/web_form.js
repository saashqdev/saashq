...
saashq.provide("saashq.web_form");
import EventEmitterMixin from "../../saashq/event_emitter";

export default class WebForm extends saashq.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		saashq.web_form = this;
		saashq.web_form.events = {};
		Object.assign(saashq.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		saashq.init_client_script && saashq.init_client_script();
		this.setup_listeners();
		saashq.web_form.events.trigger("after_load");
	...
	set_default_values() {
		...
		let values = saashq.utils.get_query_params();
		delete values.new;
	...
	render_progress_dots() {
		...
		for (let i = 0; i <= this.page_breaks.length; i++) {
			let $dot = $(`<div class="slide-step">
				...
				<div class="slide-step-complete">${saashq.utils.icon("tick", "xs")}</div>
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _page.find(".saashq-control:not(.hide-control)");
	...
	save() {
		...
		// Save
		window.saving = true;
		saashq.form_dirty = false;

		saashq.call({
			type: "POST",
			method: "saashq.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
				for_payment,
			...
			callback: (response) => {
				...
				if (!response.exc) {
					...
					saashq.web_form.events.trigger("after_save");
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						saashq.call({
							type: "POST",
							method: "saashq.handler.upload_file",
							args: {
	...
	handle_success(data) {
		...
		if (this.success_url) {
			saashq.utils.setup_timer(5, 0, $(".time"));
			setTimeout(() => {
