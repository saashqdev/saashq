...
export async function load_doctype_model(doctype) {
	await saashq.call("saashq.desk.form.load.getdoctype", { doctype });
}

export async function get_table_columns(df, child_doctype) {
	let table_columns = [];

	if (!saashq.get_meta(df.options)) {
		await load_doctype_model(df.options);
	}
	if (!child_doctype) {
		child_doctype = saashq.get_meta(df.options);
	}
	...
	for (let tf of table_fields) {
		if (!saashq.model.layout_fields.includes(tf.fieldtype) && tf.in_list_view && tf.label) {
			let colsize;
	...
	function adjust_column_size(total_colsize, increase) {
		...
		while (start_condition && passes < 11) {
			for (var i in table_columns) {
				...
				if (colsize > 1 && colsize < 11 && saashq.model.is_non_std_field(_df.fieldname)) {
					if (
						passes < 3 &&
		...
		if (d.fieldtype) {
			if (!d.fieldname) {
				if (d.label) {
					...
					if (saashq.model.restricted_fields.includes(d.fieldname)) {
					...
					d.fieldname =
						...
						"_" +
						saashq.utils.get_random(4);
				}
			} else {
				if (saashq.model.restricted_fields.includes(d.fieldname)) {
					saashq.throw(__("Fieldname {0} is restricted", [d.fieldname]));
				}
			}
...
export function clone_field(field) {
	...
	cloned_field.df.name = saashq.utils.get_random(8);
	return cloned_field;
}
