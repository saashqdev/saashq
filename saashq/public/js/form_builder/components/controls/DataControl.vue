...
if (props.df.fieldtype === "Datetime") {
	let time_zone_text = saashq.boot.time_zone
		? saashq.boot.time_zone.user
		: saashq.sys_defaults.time_zone;
...
<template>
	<div class="control saashq-control" :class="{ editable: slots.label }">
		<!-- label -->
		...
		<div
			...
			v-html="saashq.utils.icon('folder-normal', 'md')"
		/>
		<!-- phone selector icon -->
		<div
			v-if="df.fieldtype == 'Phone'"
			class="selected-phone no-value"
			v-html="saashq.utils.icon('down', 'sm')"
		/>
	</div>
</template>
