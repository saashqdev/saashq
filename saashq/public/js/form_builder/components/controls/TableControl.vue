...
let table_columns = computedAsync(async () => {
	...
	if (!doctype) return [];
	if (!saashq.get_meta(doctype)) {
		await load_doctype_model(doctype);
	}
	let child_doctype = saashq.get_meta(doctype);
...
function open_new_child_doctype_dialog() {
	...
	saashq.model.with_doctype("DocType").then(() => {
		saashq.listview_settings["DocType"].new_doctype_dialog({ is_child: 1, is_custom });
	});
}
</script>
...
<template>
	<div class="control editable">
		...
		<div class="grid-empty text-center">
			<img
				src="/assets/saashq/images/ui-states/grid-empty-state.svg"
