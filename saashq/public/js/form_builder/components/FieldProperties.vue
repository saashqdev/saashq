...
let docfield_df = computed(() => {
	let fields = store.get_docfields.filter((df) => {
		if (in_list(saashq.model.layout_fields, df.fieldtype) || df.hidden) {
			return false;
		}
		if (
			df.depends_on &&
		...
		if (
			...
			in_list(saashq.model.no_value_type, store.form.selected_field.fieldtype)
		) {
			return false;
		}
...
<template>
	<div class="header">
		...
		>
			<div v-html="saashq.utils.icon('remove', 'sm')"></div>
		</button>
	</div>
