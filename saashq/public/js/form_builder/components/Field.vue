...
function remove_field() {
	if (store.is_customize_form && props.field.df.is_custom_field == 0) {
		saashq.msgprint(__("Cannot delete standard field. You can hide it if you want"));
...
function duplicate_field() {
	...
	duplicate_field.df.owner = saashq.session.user;
...
function make_dialog(frm) {
	frm.dialog = new saashq.ui.Dialog({
	...
	if (frm.doctype === "Customize Form") {
		...
		saashq.db.exists("Property Setter", property_setter_id).then((exits) => {
			if (exits) {
				frm.dialog.set_secondary_action_label(__("Reset To Default"));
				frm.dialog.set_secondary_action(() => {
					saashq.call({
						method: "saashq.custom.doctype.customize_form.customize_form.get_link_filters_from_doc_without_customisations",
						args: {
...
function edit_filters() {
	...
	saashq.model.with_doctype(field_doctype, () => {
		frm.dialog.show();
		add_existing_filter(frm, props.field.df);
	});
}
			...
			<template #label>
				<div class="field-label">
					...
					<div
						...
						v-html="saashq.utils.icon('help', 'sm')"
					/>
				</div>
			</template>
			<template #actions>
				<div class="field-actions" :hidden="store.read_only">
					<button
						...
						@click="edit_filters"
					>
						<div v-html="saashq.utils.icon('filter', 'sm')" />
					</button>
					<AddFieldButton ref="add_field_ref" :column="column" :field="field">
						<div v-html="saashq.utils.icon('add', 'sm')" />
					</AddFieldButton>
					<button
					...
					>
						<div v-html="saashq.utils.icon('move', 'sm')" />
					</button>
					<button
						class="btn btn-xs btn-icon"
					...
					>
						<div v-html="saashq.utils.icon('duplicate', 'sm')" />
					</button>
					<button
					...
					>
						<div v-html="saashq.utils.icon('external-link', 'sm')" />
					</button>
					<button
						class="btn btn-xs btn-icon"
					...
					>
						<div v-html="saashq.utils.icon('remove', 'sm')" />
					</button>
				</div>
			</template>
		</component>
