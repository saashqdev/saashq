"""
This module provides functionality for discovering and organizing tests in the Saashq framework.
...
logger = logging.getLogger("saashq.testing.discovery")
...
def discover_doctype_tests(doctypes: list[str], runner, app: str, force: bool = False) -> "TestRunner":
	...
	for doctype in doctypes:
		try:
			...
			doctype_app = saashq.db.get_value("Module Def", module, "app_name")
			if app is None:
				_app = doctype_app
			...
			test_module = saashq.modules.utils.get_module_name(doctype, module, "test_")
			force and saashq.db.delete(doctype)
