...
def get_queues_timeout() -> dict[str, int]:
	...
	"""
	common_site_config = saashq.get_conf()
...
def enqueue(
	...
	**kwargs,
	...
	duplicate calls
	:param now: if now=True, the method is executed via saashq.call()
	...
	if deduplicate:
		if not job_id:
			saashq.throw(_("`job_id` paramater is required for deduplication."))
		job = get_job(job_id)
		if job and job.get_status() in (JobStatus.QUEUED, JobStatus.STARTED):
			saashq.logger().debug(f"Not queueing job {job.id} because it is in queue already")
			return
		elif job:
	...
	if not is_async and not saashq.flags.in_test:
		from saashq.deprecation_dumpster import deprecation_warning
	...
	call_directly = now or (not is_async and not saashq.flags.in_test)
	if call_directly:
		return saashq.call(method, **kwargs)

	try:
		q = get_queue(queue, is_async=is_async)
	except ConnectionError:
		if saashq.local.flags.in_migrate:
	...
	def enqueue_call():
		return q.enqueue_call(
			...
			kwargs=queue_args,
			at_front=at_front,
			failure_ttl=saashq.conf.get("rq_job_failure_ttl") or RQ_JOB_FAILURE_TTL,
			result_ttl=saashq.conf.get("rq_results_ttl") or RQ_RESULTS_TTL,
			job_id=job_id,
		)

	if enqueue_after_commit:
		saashq.db.after_commit.add(enqueue_call)
		return
...
def enqueue_doc(doctype, name=None, method=None, queue="default", timeout=300, now=False, **kwargs):
	...
	return enqueue(
		"saashq.utils.background_jobs.run_doc_method",
		doctype=doctype,
		name=name,
		doc_method=method,
		queue=queue,
...
def run_doc_method(doctype, name, doc_method, **kwargs):
	getattr(saashq.get_doc(doctype, name), doc_method)(**kwargs)
...
def execute_job(site, method, event, job_name, kwargs, user=None, is_async=True, retry=0):
	...
	for before_job_task in saashq.get_hooks("before_job"):
		saashq.call(before_job_task, method=method_name, kwargs=kwargs, transaction_type="job")

	try:
		retval = method(**kwargs)

	except (saashq.db.InternalError, saashq.RetryBackgroundJobError) as e:
		saashq.db.rollback()

		if retry < 5 and (
			isinstance(e, saashq.RetryBackgroundJobError)
			or (saashq.db.is_deadlocked(e) or saashq.db.is_timedout(e))
		):
			# retry the job if
			# 1213 = deadlock
			...
			saashq.log_error(title=method_name)
			raise

	except Exception as e:
		saashq.db.rollback()
		saashq.log_error(title=method_name)
		saashq.monitor.add_data_to_monitor(exception=e.__class__.__name__)
		saashq.db.commit()
		...
		for after_job_task in saashq.get_hooks("after_job"):
			saashq.call(after_job_task, method=method_name, kwargs=kwargs, result=retval)
		saashq.local.job.after_job.run()
...
class SaashqWorker(Worker):
	def work(self, *args, **kwargs):
		self.start_saashq_scheduler()
	...
	def start_saashq_scheduler(self):
...
def start_worker_pool(
	...
	import saashq.database.query  # sqlparse and indirect imports
	import saashq.query_builder  # pypika
	import saashq.utils  # common utils
	...
	pool = WorkerPool(
		...
		worker_class=SaashqWorker,  # Auto starts scheduler with workerpool
	)
...
def validate_queue(queue: str, default_queue_list: list | None = None) -> None:
	...
	if queue not in default_queue_list:
		saashq.throw(_("Queue should be one of {0}").format(", ".join(default_queue_list)))
...
def get_redis_conn(username=None, password=None):
	if not hasattr(saashq.local, "conf"):
		raise Exception("You need to call saashq.init")
	...
	try:
		...
		log(
			f"Please make sure that Redis Queue runs @ {saashq.get_conf().redis_queue}. Redis reported error: {e!s}",
			colour="red",
		)
		raise
...
def enqueue_test_job():
	enqueue("saashq.utils.background_jobs.test_job", s=100)
...
def create_job_id(job_id: str | None = None) -> str:
	...
	if not job_id:
		job_id = str(uuid4())
	return f"{saashq.local.site}::{job_id}"
...
def truncate_failed_registry(job, connection, type, value, traceback):
	...
	from saashq.utils import create_batch

	conf = saashq.conf if saashq.conf else saashq.get_conf(site=job.kwargs.get("site"))
...
def flush_telemetry():
	...
	to push events."""
	ph = getattr(saashq.local, "posthog", None)
	with suppress(Exception):
		ph and ph.flush()


def _start_sentry():
	sentry_dsn = os.getenv("SAASHQ_SENTRY_DSN")
	if not sentry_dsn:
		return
