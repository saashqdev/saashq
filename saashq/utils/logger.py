...
default_log_level = logging.WARNING if saashq._dev_server else logging.ERROR
stream_logging = os.environ.get("SAASHQ_STREAM_LOGGING")


def create_handler(module, site=None, max_size=100_000, file_count=20, stream_only=False):
	"""Create and return a Saashq-specific logging handler."""
...
def get_logger(
	...
	if allow_site is True:
		site = getattr(saashq.local, "site", None)
	...
	try:
		return saashq.loggers[logger_name]
	except KeyError:
		pass
	...
	logger.setLevel(saashq.log_level or default_log_level)
	...
	saashq.loggers[logger_name] = logger
...
class SiteContextFilter(logging.Filter):
	...
	def filter(self, record) -> bool:
		if "Form Dict" not in str(record.msg):
			site = getattr(saashq.local, "site", None)
			form_dict = sanitized_dict(getattr(saashq.local, "form_dict", None))
...
def set_log_level(level: Literal["ERROR", "WARNING", "WARN", "INFO", "DEBUG"]) -> None:
	...
	saashq.log_level = getattr(logging, (level or "").upper(), None) or default_log_level
	saashq.loggers = {}
