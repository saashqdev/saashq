...
class RedisWrapper(redis.Redis):
	...
	def __call__(self):
		"""WARNING: Added for backward compatibility to support saashq.cache().method(...)"""
		return self
	...
	def make_key(self, key, user=None, shared=False):
		...
		return f"{saashq.conf.db_name}|{key}".encode()
	...
	def get_value(self, key, generator=None, user=None, expires=False, shared=False):
		...
		:param expires: If the key is supposed to be with an expiry, don't store it in saashq.local
		"""
		original_key = key
		key = self.make_key(key, user, shared)

		if key in saashq.local.cache:
			val = saashq.local.cache[key]
	...
	def get_keys(self, key):
		...
		try:
			...
			return [k for k in list(saashq.local.cache) if regex.match(cstr(k))]
	...
	def delete_value(self, keys, user=None, make_keys=True, shared=False):
		...
		for key in keys:
			saashq.local.cache.pop(key, None)
	...
	def hset(
		...
		**kwargs,
		...
		# set in local
		saashq.local.cache.setdefault(_name, {})[key] = value
	...
	def hget(self, name, key, generator=None, shared=False):
		_name = self.make_key(name, shared=shared)
		if _name not in saashq.local.cache:
			saashq.local.cache[_name] = {}

		if not key:
			return None

		if key in saashq.local.cache[_name]:
			return saashq.local.cache[_name][key]
		...
		if value is not None:
			...
			saashq.local.cache[_name][key] = value
		elif generator:
			value = generator()
	...
	def hdel(
		...
		:param shared: shared saashq key or not
		:param pipeline: A redis.client.Pipeline object, if this transaction is to be run in a pipeline
		"""
		_name = self.make_key(name, shared=shared)

		name_in_local_cache = _name in saashq.local.cache

		if not isinstance(keys, list | tuple):
			if name_in_local_cache and keys in saashq.local.cache[_name]:
				del saashq.local.cache[_name][keys]
			if pipeline:
		...
		for key in keys:
			if name_in_local_cache:
				if key in saashq.local.cache[_name]:
					del saashq.local.cache[_name][key]
...
def setup_cache():
	if saashq.conf.redis_cache_sentinel_enabled:
		sentinels = [tuple(node.split(":")) for node in saashq.conf.get("redis_cache_sentinels", [])]
		sentinel = get_sentinel_connection(
			sentinels=sentinels,
			sentinel_username=saashq.conf.get("redis_cache_sentinel_username"),
			sentinel_password=saashq.conf.get("redis_cache_sentinel_password"),
			master_username=saashq.conf.get("redis_cache_master_username"),
			master_password=saashq.conf.get("redis_cache_master_password"),
		)
		return sentinel.master_for(
			saashq.conf.get("redis_cache_master_service"),
			redis_class=RedisWrapper,
		)

	return RedisWrapper.from_url(saashq.conf.get("redis_cache"))
