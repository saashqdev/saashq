...
def get_jenv():
	...
	if not getattr(saashq.local, "jenv", None):
		from jinja2 import DebugUndefined
		from jinja2.sandbox import SandboxedEnvironment

		from saashq.utils.safe_exec import UNSAFE_ATTRIBUTES, get_safe_globals

		UNSAFE_ATTRIBUTES = UNSAFE_ATTRIBUTES - {"format", "format_map"}

		class SaashqSandboxedEnvironment(SandboxedEnvironment):
		...
		# saashq will be loaded last, so app templates will get precedence
		jenv = SaashqSandboxedEnvironment(loader=get_jloader(), undefined=DebugUndefined)
		set_filters(jenv)
		...
		saashq.local.jenv = jenv

	return saashq.local.jenv
...
def validate_template(html):
	...
	try:
		...
		saashq.throw(f"Syntax error in template as line {e.lineno}: {e.message}")
...
def render_template(template, context=None, is_path=None, safe_render=True):
	...
	from saashq import _, get_traceback, throw
...
def get_jloader():
	...
	if not getattr(saashq.local, "jloader", None):
		from jinja2 import ChoiceLoader, PackageLoader, PrefixLoader

		apps = saashq.get_hooks("template_apps")
		if not apps:
			apps = list(
				reversed(
		...
		if "saashq" not in apps:
			apps.append("saashq")

		saashq.local.jloader = ChoiceLoader(
...
def get_jinja_hooks():
	...
	if not getattr(saashq.local, "site", None):
		return (None, None)
	...
	def get_obj_dict_from_paths(object_paths):
		...
		for obj_path in object_paths:
			try:
				obj = saashq.get_module(obj_path)
			except ModuleNotFoundError:
				obj = saashq.get_attr(obj_path)
