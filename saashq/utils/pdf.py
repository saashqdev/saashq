...
from saashq.utils import cstr, scrub_urls
from saashq.utils.caching import redis_cache
from saashq.utils.jinja_globals import bundled_asset, is_rtl
...
def get_pdf(html, options=None, output: PdfWriter | None = None):
	...
	try:
		...
		if any([error in str(e) for error in PDF_CONTENT_ERRORS]):
			if not filedata:
				print(html, options)
				saashq.throw(_("PDF generation failed because of broken image links"))
...
def prepare_options(html, options):
	...
	# page size
	pdf_page_size = (
		options.get("page-size") or saashq.db.get_single_value("Print Settings", "pdf_page_size") or "A4"
	)

	if pdf_page_size == "Custom":
		options["page-height"] = options.get("page-height") or saashq.db.get_single_value(
			"Print Settings", "pdf_page_height"
		)
		options["page-width"] = options.get("page-width") or saashq.db.get_single_value(
...
def _get_base64_image(src):
	...
	try:
		...
		saashq.logger("pdf").error("Failed to convert inline images to base64", exc_info=True)
...
def prepare_header_footer(soup: BeautifulSoup):
	...
	css = saashq.read_file(os.path.join(saashq.local.sites_path, print_css))
	...
	for html_id in ("header-html", "footer-html"):
		if content := soup.find(id=html_id):
			...
			hook_func = saashq.get_hooks(id_map.get(html_id))
			html = saashq.get_attr(hook_func[-1])(
				soup=soup,
				head=head,
				content=content,
				styles=styles,
			...
			# create temp file
			fname = os.path.join("/tmp", f"saashq-pdf-{saashq.generate_hash()}.html")
			with open(fname, "wb") as f:
				f.write(html.encode("utf-8"))

			# {"header-html": "/tmp/saashq-pdf-random.html"}
...
def get_wkhtmltopdf_version():
	wkhtmltopdf_version = saashq.cache.hget("wkhtmltopdf_version", None)

	if not wkhtmltopdf_version:
		try:
			res = subprocess.check_output(["wkhtmltopdf", "--version"])
			wkhtmltopdf_version = res.decode("utf-8").split(" ")[1]
			saashq.cache.hset("wkhtmltopdf_version", None, wkhtmltopdf_version)
		except Exception:
			pass
