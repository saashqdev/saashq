...
from saashq.locale import get_date_format, get_first_day_of_the_week, get_number_format, get_time_format
from saashq.utils.deprecations import deprecated
from saashq.utils.number_format import NUMBER_FORMAT_MAP, NumberFormat
...
def getdate(
	...
	try:
		...
		saashq.throw(
			saashq._("{} is not a valid date string.").format(saashq.bold(string_date)),
			title=saashq._("Invalid Date"),
		)
...
def add_to_date(
	...
	if isinstance(date, str):
		...
		try:
			...
			saashq.throw(saashq._("Please select a valid date filter"), title=saashq._("Invalid Date"))
...
def get_system_timezone() -> str:
	...
	return saashq.get_system_settings("time_zone") or "Asia/Kolkata"  # Default to India ?!
...
def now() -> str:
	...
	if saashq.flags.current_date:
		return (
			getdate(saashq.flags.current_date).strftime(DATE_FORMAT)
			+ " "
...
def get_user_date_format() -> str:
	...
	if getattr(saashq.local, "user_date_format", None) is None:
		saashq.local.user_date_format = get_date_format()

	return saashq.local.user_date_format
...
def get_user_time_format() -> str:
	...
	if getattr(saashq.local, "user_time_format", None) is None:
		saashq.local.user_time_format = get_time_format()

	return saashq.local.user_time_format
	...
	* dd/mm/yyyy
	...
	try:
		formatted_date = babel.dates.format_date(
			date, format_string, locale=(saashq.local.lang or "").replace("-", "_")
		)
	...
	* HH:mm
	...
	try:
		formatted_time = babel.dates.format_time(
			time_, format_string, locale=(saashq.local.lang or "").replace("-", "_")
		)
	except (UnknownLocaleError, ValueError):
	...
	* mm-dd-yyyy HH:mm
	...
	try:
		formatted_datetime = babel.dates.format_datetime(
			datetime, format_string, locale=(saashq.local.lang or "").replace("-", "_")
		)
	except (UnknownLocaleError, ValueError):
...
def validate_duration_format(duration):
	if not DURATION_PATTERN.match(duration):
		saashq.throw(
			saashq._("Value {0} must be in the valid duration format: d h m s").format(saashq.bold(duration))
		)
...
def global_date_format(date: DateTimeLikeObject, format="long") -> str:
	...
	date = getdate(date)
	return babel.dates.format_date(date, locale=(saashq.local.lang or "en").replace("-", "_"), format=format)
...
def cast_fieldtype(fieldtype, value, show_warning=True):
	if show_warning:
		message = (
			"Function `saashq.utils.data.cast_fieldtype` has been deprecated in favour"
			" of `saashq.utils.data.cast`. Use the newer util for safer type casting."
		)
...
def cast(fieldtype, value=None):
	"""Cast the value to the Python native object of the Saashq fieldtype provided.
	...
	Mapping of Python types => Saashq types:
...
def flt(s: NumericType | str, precision: int | None = None, rounding_method: str | None = None) -> float:
	...
	try:
		...
		if isinstance(e, saashq.InvalidRoundingMethod):
			raise
		num = 0.0
...
def cstr(s, encoding="utf-8") -> str:
	...
	return saashq.as_unicode(s, encoding)
...
def rounded(num, precision=0, rounding_method=None):
	...
	rounding_method = (
		rounding_method or saashq.get_system_settings("rounding_method") or "Banker's Rounding (legacy)"
	)
	...
	if rounding_method == "Banker's Rounding (legacy)":
		...
		saashq.throw(
			saashq._("Unknown Rounding Method: {}").format(rounding_method),
			exc=saashq.InvalidRoundingMethod,
		)
...
def round_based_on_smallest_currency_fraction(value, currency, precision=2):
	smallest_currency_fraction_value = flt(
		saashq.db.get_value("Currency", currency, "smallest_currency_fraction_value", cache=True)
	)
...
def fmt_money(
	...
	if precision is None:
		precision = cint(saashq.db.get_default("currency_precision")) or None
	...
	if number_format.decimal_separator:
		...
		if precision > 2:
			if len(decimals) < 3:
				if currency:
					fraction = saashq.db.get_value("Currency", currency, "fraction_units", cache=True) or 100
	...
	if currency and saashq.defaults.get_global_default("hide_currency_symbol") != "Yes":
		symbol = saashq.db.get_value("Currency", currency, "symbol", cache=True) or currency
		symbol_on_right = saashq.db.get_value("Currency", currency, "symbol_on_right", cache=True)

		if symbol_on_right:
			amount = f"{amount} {saashq._(symbol)}"
		else:
			amount = f"{saashq._(symbol)} {amount}"
...
def get_number_format_info(format: str) -> tuple[str, str, int]:
	"""DEPRECATED: use `NumberFormat.from_string()` from `saashq.utils.number_format` instead.
...
def money_in_words(
	...
	from saashq.utils import get_defaults
	...
	if not fraction_currency:
		fraction_currency = saashq.db.get_value("Currency", main_currency, "fraction", cache=True) or _(
			"Cent"
		)

	number_format = get_number_format()

	fraction_units = saashq.db.get_value("Currency", main_currency, "fraction_units", cache=True)
...
def in_words(integer: int, in_million=True) -> str:
	...
	locale = "en_IN" if not in_million else saashq.local.lang
	integer = int(integer)
	try:
...
def get_thumbnail_base64_for_image(src: str) -> dict[str, str] | None:
	...
	from saashq import cache, safe_decode
	from saashq.core.doctype.file.utils import get_local_image

	if not src:
		saashq.throw(f"Invalid source for image: {src}")
	...
	def _get_base64():
		file_path = saashq.get_site_path("public", src.lstrip("/"))
...
def pdf_to_base64(filename: str) -> bytes | None:
	...
	from saashq.utils.file_manager import get_file_path
...
def pretty_date(iso_datetime: datetime.datetime | str) -> str:
	...
	locale = saashq.local.lang.replace("-", "_") if saashq.local.lang else None
...
def comma_or(some_list: list | tuple, add_quotes=True) -> str:
	...
	e.g. ['a', 'b', 'c'] -> "'a', 'b' or 'c'"
	"""
	return comma_sep(some_list, saashq._("{0} or {1}"), add_quotes)
...
def comma_and(some_list: list | tuple, add_quotes=True) -> str:
	...
	e.g. ['a', 'b', 'c'] -> "'a', 'b' and 'c'"
	"""
	return comma_sep(some_list, saashq._("{0} and {1}"), add_quotes)
...
def comma_sep(some_list: list | tuple, pattern: str, add_quotes=True) -> str:
	...
	if isinstance(some_list, list | tuple):
		...
		if not some_list:
			...
			return pattern.format(", ".join(saashq._(s) for s in some_list[:-1]), some_list[-1])
	else:
		return some_list
...
def get_url(uri: str | None = None, full_address: bool = False) -> str:
	...
	host_name = saashq.local.conf.host_name or saashq.local.conf.hostname
	...
	if not host_name:
		...
		elif saashq.local.site:
			protocol = "http://"

			if saashq.local.conf.ssl_certificate:
				protocol = "https://"

			elif saashq.local.conf.wildcard:
				domain = saashq.local.conf.wildcard.get("domain")
				if (
					domain
					and saashq.local.site.endswith(domain)
					and saashq.local.conf.wildcard.get("ssl_certificate")
				):
					protocol = "https://"

			host_name = protocol + saashq.local.site

		else:
			host_name = saashq.db.get_single_value("Website Settings", "subdomain")
	...
	if not uri and full_address:
		uri = saashq.get_request_header("REQUEST_URI", "")

	port = saashq.conf.http_port
	if not port and saashq.conf.developer_mode:
		port = saashq.conf.webserver_port

	if (
		# XXX: This config is used as proxy for "is production mode enabled?"
		not saashq.conf.restart_supervisor_on_update
		and not saashq.conf.restart_systemd_on_update
		and host_name
...
def get_host_name_from_request() -> str:
	...
	if hasattr(saashq.local, "request") and saashq.local.request and saashq.local.request.host:
		protocol = "https://" if "https" == saashq.get_request_header("X-Forwarded-Proto", "") else "http://"
		return protocol + saashq.local.request.host
...
def filter_operator_is(value: str, pattern: str) -> bool:
	...
	if pattern == "set":
		...
		saashq.throw(saashq._(f"Invalid argument for operator 'IS': {pattern}"))
...
def get_filter(doctype: str, f: dict | list | tuple, filters_config=None) -> "saashq._dict":
	"""Return a `_dict` like:
	...
	"""
	from saashq.database.utils import NestedSetHierarchy
	from saashq.model import child_table_fields, default_fields, optional_fields
	...
	if not isinstance(f, list | tuple):
		saashq.throw(saashq._("Filter must be a tuple or list (in a list)"))
	...
	if len(f) == 3:
		...
		saashq.throw(
			saashq._("Filter must have 4 values (doctype, fieldname, operator, value): {0}").format(str(f))
		)

	f = saashq._dict(doctype=f[0], fieldname=f[1], operator=f[2], value=f[3])
	...
	if f.operator.lower() not in valid_operators:
		saashq.throw(saashq._("Operator must be one of {0}").format(", ".join(valid_operators)))
	...
	if f.doctype and (f.fieldname not in default_fields + optional_fields + child_table_fields):
		...
		meta = saashq.get_meta(f.doctype)
		if not meta.has_field(f.fieldname):
			# try and match the doctype name from child tables
			for df in meta.get_table_fields():
				if saashq.get_meta(df.options).has_field(f.fieldname):
					f.doctype = df.options
					break

	try:
		df = saashq.get_meta(f.doctype).get_field(f.fieldname) if f.doctype else None
	except saashq.exceptions.DoesNotExistError:
		df = None
...
def sanitize_column(column_name: str) -> None:
	...
	if saashq.db and saashq.db.db_type == "mariadb":
	...
	def _raise_exception():
		saashq.throw(_("Invalid field name {0}").format(column_name), saashq.DataError)
...
def generate_hash(*args, **kwargs) -> str:
	...
	"""
	return saashq.generate_hash(*args, **kwargs)
...
def get_user_info_for_avatar(user_id: str) -> _UserInfo:
	...
	"""
	try:
		user = saashq.get_cached_doc("User", user_id)
...
def validate_python_code(string: str, fieldname: str | None = None, is_expression: bool = True) -> None:
	...
	try:
		...
		msg = saashq._("{} Invalid python code on line {}").format(
			fieldname + ":" if fieldname else "", line_no + 1
		)
		msg += f"<br><pre>{error_line}</pre>"
		msg += f"<pre>{' ' * offset}^</pre>"

		saashq.throw(msg, title=saashq._("Syntax Error"))
	except Exception as e:
		saashq.msgprint(
			saashq._("{} Possibly invalid python code. <br>{}").format(fieldname + ": " or "", str(e)),
			indicator="orange",
		)
...
def format_timedelta(o: datetime.timedelta | str) -> str:
	...
	# but Saashq doesn't - I think via babel : only allows 0..23 range for hour
...
def is_site_link(link: str) -> bool:
	...
	if link.startswith("/"):
		return True
	return urlparse(link).netloc == urlparse(saashq.utils.get_url()).netloc
...
def add_trackers_to_url(
	...
	def get_utm_values():
		_source = saashq.db.get_value("UTM Source", source, "slug") or source
		_campaign = saashq.db.get_value("UTM Campaign", campaign, "slug") or campaign
		_medium = saashq.db.get_value("UTM Medium", medium, "slug") or medium
	...
	source, medium, campaign = saashq.cache.get_value(
...
def map_trackers(url_trackers: dict, create: bool = False):
	...
	if url_source := url_trackers.get("utm_source", url_trackers.get("source")):
		source = saashq.db.get_value("UTM Source", {"slug": slug(url_source)}, "name") or url_source
		if create and source == url_source and not saashq.db.exists("UTM Source", source):
			source = saashq.new_doc("UTM Source")
			source.name = url_source
			source.description = f"Autogenerated from {url_trackers}"
			source.save(ignore_permissions=True)
		saashq_trackers["utm_source"] = source

	if url_medium := url_trackers.get("utm_medium", url_trackers.get("medium")):
		medium = saashq.db.get_value("UTM Medium", {"slug": slug(url_medium)}, "name") or url_medium
		if create and medium == url_medium and not saashq.db.exists("UTM Medium", medium):
			medium = saashq.new_doc("UTM Medium")
			medium.name = url_medium
			medium.description = f"Autogenerated from {url_trackers}"
			medium.save(ignore_permissions=True)
		saashq_trackers["utm_medium"] = medium

	if url_campaign := url_trackers.get("utm_campaign", url_trackers.get("campaign")):
		campaign = saashq.db.get_value("UTM Campaign", {"slug": slug(url_campaign)}, "name") or url_campaign
		if create and campaign == url_campaign and not saashq.db.exists("UTM Campaign", campaign):
			campaign = saashq.new_doc("UTM Campaign")
			campaign.name = url_campaign
			campaign.campaign_description = f"Autogenerated from {url_trackers}"
			campaign.save(ignore_permissions=True)
		saashq_trackers["utm_campaign"] = campaign

	if url_content := url_trackers.get("utm_content", url_trackers.get("content")):
		saashq_trackers["utm_content"] = url_content

	return saashq_trackers
