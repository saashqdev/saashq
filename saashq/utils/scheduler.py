...
def _proctitle(message):
	setproctitle.setthreadtitle(f"saashq-scheduler: {message}")
...
def start_scheduler() -> NoReturn:
	...
	try:
		...
		saashq.logger("scheduler").debug("Scheduler already running")
		return
...
def enqueue_events_for_all_sites() -> None:
	...
	for site in sites:
		try:
			...
			saashq.logger("scheduler").debug(f"Failed to enqueue events for site: {site}", exc_info=True)


def enqueue_events_for_site(site: str) -> None:
	def log_exc():
		saashq.logger("scheduler").error(f"Exception in Enqueue Events for Site {site}", exc_info=True)
	...
	try:
		...
		saashq.logger("scheduler").debug(f"Queued events for site {site}")
	except Exception as e:
		if saashq.db.is_access_denied(e):
			saashq.logger("scheduler").debug(f"Access denied for site {site}")
		log_exc()
...
def enqueue_events() -> list[str] | None:
	if schedule_jobs_based_on_activity():
		enqueued_jobs = []
		all_jobs = saashq.get_all("Scheduled Job Type", filters={"stopped": 0}, fields="*")
		random.shuffle(all_jobs)
		for job_type in all_jobs:
			job_type = saashq.get_doc(doctype="Scheduled Job Type", **job_type)
			try:
				if job_type.enqueue():
					enqueued_jobs.append(job_type.method)
			except CroniterBadCronError:
				saashq.logger("scheduler").error(
					f"Invalid Job on {saashq.local.site} - {job_type.name}", exc_info=True
				)
...
def is_scheduler_inactive(verbose=True) -> bool:
	if saashq.local.conf.maintenance_mode:
		if verbose:
			cprint(f"{saashq.local.site}: Maintenance mode is ON")
		return True

	if saashq.local.conf.pause_scheduler:
		if verbose:
			cprint(f"{saashq.local.site}: saashq.conf.pause_scheduler is SET")
		return True
...
def is_scheduler_disabled(verbose=True) -> bool:
	if saashq.conf.disable_scheduler:
		if verbose:
			cprint(f"{saashq.local.site}: saashq.conf.disable_scheduler is SET")
		return True

	scheduler_disabled = not saashq.utils.cint(
		saashq.db.get_single_value("System Settings", "enable_scheduler")
	)
	if scheduler_disabled:
		if verbose:
			cprint(f"{saashq.local.site}: SystemSettings.enable_scheduler is UNSET")
	return scheduler_disabled


def toggle_scheduler(enable):
	saashq.db.set_single_value("System Settings", "enable_scheduler", int(enable))
...
def is_dormant(check_time=None):
	...
	since = (saashq.get_system_settings("dormant_days") or 4) * 86400
...
def _get_last_creation_timestamp(doctype):
	timestamp = saashq.db.get_value(doctype, filters={}, fieldname="creation", order_by="creation desc")
	if timestamp:
...
def activate_scheduler():
	...
	if saashq.local.conf.maintenance_mode:
		saashq.throw(saashq._("Scheduler can not be re-enabled when maintenance mode is active."))

	if is_scheduler_disabled():
		enable_scheduler()
	if saashq.conf.pause_scheduler:
