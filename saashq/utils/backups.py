...
from saashq.utils import cint, get_file_size, get_url, now, now_datetime
...
class BackupGenerator:
	...
	def __init__(
		...
		site = saashq.local.site or saashq.generate_hash(length=8)
	...
	def _set_existing_tables(self):
		...
		if not hasattr(self, "_existing_tables"):
			self._existing_tables = saashq.db.get_tables()
	...
	def set_backup_tables_from_config(self):
		...
		backup_conf = saashq.conf.get("backup", {})
		self._set_existing_tables()
	...
	def get_backup(self, older_than=24, ignore_files=False, force=False):
		...
		if not (last_db and last_file and last_private_file and site_config_backup_path):
			...
			if saashq.get_system_settings("encrypt_backup"):
	...
	def set_backup_file_name(self):
		...
		enc = "-enc" if saashq.get_system_settings("encrypt_backup") else ""
		...
		for path in paths:
			if os.path.exists(path):
				...
				try:
					...
					saashq.utils.execute_in_shell(command)
	...
	def print_summary(self):
		...
		print(f"Backup Summary for {saashq.local.site} at {now()}")
	...
	def backup_files(self):
		for folder in ("public", "private"):
			files_path = saashq.get_site_path(folder, "files")
			...
			try:
				saashq.utils.execute_in_shell(
					cmd_string.format(backup_path, files_path),
					verbose=self.verbose,
					low_priority=True,
					check_exit_code=True,
				)
			except saashq.CommandFailedError as e:
	...
	def copy_site_config(self):
		...
		site_config_path = os.path.join(saashq.get_site_path(), "site_config.json")
	...
	def take_dump(self):
		...
		if not gzip_exc:
			saashq.throw(
				_("gzip not found in PATH! This is required to take a backup."), exc=saashq.ExecutableNotFound
			)

		if self.old_backup_metadata:
			database_header_content = [
				f"Backup generated by Saashq {saashq.__version__} on branch {get_app_branch('saashq') or 'N/A'}",
				"",
			]
		else:
			database_header_content = [
				"begin saashq metadata",
				"[saashq]",
				f"version = {saashq.__version__}",
				f"branch = {get_app_branch('saashq') or 'N/A'}",
				"end saashq metadata",
				"",
			]
		...
		if self.partial:
			...
			database_header_content.extend(
				[
					f"Partial Backup of Saashq Site {saashq.local.site}",
		...
		saashq.utils.execute_in_shell(command, low_priority=True, check_exit_code=True)

	def send_email(self):
		"""
		Sends the link to backup file located at erpnexus/backups
		"""
		from saashq.email import get_system_managers
...
download only after 24 hours."""
		...
		saashq.sendmail(recipients=recipient_list, message=msg, subject=subject)
...
def delete_temp_backups(older_than=24):
	...
	"""
	older_than = cint(saashq.conf.keep_backups_for_hours) or older_than
	backup_path = get_backup_path()
...
def get_backup_path():
	return saashq.utils.get_site_path(conf.get("backup_path", "private/backups"))
...
def get_or_generate_backup_encryption_key():
	...
	key = saashq.conf.get(BACKUP_ENCRYPTION_CONFIG_KEY)
	if key:
		return key
