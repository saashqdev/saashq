...
def SaashqClickWrapper(cls, handler):
	class Cls(cls):
...
def get_app_groups() -> dict[str, click.Group | click.Command]:
	...
	return dict(
		saashq=click.group(
			name="saashq", commands=commands, cls=SaashqClickWrapper(click.Group, handle_exception)
		)(app_group)
	)
...
def get_app_group(app: str) -> click.Group:
	if app_commands := get_app_commands(app):
		return click.group(
			name=app, commands=app_commands, cls=SaashqClickWrapper(click.Group, handle_exception)
		)(app_group)
...
def app_group(ctx, site=False, force=False, verbose=False, profile=False):
	...
	if ctx.info_name == "saashq":
		ctx.info_name = ""


def get_sites(site_arg: str) -> list[str]:
	if site_arg == "all":
		return saashq.utils.get_sites()
	elif site_arg:
		return [site_arg]
	elif env_site := os.environ.get("SAASHQ_SITE"):
		return [env_site]
	elif default_site := saashq.get_conf().default_site:
		return [default_site]
	# This is not supported, just added here for warning.
	elif (site := saashq.read_file("currentsite.txt")) and site.strip():
		click.secho(
			dedent(
				f"""
...
@click.command("get-saashq-commands")
def get_saashq_commands():
	commands = list(get_app_commands("saashq"))
...
@click.command("get-saashq-help")
def get_saashq_help():
	print(click.Context(get_app_groups()["saashq"]).get_help())


def get_apps():
	return saashq.get_all_apps(with_internal_apps=False, sites_path=".")


if __name__ == "__main__":
	if not saashq._dev_server:
