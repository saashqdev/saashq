...
def get_secret():
	from saashq.utils.password import get_encryption_key

	return saashq.local.conf.get("secret") or get_encryption_key()


def verify_request():
	"""Verify if the incoming signed request if it is correct."""
	query_string = saashq.safe_decode(
		saashq.local.flags.signed_query_string or getattr(saashq.request, "query_string", None)
	)
	...
	if signature_string in query_string:
		...
		valid_method = saashq.request.method == "GET"
		valid_request_data = not (saashq.request.form or saashq.request.data)
...
def get_url(cmd, params, nonce=None, secret=None):
	...
	return saashq.utils.get_url("".join(["api/method/", cmd, "?", urlencode(params)]))


def get_signature(params, nonce, secret=None):
	params = "".join(saashq.utils.cstr(p) for p in params.values())
	if not secret:
		secret = saashq.local.conf.get("secret") or "secret"
