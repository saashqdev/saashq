...
from saashq.deprecation_dumpster import gzip_compress, gzip_decompress, make_esc
...
def get_fullname(user=None):
	...
	if not hasattr(saashq.local, "fullnames"):
		saashq.local.fullnames = {}

	if not saashq.local.fullnames.get(user):
		p = saashq.db.get_value("User", user, ["first_name", "last_name"], as_dict=True)
		if p:
			saashq.local.fullnames[user] = (
				" ".join(filter(None, [p.get("first_name"), p.get("last_name")])) or user
			)
		else:
			saashq.local.fullnames[user] = user

	return saashq.local.fullnames.get(user)
...
def get_email_address(user=None):
	...
	return saashq.db.get_value("User", user, "email")
...
def validate_phone_number_with_country_code(phone_number: str, fieldname: str) -> None:
	...
	try:
		...
		if not valid_number:
			saashq.throw(
				error_message.format(saashq.bold(phone_number), saashq.bold(fieldname)),
				title=error_title,
				exc=saashq.InvalidPhoneNumberError,
			)
...
def validate_phone_number(phone_number, throw=False):
	...
	if not match and throw:
		saashq.throw(
			saashq._("{0} is not a valid Phone Number").format(phone_number), saashq.InvalidPhoneNumberError
		)
...
def validate_name(name, throw=False):
	...
	* anything else is considered invalid
	...
	Note: "Name" here is name of a person, not the primary key in Saashq doctypes.
	"""
	if not name:
		return False
	...
	if not match and throw:
		saashq.throw(saashq._("{0} is not a valid Name").format(name), saashq.InvalidNameError)
...
def validate_email_address(email_str, throw=False):
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				invalid_email = saashq.utils.escape_html(e)
				saashq.throw(
					saashq._("{0} is not a valid Email Address").format(invalid_email),
...
def validate_url(
	...
	if not is_valid and throw:
		saashq.throw(saashq._("'{0}' is not a valid URL").format(saashq.bold(txt)))
...
def has_gravatar(email: str) -> str:
	...
	if saashq.flags.in_import or saashq.flags.in_install or saashq.flags.in_test:
...
def get_gravatar_url(email: str, default: Literal["mm", "404"] = "mm") -> str:
	...
	"""
	hexdigest = hashlib.md5(saashq.as_unicode(email).encode("utf-8"), usedforsecurity=False).hexdigest()
	...
	return Format(
		custom_var_printers=[
			...
			*[(variable_name, lambda *a, **kw: placeholder) for variable_name in blocklist],
			...
			(["_secret", saashq._dict, lambda *a, **kw: False], dict_printer),
		],
	)


def log(event, details):
	saashq.logger(event).info(details)
...
def get_defaults(key=None):
	...
	"""
	return saashq.db.get_defaults(key)
...
def set_default(key, val):
	...
	"""
	return saashq.db.set_default(key, val)
...
def execute_in_shell(cmd, verbose=False, low_priority=False, check_exit_code=False):
	...
	if failed:
		raise saashq.CommandFailedError(
...
def get_path(*path, **kwargs):
	...
	if not base:
		base = saashq.local.site_path
	return os.path.join(base, *path)


def get_site_base_path():
	return saashq.local.site_path
...
def get_test_client(use_cookies=True) -> Client:
	"""Return an test instance of the Saashq WSGI."""
	from saashq.app import application

	return Client(application, use_cookies=use_cookies)


def get_hook_method(hook_name, fallback=None):
	method = saashq.get_hooks().get(hook_name)
	if method:
		method = saashq.get_attr(method[0])
		return method
	if fallback:
		return fallback
...
def call_hook_method(hook, *args, **kwargs):
	out = None
	for method_name in saashq.get_hooks(hook):
		out = out or saashq.get_attr(method_name)(*args, **kwargs)
...
def update_progress_bar(txt, i, l, absolute=False):
	...
	if not getattr(saashq.local, "request", None) or is_cli():  # pragma: no cover
		lt = len(txt)
		try:
	...
	if os.path.exists(print_path):
		...
		for include_directive, path in INCLUDE_DIRECTIVE_PATTERN.findall(html_format):
			for app_name in saashq.get_installed_apps():
				include_path = saashq.get_app_path(app_name, *path.split(os.path.sep))
...
def get_sites(sites_path=None):
	if not sites_path:
		sites_path = getattr(saashq.local, "sites_path", None) or "."
...
def markdown(text, sanitize=True, linkify=True):
	html = text if is_html(text) else saashq.utils.md_to_html(text)
...
def get_site_info():
	from saashq.email.queue import get_emails_sent_this_month
	from saashq.utils.user import get_system_managers

	# only get system users
	users = saashq.get_all(
		"User",
		filters={"user_type": "System User", "name": ("not in", saashq.STANDARD_USERS)},
	...
	system_settings = saashq.db.get_singles_dict("System Settings")
	space_usage = saashq._dict((saashq.local.conf.limits or {}).get("space_usage", {}))
	...
	site_info = {
		...
		"last_logins": saashq.get_all("Activity Log", **kwargs),
	}

	# from other apps
	for method_name in saashq.get_hooks("get_site_info"):
		site_info.update(saashq.get_attr(method_name)(site_info) or {})

	# dumps -> loads to prevent datatype conflicts
	return json.loads(saashq.as_json(site_info))
...
def get_db_count(*args):
	...
	db_count = {}
	for doctype in args:
		db_count[doctype] = saashq.db.count(doctype)

	return json.loads(saashq.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	Pass a doctype or a series of doctypes to get the count of docs in them
	Parameters:
	        fn: saashq function to be called
...
def get_safe_filters(filters):
	try:
		...
		if isinstance(filters, int | float):
			filters = saashq.as_unicode(filters)
...
def set_request(**kwargs):
	...
	saashq.local.request = Request(builder.get_environ())
...
def get_html_for_route(route):
	...
	return saashq.safe_decode(response.get_data())
...
def get_build_version():
	try:
		return str(os.path.getmtime(os.path.join(saashq.local.sites_path, "assets/assets.json")))
	except OSError:
		# .build can sometimes not exist
		# this is not a major problem so send fallback
		return saashq.utils.random_string(8)


def get_assets_json():
	def _get_assets():
		# get merged assets.json and assets-rtl.json
		assets = saashq.parse_json(saashq.read_file("assets/assets.json"))

		if assets_rtl := saashq.read_file("assets/assets-rtl.json"):
			assets.update(saashq.parse_json(assets_rtl))

		return assets

	if not hasattr(saashq.local, "assets_json"):
		if not saashq.conf.developer_mode:
			saashq.local.assets_json = saashq.cache.get_value(
				"assets_json",
				_get_assets,
				shared=True,
			)
			...
			saashq.local.assets_json = _get_assets()
...
def add_user_info(user: str | list[str] | set[str], user_info: dict[str, _UserInfo]) -> None:
	...
	for info in missing_info:
		user_info.setdefault(info.name, saashq._dict()).update(
...
class CallbackManager:
	...
	Example usage: saashq.db.after_commit
	"""
