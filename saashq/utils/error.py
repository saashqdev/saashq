# Copyleft (l) 2023-Present, Maxwell Morais and contributors
...
EXCLUDE_EXCEPTIONS = (
	...
	saashq.CSRFTokenError,  # CSRF covers OAuth too
	saashq.SecurityException,
	saashq.InReadOnlyMode,
)
...
def log_error(title=None, message=None, reference_doctype=None, reference_name=None, *, defer_insert=False):
	...
	from saashq.utils.sentry import capture_exception
	...
	traceback = saashq.as_unicode(traceback or saashq.get_traceback(with_context=True))
	...
	if saashq.flags.read_only or defer_insert:
...
def log_error_snapshot(exception: Exception):
	...
	logger = saashq.logger(with_more_info=True)
	...
	def decorator_raise_error_on_no_output(func):
		...
		def wrapper_raise_error_on_no_output(*args, **kwargs):
			...
			if (not response) and raise_error:
				saashq.throw(error_message, error_type or Exception)
			return response
...
def guess_exception_source(exception: str) -> str | None:
	...
	- For saashq.throws the exception source is possibly present after skipping saashq.throw frames
	...
	with suppress(Exception):
		...
		if (probably_source := apps.most_common(1)) and probably_source[0][0] != "saashq":
