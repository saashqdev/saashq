...
def sync_fixtures(app=None):
	...
	saashq.flags.in_fixtures = True

	for app in apps:
		import_fixtures(app)
		import_custom_scripts(app)

	saashq.flags.in_fixtures = False


def import_fixtures(app):
	fixtures_path = saashq.get_app_path(app, "fixtures")
	...
	for fname in fixture_files:
		...
		file_path = saashq.get_app_path(app, "fixtures", fname)
		try:
			import_doc(file_path, sort=True)
		except (ImportError, saashq.DoesNotExistError) as e:
...
def import_custom_scripts(app):
	...
	scripts_folder = saashq.get_app_path(app, "fixtures", "custom_scripts")
...
def export_fixtures(app=None):
	...
	if app:
		...
		apps = saashq.get_installed_apps()
	for app in apps:
		fixture_auto_order = bool(next(iter(saashq.get_hooks("fixture_auto_order", app_name=app)), False))
		fixtures = saashq.get_hooks("fixtures", app_name=app)
		prefix = None
		for index, fixture in enumerate(fixtures, start=1):
			...
			if not os.path.exists(saashq.get_app_path(app, "fixtures")):
				os.mkdir(saashq.get_app_path(app, "fixtures"))

			filename = saashq.scrub(fixture)
			if prefix:
			...
			export_json(
				fixture,
				saashq.get_app_path(app, "fixtures", filename + ".json"),
				filters=filters,
