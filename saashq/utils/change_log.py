...
import saashq
from saashq import _, safe_decode
...
def get_change_log(user=None):
	...
	last_known_versions = saashq._dict(
		json.loads(saashq.db.get_value("User", user, "last_known_versions") or "{}")
	)
	...
	if "saashq" in current_versions:
		set_in_change_log("saashq", current_versions["saashq"], change_log)

	return change_log


def get_change_log_for_app(app, from_version, to_version):
	change_log_folder = os.path.join(saashq.get_app_path(app), "change_log")
		...
		if folder in major_version_folders:
			for file in os.listdir(os.path.join(change_log_folder, folder)):
				...
				if from_version < version <= to_version:
					...
					content = saashq.read_file(file_path)
...
def get_versions():
	...
	for app in saashq.get_installed_apps(_ensure_on_wrench=True):
		...
		try:
			versions[app]["version"] = saashq.get_attr(app + ".__version__")
...
def check_for_update():
	if saashq.get_system_settings("disable_system_update_notification"):
		return

	updates = saashq._dict(major=[], minor=[], patch=[])
	apps = get_versions()
...
def has_app_update_notifications() -> bool:
	return bool(saashq.cache.sismember("changelog-update-user-set", saashq.session.user))
		...
		for vuln in advisory["vulnerabilities"]:
			with suppress(Exception):
				...
				if (
					...
					# XXX: this is not 100% correct, but works for saashq
...
def add_message_to_redis(update_json):
	...
	saashq.cache.set_value("changelog-update-info", json.dumps(update_json))
	user_list = [x.name for x in saashq.get_all("User", filters={"enabled": True})]
	system_managers = [user for user in user_list if "System Manager" in saashq.get_roles(user)]
	saashq.cache.sadd("changelog-update-user-set", *system_managers)
...
def show_update_popup():
	...
	update_info = saashq.cache.get_value("changelog-update-info")
	if not update_info:
		return
	...
	update_message = ""
	if saashq.cache.sismember("changelog-update-user-set", user):
		for update_type in updates:
			release_links = ""
			for app in updates[update_type]:
				app = saashq._dict(app)
				security_msg = ""
				if app.security_issues:
					...
					security_msg = security_msg.format(saashq.bold(app.security_issues))
	...
	if on_saashqcloud():
		primary_action = {
			"label": _("Update from Saashq Cloud"),
			"client_action": "window.open",
			"args": f"https://saashqcloud.com/dashboard/sites/{saashq.local.site}",
		}
	...
	if update_message:
		...
		saashq.cache.srem("changelog-update-user-set", user)


def get_pyproject(app: str) -> dict | None:
	from tomli import load

	pyproject_path = saashq.get_app_path(app, "..", "pyproject.toml")
