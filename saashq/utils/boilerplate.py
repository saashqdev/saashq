...
def make_boilerplate(dest, app_name, no_git=False):
	...
	app_name = saashq.scrub(app_name)
...
def _get_user_inputs(app_name):
	...
	new_app_config = {
		...
		"branch_name": {"prompt": "Branch Name", "default": get_app_branch("saashq")},
	}
...
def copy_from_saashq(rel_path: str, new_app_path: str):
	"""Copy files from saashq app to new app."""
	src = Path(saashq.get_app_path("saashq", "..")) / rel_path
...
def _create_app_boilerplate(dest, hooks, no_git=False):
	saashq.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, saashq.scrub(hooks.app_title)),
		with_init=True,
	)
	saashq.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates"), with_init=True)
	saashq.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "www"))
	saashq.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "pages"), with_init=True
	)
	saashq.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "includes"))
	saashq.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "config"), with_init=True)
	saashq.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "css"))
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "hooks.py"), "w") as f:
		f.write(saashq.as_unicode(hooks_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "patches.txt"), "w") as f:
		f.write(saashq.as_unicode(patches_template.format(**hooks)))

	app_directory = os.path.join(dest, hooks.app_name)

	copy_from_saashq(".editorconfig", app_directory)
	copy_from_saashq(".eslintrc", app_directory)
	...
	with open(os.path.join(dest, hooks.app_name, "README.md"), "w") as f:
		f.write(saashq.as_unicode(readme_template.format(**hooks)))

	if not no_git:
		with open(os.path.join(dest, hooks.app_name, ".gitignore"), "w") as f:
			f.write(saashq.as_unicode(gitignore_template.format(app_name=hooks.app_name)))
...
class PatchCreator:
	def __init__(self):
		self.all_apps = saashq.get_all_apps(sites_path=".", with_internal_apps=False)
	...
	def _ask_app_name(self):
		...
		self.app_dir = pathlib.Path(saashq.get_app_path(self.app))
	...
	def _ask_patch_meta_info(self):
		...
		default_filename = saashq.scrub(self.docstring) + ".py"
...
typing-modules = ["saashq.types.DF"]
...
# Desk Notifications
# ------------------
# See saashq.core.notifications.get_notification_config
...
# 	"saashq.desk.doctype.event.event.get_events": "{app_name}.event.get_events"
# }}
#
# each overriding function accepts a `data` argument;
# generated from the base implementation of the doctype dashboard,
# along with any modifications made in other Saashq apps
...
# Read docs to understand patches: https://saashqframework.com/docs/v14/user/en/database-migrations
  ...
  linter:
    ...
    steps:
      ...
      - name: Run Semgrep rules
        run: |
          pip install semgrep
          semgrep ci --config ./saashq-semgrep-rules/rules --config r/python.lang.correctness
