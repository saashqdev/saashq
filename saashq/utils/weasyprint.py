...
def download_pdf(doctype, name, print_format, letterhead=None):
	...
	saashq.local.response.filecontent = pdf
...
class PrintFormatGenerator:
	...
	def __init__(self, print_format, doc, letterhead=None):
		...
		"""
		self.base_url = saashq.utils.get_url()
		self.print_format = saashq.get_doc("Print Format", print_format)
		self.doc = doc

		if letterhead == _("No Letterhead"):
			letterhead = None
		self.letterhead = saashq.get_doc("Letter Head", letterhead) if letterhead else None
	...
	def build_context(self):
		self.print_settings = saashq.get_doc("Print Settings")
		...
		print_style = (
			saashq.get_doc("Print Style", self.print_settings.print_style)
	...
	def get_main_html(self):
		self.context.css = saashq.render_template("templates/print_format/print_format.css", self.context)
		return saashq.render_template("templates/print_format/print_format.html", self.context)

	def get_header_footer_html(self):
		header_html = footer_html = None
		if self.letterhead:
			header_html = saashq.render_template("templates/print_format/print_header.html", self.context)
		if self.letterhead:
			footer_html = saashq.render_template("templates/print_format/print_footer.html", self.context)
	...
	def get_layout(self, print_format):
		layout = saashq.parse_json(print_format.format_data)
	...
	def process_margin_texts(self, layout):
		...
		for key in margin_texts:
			...
			if text and "{{" in text:
				layout["text_" + key] = saashq.render_template(text, self.context)
