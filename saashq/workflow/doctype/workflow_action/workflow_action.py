...
from saashq.desk.form.utils import get_pdf_link
from saashq.desk.notifications import clear_doctype_notifications
from saashq.email.doctype.email_template.email_template import get_email_template
from saashq.model.document import Document
from saashq.model.workflow import (
	apply_workflow,
	get_workflow_name,
...
from saashq.utils import get_datetime, get_url
from saashq.utils.background_jobs import enqueue
from saashq.utils.data import get_link_to_form
from saashq.utils.user import get_users_with_role
...
class WorkflowAction(Document):
	...
	if TYPE_CHECKING:
		...
		from saashq.workflow.doctype.workflow_action_permitted_role.workflow_action_permitted_role import (
...
def on_doctype_update():
	...
	saashq.db.add_index("Workflow Action", ["reference_name", "reference_doctype", "status"])
...
def get_permission_query_conditions(user):
	...
	permitted_workflow_actions = (
		saashq.qb.from_(WorkflowAction)
...
def has_permission(doc, user):
	...
	return not permitted_roles.isdisjoint(saashq.get_roles(user))
...
def confirm_action(doctype, docname, user, action):
	...
	logged_in_user = saashq.session.user
	...
	if logged_in_user == "Guest":
		saashq.set_user(logged_in_user)
...
def return_success_page(doc):
	saashq.respond_as_web_page(
		...
		_("{0}: {1} is set to state {2}").format(
			doc.get("doctype"), saashq.bold(doc.get("name")), saashq.bold(get_doc_workflow_state(doc))
		),
...
def return_link_expired_page(doc, doc_workflow_state):
	saashq.respond_as_web_page(
		...
		_("Document {0} has been set to state {1} by {2}").format(
			saashq.bold(doc.get("name")),
			saashq.bold(doc_workflow_state),
			saashq.bold(saashq.get_value("User", doc.get("modified_by"), "full_name")),
		),
		indicator_color="blue",
	)
...
def get_allowed_roles(user, workflow, workflow_state):
	user = user if user else saashq.session.user

	allowed_roles = saashq.get_all(
		"Workflow Transition",
		fields="allowed",
	...
	user_roles = set(saashq.get_roles(user))
...
def update_completed_workflow_actions_using_role(user=None, workflow_action=None):
	...
	(
		saashq.qb.update(WorkflowAction)
	...
	for transition in transitions:
		...
		for user in filtered_users:
			if not user_data_map.get(user):
				user_data_map[user] = saashq._dict(
					{
						"possible_actions": [],
						"email": saashq.db.get_value("User", user, "email"),
					}
				)
...
def create_workflow_actions_for_roles(roles, doc):
	if not roles:
		return
	workflow_action = saashq.get_doc(
		{
...
def get_workflow_action_url(action, doc, user):
	apply_action_method = "/api/method/saashq.workflow.doctype.workflow_action.workflow_action.apply_action"
...
def get_confirm_workflow_action_url(doc, action, user):
	confirm_action_method = (
		"/api/method/saashq.workflow.doctype.workflow_action.workflow_action.confirm_action"
	)
...
def get_common_email_args(doc):
	...
	lang = doc.get("language") or (
		saashq.get_cached_value("Print Format", print_format, "default_print_language")
		if print_format
		else None
	)
...
def get_email_template_from_workflow(doc):
	...
	template_name = saashq.db.get_value(
