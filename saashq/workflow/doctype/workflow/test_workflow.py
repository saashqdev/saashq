...
class TestWorkflow(IntegrationTestCase):
	...
	def setUp(self):
		self.patcher = patch("saashq.attach_print", return_value={})
		self.patcher.start()
		saashq.db.delete("Workflow Action")
		self.workflow = create_todo_workflow()

	def tearDown(self):
		saashq.set_user("Administrator")
		self.patcher.stop()
		saashq.delete_doc("Workflow", "Test ToDo")
	...
	def test_if_workflow_actions_were_processed_using_role(self):
		...
		workflow_actions = saashq.get_all("Workflow Action", fields=["*"])
...
def create_todo_workflow():
	from saashq.tests.ui_test_helpers import UI_TEST_USER

	if saashq.db.exists("Workflow", "Test ToDo"):
		saashq.delete_doc("Workflow", "Test ToDo")

	TEST_ROLE = "Test Approver"

	if not saashq.db.exists("Role", TEST_ROLE):
		saashq.get_doc(doctype="Role", role_name=TEST_ROLE).insert(ignore_if_duplicate=True)
		if saashq.db.exists("User", UI_TEST_USER):
			saashq.get_doc("User", UI_TEST_USER).add_roles(TEST_ROLE)

	workflow = saashq.new_doc("Workflow")
...
def create_new_todo():
	return saashq.get_doc(doctype="ToDo", description="workflow " + random_string(10)).insert()
