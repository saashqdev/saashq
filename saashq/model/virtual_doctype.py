...
class VirtualDoctype(Protocol):
	...
	- DocType controller has to inherit from `saashq.model.document.Document` class
	...
	@staticmethod
	def get_list(**kwargs) -> list[saashq._dict]:
...
def validate_controller(doctype: str) -> None:
	try:
		...
		saashq.msgprint(_("Failed to import virtual doctype {}, is controller file present?").format(doctype))
		return
		...
		if not isinstance(method, staticmethod):
			saashq.msgprint(
				_("Virtual DocType {} requires a static method called {} found {}").format(
					saashq.bold(doctype), saashq.bold(m), saashq.bold(_as_str(method))
				),
