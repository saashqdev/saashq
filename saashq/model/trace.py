"""
Traced Fields for Saashq

This module provides utilities for creating traced fields in Saashq documents,
...
- Seamlessly integrate with Saashq's document model

Example of standard usage:
		from saashq.model.trace import TracedDocument, traced_field
...
See saashq.tests.classes.context_managers for a context manager built into test classes.
"""
...
class TracedValue:
	"""
	A descriptor class for creating traced fields in Saashq documents.

	This class allows for monitoring and validating changes to specific fields
	in a Saashq document. It can enforce forbidden values and apply custom
	validation logic.
	...
	def __set__(self, obj, value):
		...
		if value in self.forbidden_values:
			if saashq.flags.in_test:
				saashq.throw(f"{self.field_name} cannot be set to {value}", AssertionError)
			else:
				saashq.throw(f"{self.field_name} cannot be set to {value}")
		...
		if self.custom_validation:
			try:
				...
				if saashq.flags.in_test:
					saashq.throw(str(e), AssertionError)
				else:
					saashq.throw(str(e))
...
class TracedDocument(Document):
	"""
	A base class for Saashq documents with traced fields.

	This class extends Saashq's Document class to provide support for
	traced fields created with TracedValue.

	Attributes:
	        Inherits all attributes from saashq.model.document.Document
	"""
...
def traced_field(*args, **kwargs):
	...
	This function simplifies the creation of traced fields in Saashq documents.
...
from saashq.deprecation_dumpster import model_trace_traced_field_context as traced_field_context
