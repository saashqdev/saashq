...
from saashq import _, get_module_path
from saashq.desk.doctype.tag.tag import delete_tags_for_document
from saashq.model.docstatus import DocStatus
from saashq.model.dynamic_links import get_dynamic_link_map
from saashq.model.naming import revert_series_if_last
from saashq.model.utils import is_virtual_doctype
from saashq.utils.file_manager import remove_all
from saashq.utils.global_search import delete_for_document
from saashq.utils.password import delete_all_passwords_for
...
def delete_doc(
	...
	# get from form
	if not doctype:
		doctype = saashq.form_dict.get("dt")
		name = saashq.form_dict.get("dn")
	...
	for name in names or []:
		if is_virtual:
			saashq.get_doc(doctype, name).delete()
			continue
		...
		if doctype == "DocType":
			if for_reload:
				...
				doc = saashq.get_doc(doctype, name)
				if not (doc.custom or saashq.conf.developer_mode or saashq.flags.in_patch or force):
					saashq.throw(_("Standard DocType can not be deleted."))

				update_flags(doc, flags, ignore_permissions)
				check_permission_and_not_submitted(doc)
				# delete custom table fields using this doctype.
				saashq.db.delete(
					"Custom Field", {"options": name, "fieldtype": ("in", saashq.model.table_fields)}
				)
				saashq.db.delete("__global_search", {"doctype": name})

			delete_from_table(doctype, name, ignore_doctypes, None)

			if (
				saashq.conf.developer_mode
				and not doc.custom
				and not (
					for_reload
			...
			try:
				saashq.db.get_value(doctype, name, for_update=True, wait=False)
			except saashq.QueryTimeoutError:
				saashq.throw(
					_(
						"This document can not be deleted right now as it's being modified by another user. Please try again after some time."
					),
					exc=saashq.QueryTimeoutError,
				)
			doc = saashq.get_doc(doctype, name)
			...
			if not for_reload:
				...
				if not force:
					...
					except saashq.LinkExistsError as e:
			...
			if not for_reload:
				...
				saashq.enqueue(
					"saashq.model.delete_doc.delete_dynamic_links",
					doctype=doc.doctype,
					name=doc.name,
...
def add_to_deleted_document(doc):
	...
	if doc.doctype != "Deleted Document" and saashq.flags.in_install != "saashq":
		saashq.get_doc(
...
def delete_from_table(doctype: str, name: str, ignore_doctypes: list[str], doc):
	if doctype != "DocType" and doctype == name:
		saashq.db.delete("Singles", {"doctype": name})
	else:
		saashq.db.delete(doctype, {"name": name})
	if doc:
		child_doctypes = [
			d.options for d in doc.meta.get_table_fields() if saashq.get_meta(d.options).is_virtual == 0
		]
		...
		child_doctypes = saashq.get_all(
			"DocField",
			fields="options",
			filters={"fieldtype": ["in", saashq.model.table_fields], "parent": doctype},
			pluck="options",
		)

	child_doctypes_to_delete = set(child_doctypes) - set(ignore_doctypes)
	for child_doctype in child_doctypes_to_delete:
		saashq.db.delete(child_doctype, {"parenttype": doctype, "parent": name})
...
def check_permission_and_not_submitted(doc):
	...
	if (
		...
		and saashq.session.user != "Administrator"
		...
		saashq.msgprint(
			...
			raise_exception=saashq.PermissionError,
		)
	...
	if doc.meta.is_submittable and doc.docstatus.is_submitted():
		saashq.msgprint(
			_("{0} {1}: Submitted Record cannot be deleted. You must {2} Cancel {3} it first.").format(
				_(doc.doctype),
				doc.name,
				"<a href='https://docs.erpnexus.com//docs/user/manual/en/setting-up/articles/delete-submitted-document' target='_blank'>",
				"</a>",
			),
...
def check_if_doc_is_linked(doc, method="Delete"):
	...
	"""
	from saashq.model.rename_doc import get_link_fields
	...
	if method == "Delete":
		ignored_doctypes.update(saashq.get_hooks("ignore_links_on_delete"))
	...
	for lf in link_fields:
		...
		if issingle:
			if saashq.db.get_single_value(link_dt, link_field) == doc.name:
		...
		for item in saashq.db.get_values(link_dt, {link_field: doc.name}, fields, as_dict=True):
...
def check_if_doc_is_dynamically_linked(doc, method="Delete"):
	"""Raise `saashq.LinkExistsError` if the document is dynamically linked"""
	for df in get_dynamic_link_map().get(doc.doctype, []):
		ignore_linked_doctypes = doc.get("ignore_linked_doctypes") or []

		if df.parent in saashq.get_hooks("ignore_links_on_delete") or (
		...
		if meta.issingle:
			...
			for refdoc in saashq.db.sql(
				...
				if (method == "Delete" and not DocStatus(refdoc.docstatus).is_cancelled()) or (
					...
					if reference_doctype in saashq.get_hooks("ignore_links_on_delete") or (
...
def clear_timeline_references(link_doctype, link_name):
	saashq.db.delete("Communication Link", {"link_doctype": link_doctype, "link_name": link_name})
...
def delete_controllers(doctype, module):
	...
	dir_path = os.path.join(module_path, "doctype", saashq.scrub(doctype))
