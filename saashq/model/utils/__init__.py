...
def set_default(doc, key):
	...
	if not doc.is_default:
		saashq.db.set(doc, "is_default", 1)
...
def set_field_property(filters, key, value):
	...
	docs = [
		saashq.get_doc("DocType", d.parent)
		for d in saashq.get_all("DocField", fields=["parent"], filters=filters)
	]
...
class InvalidIncludePath(saashq.ValidationError):
	pass
	...
	for _ in range(5):
		if "{% include" in content:
			...
			for path in paths:
				...
				with open(saashq.get_app_path(app, app_path), encoding="utf-8") as f:
					include = f.read()
...
def is_virtual_doctype(doctype: str):
	if saashq.flags.in_install or saashq.flags.in_migrate:
		if saashq.db.has_column("DocType", "is_virtual"):
			return saashq.db.get_value("DocType", doctype, "is_virtual")
	else:
		return getattr(saashq.get_meta(doctype), "is_virtual", False)
...
def is_single_doctype(doctype: str) -> bool:
	...
	if saashq.flags.in_install or saashq.flags.in_migrate:
		...
		return getattr(saashq.get_meta(doctype), "issingle", False)
