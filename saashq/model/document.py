...
import saashq
from saashq import _, is_whitelisted, msgprint
from saashq.core.doctype.file.utils import relink_mismatched_files
from saashq.core.doctype.server_script.server_script_utils import run_server_script_for_doc_event
from saashq.desk.form.document_follow import follow_document
from saashq.integrations.doctype.webhook import run_webhooks
from saashq.model import optional_fields, table_fields
from saashq.model.base_document import BaseDocument, get_controller
from saashq.model.docstatus import DocStatus
from saashq.model.naming import set_new_name, validate_name
from saashq.model.utils import is_virtual_doctype, simple_singledispatch
from saashq.model.workflow import set_workflow_state_on_action, validate_workflow
from saashq.types import DF, DocRef
from saashq.utils import compare, cstr, date_diff, file_lock, flt, now
from saashq.utils.data import get_absolute_url, get_datetime, get_timedelta, getdate
from saashq.utils.global_search import update_global_search
...
def get_doc(*args, **kwargs) -> "Document":
	"""Return a `saashq.model.Document` object.
...
def read_only_document(context=None):
	...
	def read_only_method(func):
		...
		def wrapper(self, *args, **kwargs):
			...
			if context:
				error_msg += f" ({context})"
			raise saashq.DatabaseModificationError(error_msg)

		return wrapper

	# Use a thread-local variable to track nested invocations
	if not hasattr(saashq.local, "read_only_depth"):
		saashq.local.read_only_depth = 0

	try:
		# Increment the depth counter
		saashq.local.read_only_depth += 1

		# Only apply read-only methods if this is the outermost invocation
		if saashq.local.read_only_depth == 1:
...
class Document(BaseDocument, DocRef):
	...
	def load_from_db(self) -> "Self":
		...
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			single_doc = saashq.db.get_singles_dict(self.doctype, for_update=self.flags.for_update)
			if not single_doc:
				single_doc = saashq.new_doc(self.doctype, as_dict=True)
			...
			if not d:
				saashq.throw(
					_("{0} {1} not found").format(_(self.doctype), self.name), saashq.DoesNotExistError
				)
	...
	def load_children_from_db(self):
		for df in self._get_table_fields():
			...
			# During saashq is installed, the property "is_virtual" is not available in tabDocType, so
			# we need to filter those cases for the access to saashq.db.get_value() as it would crash otherwise.
	...
	def check_permission(self, permtype="read", permlevel=None):
		"""Raise `saashq.PermissionError` if not permitted"""
		if not self.has_permission(permtype):
			self.raise_no_permission_to(permtype)

	def has_permission(self, permtype="read", *, debug=False, user=None) -> bool:
		"""
		Call `saashq.permissions.has_permission` if `ignore_permissions` flag isn't truthy
		...
		return saashq.permissions.has_permission(self.doctype, permtype, self, debug=debug, user=user)

	def raise_no_permission_to(self, perm_type):
		"""Raise `saashq.PermissionError`."""
		saashq.flags.error_message = (
			_("Insufficient Permission for {0}").format(_(self.doctype)) + f" ({saashq.bold(_(perm_type))})"
		)
		raise saashq.PermissionError
	...
	def insert(
		...
		if not (saashq.flags.in_migrate or saashq.local.flags.in_install or saashq.flags.in_setup_wizard):
			if saashq.get_cached_value("User", saashq.session.user, "follow_created_documents"):
				follow_document(self.doctype, self.name, saashq.session.user)
		return self

	def check_if_locked(self):
		if self.creation and self.is_locked:
			raise saashq.DocumentLockedError
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Self":
		...
		self.flags.ignore_version = saashq.flags.in_test if ignore_version is None else ignore_version
	...
	def set_new_name(self, force=False, set_name=None, set_child_names=True):
		"""Calls `saashq.naming.set_new_name` for parent and child docs."""
	...
	def update_single(self, d):
		...
		saashq.db.delete("Singles", {"doctype": self.doctype})
		...
		if self.doctype in saashq.db.value_cache:
			del saashq.db.value_cache[self.doctype]

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = saashq.session.user
		...
		if self.is_new() and not (
			saashq.flags.in_install or saashq.flags.in_patch or saashq.flags.in_migrate
		):
		...
		saashq.flags.currently_saving.append((self.doctype, self.name))
	...
	def _validate_non_negative(self):
		def get_msg(df):
			if self.get("parentfield"):
				return "{} {} #{}: {} {}".format(
					saashq.bold(_(self.doctype)),
					_("Row"),
					self.idx,
					_("Value cannot be negative for"),
					saashq.bold(_(df.label, context=df.parent)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), saashq.bold(_(df.label, context=df.parent))
				)
		...
		for df in self.meta.get(
			...
			if flt(self.get(df.fieldname)) < 0:
				msg = get_msg(df)
				saashq.throw(msg, saashq.NonNegativeError, title=_("Negative Value"))
				...
				if fail:
					saashq.throw(
						_("Value cannot be changed for {0}").format(
							saashq.bold(self.meta.get_label(field.fieldname))
						),
						exc=saashq.CannotChangeConstantError,
					)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields():
			all_fields += saashq.get_meta(table_field.options).fields or []
		...
		for table_field in self.meta.get_table_fields():
			for df in saashq.get_meta(table_field.options).fields or []:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or saashq.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = saashq.get_meta(df.options).get_high_permlevel_fields()
			if high_permlevel_fields:
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = saashq.get_meta(self.parenttype).permissions
		else:
	...
	def _set_defaults(self):
		...
		if self.is_new():
			new_doc = saashq.new_doc(self.doctype, as_dict=True)
			self.update_if_missing(new_doc)

		# children
		for df in self.meta.get_table_fields():
			new_doc = saashq.new_doc(df.options, parent_doc=self, parentfield=df.fieldname, as_dict=True)
	...
	def check_if_latest(self):
		...
		if cstr(previous.modified) != cstr(self._original_modified):
			saashq.msgprint(
				...
				raise_exception=saashq.TimestampMismatchError,
			)
	...
	def check_docstatus_transition(self, to_docstatus):
		...
		if to_docstatus == DocStatus.draft():
			if self.docstatus.is_draft():
				...
				raise saashq.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise saashq.ValidationError(_("Invalid docstatus"), self.docstatus)
			...
			raise saashq.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		if saashq.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise saashq.MandatoryError(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			saashq.throw(_("Could not find {0}").format(msg), saashq.LinkValidationError)

		if cancelled_links:
			msg = ", ".join(each[2] for each in cancelled_links)
			saashq.throw(_("Cannot link cancelled document: {0}").format(msg), saashq.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			(saashq.flags.in_import and saashq.flags.mute_emails)
		...
		from saashq.email.doctype.notification.notification import evaluate_alert
		...
		if self.flags.notifications is None:
			...
			self.flags.notifications = saashq.cache.hget("notifications", self.doctype, _get_notifications)
	...
	def _rename(self, name: str, merge: bool = False, force: bool = False, validate_rename: bool = True):
		"""Rename the document. Triggers saashq.rename_doc, then reloads."""
	...
	def discard(self):
		...
		if not self.docstatus == DocStatus.draft():
			raise saashq.ValidationError(_("Only draft documents can be discarded"), self.docstatus)
	...
	def load_doc_before_save(self, *, raise_exception: bool = False):
		...
		try:
			self._doc_before_save = saashq.get_doc(self.doctype, self.name, for_update=True)
	...
	def run_post_save_methods(self):
		...
		if (self.doctype, self.name) in saashq.flags.currently_saving:
			saashq.flags.currently_saving.remove((self.doctype, self.name))

	def clear_cache(self):
		saashq.clear_document_cache(self.doctype, self.name)
	...
	def reset_seen(self):
		...
		if (
			...
			saashq.db.set_value(
				self.doctype, self.name, "_seen", json.dumps([saashq.session.user]), update_modified=False
			)
	...
	def notify_update(self):
		...
		if not self.meta.get("read_only") and not self.meta.get("issingle") and not self.meta.get("istable"):
			data = {"doctype": self.doctype, "name": self.name, "user": saashq.session.user}
			saashq.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		:param commit: default False. run saashq.db.commit()
		"""
		...
		if update_modified and (self.doctype, self.name) not in saashq.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", saashq.session.user)
	...
	def db_get(self, fieldname):
		...
		return saashq.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from saashq.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def hook(f):
		...
		def compose(fn, *hooks):
			def runner(self, method, *args, **kwargs):
				...
				for f in hooks:
					try:
						saashq.db._disable_transaction_control += 1
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			saashq.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or saashq.EmptyTableError
			)
		...
		return saashq.get_doc(
			{
				...
				"comment_email": comment_email or saashq.session.user,
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen and not saashq.flags.read_only and not self.meta.issingle:
			_seen = self.get("_seen") or []
			_seen = saashq.parse_json(_seen)
			...
			if user not in _seen:
				...
				saashq.local.flags.commit = True
	...
	def add_viewed(self, user=None, force=False, unique_views=False):
		...
		if unique_views and saashq.db.exists(
	...
	def get_document_share_key(self, expires_on=None, no_expiry=False):
		...
		if existing_key:
			doc = saashq.get_doc("Document Share Key", existing_key)
		else:
			doc = saashq.new_doc("Document Share Key")
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", saashq._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", saashq._dict())
	...
	def queue_action(self, action, **kwargs):
		...
		return enqueue(
			"saashq.model.document.execute_action",
	...
	def lock(self, timeout=None):
		...
		saashq.local.locked_documents.append(self)

	def unlock(self):
		"""Delete the lock file for this document"""
		file_lock.delete_lock(self.get_signature())
		if self in saashq.local.locked_documents:
			saashq.local.locked_documents.remove(self)
		...
		if date_diff(to_date, from_date) < 0:
			saashq.throw(
				_("{0} must be after {1}").format(
					saashq.bold(_(self.meta.get_label(to_date_field))),
					saashq.bold(_(self.meta.get_label(from_date_field))),
				),
				saashq.exceptions.InvalidDates,
			)

	def get_assigned_users(self):
		assigned_users = saashq.get_all(
			"ToDo",
			fields=["allocated_to"],
			filters={
	...
	def add_tag(self, tag):
		...
		from saashq.desk.doctype.tag.tag import DocTags
	...
	def deferred_insert(self) -> None:
		...
		from saashq.deferred_insert import deferred_insert
...
def execute_action(__doctype, __name, __action, **kwargs):
	...
	doc = saashq.get_doc(__doctype, __name)
	doc.unlock()
	try:
		...
		if saashq.message_log:
			msg = saashq.message_log[-1].get("message")
		else:
			msg = "<pre><code>" + saashq.get_traceback() + "</pre></code>"
...
def bulk_insert(
	...
	for child_table in doctype_meta.get_table_fields():
		valid_column_map[child_table.options] = saashq.get_meta(child_table.options).get_valid_columns()
...
def _document_values_generator(
	...
	for doc in documents:
		...
		doc.owner = doc.modified_by = saashq.session.user
...
def unlock_document(doctype: str, name: str):
	saashq.get_doc(doctype, name).unlock()
	saashq.msgprint(saashq._("Document Unlocked"), alert=True)
