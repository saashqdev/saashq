...
from saashq.modules.patch_handler import _patch_mode
...
def sync_for(app_name, force=0, reset_permissions=False):
	...
	if app_name == "saashq":
		...
		SAASHQ_PATH = saashq.get_app_path("saashq")
		...
		for core_module in [
			...
			files.append(os.path.join(SAASHQ_PATH, "core", "doctype", core_module, f"{core_module}.json"))

		for custom_module in ["custom_field", "property_setter"]:
			files.append(
				os.path.join(SAASHQ_PATH, "custom", "doctype", custom_module, f"{custom_module}.json")
			)

		for website_module in ["web_form", "web_template", "web_form_field", "portal_menu_item"]:
			files.append(
				os.path.join(SAASHQ_PATH, "website", "doctype", website_module, f"{website_module}.json")
			)
		...
		for desk_module in [
			...
			files.append(os.path.join(SAASHQ_PATH, "desk", "doctype", desk_module, f"{desk_module}.json"))

		for module_name, document_type in IMPORTABLE_DOCTYPES:
			file = os.path.join(SAASHQ_PATH, module_name, "doctype", document_type, f"{document_type}.json")
			if file not in files:
				files.append(file)

	for module_name in saashq.local.app_modules.get(app_name) or []:
		folder = os.path.dirname(saashq.get_module(app_name + "." + module_name).__file__)
...
def remove_orphan_doctypes():
	...
	doctype_names = saashq.get_all("DocType", {"custom": 0}, pluck="name")
	orphan_doctypes = []
	...
	for i, name in enumerate(orphan_doctypes):
		saashq.delete_doc("DocType", name, force=True, ignore_missing=True)
