...
from saashq.model.dynamic_links import get_dynamic_link_map
from saashq.model.naming import validate_name
from saashq.model.utils.user_settings import sync_user_settings, update_user_settings_data
from saashq.query_builder import Field
from saashq.utils.data import sbool
from saashq.utils.password import rename_password
	...
	if title_updated:
		if action_enqueued and name_updated:
			...
			try:
				...
				doc.save()
				saashq.msgprint(_("Saved"), alert=True, indicator="green")
			except Exception as e:
				if saashq.db.is_duplicate_entry(e):
					saashq.throw(
						_("{0} {1} already exists").format(doctype, saashq.bold(docname)),
						title=_("Duplicate Name"),
						exc=saashq.DuplicateEntryError,
					)
				raise
...
def rename_doc(
	...
	if not (new_usage_style or old_usage_style):
		raise TypeError(
			"{doctype, old, new} or {doc, new} are required arguments for saashq.model.rename_doc"
		)
	...
	if validate:
		old_doc = doc or saashq.get_doc(doctype, old)
	...
	# call after_rename
	new_doc = saashq.get_doc(doctype, new)
	...
	if merge:
		new_doc.add_comment("Edit", _("merged {0} into {1}").format(saashq.bold(old), saashq.bold(new)))
	else:
		new_doc.add_comment("Edit", _("renamed from {0} to {1}").format(saashq.bold(old), saashq.bold(new)))

	if merge:
		saashq.delete_doc(doctype, old)

	new_doc.clear_cache()
	saashq.clear_cache()
	if rebuild_search:
		saashq.enqueue("saashq.utils.global_search.rebuild_for_doctype", doctype=doctype)
...
def update_assignments(old: str, new: str, doctype: str) -> None:
	old_assignments = saashq.parse_json(saashq.db.get_value(doctype, old, "_assign")) or []
	new_assignments = saashq.parse_json(saashq.db.get_value(doctype, new, "_assign")) or []
	...
	for user in common_assignments:
		...
		for todo in todos:
			saashq.delete_doc("ToDo", todo.name)

	unique_assignments = list(set(old_assignments + new_assignments))
	saashq.db.set_value(doctype, new, "_assign", saashq.as_json(unique_assignments, indent=0))
...
def update_user_settings(old: str, new: str, link_fields: list[dict]) -> None:
	...
	UserSettings = saashq.qb.Table("__UserSettings")

	user_settings_details = (
		saashq.qb.from_(UserSettings)
...
def update_customizations(old: str, new: str) -> None:
	saashq.db.set_value("Custom DocPerm", {"parent": old}, "parent", new, update_modified=False)


def update_attachments(doctype: str, old: str, new: str) -> None:
	if doctype != "DocType":
		File = saashq.qb.DocType("File")

		saashq.qb.update(File).set(File.attached_to_name, new).where(
			(File.attached_to_name == old) & (File.attached_to_doctype == doctype)
		).run()


def rename_versions(doctype: str, old: str, new: str) -> None:
	Version = saashq.qb.DocType("Version")

	saashq.qb.update(Version).set(Version.docname, new).where(
		(Version.docname == old) & (Version.ref_doctype == doctype)
	).run()


def rename_eps_records(doctype: str, old: str, new: str) -> None:
	EPL = saashq.qb.DocType("Energy Point Log")

	saashq.qb.update(EPL).set(EPL.reference_name, new).where(
...
def rename_parent_and_child(doctype: str, old: str, new: str, meta: "Meta") -> None:
	saashq.qb.update(doctype).set("name", new).where(Field("name") == old).run()
...
def update_autoname_field(doctype: str, new: str, meta: "Meta") -> None:
	...
	if meta.get("autoname"):
		...
		if field and field[0] == "field":
			saashq.qb.update(doctype).set(field[1], new).where(Field("name") == new).run()
...
def validate_rename(
	...
	if save_point:
		_SAVE_POINT = f"validate_rename_{saashq.generate_hash(length=8)}"
		saashq.db.savepoint(_SAVE_POINT)

	exists = saashq.qb.from_(doctype).where(Field("name") == new).for_update().select("name").run(pluck=True)
	exists = exists[0] if exists else None

	if not saashq.db.exists(doctype, old):
		saashq.throw(_("Can't rename {0} to {1} because {0} doesn't exist.").format(old, new))

	if old == new:
		saashq.throw(_("No changes made because old and new name are the same.").format(old, new))
	...
	if merge and not exists:
		saashq.throw(_("{0} {1} does not exist, select a new target to merge").format(doctype, new))

	if not merge and exists and not ignore_if_exists:
		saashq.throw(_("Another {0} with name {1} exists, select another name").format(doctype, new))

	kwargs = {"doctype": doctype, "ptype": "write", "print_logs": False}
	if old_doc:
		kwargs |= {"doc": old_doc}

	if not (ignore_permissions or saashq.permissions.has_permission(**kwargs)):
		saashq.throw(_("You need write permission to rename"))

	if not force and not ignore_permissions and not meta.allow_rename:
		saashq.throw(_("{0} not allowed to be renamed").format(_(doctype)))

	# validate naming like it's done in doc.py
	new = validate_name(doctype, new)

	if save_point:
		saashq.db.rollback(save_point=_SAVE_POINT)
...
def rename_doctype(doctype: str, old: str, new: str) -> None:
	...
	fields_with_options = ("Link", *saashq.model.table_fields)
...
def update_link_field_values(link_fields: list[dict], old: str, new: str, doctype: str) -> None:
	for field in link_fields:
		if field["issingle"]:
			try:
				single_doc = saashq.get_doc(field["parent"])
			...
			saashq.db.set_value(parent, {docfield: old}, docfield, new, update_modified=False)
...
def get_link_fields(doctype: str) -> list[dict]:
	...
	if not saashq.flags.link_fields:
		saashq.flags.link_fields = {}

	if doctype not in saashq.flags.link_fields:
		dt = saashq.qb.DocType("DocType")
		df = saashq.qb.DocType("DocField")
		cf = saashq.qb.DocType("Custom Field")
		ps = saashq.qb.DocType("Property Setter")
		...
		if saashq.db.has_column("DocField", "is_virtual"):
			standard_fields_query = standard_fields_query.where(df.is_virtual == 0)

		virtual_doctypes = []
		if saashq.db.has_column("DocType", "is_virtual"):
			virtual_doctypes = saashq.get_all("DocType", {"is_virtual": 1}, pluck="name")
			standard_fields_query = standard_fields_query.where(dt.is_virtual == 0)

		standard_fields = standard_fields_query.run(as_dict=True)

		cf_issingle = saashq.qb.from_(dt).select(dt.issingle).where(dt.name == cf.dt).as_("issingle")
		custom_fields = (
			saashq.qb.from_(cf)
		...
		ps_issingle = saashq.qb.from_(dt).select(dt.issingle).where(dt.name == ps.doc_type).as_("issingle")
		...
		saashq.flags.link_fields[doctype] = standard_fields + custom_fields + property_setter_fields

	return saashq.flags.link_fields[doctype]


def update_options_for_fieldtype(fieldtype: str, old: str, new: str) -> None:
	CustomField = saashq.qb.DocType("Custom Field")
	PropertySetter = saashq.qb.DocType("Property Setter")

	if saashq.conf.developer_mode:
		for name in saashq.get_all("DocField", filters={"options": old}, pluck="parent"):
			if name in (old, new):
				continue

			doctype = saashq.get_doc("DocType", name)
			save = False
	...
	DocField = saashq.qb.DocType("DocField")
	saashq.qb.update(DocField).set(DocField.options, new).where(
		(DocField.fieldtype == fieldtype) & (DocField.options == old)
	).run()

	saashq.qb.update(CustomField).set(CustomField.options, new).where(
		(CustomField.fieldtype == fieldtype) & (CustomField.options == old)
	).run()

	saashq.qb.update(PropertySetter).set(PropertySetter.value, new).where(
...
def get_select_fields(old: str, new: str) -> list[dict]:
	...
	# get link fields from tabDocField
	st_issingle = saashq.qb.from_(dt).select(dt.issingle).where(dt.name == df.parent).as_("issingle")
	standard_fields = (
		saashq.qb.from_(df)
...
def update_select_field_values(old: str, new: str):
	from saashq.query_builder.functions import Replace

	DocField = saashq.qb.DocType("DocField")
	CustomField = saashq.qb.DocType("Custom Field")
	PropertySetter = saashq.qb.DocType("Property Setter")

	saashq.qb.update(DocField).set(DocField.options, Replace(DocField.options, old, new)).where(
		(DocField.fieldtype == "Select")
		& (DocField.parent != new)
		& (DocField.options.like(f"%\n{old}%") | DocField.options.like(f"%{old}\n%"))
	).run()

	saashq.qb.update(CustomField).set(CustomField.options, Replace(CustomField.options, old, new)).where(
		(CustomField.fieldtype == "Select")
		& (CustomField.dt != new)
		& (CustomField.options.like(f"%\n{old}%") | CustomField.options.like(f"%{old}\n%"))
	).run()

	saashq.qb.update(PropertySetter).set(PropertySetter.value, Replace(PropertySetter.value, old, new)).where(
...
def update_parenttype_values(old: str, new: str):
	child_doctypes = saashq.get_all(
		"DocField",
		fields=["options", "fieldname"],
		filters={"parent": new, "fieldtype": ["in", saashq.model.table_fields]},
	)

	custom_child_doctypes = saashq.get_all(
		"Custom Field",
		fields=["options", "fieldname"],
		filters={"dt": new, "fieldtype": ["in", saashq.model.table_fields]},
	)

	child_doctypes += custom_child_doctypes
	fields = [d["fieldname"] for d in child_doctypes]

	property_setter_child_doctypes = saashq.get_all(
		"Property Setter",
	...
	for doctype in child_doctypes:
		...
		saashq.qb.update(table).set(table.parenttype, new).where(table.parenttype == old).run()


def rename_dynamic_links(doctype: str, old: str, new: str):
	Singles = saashq.qb.DocType("Singles")
	for df in get_dynamic_link_map().get(doctype, []):
		...
		if meta.is_virtual:
			continue
		if meta.issingle:
			refdoc = saashq.db.get_singles_dict(df.parent)
			if refdoc.get(df.options) == doctype and refdoc.get(df.fieldname) == old:
				saashq.qb.update(Singles).set(Singles.value, new).where(
			...
			saashq.qb.update(parent).set(df.fieldname, new).where(
...
def bulk_rename(doctype: str, rows: list[list] | None = None, via_console: bool = False) -> list[str] | None:
	...
	if not rows:
		saashq.throw(_("Please select a valid csv file with data"))

	if not via_console:
		max_rows = 500
		if len(rows) > max_rows:
			saashq.throw(_("Maximum {0} rows allowed").format(max_rows))
