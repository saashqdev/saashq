...
from saashq.database.utils import DefaultOrderBy, FallBackDateTimeStr, NestedSetHierarchy
from saashq.model import get_permitted_fields, optional_fields
from saashq.model.meta import get_table_columns
from saashq.model.utils import is_virtual_doctype
from saashq.model.utils.user_settings import get_user_settings, update_user_settings
from saashq.query_builder.utils import Column
...
from saashq.utils.data import DateTimeLikeObject, get_datetime, getdate, sbool
...
class DatabaseQuery:
	def __init__(self, doctype, user=None):
		...
		self.or_conditions = []
		self.fields = None
		self.user = user or saashq.session.user
	...
	def doctype_meta(self):
		if not hasattr(self, "_doctype_meta"):
			self._doctype_meta = saashq.get_meta(self.doctype)
	...
	def execute(
		...
		*,
		...
		if as_list and not isinstance(self.fields, (Sequence | str)) and len(self.fields) > 1:
			saashq.throw(_("Fields must be a list or tuple when as_list is enabled"))
		...
		if is_virtual_doctype(self.doctype):
			...
			return saashq.call(controller.get_list, args=kwargs, **kwargs)
	...
	def build_and_run(self):
		...
		if saashq.db.db_type == "postgres" and args.order_by and args.group_by:
	...
	def prepare_args(self):
		...
		for child in self.tables[1:]:
			...
			args.tables += f" {self.join} {child} on ({child}.parenttype = {saashq.db.escape(self.doctype)} and {child}.parent = {parent_name})"
	...
	def parse_args(self):
		...
		for field in self.fields:
			if "." in field:
				...
				linked_field = saashq.get_meta(self.doctype).get_field(linked_fieldname)
	...
	def sanitize_fields(self):
		...
		def _raise_exception():
			saashq.throw(_("Use of sub-query or function is restricted"), saashq.DataError)
		...
		for field in self.fields:
			...
			if SUB_QUERY_PATTERN.match(field):
				...
				if function in blacklisted_functions:
					saashq.throw(
						_("Use of function {0} in field is restricted").format(function), exc=saashq.DataError
					)
			...
			if self.strict:
				if STRICT_FIELD_PATTERN.match(field):
					saashq.throw(_("Illegal SQL Query"))
	...
	def _set_permission_map(self, doctype: str, parent_doctype: str | None = None):
		ptype = "select" if saashq.only_has_select_perm(doctype) else "read"
		saashq.has_permission(
			doctype,
			ptype=ptype,
	...
	def apply_fieldlevel_read_permissions(self):
		...
		Note: Does not apply to `saashq.model.core_doctype_list`
		...
		Example:
		        ...
		        - Query: fields=["*"]
		        - Result: fields=["title", ...] // will also include Saashq's meta field like `name`, `owner`, etc.
		"""
		from saashq.desk.reportview import extract_fieldnames
		...
		for i, field in enumerate(self.fields):
			...
			# labels / pseudo columns or saashq internals
			...
			# handle child / joined table fields
				...
				if wrap_grave_quotes(table) in self.query_tables:
					...
					raise saashq.PermissionError(ch_doctype)
	...
	def prepare_filter_condition(self, f):
		...
		from saashq.boot import get_additional_filters_from_hooks
		...
		if f.operator.lower() in NestedSetHierarchy:
			...
			lft, rgt = "", ""
			if f.value:
				lft, rgt = saashq.db.get_value(ref_doctype, f.value, ["lft", "rgt"]) or (0, 0)
			...
			fallback = "''"
			value = [saashq.db.escape((cstr(v)).strip(), percent=False) for v in nodes]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
		...
		if f.operator.lower() in ("in", "not in"):
			...
			if value is None:
				...
				fallback = "''"
				value = [saashq.db.escape((cstr(v) or "").strip(), percent=False) for v in values]
				if len(value):
					value = f"({', '.join(value)})"
				else:
					value = "('')"
			...
			if f.operator in (">", "<", ">=", "<=") and (f.fieldname in ("creation", "modified")):
				...
				value = saashq.db.format_date(f.value)
				fallback = "'0001-01-01'"

			elif (df and df.fieldtype == "Datetime") or isinstance(f.value, datetime.datetime):
				value = saashq.db.format_datetime(f.value)
				fallback = f"'{FallBackDateTimeStr}'"
			...
			if isinstance(f.value, Column):
				...
				quote = '"' if saashq.conf.db_type == "postgres" else "`"
				value = f"{tname}.{quote}{f.value.name}{quote}"

			# escape value
			elif escape and isinstance(value, str):
				value = f"{saashq.db.escape(value, percent=False)}"
		...
		if (
			...
			if f.operator.lower() == "like" and saashq.conf.get("db_type") == "postgres":
				f.operator = "ilike"
	...
	def build_match_conditions(self, as_condition=True) -> str | list:
		...
		role_permissions = saashq.permissions.get_role_permissions(self.doctype_meta, user=self.user)
		if (
			...
			only_if_shared = True
			self.shared = saashq.share.get_shared(self.doctype, self.user)
			if not self.shared:
				saashq.throw(_("No permission to read {0}").format(_(self.doctype)), saashq.PermissionError)
			else:
			...
			if requires_owner_constraint(role_permissions):
				...
				self.match_conditions.append(
					f"`tab{self.doctype}`.`owner` = {saashq.db.escape(self.user, percent=False)}"
				)

			# add user permission only if role has read perm
			elif role_permissions.get("read") or role_permissions.get("select"):
				# get user permissions
				user_permissions = saashq.permissions.get_user_permissions(self.user)
	...
	def get_share_condition(self):
		return (
			...
			+ f" in ({', '.join(saashq.db.escape(s, percent=False) for s in self.shared)})"
		)
	...
	def add_user_permissions(self, user_permissions):
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				docs = []
				if saashq.get_system_settings("apply_strict_user_permissions"):
					condition = ""
				else:
				...
				if docs:
					values = ", ".join(saashq.db.escape(doc, percent=False) for doc in docs)
	...
	def get_permission_query_conditions(self) -> str:
		conditions = []
		hooks = saashq.get_hooks("permission_query_conditions", {})
		condition_methods = hooks.get(self.doctype, []) + hooks.get("*", [])
		for method in condition_methods:
			if c := saashq.call(saashq.get_attr(method), self.user, doctype=self.doctype):
				conditions.append(c)

		if permission_script_name := get_server_script_map().get("permission_query", {}).get(self.doctype):
			script = saashq.get_doc("Server Script", permission_script_name)
	...
	def validate_order_by_and_group_by(self, parameters: str):
		...
		if "select" in _lower and "from" in _lower:
			saashq.throw(_("Cannot use sub-query in order by"))
		...
		for field in parameters.split(","):
			...
			if full_field_name:
				...
				if tbl not in self.tables:
					if tbl.startswith("`"):
						tbl = tbl[4:-1]
					saashq.throw(_("Please select atleast 1 column from {0} to sort/group").format(tbl))

			if function in blacklisted_sql_functions:
				saashq.throw(_("Cannot use {0} in order/group by").format(field))
...
def check_parent_permission(parent, child_doctype):
	if parent:
		...
		if child_doctype and not (
			saashq.db.exists("DocField", {"parent": parent, "options": child_doctype})
			or saashq.db.exists("Custom Field", {"dt": parent, "options": child_doctype})
		):
			raise saashq.PermissionError

		if saashq.permissions.has_permission(parent):
			return
...
def has_any_user_permission_for_doctype(doctype, user, applicable_for):
	user_permissions = saashq.permissions.get_user_permissions(user=user)
...
def get_between_date_filter(value, df=None):
	...
	from_date = saashq.utils.nowdate()
	to_date = saashq.utils.nowdate()
	...
	if fieldtype == "Datetime":
		cond = f"'{saashq.db.format_datetime(from_date)}' AND '{saashq.db.format_datetime(to_date)}'"
	else:
		cond = f"'{saashq.db.format_date(from_date)}' AND '{saashq.db.format_date(to_date)}'"
...
def get_additional_filter_field(additional_filters_config, f, value):
	...
	f = saashq._dict(saashq.get_attr(additional_filter["get_field"])())
	if f.query_value:
		for option in f.options:
			option = saashq._dict(option)
