...
from saashq.model.base_document import (
...
from saashq.types import DocRef
from saashq.utils import cast, cint, cstr
...
def get_meta(doctype: str | DocRef | Document, cached=True) -> "_Meta":
	...
	if cached and not isinstance(doctype, Document):
		if meta := saashq.cache.hget("doctype_meta", doctype_name):
			return meta

	meta = Meta(doctype)
	saashq.cache.hset("doctype_meta", meta.name, meta)
	return meta
...
def get_table_columns(doctype):
	return saashq.db.get_table_columns(doctype)


def load_doctype_from_file(doctype):
	fname = saashq.scrub(doctype)
	with open(saashq.get_app_path("saashq", "core", "doctype", fname, fname + ".json")) as f:
...
class Meta(Document):
	...
	standard_set_once_fields = (
		saashq._dict(fieldname="creation", fieldtype="Datetime"),
		saashq._dict(fieldname="owner", fieldtype="Data"),
	)
	...
	def get_global_search_fields(self):
		...
		if getattr(self, "show_name_in_global_search", None):
			fields.append(saashq._dict(fieldtype="Data", fieldname="name", label="Name"))

		return fields

	def get_valid_columns(self) -> list[str]:
		if not hasattr(self, "_valid_columns"):
			table_exists = saashq.db.table_exists(self.name)
	...
	def get_valid_fields(self) -> list[str]:
		if not hasattr(self, "_valid_fields"):
			if (saashq.flags.in_install or saashq.flags.in_migrate) and self.name in self.special_doctypes:
	...
	def add_custom_fields(self):
		if not saashq.db.table_exists("Custom Field"):
			return

		custom_fields = saashq.db.get_values(
			"Custom Field",
	...
	def apply_property_setters(self):
		...
		"""
		if not saashq.db.table_exists("Property Setter"):
			return

		property_setters = saashq.db.get_values(
			"Property Setter",
	...
	def get_fields_to_check_permissions(self, user_permission_doctypes):
		...
		if self.name in user_permission_doctypes:
			fields.append(saashq._dict({"label": "Name", "fieldname": "name", "options": self.name}))
	...
	def get_permitted_fieldnames(
		...
		*,
		...
		if not permission_type:
			permission_type = "select" if saashq.only_has_select_perm(self.name, user=user) else "read"
		...
		if 0 not in permlevel_access and permission_type in ("read", "select"):
			if saashq.share.get_shared(self.name, user, rights=[permission_type], limit=1):
	...
	def get_permissions(self, parenttype=None):
		if self.istable and parenttype:
			...
			permissions = saashq.get_meta(parenttype).permissions
		else:
	...
	def get_dashboard_data(self):
		...
		implemented in other Saashq applications via hooks.
		"""
		data = saashq._dict()
		if not self.custom:
			try:
				...
				if hasattr(module, "get_data"):
					data = saashq._dict(module.get_data())
			except ImportError:
				pass
		...
		if not self.custom:
			for hook in saashq.get_hooks("override_doctype_dashboards", {}).get(self.name, []):
				data = saashq._dict(saashq.get_attr(hook)(data=data))
	...
	def get_web_template(self, suffix=""):
		...
		module_name = saashq.scrub(self.module)
		doctype = saashq.scrub(self.name)
		template_path = saashq.get_module_path(
...
def is_single(doctype):
	try:
		return saashq.db.get_value("DocType", doctype, "issingle")
	except IndexError:
		raise Exception("Cannot determine whether %s is single" % doctype)


def get_parent_dt(dt):
	if not saashq.is_table(dt):
		return ""

	return (
		saashq.db.get_value(
			"DocField",
			{"fieldtype": ("in", saashq.model.table_fields), "options": dt},
			"parent",
		)
		or ""
	)
...
def set_fieldname(field_id, fieldname):
	saashq.db.set_value("DocField", field_id, "fieldname", fieldname)
...
def get_field_currency(df, doc=None):
	...
	if not getattr(saashq.local, "field_currency", None):
		saashq.local.field_currency = saashq._dict()

	if not (
		saashq.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname)
		or (
			doc.get("parent")
			and saashq.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
		)
	):
		...
		if ":" in cstr(df.get("options")):
			...
			if len(split_opts) == 3 and doc.get(split_opts[1]):
				currency = saashq.get_cached_value(split_opts[0], doc.get(split_opts[1]), split_opts[2])
		else:
			...
			if doc.get("parenttype"):
				if currency:
					...
					if saashq.get_meta(doc.parenttype).has_field(df.get("options")):
						# only get_value if parent has currency field
						currency = saashq.db.get_value(doc.parenttype, doc.parent, df.get("options"))

		if currency:
			saashq.local.field_currency.setdefault((doc.doctype, ref_docname), saashq._dict()).setdefault(
				df.fieldname, currency
			)

	return saashq.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or (
		doc.get("parent") and saashq.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
	)


def get_field_precision(df, doc=None, currency=None):
	"""get precision based on DocField options and fieldvalue in doc"""
	from saashq.locale import get_number_format

	if df.precision:
		precision = cint(df.precision)

	elif df.fieldtype == "Currency":
		precision = cint(saashq.db.get_default("currency_precision"))
		if not precision:
			number_format = get_number_format()
			precision = number_format.precision
	else:
		precision = cint(saashq.db.get_default("float_precision")) or 3
...
def get_default_df(fieldname):
	if fieldname in (default_fields + child_table_fields):
		if fieldname in ("creation", "modified"):
			return saashq._dict(fieldname=fieldname, fieldtype="Datetime")

		elif fieldname in ("idx", "docstatus"):
			return saashq._dict(fieldname=fieldname, fieldtype="Int")

		elif fieldname in ("owner", "modified_by"):
			return saashq._dict(fieldname=fieldname, fieldtype="Link", options="User")

		return saashq._dict(fieldname=fieldname, fieldtype="Data")
...
def trim_tables(doctype=None, dry_run=False, quiet=False):
	...
	for doctype in saashq.get_all("DocType", filters=filters, pluck="name"):
		try:
...
def trim_table(doctype, dry_run=True):
	saashq.cache.hdel("table_columns", f"tab{doctype}")
	ignore_fields = default_fields + optional_fields + child_table_fields
	columns = saashq.db.get_table_columns(doctype)
	fields = saashq.get_meta(doctype, cached=False).get_fieldnames_with_value()
	...
	if columns_to_remove and not dry_run:
		...
		saashq.db.sql_ddl(f"ALTER TABLE `tab{doctype}` {columns_to_remove}")
