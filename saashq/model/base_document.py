...
from saashq.model.naming import set_new_name
from saashq.model.utils.link_count import notify_link_count
...
def get_controller(doctype):
	...
	For `custom` type, return `saashq.model.document.Document`.

	:param doctype: DocType name as string.
	"""

	if saashq.local.dev_server or saashq.flags.in_migrate:
		return import_controller(doctype)

	site_controllers = saashq.controllers.setdefault(saashq.local.site, {})
...
def import_controller(doctype):
	...
	if doctype not in DOCTYPES_FOR_DOCTYPE:
		doctype_info = saashq.db.get_value("DocType", doctype, fieldname="*")
		if doctype_info:
	...
	module_path = None
	class_overrides = saashq.get_hooks("override_doctype_class")
	...
	if class_overrides and class_overrides.get(doctype):
		...
		custom_module = saashq.get_module(module_path)
		custom_class_ = getattr(custom_module, custom_classname, None)
		if not issubclass(custom_class_, class_):
			original_class_path = saashq.bold(f"{class_.__module__}.{class_.__name__}")
			saashq.throw(
				f"{doctype}: {saashq.bold(import_path)} must be a subclass of {original_class_path}",
...
class BaseDocument:
	...
	@cached_property
	def meta(self):
		return saashq.get_meta(self.doctype)
	...
	def get_db_value(self, key):
		return saashq.db.get_value(self.doctype, self.name, key)
		...
		for fieldname in self.meta.get_valid_fields():
			...
			if df:
				...
				if isinstance(value, list) and df.fieldtype not in table_fields:
					saashq.throw(_("Value for {0} cannot be a list").format(_(df.label, context=df.parent)))
	...
	def get_valid_columns(self) -> list[str]:
		if self.doctype not in saashq.local.valid_columns:
			if self.doctype in DOCTYPES_FOR_DOCTYPE:
				from saashq.model.meta import get_table_columns

				valid = get_table_columns(self.doctype)
			else:
				valid = self.meta.get_valid_columns()

			saashq.local.valid_columns[self.doctype] = valid

		return saashq.local.valid_columns[self.doctype]
	...
	def as_json(self):
		return saashq.as_json(self.as_dict())
	...
	def db_insert(self, ignore_if_duplicate=False):
		...
		if ignore_if_duplicate and saashq.db.db_type == "postgres":
			conflict_handler = "on conflict (name) do nothing"

		if not self.creation:
			self.creation = self.modified = now()
			self.owner = self.modified_by = saashq.session.user
		...
		try:
			...
			if saashq.db.is_primary_key_violation(e):
				...
				if not ignore_if_duplicate:
					saashq.msgprint(
						_("{0} {1} already exists").format(_(self.doctype), saashq.bold(self.name)),
						title=_("Duplicate Name"),
						indicator="red",
					)
					raise saashq.DuplicateEntryError(self.doctype, self.name, e)

			elif saashq.db.is_unique_key_violation(e):
				# unique constraint
	...
	def db_update(self):
		...
		try:
			...
			if saashq.db.is_unique_key_violation(e):
				self.show_unique_validation_message(e)
			else:
				raise
	...
	def show_unique_validation_message(self, e):
		if saashq.db.db_type != "postgres":
			...
			saashq.msgprint(_("{0} must be unique").format(label or fieldname))

		# this is used to preserve traceback
		raise saashq.UniqueValidationError(self.doctype, self.name, e)
	...
	def update_modified(self):
		...
		if getattr(self.meta, "issingle", False):
			saashq.db.set_single_value(self.doctype, "modified", self.modified, update_modified=False)
		else:
			saashq.db.set_value(self.doctype, self.name, "modified", self.modified, update_modified=False)
		...
		for df in self.meta.get_link_fields() + self.meta.get("fields", {"fieldtype": ("=", "Dynamic Link")}):
			...
			if docname:
				if df.fieldtype == "Link":
					doctype = df.options
					if not doctype:
						saashq.throw(_("Options not set for link field {0}").format(df.fieldname))
				else:
					doctype = self.get(df.options)
					if not doctype:
						saashq.throw(_("{0} must be set first").format(self.meta.get_label(df.options)))
				...
				if not meta.get("is_virtual"):
					if not fields_to_fetch:
						...
						values = _dict(name=saashq.db.get_value(doctype, docname, "name", cache=True))
					else:
						...
						values = (
							saashq.db.get_value(doctype, docname, values_to_fetch, as_dict=True)
							or empty_values
						)
				...
				if meta.get("is_virtual"):
					values = saashq.get_doc(doctype, docname).as_dict()
				...
				if values:
					...
					if not values.name:
						...
						and saashq.get_meta(doctype).is_submittable
						and cint(saashq.db.get_value(doctype, docname, "docstatus")) == DocStatus.cancelled()
					):
	...
	def set_fetch_from_value(self, doctype, df, values):
		...
		if df.fieldtype in ["Small Text", "Text", "Data"]:
			from saashq.model.meta import get_default_df

			fetch_from_df = get_default_df(fetch_from_fieldname) or saashq.get_meta(doctype).get_field(
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (saashq.flags.in_test and value.startswith("_T-")):
	...
	def _validate_data_fields(self):
		...
		# data_field options defined in saashq.model.data_field_options
		for data_field in self.meta.get_data_fields():
			...
			if data_field_options == "Email":
				if (self.owner in saashq.STANDARD_USERS) and (data in saashq.STANDARD_USERS):
					continue
	...
	def _validate_constants(self):
		if saashq.flags.in_import or self.is_new() or self.flags.ignore_validate_constants:
			return

		constants = [d.fieldname for d in self.meta.get("fields", {"set_only_once": ("=", 1)})]
		if constants:
			values = saashq.db.get_value(self.doctype, self.name, constants, as_dict=True)
		...
		for fieldname in constants:
			...
			if self.get(fieldname) != value:
				saashq.throw(
					...
					saashq.CannotChangeConstantError,
				)
	...
	def _validate_code_fields(self):
		for field in self.meta.get_code_fields():
			...
			if language == "Python":
				saashq.utils.validate_python_code(code_string, fieldname=field.label, is_expression=False)

			elif language == "PythonExpression":
				saashq.utils.validate_python_code(code_string, fieldname=field.label)
	...
	def throw_length_exceeded_error(self, df, max_length, value):
		...
		saashq.throw(
			_("{0}: '{1}' ({3}) will get truncated, as max characters allowed is {2}").format(
				reference, saashq.bold(_(df.label, context=df.parent)), max_length, value
			),
			saashq.CharacterLengthExceededError,
			title=_("Value too big"),
		)

	def _validate_update_after_submit(self):
		# get the full doc with children
		db_values = saashq.get_doc(self.doctype, self.name).as_dict()
				...
				if self_value != db_value:
					saashq.throw(
						_("{0} Not allowed to change {1} after submission from {2} to {3}").format(
							...
							saashq.bold(db_value),
							saashq.bold(self_value),
						),
						saashq.UpdateAfterSubmitError,
	...
	def _save_passwords(self):
		...
		from saashq.utils.password import remove_encrypted_password, set_encrypted_password
	...
	def get_formatted(
		...
		from saashq.utils.formatters import format_value
		...
		if (
			...
			currency = saashq.db.get_value("Currency", currency_value, cache=True)
	...
	def reset_values_if_no_permlevel_access(self, has_access_to, high_permlevel_fields):
		...
		if to_reset:
			if self.is_new():
				...
				ref_doc = saashq.new_doc(self.doctype)
			else:
	...
	def _extract_images_from_text_editor(self):
		from saashq.core.doctype.file.utils import extract_images_from_doc
