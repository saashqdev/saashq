...
from saashq.model import child_table_fields, default_fields, table_fields
from saashq.model.document import read_only_document
...
def make_mapped_doc(method, source_name, selected_children=None, args=None):
	...
	saashq.is_whitelisted(method)

	if selected_children:
		selected_children = json.loads(selected_children)

	if args:
		saashq.flags.args = saashq._dict(json.loads(args))

	saashq.flags.selected_children = selected_children or None
...
def get_mapped_doc(
	...
	apply_strict_user_permissions = saashq.get_system_settings("apply_strict_user_permissions")
	...
	if not target_doc:
		...
		if table_maps[from_doctype].get("on_parent"):
			...
			if isinstance(target_parent, str):
				target_parent = saashq.get_doc(json.loads(target_parent))
			target_parentfield = target_parent.get_parentfield_of_doctype(target_doctype)
			target_doc = saashq.new_doc(
			...
			target_doc = saashq.new_doc(target_doctype)
			ret_doc = target_doc
	elif isinstance(target_doc, str):
		target_doc = saashq.get_doc(json.loads(target_doc))
		ret_doc = target_doc
	else:
		ret_doc = target_doc
	...
	if cached:
		source_doc = saashq.get_cached_doc(from_doctype, from_docname)
	else:
		source_doc = saashq.get_doc(from_doctype, from_docname)
		...
		for df in source_doc.meta.get_table_fields():
			...
			if table_map:
				...
				for source_d in source_doc.get(df.fieldname):
					...
					if (
						saashq.flags.selected_children
						and (df.fieldname in saashq.flags.selected_children)
						and source_d.name not in saashq.flags.selected_children[df.fieldname]
					):
						continue
...
def map_fetch_fields(target_doc, df, no_copy_fields):
	...
	for fetch_df in target_doc.meta.get("fields", {"fetch_from": f"^{df.fieldname}."}):
		...
		if (
			...
			if not linked_doc:
				try:
					linked_doc = saashq.get_doc(df.options, target_doc.get(df.fieldname))
				except Exception:
					return
...
def map_child_doc(source_d, target_parent, table_map, source_parent=None):
	...
	target_d = saashq.new_doc(target_child_doctype, parent_doc=target_parent, parentfield=target_parentfield)
