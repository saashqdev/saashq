...
class TestFullTextSearch(IntegrationTestCase):
	...
	def test_search_term(self):
		...
		self.assertEqual(res[0], "sw/erpnexus")
	...
	def test_update_index(self):
		...
		self.index.update_index({"name": "sw/erpnexus", "content": """AwesomeERPNexus"""})

		res = self.index.search("CommonSearchTerm")
		self.assertTrue("sw/erpnexus" not in res)

		res = self.index.search("AwesomeERPNexus")
		self.assertEqual(res[0], "sw/erpnexus")

		# Update new doc
		self.index.update_index({"name": "sw/saashqbooks", "content": """DesktopAccounting"""})

		res = self.index.search("DesktopAccounting")
		self.assertEqual(res[0], "sw/saashqbooks")
...
def get_index():
	return TestWrapper("test_saashq_index")
...
def get_documents():
	...
	docs.append(
		{
			"name": "sw/saashq",
			"content": """Saashq Framework is a full-stack web framework, that includes everything you need to build and
