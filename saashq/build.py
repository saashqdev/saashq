...
def build_missing_files():
	...
	for type in ["css", "js"]:
		folder = os.path.join(sites_path, "assets", "saashq", "dist", type)
	...
	assets_json = saashq.read_file("assets/assets.json")
	if assets_json:
		assets_json = saashq.parse_json(assets_json)

		for bundle_file, output_file in assets_json.items():
			if not output_file.startswith("/assets/saashq"):
				continue
		...
		if missing_assets:
			...
			files_to_build = ["saashq/" + name for name in missing_assets]
			bundle(build_mode, files=files_to_build)
	else:
		# no assets.json, run full build
		bundle(build_mode, apps="saashq")


def get_assets_link(saashq_head) -> str:
	import requests

	tag = getoutput(
		r"cd ../apps/saashq && git show-ref --tags -d | grep %s | sed -e 's,.*"
		r" refs/tags/,,' -e 's/\^{}//'" % saashq_head
	)
...
def fetch_assets(url, saashq_head):
	click.secho("Retrieving assets...", fg="yellow")

	prefix = mkdtemp(prefix="saashq-assets-", suffix=saashq_head)
	assets_archive = download_file(url, prefix)

	if not assets_archive:
		raise AssetsNotDownloadedError(f"Assets could not be retrived from {url}")

	click.echo(click.style("✔", fg="green") + f" Downloaded Saashq assets from {url}")
...
def download_saashq_assets(verbose=True) -> bool:
	"""Download and set up Saashq assets if they exist based on the current commit HEAD.
	Return True if correctly setup else return False.
	"""
	saashq_head = getoutput("cd ../apps/saashq && git rev-parse HEAD")

	if not saashq_head:
		return False

	try:
		url = get_assets_link(saashq_head)
		assets_archive = fetch_assets(url, saashq_head)
...
def symlink(target, link_name, overwrite=False):
	...
	while True:
		temp_link_name = f"tmp{saashq.generate_hash()}"
...
def setup():
	...
	for app in saashq.get_all_apps(True):
		try:
			pymodules.append(saashq.get_module(app))
		except ImportError:
			pass
	app_paths = [os.path.dirname(pymodule.__file__) for pymodule in pymodules]
	assets_path = os.path.join(saashq.local.sites_path, "assets")
...
def watch(apps=None):
	...
	live_reload = saashq.utils.cint(os.environ.get("LIVE_RELOAD", saashq.conf.live_reload))
...
def generate_assets_map():
	...
	for app_name in saashq.get_all_apps():
		app_doc_path = None

		pymodule = saashq.get_module(app_name)
...
def html_to_js_template(path, content):
	"""Return HTML template content as Javascript code, by adding it to `saashq.templates`."""
	return """saashq.templates["{key}"] = '{content}';\n""".format(
