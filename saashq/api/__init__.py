...
def handle(request: Request):
	...
	if data is not None:
		saashq.response["data"] = data
	return build_response("json")


# Merge all API version routing rules
from saashq.api.v1 import url_rules as v1_rules
from saashq.api.v2 import url_rules as v2_rules
...
def get_api_version() -> ApiVersion | None:
	...
	if saashq.request.path.startswith(f"/api/{ApiVersion.V2.value}"):
