...
class ParallelTestRunner:
	def __init__(self, app, site, build_number=1, total_builds=1, dry_run=False):
		self.app = app
		self.site = site
		self.build_number = saashq.utils.cint(build_number) or 1
		self.total_builds = saashq.utils.cint(total_builds)
	...
	def before_test_setup(self):
		...
		for fn in saashq.get_hooks("before_tests", app_name=self.app):
			saashq.get_attr(fn)()

		test_module = saashq.get_module(f"{self.app}.tests")
	...
	def get_module(self, path, filename):
		app_path = saashq.get_app_path(self.app)
...
def get_all_tests(app):
	test_file_list = []
	for path, folders, files in os.walk(saashq.get_app_path(app)):
...
class ParallelTestWithOrchestrator(ParallelTestRunner):
	...
	def __init__(self, app, site):
		...
		if not self.orchestrator_url:
			...
			click.echo("Pass public URL after hosting https://github.com/saashqdev/test-orchestrator")
			sys.exit(1)

		self.ci_build_id = os.environ.get("CI_BUILD_ID")
		self.ci_instance_id = os.environ.get("CI_INSTANCE_ID") or saashq.generate_hash(length=10)
