...
import saashq
from saashq import DoesNotExistError, ValidationError, _, _dict
from saashq.boot import get_allowed_pages, get_allowed_reports
from saashq.cache_manager import (
...
class Workspace:
	def __init__(self, page, minimal=False):
		...
		self.workspace_manager = "Workspace Manager" in saashq.get_roles()

		self.user = saashq.get_user()
		self.allowed_modules = self.get_cached("user_allowed_modules", self.get_allowed_modules)

		self.doc = saashq.get_cached_doc("Workspace", self.page_name)
		if (
			self.doc
			and self.doc.module
		...
		self.restricted_doctypes = (
			saashq.cache.get_value("domain_restricted_doctypes") or build_domain_restriced_doctype_cache()
		)
		self.restricted_pages = (
			saashq.cache.get_value("domain_restricted_pages") or build_domain_restriced_page_cache()
		)
	...
	def get_cached(self, cache_key, fallback_fn):
		value = saashq.cache.get_value(cache_key, user=saashq.session.user)
		if value:
			return value
		...
		saashq.cache.set_value(cache_key, value, saashq.session.user, 21600)
		return value
	...
	def get_onboarding_doc(self, onboarding):
		...
		if saashq.db.get_value("Module Onboarding", onboarding, "is_complete"):
			return None

		doc = saashq.get_doc("Module Onboarding", onboarding)

		# Check if user is allowed
		allowed_roles = set(doc.get_allowed_roles())
		user_roles = set(saashq.get_roles())
	...
	def _doctype_contains_a_record(self, name):
		...
		if not exists and saashq.db.exists(name):
			if not saashq.db.get_value("DocType", name, "issingle"):
				exists = bool(saashq.get_all(name, limit=1))
			else:
				exists = True
	...
	def _prepare_item(self, item):
		...
		if item.get("link_type") == "DocType":
			item["description"] = saashq.get_meta(item.link_to).description
	...
	def is_custom_block_permitted(self, custom_block_name):
		...
		allowed = [
			d.role for d in saashq.get_all("Has Role", fields=["role"], filters={"parent": custom_block_name})
		]
	...
	def get_links(self):
		...
		default_country = saashq.db.get_default("country")
	...
	def get_charts(self):
		all_charts = []
		if saashq.has_permission("Dashboard Chart", throw=False):
			charts = self.doc.charts

			for chart in charts:
				if saashq.has_permission("Dashboard Chart", doc=chart.chart_name):
					# Translate label
	...
	def get_shortcuts(self):
		def _in_active_domains(item):
			if not item.restrict_to_domain:
				...
				return item.restrict_to_domain in saashq.get_active_domains()
	...
	def get_number_cards(self):
		all_number_cards = []
		if saashq.has_permission("Number Card", throw=False):
			number_cards = self.doc.number_cards
			for number_card in number_cards:
				if saashq.has_permission("Number Card", doc=number_card.number_card_name):
					# Translate label
					number_card.label = (
	...
	def get_custom_blocks(self):
		all_custom_blocks = []
		if saashq.has_permission("Custom HTML Block", throw=False):
			custom_blocks = self.doc.custom_blocks

			for custom_block in custom_blocks:
				if saashq.has_permission("Custom HTML Block", doc=custom_block.custom_block_name):
...
def get_desktop_page(page):
	...
	try:
		...
		saashq.log_error("Workspace Missing")
		return {}
...
def get_workspace_sidebar_items():
	...
	has_access = "Workspace Manager" in saashq.get_roles()

	# don't get domain restricted pages
	blocked_modules = saashq.get_cached_doc("User", saashq.session.user).get_blocked_modules()
	blocked_modules.append("Dummy Module")

	# adding None to allowed_domains to include pages without domain restriction
	allowed_domains = [None, *saashq.get_active_domains()]
	...
	try:
		workspace_visibilty = loads(
			saashq.db.get_single_value("Workspace Settings", "workspace_visibility_json") or "{}"
		)
		...
		try:
			...
			if has_access or workspace.is_permitted():
				if page.public and (has_access or not page.is_hidden) and page.title != "Welcome Workspace":
					pages.append(page)
				elif page.for_user == saashq.session.user:
			...
			if not page["app"] and page["module"]:
				page["app"] = saashq.db.get_value("Module Def", page["module"], "app_name") or get_module_app(
					page["module"]
				)
			if page["link_type"] == "Report":
				report_type, ref_doctype = saashq.db.get_value(
	...
	return {
		"workspace_setup_completed": saashq.db.get_single_value(
			"Workspace Settings", "workspace_setup_completed"
		),
		"pages": pages,
		"has_access": has_access,
		"has_create_access": saashq.has_permission(doctype="Workspace", ptype="create"),
	}


def get_table_with_counts():
	counts = saashq.cache.get_value("information_schema:counts")
	if not counts:
...
def update_onboarding_step(name, field, value):
	...
	saashq.db.set_value("Onboarding Step", name, field, value)

	capture(saashq.scrub(name), app="saashq_onboarding", properties={field: value})
