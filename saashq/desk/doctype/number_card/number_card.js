...
saashq.ui.form.on("Number Card", {
	...
	create_add_to_dashboard_button: function (frm) {
		frm.add_custom_button(__("Add Card to Dashboard"), () => {
			const dialog = saashq.dashboard_utils.get_add_to_dashboard_dialog(
				frm.doc.name,
				"Number Card",
				"saashq.desk.doctype.number_card.number_card.add_card_to_dashboard"
			);

			if (!frm.doc.name) {
				saashq.msgprint(__("Please create Card first"));
			} else {
				dialog.show();
			}
		});
	...
	filters_config: function (frm) {
		frm.filters = eval(frm.doc.filters_config);
		const filter_values = saashq.report_utils.get_filter_values(frm.filters);
	...
	set_report_filters: function (frm) {
		...
		if (report_name) {
			saashq.report_utils.get_report_filters(report_name).then((filters) => {
				if (filters) {
					frm.filters = filters;
					const filter_values = saashq.report_utils.get_filter_values(filters);
	...
	render_filters_table: function (frm) {
		...
		table.on("click", () => {
			if (!saashq.boot.developer_mode && frm.doc.is_standard) {
				saashq.throw(__("Cannot edit filters for standard number cards"));
			}
	...
	set_parent_document_type: async function (frm) {
		...
		let doc_is_table =
			document_type &&
			(await saashq.db.get_value("DocType", document_type, "istable")).message.istable;
