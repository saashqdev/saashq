...
from saashq.contacts.doctype.contact.contact import get_default_contact
...
class Event(Document):
	...
	if TYPE_CHECKING:
		from saashq.desk.doctype.event_participants.event_participants import EventParticipants
	...
	def validate(self):
		...
		if self.repeat_on == "Daily" and self.ends_on and getdate(self.starts_on) != getdate(self.ends_on):
			saashq.throw(_("Daily Events should finish on the Same Day."))

		if self.sync_with_google_calendar and not self.google_calendar:
			saashq.throw(_("Select Google Calendar to which event should be synced."))
	...
	def on_trash(self):
		...
		if communications:
			for communication in communications:
				saashq.delete_doc_if_exists("Communication", communication.name)
	...
	def sync_communication(self):
		if self.event_participants:
			for participant in self.event_participants:
				...
				if comms := saashq.get_all("Communication", filters=filters, fields=["name"], distinct=True):
					for comm in comms:
						communication = saashq.get_doc("Communication", comm.name)
						self.update_communication(participant, communication)
				else:
					meta = saashq.get_meta(participant.reference_doctype)
	...
	def update_communication(self, participant, communication):
		...
		communication.sender_full_name = saashq.utils.get_fullname(self.owner)
		communication.reference_doctype = self.doctype
		communication.reference_name = self.name
	...
	def set_participants_email(self):
		for participant in self.event_participants:
			...
			participant.email = (
				saashq.get_value("Contact", participant_contact, "email_id") if participant_contact else None
			)


@saashq.whitelist()
def delete_communication(event, reference_doctype, reference_docname):
	deleted_participant = saashq.get_doc(reference_doctype, reference_docname)
	...
	comms = saashq.get_list("Communication", filters=filters, fields=["name"])

	if comms:
		deletion = []
		for comm in comms:
			delete = saashq.get_doc("Communication", comm.name).delete()
...
def get_permission_query_conditions(user):
	...
	return f"""(`tabEvent`.`event_type`='Public' or `tabEvent`.`owner`={saashq.db.escape(user)})"""
...
def send_event_digest():
	...
	for user in users:
		...
		if events:
			saashq.set_user_lang(user.name, user.language)
			...
			saashq.sendmail(
				...
				subject=saashq._("Upcoming Events for Today"),
				template="upcoming_events",
				args={
					"events": events,
				},
				header=[saashq._("Events in Today's Calendar"), "blue"],
			)


@saashq.whitelist()
def get_events(start, end, user=None, for_reminder=False, filters=None) -> list[saashq._dict]:
	if not user:
		user = saashq.session.user
...
def delete_events(ref_type, ref_name, delete_event=False):
	...
	if participations:
		for participation in participations:
			if delete_event:
				saashq.delete_doc("Event", participation.parent, for_reload=True)
			else:
				total_participants = saashq.get_all(
					"Event Participants", filters={"parenttype": "Event", "parent": participation.parent}
				)

				if len(total_participants) <= 1:
					saashq.db.delete("Event", {"name": participation.parent})
					saashq.db.delete("Event Participants", {"name": participation.name})


# Close events if ends_on or repeat_till is less than now_datetime
def set_status_of_events():
	events = saashq.get_list("Event", filters={"status": "Open"}, fields=["name", "ends_on", "repeat_till"])
	for event in events:
		if (event.ends_on and getdate(event.ends_on) < getdate(nowdate())) or (
			...
			saashq.db.set_value("Event", event.name, "status", "Closed")
