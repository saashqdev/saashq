...
from saashq.desk.doctype.bulk_update.bulk_update import submit_cancel_or_update_docs
...
class TestBulkUpdate(IntegrationTestCase):
	...
	def setUpClass(cls) -> None:
		...
		saashq.db.commit()
		for _ in range(50):
			saashq.new_doc(cls.doctype, some_fieldname=saashq.mock("name")).insert()
	...
	def test_bulk_submit_in_background(self):
		...
		submitted = saashq.get_all(self.doctype, {"docstatus": 1}, limit=20, pluck="name")
