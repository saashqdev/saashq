...
import saashq
from saashq.desk.doctype.dashboard_chart.dashboard_chart import get
from saashq.tests import IntegrationTestCase, UnitTestCase
from saashq.utils import formatdate, get_last_day, getdate
from saashq.utils.dateutils import get_period, get_period_ending
...
class TestDashboardChart(IntegrationTestCase):
	...
	def test_dashboard_chart(self):
		if saashq.db.exists("Dashboard Chart", "Test Dashboard Chart"):
			saashq.delete_doc("Dashboard Chart", "Test Dashboard Chart")
	...
	def test_empty_dashboard_chart(self):
		if saashq.db.exists("Dashboard Chart", "Test Empty Dashboard Chart"):
			saashq.delete_doc("Dashboard Chart", "Test Empty Dashboard Chart")

		saashq.db.delete("Error Log")
	...
	def test_chart_wih_one_value(self):
		if saashq.db.exists("Dashboard Chart", "Test Empty Dashboard Chart 2"):
			saashq.delete_doc("Dashboard Chart", "Test Empty Dashboard Chart 2")

		saashq.db.delete("Error Log")

		# create one data point
		saashq.get_doc(doctype="Error Log", creation="2018-06-01 00:00:00").insert()
	...
	def test_group_by_chart_type(self):
		if saashq.db.exists("Dashboard Chart", "Test Group By Dashboard Chart"):
			saashq.delete_doc("Dashboard Chart", "Test Group By Dashboard Chart")

		saashq.get_doc({"doctype": "ToDo", "description": "test"}).insert()
		...
		todo_status_count = saashq.db.count("ToDo", {"status": result.get("labels")[0]})
	...
	def test_daily_dashboard_chart(self):
		...
		if saashq.db.exists("Dashboard Chart", "Test Daily Dashboard Chart"):
			saashq.delete_doc("Dashboard Chart", "Test Daily Dashboard Chart")
	...
	def test_weekly_dashboard_chart(self):
		...
		if saashq.db.exists("Dashboard Chart", "Test Weekly Dashboard Chart"):
			saashq.delete_doc("Dashboard Chart", "Test Weekly Dashboard Chart")
	...
	def test_avg_dashboard_chart(self):
		...
		if saashq.db.exists("Dashboard Chart", "Test Average Dashboard Chart"):
			saashq.delete_doc("Dashboard Chart", "Test Average Dashboard Chart")
	...
	def test_user_date_label_dashboard_chart(self):
		saashq.delete_doc_if_exists("Dashboard Chart", "Test Dashboard Chart Date Label")
		...
		with patch.object(saashq.utils.data, "get_user_date_format", return_value="dd.mm.yyyy"):
		...
		with patch.object(saashq.utils.data, "get_user_date_format", return_value="mm-dd-yyyy"):
			result = get(chart_name="Test Dashboard Chart Date Label")
...
def create_new_communication(subject, date, rating):
	...
	comm = saashq.get_doc(communication)
	if not saashq.db.exists("Communication", {"subject": comm.subject}):
		comm.insert()
