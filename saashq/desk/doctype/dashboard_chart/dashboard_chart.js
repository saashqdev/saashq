...
saashq.ui.form.on("Dashboard Chart", {
	setup: function (frm) {
	...
	before_save: function (frm) {
		...
		static_filters = saashq.dashboard_utils.remove_common_static_filter_values(
			static_filters,
			dynamic_filters
		);
	...
	refresh: function (frm) {
		...
		frm.is_disabled = !saashq.boot.developer_mode && frm.doc.is_standard;
		...
		if (!frm.is_new()) {
			frm.add_custom_button("Add Chart to Dashboard", () => {
				const dialog = saashq.dashboard_utils.get_add_to_dashboard_dialog(
					frm.doc.name,
					"Dashboard Chart",
					"saashq.desk.doctype.dashboard_chart.dashboard_chart.add_chart_to_dashboard"
				);

				if (!frm.doc.chart_name) {
					saashq.msgprint(__("Please create chart first"));
				} else {
					dialog.show();
				}
			});
	...
	set_heatmap_year_options: function (frm) {
		if (frm.doc.type == "Heatmap") {
			saashq.db.get_doc("System Settings").then((doc) => {
				const creation_date = doc.creation;
				frm.set_df_property(
					"heatmap_year",
					"options",
					saashq.dashboard_utils.get_years_since_creation(creation_date)
				);
			});
		}
	},
	...
	set_chart_report_filters: function (frm) {
		...
		if (report_name) {
			if (frm.doc.filters_json.length > 2) {
				...
				saashq.report_utils.get_report_filters(report_name).then((filters) => {
					if (filters) {
						frm.chart_filters = filters;
						let filter_values = saashq.report_utils.get_filter_values(filters);
	...
	set_chart_field_options: function (frm) {
		...
		if (frm.doc.dynamic_filters_json && frm.doc.dynamic_filters_json.length > 2) {
			filters = saashq.dashboard_utils.get_all_filters(frm.doc);
		}
		saashq
			.xcall("saashq.desk.query_report.run", {
			...
			.then((data) => {
				...
				if (!frm.doc.use_report_chart) {
					if (data.result.length) {
						frm.field_options = saashq.report_utils.get_field_options_from_report(
							data.columns,
							data
						);
						frm.set_df_property(
						...
						if (!frm.field_options.numeric_fields.length) {
							...
							let y_field_df = saashq.meta.get_docfield(
	...
	show_filters: function (frm) {
		frm.chart_filters = [];
		saashq.dashboard_utils.get_filters_for_chart_type(frm.doc).then((filters) => {
			if (filters) {
				frm.chart_filters = filters;
			}
	...
	render_filters_table: function (frm) {
		...
		table.on("click", () => {
			frm.is_disabled && saashq.throw(__("Cannot edit filters for standard charts"));
			...
			saashq.dashboards.filters_dialog = dialog;

			if (is_document_type) {
				frm.filter_group = new saashq.ui.FilterGroup({
			...
			if (frm.doc.chart_type == "Report") {
				...
				saashq.query_reports[frm.doc.report_name] &&
					saashq.query_reports[frm.doc.report_name].onload &&
					saashq.query_reports[frm.doc.report_name].onload(saashq.query_report);
			}
	...
	render_dynamic_filters_table(frm) {
		...
		let fields = saashq.dashboard_utils.get_fields_for_dynamic_filter_dialog(
			is_document_type,
			filters,
			frm.dynamic_filters
		);
	...
	set_parent_document_type: async function (frm) {
		...
		saashq.model.with_doctype(document_type, async () => {
			let doc_is_table = saashq.get_meta(document_type).istable;
			frm.set_df_property("parent_document_type", "hidden", !doc_is_table);

			if (doc_is_table) {
				let parents = await saashq.xcall(
					"saashq.desk.doctype.dashboard_chart.dashboard_chart.get_parent_doctypes",
