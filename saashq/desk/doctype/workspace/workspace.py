...
from saashq.desk.desktop import get_workspace_sidebar_items, save_new_widget
from saashq.desk.utils import validate_route_conflict
from saashq.model.document import Document
from saashq.model.rename_doc import rename_doc
from saashq.modules.export_file import delete_folder, export_to_files
...
class Workspace(Document):
	...
	if TYPE_CHECKING:
		...
		from saashq.desk.doctype.workspace_chart.workspace_chart import WorkspaceChart
		from saashq.desk.doctype.workspace_custom_block.workspace_custom_block import WorkspaceCustomBlock
		from saashq.desk.doctype.workspace_link.workspace_link import WorkspaceLink
		from saashq.desk.doctype.workspace_number_card.workspace_number_card import WorkspaceNumberCard
		from saashq.desk.doctype.workspace_quick_list.workspace_quick_list import WorkspaceQuickList
		from saashq.desk.doctype.workspace_shortcut.workspace_shortcut import WorkspaceShortcut
	...
	def validate(self):
		...
		if self.public and not is_workspace_manager() and not disable_saving_as_public():
			saashq.throw(_("You need to be Workspace Manager to edit this document"))
		...
		try:
			...
			saashq.throw(_("Content data shoud be a list"))

		for d in self.get("links"):
			if d.link_type == "Report" and d.is_query_report != 1:
				d.report_ref_doctype = saashq.get_value("Report", d.link_to, "ref_doctype")
	...
	def clear_cache(self):
		super().clear_cache()
		if self.for_user:
			saashq.cache.hdel("bootinfo", self.for_user)
		else:
	...
	def on_update(self):
		...
		if saashq.conf.developer_mode and self.public:
			if self.module:
		...
		for link in self.links:
			...
			if link.type == "Card Break":
				if card_links and (
					...
					or current_card.get("only_for") == saashq.get_system_settings("country")
				):
				...
				current_card = link
				card_links = []
			elif not link.get("only_for") or link.get("only_for") == saashq.get_system_settings("country"):
...
def get_report_type(report):
	report_type = saashq.get_value("Report", report, "report_type")
...
def new_page(new_page):
	...
	if page.get("public") and not is_workspace_manager():
		...
		not page.get("public") and page.get("for_user") != saashq.session.user and not is_workspace_manager()
	):
		saashq.throw(_("Cannot create private workspace of other users"), saashq.PermissionError)

	elif not saashq.has_permission(doctype="Workspace", ptype="create"):
		saashq.flags.error_message = _("User {0} does not have the permission to create a Workspace.").format(
			saashq.bold(saashq.session.user)
		)
		raise saashq.PermissionError

	doc = saashq.new_doc("Workspace")
...
def save_page(name, public, new_widgets, blocks):
	public = saashq.parse_json(public)

	doc = saashq.get_doc("Workspace", name)
	doc.content = blocks
...
def update_page(name, title, icon, indicator_color, parent, public):
	...
	if not doc.get("public") and doc.get("for_user") != saashq.session.user and not is_workspace_manager():
		saashq.throw(
	...
	if doc:
		child_docs = saashq.get_all("Workspace", filters={"parent_page": doc.title, "public": doc.public})
		doc.title = title
		doc.icon = icon
		doc.indicator_color = indicator_color
		doc.parent_page = parent
		if doc.public != public:
			doc.sequence_id = saashq.db.count("Workspace", {"public": public}, cache=True)
			doc.public = public
		doc.for_user = "" if public else doc.for_user or saashq.session.user
		...
		if child_docs:
			for child in child_docs:
				child_doc = saashq.get_doc("Workspace", child.name)
				child_doc.parent_page = doc.title
				if child_doc.public != public:
					child_doc.public = public
				child_doc.for_user = "" if public else child_doc.for_user or saashq.session.user
...
def last_sequence_id(doc):
	doc_exists = saashq.db.exists({"doctype": "Workspace", "public": doc.public, "for_user": doc.for_user})
...
def get_page_list(fields, filters):
	return saashq.get_all("Workspace", fields=fields, filters=filters, order_by="sequence_id asc")


def is_workspace_manager():
	return "Workspace Manager" in saashq.get_roles()
