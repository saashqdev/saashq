...
class Dashboard(Document):
	...
	if TYPE_CHECKING:
		from saashq.desk.doctype.dashboard_chart_link.dashboard_chart_link import DashboardChartLink
		from saashq.desk.doctype.number_card_link.number_card_link import NumberCardLink
	...
	def on_update(self):
		if self.is_default:
			...
			saashq.qb.update(DashBoard).set(DashBoard.is_default, 0).where(DashBoard.name != self.name).run()
	...
	def validate(self):
		if not saashq.conf.developer_mode and self.is_standard:
			saashq.throw(_("Cannot edit Standard Dashboards"))
		...
		if self.is_standard:
			...
			if non_standard_docs_map["Dashboard Chart"] or non_standard_docs_map["Number Card"]:
				...
				saashq.throw(message, title=_("Standard Not Set"), is_minimizable=True)
...
def get_permission_query_conditions(user):
	...
	if user == "Administrator" or "System Manager" in saashq.get_roles(user):
		return
...
def get_permitted_charts(dashboard_name):
	permitted_charts = []
	dashboard = saashq.get_doc("Dashboard", dashboard_name)
	for chart in dashboard.charts:
		if saashq.has_permission("Dashboard Chart", doc=chart.chart):
			chart_dict = saashq._dict()
...
def get_permitted_cards(dashboard_name):
	dashboard = saashq.get_doc("Dashboard", dashboard_name)
	return [card for card in dashboard.cards if saashq.has_permission("Number Card", doc=card.card)]


def get_non_standard_charts_in_dashboard(dashboard):
	non_standard_charts = [doc.name for doc in saashq.get_list("Dashboard Chart", {"is_standard": 0})]
...
def get_non_standard_cards_in_dashboard(dashboard):
	non_standard_cards = [doc.name for doc in saashq.get_list("Number Card", {"is_standard": 0})]
...
def get_non_standard_warning_message(non_standard_docs_map):
	...
	def get_html(docs, doctype):
		html = f"<p>{saashq.bold(doctype)}</p>"
		for doc in docs:
