...
import saashq
from saashq.core.doctype.scheduled_job_type.scheduled_job_type import ScheduledJobType
from saashq.model.document import Document
from saashq.utils.background_jobs import get_queue, get_queue_list, get_redis_conn
from saashq.utils.caching import redis_cache
from saashq.utils.data import add_to_date
from saashq.utils.scheduler import get_scheduler_status, get_scheduler_tick
	...
	def suppress_exception(func: Callable):
		...
		def wrapper(*args, **kwargs):
			try:
				...
				saashq.log(saashq.get_traceback())
				# nosemgrep
				saashq.msgprint(
					f"System Health check step {saashq.bold(step)} failed: {e}", alert=True, indicator="red"
				)
...
class SystemHealthReport(Document):
	...
	if TYPE_CHECKING:
		from saashq.desk.doctype.system_health_report_errors.system_health_report_errors import (
			SystemHealthReportErrors,
		)
		from saashq.desk.doctype.system_health_report_failing_jobs.system_health_report_failing_jobs import (
			SystemHealthReportFailingJobs,
		)
		from saashq.desk.doctype.system_health_report_queue.system_health_report_queue import (
			SystemHealthReportQueue,
		)
		from saashq.desk.doctype.system_health_report_tables.system_health_report_tables import (
			SystemHealthReportTables,
		)
		from saashq.desk.doctype.system_health_report_workers.system_health_report_workers import (
	...
	def fetch_background_jobs(self):
		...
		self.test_job_id = saashq.enqueue("saashq.ping", at_front=True).id
		self.background_jobs_check = "queued"
		self.scheduler_status = get_scheduler_status().get("status")
		workers = saashq.get_all("RQ Worker")
	...
	def fetch_scheduler(self):
		...
		failing_jobs = saashq.db.multisql(
			{
		...
		for job_type in saashq.get_all(
			"Scheduled Job Type",
			filters={"stopped": 0, "last_execution": ("<", threshold)},
			fields="*",
			order_by="last_execution asc",
		):
			job_type: ScheduledJobType = saashq.get_doc(doctype="Scheduled Job Type", **job_type)
	...
	def fetch_email_stats(self):
		...
		self.total_outgoing_emails = saashq.db.count("Email Queue", filters)
		self.pending_emails = saashq.db.count("Email Queue", {"status": "Not Sent", **filters})
		self.failed_emails = saashq.db.count("Email Queue", {"status": "Error", **filters})
		self.unhandled_emails = saashq.db.count("Unhandled Email", filters)
		self.handled_emails = saashq.db.count(
			"Communication",
	...
	def fetch_errors(self):
		...
		filters = {"creation": (">", threshold), "modified": (">", threshold)}
		self.total_errors = saashq.db.count("Error Log", filters)

		top_errors = saashq.db.sql(
	...
	def fetch_database_details(self):
		...
		self.database = saashq.db.db_type
		self.db_storage_usage = sum(table.size for table in data)
		for row in data[:5]:
			self.append("top_db_tables", row)
		self.database_version = saashq.db.sql("select version()")[0][0]

		if saashq.db.db_type == "mariadb":
			self.bufferpool_size = saashq.db.sql("show variables like 'innodb_buffer_pool_size'")[0][1]
			self.binary_logging = saashq.db.sql("show variables like 'log_bin'")[0][1]

	@health_check("Cache")
	def fetch_cache_details(self):
		self.cache_keys = len(saashq.cache.get_keys(""))
		self.cache_memory_usage = saashq.cache.execute_command("INFO", "MEMORY").get("used_memory_human")

	@health_check("Storage")
	def fetch_storage_details(self):
		from saashq.desk.page.backups.backups import get_context
	...
	def fetch_user_stats(self):
		...
		self.total_users = saashq.db.count("User", {"enabled": 1})
		self.new_users = saashq.db.count("User", {"enabled": 1, "creation": (">", threshold)})
		self.failed_logins = saashq.db.count(
			"Activity Log",
			{
				"operation": "login",
				"status": "Failed",
		...
		self.active_sessions = saashq.db.count("Sessions")
...
def get_job_status(job_id: str | None = None):
	saashq.only_for("System Manager")
	try:
		return saashq.get_doc("RQ Job", job_id).status
	except Exception:
...
def _get_directory_size(*path):
	folder = os.path.abspath(saashq.get_site_path(*path))
