...
saashq.ui.form.on("System Health Report", {
	onload(frm) {
		let poll_attempts = 0;
		const interval = setInterval(() => {
			saashq
				.xcall(
					"saashq.desk.doctype.system_health_report.system_health_report.get_job_status",
	...
	refresh(frm) {
		...
		saashq.realtime.on("pong", () => {
			frm.set_value("socketio_ping_check", "Pass");
			frm.set_value(
				"socketio_transport_mode",
				saashq.realtime.socket.io?.engine?.transport?.name
			);
		});
		saashq.realtime.emit("ping");
		frm.disable_save();
	...
	setup_highlight(frm) {
		...
		const update_fields = () => {
			if (!saashq.get_route().includes(frm.doc.name)) {
				clearInterval(interval);
			}
