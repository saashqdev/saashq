...
from saashq.desk.form.document_follow import is_document_followed
from saashq.model.utils.user_settings import get_user_settings
from saashq.permissions import get_doc_permissions, has_permission
...
def getdoc(doctype, name, user=None):
	...
	if not doc.has_permission("read"):
		saashq.flags.error_message = _("Insufficient Permission for {0}").format(
			saashq.bold(_(doctype) + " " + name)
		)
		raise saashq.PermissionError(("read", doctype, name))
	...
	doc.add_seen()
	set_link_titles(doc)
	if saashq.response.docs is None:
		saashq.local.response = _dict({"docs": []})
...
def getdoctype(doctype, with_parent=False, cached_timestamp=None):
	...
	if with_parent and (parent_dt := saashq.model.meta.get_parent_dt(doctype)):
		docs = get_meta_bundle(parent_dt)
		saashq.response["parent_dt"] = parent_dt

	if not docs:
		docs = get_meta_bundle(doctype)

	saashq.response["user_settings"] = get_user_settings(parent_dt or doctype)

	if cached_timestamp and docs[0].modified == cached_timestamp:
		return "use_cache"

	saashq.response.docs.extend(docs)


def get_meta_bundle(doctype):
	bundle = [saashq.desk.form.meta.get_meta(doctype)]
	bundle.extend(
		saashq.desk.form.meta.get_meta(df.options)
		for df in bundle[0].fields
		if df.fieldtype in saashq.model.table_fields
	)
	return bundle
...
def get_docinfo(doc=None, doctype=None, name=None):
	from saashq.share import _get_users as get_docshares
	...
	docinfo = saashq._dict(user_info={})
	...
	docinfo.update(
		{
			...
			"is_document_followed": is_document_followed(doc.doctype, doc.name, saashq.session.user),
	...
	saashq.response["docinfo"] = docinfo
...
def add_comments(doc, docinfo):
	...
	for c in comments:
		match c.comment_type:
			case "Comment":
				c.content = saashq.utils.markdown(c.content)
...
def get_error_log_exists(doc: "Document") -> bool:
	if has_permission("Error Log", print_logs=False):
		return saashq.db.exists("Error Log", {"reference_doctype": doc.doctype, "reference_name": doc.name})
	return False


def get_webhook_request_log_exists(doc: "Document") -> bool:
	if has_permission("Webhook Request Log", print_logs=False):
		return saashq.db.exists(
...
def get_comments(doctype: str, name: str, comment_type: str | list[str] = "Comment") -> list[saashq._dict]:
...
def get_additional_timeline_content(doctype, docname):
	contents = []
	hooks = saashq.get_hooks().get("additional_timeline_content", {})
	...
	for method in methods_for_all_doctype + methods_for_current_doctype:
		contents.extend(saashq.get_attr(method)(doctype, docname) or [])
...
def get_title_values_for_link_and_dynamic_link_fields(doc, link_fields=None):
	...
	for field in link_fields:
		...
		link_title = saashq.db.get_value(doctype, doc_fieldvalue, meta.title_field, cache=True, order_by=None)
...
def send_link_titles(link_titles):
	"""Append link titles dict in `saashq.local.response`."""
	if "_link_titles" not in saashq.local.response:
		saashq.local.response["_link_titles"] = {}

	saashq.local.response["_link_titles"].update(link_titles)
...
def update_user_info(docinfo):
	...
	saashq.utils.add_user_info(users, docinfo.user_info)
