...
def update_follow(doctype: str, doc_name: str, following: bool):
	if following:
		return follow_document(doctype, doc_name, saashq.session.user) and True or False
	else:
		return unfollow_document(doctype, doc_name, saashq.session.user)
...
def follow_document(doctype, doc_name, user):
	...
	if not saashq.get_meta(doctype).track_changes:
		saashq.toast(_("Can't follow since changes are not tracked."))
		return False

	if user == "Administrator":
		saashq.toast(_("Administrator can't follow"))
		return False

	if not saashq.db.get_value("User", user, "document_follow_notify", ignore=True, cache=True):
		saashq.toast(_("Document follow is not enabled for this user."))
		return False

	if not is_document_followed(doctype, doc_name, user):
		doc = saashq.new_doc("Document Follow")
		doc.update({"ref_doctype": doctype, "ref_docname": doc_name, "user": user})
		doc.save()
		saashq.toast(_("Following document {0}").format(doc_name))
		return doc
...
def unfollow_document(doctype, doc_name, user):
	...
	if doc:
		saashq.delete_doc("Document Follow", doc[0].name)
		saashq.toast(_("Un-following document {0}").format(doc_name))
		return False
	return False
...
def get_version(doctype, doc_name, frequency, user):
	...
	if version:
		for v in version:
			change = saashq.parse_json(v.data)
			time = saashq.utils.format_datetime(v.modified, "hh:mm a")
			timeline_items = []
			if change.changed:
...
def get_comments(doctype, doc_name, frequency, user):
	...
	for comment in comments:
		...
		time = saashq.utils.format_datetime(comment.modified, "hh:mm a")
		timeline.append(
			{
...
def _get_filters(frequency, user):
	...
	if frequency == "Weekly":
		filters += [
			["modified", ">", saashq.utils.add_days(saashq.utils.nowdate(), -7)],
			["modified", "<", saashq.utils.nowdate()],
		]

	elif frequency == "Daily":
		filters += [
			["modified", ">", saashq.utils.add_days(saashq.utils.nowdate(), -1)],
			["modified", "<", saashq.utils.nowdate()],
		]

	elif frequency == "Hourly":
		filters += [
			["modified", ">", saashq.utils.add_to_date(saashq.utils.now_datetime(), hours=-1)],
			["modified", "<", saashq.utils.now_datetime()],
		]
