import BaseTimeline from "../../../public/js/saashq/form/footer/base_timeline";
...
class UserProfile {
	...
	show() {
		...
		this.user_id = route[1] || saashq.session.user;
		saashq.dom.freeze(__("Loading user profile") + "...");
		saashq.db.exists("User", this.user_id).then((exists) => {
			saashq.dom.unfreeze();
			if (exists) {
				this.make_user_profile();
			} else {
				saashq.msgprint(__("User does not exist"));
			}
		});
	}

	make_user_profile() {
		this.user = saashq.user_info(this.user_id);
		this.page.set_title(this.user.fullname);
		this.setup_user_search();
		this.main_section.empty().append(saashq.render_template("user_profile"));
	...
	show_user_search_dialog() {
		let dialog = new saashq.ui.Dialog({
			...
			primary_action: ({ user }) => {
				dialog.hide();
				saashq.set_route("user-profile", user);
			},
		});
		dialog.show();
	}
	...
	render_heatmap() {
		this.heatmap = new saashq.Chart(".performance-heatmap", {
			type: "heatmap",
	...
	update_heatmap_data(date_from) {
		saashq
			.xcall("saashq.desk.page.user_profile.user_profile.get_energy_points_heatmap_data", {
				user: this.user_id,
				date: date_from || saashq.datetime.year_start(),
			})
			.then((r) => {
	...
	render_line_chart() {
		...
		this.line_chart = new saashq.Chart(".performance-line-chart", {
			type: "line",
			height: 200,
			data: {
				labels: [],
	...
	update_line_chart_data() {
		...
		saashq
			.xcall("saashq.desk.doctype.dashboard_chart.dashboard_chart.get", {
	...
	render_percentage_chart(field, title) {
		saashq
			.xcall(
				"saashq.desk.page.user_profile.user_profile.get_energy_points_percentage_chart_data",
				{
					user: this.user_id,
					field: field,
				}
			...
			.then((chart) => {
				if (chart.labels.length) {
					this.percentage_chart = new saashq.Chart(".performance-percentage-chart", {
						type: "percentage",
						data: {
							labels: chart.labels,
	...
	create_line_chart_filters() {
		...
		saashq.dashboard_utils.render_chart_filters(
			filters,
			"chart-filter",
			".line-chart-options",
			1
	...
	create_heatmap_chart_filters() {
		let filters = [
			{
				label: saashq.dashboard_utils.get_year(saashq.datetime.now_date()),
				options: saashq.dashboard_utils.get_years_since_creation(
					saashq.boot.user.creation
				),
				action: (selected_item) => {
					this.update_heatmap_data(saashq.datetime.obj_to_str(selected_item));
				},
			},
		];
		saashq.dashboard_utils.render_chart_filters(filters, "chart-filter", ".heatmap-options");
	}

	edit_profile() {
		let edit_profile_dialog = new saashq.ui.Dialog({
			...
			primary_action: (values) => {
				...
				saashq
					.xcall("saashq.desk.page.user_profile.user_profile.update_profile_info", {
						profile_info: values,
					})
					.then((user) => {
	...
	render_user_details() {
		this.sidebar.empty().append(
			saashq.render_template("user_profile_sidebar", {
				user_image: this.user.image,
	...
	setup_user_profile_links() {
		if (this.user_id !== saashq.session.user) {
	...
	get_user_rank() {
		return saashq
			.xcall("saashq.desk.page.user_profile.user_profile.get_user_rank", {
				user: this.user_id,
			})
			.then((r) => {
	...
	get_user_points() {
		return saashq
			.xcall(
				"saashq.social.doctype.energy_point_log.energy_point_log.get_user_energy_and_review_points",
				{
					user: this.user_id,
				}
			)
	...
	render_points_and_rank() {
		...
		const _get_stat_dom = (value, label, icon) => {
			return `<div class="user-stats-item mt-4">
				${saashq.utils.icon(icon, "lg", "no-stroke")}
				<div>
	...
	go_to_user_settings() {
		saashq.set_route("Form", "User", this.user_id);
	}
...
class UserProfileTimeline extends BaseTimeline {
	...
	get_user_activity_data() {
		return saashq.xcall("saashq.desk.page.user_profile.user_profile.get_energy_points_list", {
	...
	get_activity_timeline_item(data) {
		...
		return {
			...
			is_card: true,
			content: saashq.energy_points.format_history_log(data),
		};
	}
...
saashq.ui.UserProfile = UserProfile;
