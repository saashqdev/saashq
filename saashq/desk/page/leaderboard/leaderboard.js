saashq.pages["leaderboard"].on_page_load = (wrapper) => {
	saashq.leaderboard = new Leaderboard(wrapper);

	$(wrapper).bind("show", () => {
		// Get which leaderboard to show
		let doctype = saashq.get_route()[1];
		saashq.leaderboard.show_leaderboard(doctype);
	});
};
...
class Leaderboard {
	...
	get_leaderboard_config() {
		...
		saashq
			.xcall("saashq.desk.page.leaderboard.leaderboard.get_leaderboard_config")
			.then((config) => {
				...
				const _initial_doctype = saashq.get_route()[1] || this.doctypes[0];
	...
	setup_leaderboard_fields() {
		...
		this.type_select = this.page.add_select(
			...
			this.options.selected_filter.map((d) => {
				return { label: __(saashq.model.unscrub(d)), value: d };
			})
		);
	...
	create_date_range_field() {
		let timespan_field = $(this.parent).find(
			`.saashq-control[data-original-title="${__("Timespan")}"]`
		);
		this.date_range_field = $(`<div class="from-date-field"></div>`)
			.insertAfter(timespan_field)
			.hide();

		let date_field = saashq.ui.form.make_control({
			df: {
				fieldtype: "DateRange",
				fieldname: "selected_date_range",
				placeholder: __("Date Range"),
				default: [saashq.datetime.month_start(), saashq.datetime.now_date()],
	...
	render_selected_doctype() {
		this.$sidebar_list.on("click", "li", (e) => {
			...
			this.company_select.set_value(
				saashq.defaults.get_default("company") || this.company_select.get_value()
			);
			...
			saashq.set_route("leaderboard", this.options.selected_doctype);
			this.make_request();
		});
	}
	...
	make_request() {
		saashq.model.with_doctype(this.options.selected_doctype, () => {
	...
	get_leaderboard(notify) {
		...
		if (!company && !this.leaderboard_config[this.options.selected_doctype].company_disabled) {
			notify(this, null);
			saashq.show_alert(__("Please select Company"));
			return;
		}
		saashq
			...
			.then((r) => {
				...
				saashq.utils.make_chart(".leaderboard-graph", custom_options);
	...
	get_leaderboard_data(me, res) {
		if (res && res.message.length) {
			...
			saashq.utils.setup_search($(me.parent), ".list-item-container", ".list-id");
		} else {
			me.$graph_area.hide();
	...
	render_list_header() {
		const _selected_filter = this.options.selected_filter.map((i) => saashq.model.unscrub(i));
		const fields = ["rank", "name", this.options.selected_filter_item];
		const filters = fields
			.map((filter) => {
				const col = __(saashq.model.unscrub(filter));
	...
	render_message() {
		...
		return `<div class="leaderboard-empty-state ${display_class}">
  			<div class="no-result text-center">
  				<img src="/assets/saashq/images/ui-states/search-empty-state.svg"
  					alt="Empty State"
  					class="null-state"
  				>
	...
	get_item_html(item, index) {
		...
		const link = `/app/${saashq.router.slug(this.options.selected_doctype)}/${item.name}`;
	...
	get_sidebar_item(item, icon) {
		let icon_html = icon ? saashq.utils.icon(icon, "md") : "";
	...
	get_date_range() {
		...
		let current_date = saashq.datetime.now_date();
		let date_range_map = {
			"this week": [saashq.datetime.week_start(), saashq.datetime.week_end()],
			"this month": [saashq.datetime.month_start(), saashq.datetime.month_end()],
			"this quarter": [saashq.datetime.quarter_start(), saashq.datetime.quarter_end()],
			"this year": [saashq.datetime.year_start(), saashq.datetime.year_end()],
			"last week": [saashq.datetime.add_days(current_date, -7), current_date],
			"last month": [saashq.datetime.add_months(current_date, -1), current_date],
			"last quarter": [saashq.datetime.add_months(current_date, -3), current_date],
			"last year": [saashq.datetime.add_months(current_date, -12), current_date],
			"all time": null,
			"select date range": this.selected_date_range || [
				saashq.datetime.month_start(),
				current_date,
			],
		};
