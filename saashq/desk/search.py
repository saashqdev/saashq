...
from saashq import _, is_whitelisted, validate_and_sanitize_search_inputs
from saashq.database.schema import SPECIAL_CHAR_PATTERN
from saashq.model.db_query import get_order_by
from saashq.permissions import has_permission
from saashq.utils import cint, cstr, unique
from saashq.utils.data import make_filter_tuple
...
def sanitize_searchfield(searchfield: str):
	...
	if SPECIAL_CHAR_PATTERN.search(searchfield):
		saashq.throw(_("Invalid Search Field {0}").format(searchfield), saashq.DataError)
...
def search_widget(
	...
	standard_queries = saashq.get_hooks().standard_queries or {}
	...
	if query:  # Query = custom search query i.e. python function
		try:
			is_whitelisted(saashq.get_attr(query))
			return saashq.call(
				query,
				doctype,
				txt,
		...
		except (saashq.PermissionError, saashq.AppNotInstalledError, ImportError):
			if saashq.local.conf.developer_mode:
				raise
			else:
	...
	if not meta.translated_doctype:
		_txt = saashq.db.escape((txt or "").replace("%", "").replace("@", ""))
...
def get_names_for_mentions(search_term):
	users_for_mentions = saashq.cache.get_value("users_for_mentions", get_users_for_mentions)
	user_groups = saashq.cache.get_value("user_groups", get_user_groups)
	...
	for mention_data in users_for_mentions + user_groups:
		...
		mention_data["link"] = saashq.utils.get_url_to_form(
...
def get_user_groups():
	return saashq.get_all("User Group", fields=["name as id", "name as value"], update={"is_group": True})
...
def get_link_title(doctype, docname):
	...
	if meta.show_title_field_in_link:
		return saashq.db.get_value(doctype, docname, meta.title_field)
