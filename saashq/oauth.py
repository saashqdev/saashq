...
from saashq.utils.data import cstr, get_system_timezone, now_datetime
...
class OAuthWebRequestValidator(RequestValidator):
	...
	def validate_client_id(self, client_id, request, *args, **kwargs):
		...
		cli_id = saashq.db.get_value("OAuth Client", {"name": client_id})
		if cli_id:
			client = saashq.get_doc("OAuth Client", client_id)
	...
	def validate_redirect_uri(self, client_id, redirect_uri, request, *args, **kwargs):
		...
		redirect_uris = (
			cstr(saashq.db.get_value("OAuth Client", client_id, "redirect_uris"))
			.strip()
	...
	def get_default_redirect_uri(self, client_id, request, *args, **kwargs):
		...
		return saashq.db.get_value("OAuth Client", client_id, "default_redirect_uri")
	...
	def save_authorization_code(self, client_id, code, request, *args, **kwargs):
		...
		oac = saashq.new_doc("OAuth Authorization Code")
	...
	def authenticate_client(self, request, *args, **kwargs):
		...
		if request.client_id:
			oc = saashq.get_doc("OAuth Client", request.client_id)
		else:
			# Extract token, instantiate OAuth Bearer Token and use clientid from there.
			if "refresh_token" in saashq.form_dict:
				oc = saashq.get_doc(
					"OAuth Client",
					saashq.db.get_value(
						"OAuth Bearer Token",
						{"refresh_token": saashq.form_dict["refresh_token"]},
						"client",
					),
				)
			elif "token" in saashq.form_dict:
				oc = saashq.get_doc(
					"OAuth Client",
					saashq.db.get_value("OAuth Bearer Token", saashq.form_dict["token"], "client"),
				)
			else:
				oc = saashq.get_doc(
					"OAuth Client",
					saashq.db.get_value(
						"OAuth Bearer Token",
						saashq.get_request_header("Authorization").split(" ")[1],
						"client",
					),
				)
		try:
		...
		return saashq.session.user == user_id

	def authenticate_client_id(self, client_id, request, *args, **kwargs):
		cli_id = saashq.db.get_value("OAuth Client", client_id, "name")
		if not cli_id:
			# Don't allow public (non-authenticated) clients
			return False
		else:
			request["client"] = saashq.get_doc("OAuth Client", cli_id)
			return True
	...
	def validate_code(self, client_id, code, client, request, *args, **kwargs):
		...
		validcodes = saashq.get_all(
			"OAuth Authorization Code",
			filters={"client": client_id, "validity": "Valid"},
		)

		if code in [vcode["name"] for vcode in validcodes]:
			request.scopes = saashq.db.get_value("OAuth Authorization Code", code, "scopes").split(
				get_url_delimiter()
			)
			request.user = saashq.db.get_value("OAuth Authorization Code", code, "user")
			code_challenge_method = saashq.db.get_value(
				"OAuth Authorization Code", code, "code_challenge_method"
			)
			code_challenge = saashq.db.get_value("OAuth Authorization Code", code, "code_challenge")

			if code_challenge and not request.code_verifier:
				if saashq.db.exists("OAuth Authorization Code", code):
					saashq.delete_doc("OAuth Authorization Code", code, ignore_permissions=True)
					saashq.db.commit()
				return False
	...
	def confirm_redirect_uri(self, client_id, code, redirect_uri, client, *args, **kwargs):
		saved_redirect_uri = saashq.db.get_value("OAuth Client", client_id, "default_redirect_uri")

		redirect_uris = saashq.db.get_value("OAuth Client", client_id, "redirect_uris")
	...
	def save_bearer_token(self, token, request, *args, **kwargs):
		...
		otoken = saashq.new_doc("OAuth Bearer Token")
		otoken.client = request.client["name"]
		try:
			otoken.user = (
				request.user
				if request.user
				else saashq.db.get_value(
					"OAuth Bearer Token",
			...
			otoken.user = saashq.session.user
		...
		return saashq.db.get_value("OAuth Client", request.client["name"], "default_redirect_uri")

	def invalidate_authorization_code(self, client_id, code, request, *args, **kwargs):
		# Authorization codes are use once, invalidate it when a Bearer token
		# has been acquired.

		saashq.db.set_value("OAuth Authorization Code", code, "validity", "Invalid")
		saashq.db.commit()
	...
	def validate_bearer_token(self, token, scopes, request):
		...
		otoken = saashq.get_doc("OAuth Bearer Token", token)
		is_token_valid = (now_datetime() < otoken.expiration_time) and otoken.status != "Revoked"
		client_scopes = saashq.db.get_value("OAuth Client", otoken.client, "scopes").split(
			get_url_delimiter()
		)
	...
	def get_original_scopes(self, refresh_token, request, *args, **kwargs):
		...
		# request.
		obearer_token = saashq.get_doc("OAuth Bearer Token", {"refresh_token": refresh_token})
	...
	def revoke_token(self, token, token_type_hint, request, *args, **kwargs):
		...
		if token_type_hint == "access_token":
			saashq.db.set_value("OAuth Bearer Token", token, "status", "Revoked")
		elif token_type_hint == "refresh_token":
			saashq.db.set_value("OAuth Bearer Token", {"refresh_token": token}, "status", "Revoked")
		else:
			saashq.db.set_value("OAuth Bearer Token", token, "status", "Revoked")
		saashq.db.commit()
	...
	def validate_refresh_token(self, refresh_token, client, request, *args, **kwargs):
		...
		otoken = saashq.get_doc("OAuth Bearer Token", {"refresh_token": refresh_token, "status": "Active"})
	...
	def finalize_id_token(self, id_token, token, token_handler, request):
		# Check whether saashq server URL is set
		id_token_header = {"typ": "jwt", "alg": "HS256"}

		user = saashq.get_doc("User", request.user)
		...
		return saashq.safe_decode(id_token_encoded)

	def get_authorization_code_nonce(self, client_id, code, redirect_uri, request):
		if saashq.get_value("OAuth Authorization Code", code, "validity") == "Valid":
			return saashq.get_value("OAuth Authorization Code", code, "nonce")

		return None

	def get_authorization_code_scopes(self, client_id, code, redirect_uri, request):
		scope = saashq.get_value("OAuth Client", client_id, "scopes")
		if not scope:
			scope = []
		else:
	...
	def validate_id_token(self, token, scopes, request):
		try:
			id_token = saashq.get_doc("OAuth Bearer Token", token)
	...
	def validate_jwt_bearer_token(self, token, scopes, request):
		try:
			jwt = saashq.get_doc("OAuth Bearer Token", token)
	...
	def validate_silent_login(self, request):
		...
		- OpenIDConnectHybrid
		"""
		if saashq.session.user == "Guest" or request.prompt.lower() == "login":
			return False
		else:
			return True
	...
	def validate_user_match(self, id_token_hint, scopes, claims, request):
		...
		if id_token_hint:
			try:
				...
				client_id, client_secret = saashq.get_value(
					"OAuth Client",
					payload.get("aud"),
				...
				if payload.get("sub") and client_id and client_secret:
					user = saashq.db.get_value(
						"User Social Login",
						{"userid": payload.get("sub"), "provider": "saashq"},
						"parent",
					)
					...
					if verified_payload:
						return user.name == saashq.session.user

			except Exception:
				return False

		elif saashq.session.user != "Guest":
			return True
...
def delete_oauth2_data():
	saashq.db.delete("OAuth Authorization Code", {"validity": "Invalid"})
	saashq.db.delete("OAuth Bearer Token", {"status": "Revoked"})


def get_client_scopes(client_id):
	scopes_string = saashq.db.get_value("OAuth Client", client_id, "scopes")
...
def get_userinfo(user):
	...
	if user.user_image:
		if saashq.utils.validate_url(user.user_image, valid_schemes=valid_url_schemes):
			picture = user.user_image
		else:
			picture = urljoin(saashq_server_url, user.user_image)

	return saashq._dict(
		{
			"sub": saashq.db.get_value(
				"User Social Login",
				{"parent": user.name, "provider": "saashq"},
				"userid",
			),
			...
			"email": user.email,
			"picture": picture,
			"roles": saashq.get_roles(user.name),
			"iss": saashq_server_url,
		}
	)
...
def generate_json_error_response(e):
	...
	saashq.local.response["http_status_code"] = getattr(e, "status_code", 500)
	return


def get_server_url():
	request_url = urlparse(saashq.request.url)
	request_url = f"{request_url.scheme}://{request_url.netloc}"
	return saashq.get_value("Social Login Key", "saashq", "base_url") or request_url
