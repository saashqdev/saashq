...
"""
Saashq - Low Code Open Source Framework in Python and JS

Saashq, pronounced fra-pay, is a full stack, batteries-included, web
framework written in Python and Javascript with MariaDB as the database.
It is the framework which powers ERPNexus. It is pretty generic and can
be used to build database driven apps.

Read the documentation: https://saashqframework.com/docs
"""
...
import saashq
from saashq.query_builder.utils import (
	get_query,
	get_query_builder,
	patch_query_aggregation,
	patch_query_execute,
)
from saashq.utils.caching import request_cache
from saashq.utils.data import cint, cstr, sbool

# Local application imports
from .exceptions import *
from .types.saashqdict import _dict
...
if TYPE_CHECKING:  # pragma: no cover
	...
	from saashq.database.mariadb.database import MariaDBDatabase
	from saashq.database.postgres.database import PostgresDatabase
	from saashq.email.doctype.email_queue.email_queue import EmailQueue
	from saashq.model.document import Document
	from saashq.query_builder.builder import MariaDB, Postgres
	from saashq.types.lazytranslatedstring import _LazyTranslate
	from saashq.utils.redis_wrapper import RedisWrapper
...
_tune_gc = bool(sbool(os.environ.get("SAASHQ_TUNE_GC", True)))
...
def _(msg: str, lang: str | None = None, context: str | None = None) -> str:
	...
	"""
	from saashq.translate import get_all_translations
...
def set_user_lang(user: str, user_language: str | None = None) -> None:
	"""Guess and set user language for the session. `saashq.local.lang`"""
	from saashq.translate import get_user_lang
...
def init(site: str, sites_path: str = ".", new_site: bool = False, force: bool = False) -> None:
	"""Initialize saashq for the current site. Reset thread locals `saashq.local`"""
	...
	setup_module_map(include_all_apps=not (saashq.request or saashq.job or saashq.flags.in_migrate))
...
def connect(site: str | None = None, db_name: str | None = None, set_admin_as_user: bool = True) -> None:
	...
	:param site: (Deprecated) If site is given, calls `saashq.init`.
	:param db_name: (Deprecated) Optional. Will use from `site_config.json`.
	:param set_admin_as_user: Set Administrator as current user.
	"""
	from saashq.database import get_db
	...
	if site:
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling saashq.connect with the site argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke saashq.init(site) prior to calling saashq.connect(), if initializing the site is necessary.",
		)
		init(site)
	if db_name:
		from saashq.deprecation_dumpster import deprecation_warning
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling saashq.connect with the db_name argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke saashq.init(site) with the right config prior to calling saashq.connect(), if necessary.",
		)
...
def get_site_config(sites_path: str | None = None, site_path: str | None = None) -> _dict[str, Any]:
	...
	config["redis_queue"] = (
		os.environ.get("SAASHQ_REDIS_QUEUE") or config.get("redis_queue") or "redis://127.0.0.1:11311"
	)
	config["redis_cache"] = (
		os.environ.get("SAASHQ_REDIS_CACHE") or config.get("redis_cache") or "redis://127.0.0.1:13311"
	)
	config["db_type"] = os.environ.get("SAASHQ_DB_TYPE") or config.get("db_type") or "mariadb"
	config["db_socket"] = os.environ.get("SAASHQ_DB_SOCKET") or config.get("db_socket")
	config["db_host"] = os.environ.get("SAASHQ_DB_HOST") or config.get("db_host") or "127.0.0.1"
	config["db_port"] = int(
		os.environ.get("SAASHQ_DB_PORT") or config.get("db_port") or db_default_ports(config["db_type"])
	)

	# Set the user as database name if not set in config
	config["db_user"] = os.environ.get("SAASHQ_DB_USER") or config.get("db_user") or config.get("db_name")

	# vice versa for dbname if not defined
	config["db_name"] = os.environ.get("SAASHQ_DB_NAME") or config.get("db_name") or config["db_user"]

	# read password
	config["db_password"] = os.environ.get("SAASHQ_DB_PASSWORD") or config.get("db_password")
...
def setup_redis_cache_connection():
	"""Defines `saashq.cache` as `RedisWrapper` instance"""
	global cache

	if not cache:
		from saashq.utils.redis_wrapper import setup_cache
...
def get_traceback(with_context: bool = False) -> str:
	...
	from saashq.utils import get_traceback
	...
	*,
	...
	def _raise_exception():
		if raise_exception:
			...
			if out.__saashq_exc_id:
				exc.__saashq_exc_id = out.__saashq_exc_id
			raise exc
	...
	if raise_exception:
		...
		out.__saashq_exc_id = generate_hash()
...
def toast(message: str, indicator: Literal["blue", "green", "orange", "red", "yellow"] | None = None):
	saashq.msgprint(message, indicator=indicator, alert=True)
...
def throw(
	...
	:param msg: Message.
	:param exc: Exception class. Default `saashq.ValidationError`
	:param title: [optional] Message title. Default: "Message".
	:param is_minimizable: [optional] Allow users to minimize the modal
	:param wide: [optional] Show wide modal
	:param as_list: [optional] If `msg` is a list, render as un-ordered list.
...
def create_folder(path, with_init=False):
	...
	from saashq.utils import touch_file
...
def get_roles(username=None) -> list[str]:
	...
	return saashq.permissions.get_roles(username or local.session.user)
...
def sendmail(
	...
	if as_markdown:
		from saashq.utils import md_to_html
	...
	from saashq.email.doctype.email_queue.email_queue import QueueBuilder
...
def whitelist(allow_guest=False, xss_safe=False, methods=None):
	...
	def innerfn(fn):
		from saashq.utils.typing_validations import validate_argument_types
...
def is_whitelisted(method):
	from saashq.utils import sanitize_html
...
def read_only():
	def innfn(fn):
		...
		def wrapper_fn(*args, **kwargs):
			# saashq.read_only could be called from nested functions, in such cases don't swap the
			# connection again.
...
def only_for(roles: list[str] | tuple[str] | str, message=False):
	"""
	Raises `saashq.PermissionError` if the user does not have any of the permitted roles.
...
def clear_cache(user: str | None = None, doctype: str | None = None):
	...
	import saashq.utils.caching
	from saashq.website.router import clear_routing_cache

	if doctype:
		saashq.cache_manager.clear_doctype_cache(doctype)
		reset_metadata_version()
	elif user:
		saashq.cache_manager.clear_user_cache(user)
	else:  # everything
		# Delete ALL keys associated with this site.
		keys_to_delete = set(saashq.cache.get_keys(""))
		for key in saashq.get_hooks("persistent_cache_keys"):
			keys_to_delete.difference_update(saashq.cache.get_keys(key))
		saashq.cache.delete_value(list(keys_to_delete), make_keys=False)
	...
	saashq.utils.caching._SITE_CACHE.clear()
	local.role_permissions = {}
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	from saashq.permissions import get_role_permissions
...
def has_permission(
	...
	*,
	...
	Raise `saashq.PermissionError` if user isn't permitted and `throw` is truthy
	...
	out = saashq.permissions.has_permission(
		doctype,
		ptype,
		doc=doc,
		user=user,
	...
	if throw and not out:
		...
		saashq.flags.error_message = _("No permission for {0}").format(document_label)
...
def has_website_permission(doc=None, ptype="read", user=None, verbose=False, doctype=None):
	"""Raises `saashq.PermissionError` if not permitted.
...
def get_precision(
	...
	from saashq.model.meta import get_field_precision
...
def new_doc(
	...
	**kwargs,
	...
	from saashq.model.create_new import get_new_doc
...
def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `saashq.client.set_value`"""
	import saashq.client

	return saashq.client.set_value(doctype, docname, fieldname, value)


def get_cached_doc(*args: Any, **kwargs: Any) -> "Document":
	"""Identical to `saashq.get_doc`, but return from cache if available."""
...
def get_doc(**kwargs: dict) -> "_NewDocument":
	...
	Not recommended. Use `saashq.new_doc` instead."""
	pass
...
def get_doc(*args: Any, **kwargs: Any) -> "Document":
	"""Return a `saashq.model.document.Document` object of the given type and name.
	...
	Examples:
	        ...
	        todo = saashq.get_doc({"doctype":"ToDo", "description": "test"})
	        todo.insert()

	        # open an existing document
	        todo = saashq.get_doc("ToDo", "TD0001")

	"""
	import saashq.model.document

	doc = saashq.model.document.get_doc(*args, **kwargs)
...
def get_single(doctype):
	"""Return a `saashq.model.document.Document` object of the given Single doctype."""
	return get_doc(doctype, doctype)


def get_meta(doctype, cached=True):
	"""Get `saashq.model.meta.Meta` instance of given doctype name."""
	import saashq.model.meta

	return saashq.model.meta.get_meta(doctype, cached=cached)


def get_meta_module(doctype):
	import saashq.modules

	return saashq.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	"""Delete a document. Calls `saashq.model.delete_doc.delete_doc`.
	...
	import saashq.model.delete_doc

	return saashq.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def reload_doc(
	...
	return saashq.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def rename_doc(
	...
	*,
	...
	Calls `saashq.model.rename_doc.rename_doc`
	"""
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = saashq.utils.strip(content)
...
def import_doc(path):
	...
	from saashq.core.doctype.data_import.data_import import import_doc
...
def copy_doc(doc: "Document", ignore_no_copy: bool = True) -> "Document":
	...
	from saashq.model.base_document import BaseDocument
...
def redirect(url):
	...
	from saashq.exceptions import Redirect
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
	        saashq.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	import saashq.desk.reportview

	return saashq.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)


def get_list(doctype, *args, **kwargs):
	"""List database query via `saashq.model.db_query`. Will also check for permissions.
	...
	Example usage:
	        ...
	        # simple dict filter
	        saashq.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        saashq.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])
	"""
	import saashq.model.db_query

	return saashq.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)


def get_all(doctype, *args, **kwargs):
	"""List database query via `saashq.model.db_query`. Will **not** check for permissions.
	Parameters are same as `saashq.get_list`
	...
	Example usage:
	        ...
	        # simple dict filter
	        saashq.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        saashq.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])
	"""
...
from saashq.deprecation_dumpster import saashq_get_test_records as get_test_records
...
def format_value(*args, **kwargs):
	...
	import saashq.utils.formatters

	return saashq.utils.formatters.format_value(*args, **kwargs)
...
def get_print(
	...
	from saashq.website.serve import get_response_without_exception_handling
...
def attach_print(
	...
	from saashq.utils import scrub_urls
	from saashq.utils.pdf import get_pdf
...
def publish_progress(*args, **kwargs):
	...
	"""
	import saashq.realtime

	return saashq.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return saashq.realtime.publish_realtime(*args, **kwargs)


def local_cache(namespace, key, generator, regenerate_if_none=False):
	"""A key value store for caching within a request

	:param namespace: saashq.local.cache[namespace]
	:param key: saashq.local.cache[namespace][key] used to retrieve value
...
def enqueue(*args, **kwargs):
	...
	"""
	import saashq.utils.background_jobs

	return saashq.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	return saashq.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def logger(module=None, with_more_info=False, allow_site=True, filter=None, max_size=100_000, file_count=20):
	...
	from saashq.utils.logger import get_logger
...
def safe_eval(code, eval_globals=None, eval_locals=None):
	...
	from saashq.utils.safe_exec import safe_eval
...
def get_active_domains():
	from saashq.core.doctype.domain_settings.domain_settings import get_active_domains
...
def get_version(doctype, name, limit=None, head=False, raise_err=True):
	...
	Example
	>>> saashq.get_version("User", "foobar@gmail.com")
	>>>
	[
	        {
	...
	if meta.track_changes:
		...
		from saashq.utils import dictify, safe_json_loads, squashify
...
def parse_json(val):
	from saashq.utils import parse_json
...
def mock(type, size=1, locale="en"):
	...
	from saashq.utils import squashify
...
def validate_and_sanitize_search_inputs(fn):
	...
	def wrapper(*args, **kwargs):
		from saashq.desk.search import sanitize_searchfield
...
from saashq.utils.error import log_error

if _tune_gc:
	# generational GC gets triggered after certain allocs (g0) which is 700 by default.
	# This number is quite small for saashq where a single query can potentially create 700+
	# objects easily.
