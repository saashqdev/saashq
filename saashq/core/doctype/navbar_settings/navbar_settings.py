...
class NavbarSettings(Document):
	...
	if TYPE_CHECKING:
		from saashq.core.doctype.navbar_item.navbar_item import NavbarItem
...
def get_app_logo():
	app_logo = saashq.db.get_single_value("Navbar Settings", "app_logo", cache=True)
	if not app_logo:
		logos = saashq.get_hooks("app_logo_url")
		app_logo = logos[0]
		if len(logos) == 2:
			app_logo = logos[1]
...
def get_navbar_settings():
	return saashq.get_single("Navbar Settings")
...
def sync_table(key, hook):
	...
	for item in saashq.get_hooks(hook):
