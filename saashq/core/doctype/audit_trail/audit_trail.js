...
saashq.ui.form.on("Audit Trail", {
	refresh(frm) {
		let prev_route = saashq.get_prev_route();
		if (
			prev_route.length > 2 &&
			prev_route[0] == "Form" &&
	...
	start_date(frm) {
		...
		saashq.db
			...
			.then((creation) => {
				if (saashq.datetime.obj_to_str(creation) < frm.doc.start_date) {
					frm.doc.document = "";
					frm.refresh_fields();
				}
			});
	...
	end_date(frm) {
		saashq.db
			.get_value(frm.doc.doctype_name, frm.doc.document, "creation")
			.then((creation) => {
				if (saashq.datetime.obj_to_str(creation) > frm.doc.end_date) {
					frm.doc.document = "";
					frm.refresh_fields();
				}
			});
	...
	render_changed_fields(frm, document_names, changed_fields) {
		...
		$(saashq.render_template("audit_trail", render_dict)).appendTo(
	...
	render_rows_added_or_removed(frm, changed_fields) {
		...
		for (let key in added_or_removed) {
			...
			$(saashq.render_template("audit_trail_rows_added_removed", section_dict)).appendTo(
