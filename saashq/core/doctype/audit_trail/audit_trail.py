...
from saashq.utils import compare
...
class AuditTrail(Document):
	...
	def validate_fields(self):
		...
		for field in fields_dict:
			if not fields_dict[field]:
				saashq.throw(_("{} field cannot be empty.").format(saashq.bold(field)))

	def validate_document(self):
		if not saashq.db.exists(self.doctype_name, self.document):
			saashq.throw(
				_("The selected document {0} is not a {1}.").format(
					saashq.bold(self.document), saashq.bold(self.doctype_name)
				)
			)
	...
	def compare_document(self):
		...
		self.amended_docs = [saashq.get_doc(self.doctype_name, name) for name in amended_document_names]
	...
	def get_amended_documents(self):
		...
		creation = saashq.db.get_value(self.doctype_name, self.document, "creation")
		while (
			curr_doc
			...
			curr_doc = saashq.db.get_value(self.doctype_name, curr_doc, "amended_from")
			creation = saashq.db.get_value(self.doctype_name, curr_doc, "creation")
...
def filter_fields_for_gridview(row):
	grid_row = {}
	meta = saashq.get_meta(row.doctype)
	for field in meta.fields:
