...
class TestVersion(IntegrationTestCase):
	def test_get_diff(self):
		...
		old_doc = saashq.get_doc("Event", test_records[0])
	...
	def test_no_version_on_new_doc(self):
		from saashq.desk.form.load import get_versions

		t = saashq.get_doc(doctype="ToDo", description="something")
		t.save(ignore_version=False)

		self.assertFalse(get_versions(t))

		t = saashq.get_doc(t.doctype, t.name)
