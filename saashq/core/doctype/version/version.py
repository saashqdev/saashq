...
class Version(Document):
	...
	def set_impersonator(data):
		if not saashq.session:
			return
		if impersonator := saashq.session.data.get("impersonated_by"):
	...
	def set_diff(self, old: Document, new: Document) -> bool:
		...
		if diff:
			...
			self.data = saashq.as_json(diff, indent=None, separators=(",", ":"))
			return True
		else:
			return False
	...
	def for_insert(self, doc: Document) -> bool:
		...
		self.data = saashq.as_json(data, indent=None, separators=(",", ":"))
		return True
...
def on_doctype_update():
	saashq.db.add_index("Version", ["ref_doctype", "docname"])
