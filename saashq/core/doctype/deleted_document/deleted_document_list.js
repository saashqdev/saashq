saashq.listview_settings["Deleted Document"] = {
	onload: function (doclist) {
		const action = () => {
			...
			if (selected_docs.length > 0) {
				...
				saashq.call({
					method: "saashq.core.doctype.deleted_document.deleted_document.bulk_restore",
					args: { docnames },
					callback: function (r) {
						if (r.message) {
							let body = (docnames) => {
							...
							saashq.msgprint(summary, __("Document Restoration Summary"), true);
