...
from saashq.utils.background_jobs import get_queues, get_redis_conn
...
class RQJob(Document):
	...
	def stop_job(self):
		try:
			...
			saashq.msgprint(_("Job is not running."), title=_("Invalid Operation"))
...
def serialize_job(job: Job) -> saashq._dict:
	...
	if job_name == "saashq.utils.background_jobs.run_doc_method":
	...
	return saashq._dict(
		...
		exc_info=exc_info,
		arguments=saashq.as_json(job.kwargs),
		timeout=job.timeout,
...
def for_current_site(job: Job) -> bool:
	return job.kwargs.get("site") == saashq.local.site
...
@saashq.whitelist()
def stop_job(job_id):
	saashq.get_doc("RQ Job", job_id).stop_job()
