...
import saashq
from saashq.core.doctype.rq_job.rq_job import RQJob, remove_failed_jobs, stop_job
from saashq.installer import update_site_config
from saashq.tests import IntegrationTestCase, UnitTestCase, timeout
from saashq.utils import cstr, execute_in_shell
from saashq.utils.background_jobs import get_job_status, is_job_enqueued
...
class TestRQJob(IntegrationTestCase):
	BG_JOB = "saashq.core.doctype.rq_job.test_rq_job.test_func"
	...
	def test_serialization(self):
		job = saashq.enqueue(method=self.BG_JOB, queue="short")
		rq_job = saashq.get_doc("RQ Job", job.id)
	...
	def test_configurable_ttl(self):
		saashq.conf.rq_job_failure_ttl = 600
		job = saashq.enqueue(method=self.BG_JOB, queue="short")

		self.assertEqual(job.failure_ttl, 600)

	def test_func_obj_serialization(self):
		job = saashq.enqueue(method=test_func, queue="short")
		rq_job = saashq.get_doc("RQ Job", job.id)
		self.assertEqual(rq_job.job_name, "saashq.core.doctype.rq_job.test_rq_job.test_func")

	@timeout
	def test_get_list_filtering(self):
		# Check failed job clearning and filtering
		remove_failed_jobs()
		jobs = saashq.get_all("RQ Job", {"status": "failed"})
		...
		# Fail a job
		job = saashq.enqueue(method=self.BG_JOB, queue="short", fail=True)
		...
		non_failed_jobs = saashq.get_all("RQ Job", {"status": ("!=", "failed")})
		self.assertGreaterEqual(len(non_failed_jobs), 1)

		# Create a slow job and check if it's stuck in "Started"
		job = saashq.enqueue(method=self.BG_JOB, queue="short", sleep=10)
		time.sleep(3)
	...
	def test_delete_doc(self):
		job = saashq.enqueue(method=self.BG_JOB, queue="short")
		saashq.get_doc("RQ Job", job.id).delete()
	...
	def test_job_id_manual_dedup(self):
		job_id = "test_dedup"
		job = saashq.enqueue(self.BG_JOB, sleep=5, job_id=job_id)
	...
	def test_auto_job_dedup(self):
		job_id = "test_dedup"
		job1 = saashq.enqueue(self.BG_JOB, sleep=2, job_id=job_id, deduplicate=True)
		job2 = saashq.enqueue(self.BG_JOB, sleep=5, job_id=job_id, deduplicate=True)
		...
		job3 = saashq.enqueue(self.BG_JOB, fail=True, job_id=job_id, deduplicate=True)
		self.check_status(job3, "failed")
		job4 = saashq.enqueue(self.BG_JOB, sleep=1, job_id=job_id, deduplicate=True)
		self.check_status(job4, "finished")

	@timeout
	def test_enqueue_after_commit(self):
		job_id = saashq.generate_hash()

		saashq.enqueue(self.BG_JOB, enqueue_after_commit=True, job_id=job_id)
	...
	def test_memory_usage(self):
		if saashq.db.db_type != "mariadb":
			return
		job = saashq.enqueue("saashq.utils.data._get_rss_memory_usage")
		self.check_status(job, "finished")
	...
	def test_clear_failed_jobs(self):
		...
		jobs = [saashq.enqueue(method=self.BG_JOB, queue="short", fail=True) for _ in range(limit * 2)]
