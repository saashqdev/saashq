...
from saashq.core.doctype.document_naming_settings.document_naming_settings import (
	DocumentNamingSettings,
)
from saashq.model.naming import NamingSeries, get_default_naming_series
...
class TestNamingSeries(IntegrationTestCase):
	...
	def setUp(self):
		self.dns: DocumentNamingSettings = saashq.get_doc("Document Naming Settings")
	...
	def test_get_transactions(self):
		...
		existing_naming_series = saashq.get_meta(self.ns_doctype).get_field("naming_series").options
	...
	def test_updates_naming_options(self):
		...
		self.assertIn(test_series, saashq.get_meta(self.ns_doctype).get_naming_series_options())
	...
	def test_amended_naming(self):
		...
		new_amended_doc = saashq.get_doc(
			doctype=self.ns_doctype, some_fieldname="test doc with submit", amended_from=submittable_doc.name
		).insert()
