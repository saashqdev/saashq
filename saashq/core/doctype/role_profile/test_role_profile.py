...
class TestRoleProfile(IntegrationTestCase):
	def test_make_new_role_profiles(self):
		saashq.delete_doc_if_exists("Role Profile", "Test 1", force=1)
		new_role_profile = saashq.get_doc(doctype="Role Profile", role_profile="Test 1").insert()
	...
	def test_multiple_role_profiles(self):
		saashq.delete_doc_if_exists("Role Profile", "_Test Role Profile 1", force=1)
		saashq.delete_doc_if_exists("Role Profile", "_Test Role Profile 2", force=1)

		role_profile_one = saashq.get_doc(doctype="Role Profile", role_profile="_Test Role Profile 1").insert(
			ignore_if_duplicate=True
		)
		role_profile_two = saashq.get_doc(doctype="Role Profile", role_profile="_Test Role Profile 2").insert(
			ignore_if_duplicate=True
		)
		...
		saashq.get_doc(doctype="Role", role_name="_Test Role 1").insert(ignore_if_duplicate=True)
		saashq.get_doc(doctype="Role", role_name="_Test Role 2").insert(ignore_if_duplicate=True)
		saashq.get_doc(doctype="Role", role_name="_Test Role 3").insert(ignore_if_duplicate=True)
		# add role
		...
		user_one = saashq.get_doc(
			{
				"doctype": "User",
				"email": saashq.mock("email"),
				"enabled": 1,
				"first_name": saashq.mock("name"),
				"new_password": "Eastern_43A1W",
				"role_profiles": [
	...
	def test_update_role_profile(self):
		role_profile = saashq.get_doc("Role Profile", "_Test Role Profile 1")
