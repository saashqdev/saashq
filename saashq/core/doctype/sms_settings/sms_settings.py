...
import saashq
from saashq import _, msgprint, throw
...
class SMSSettings(Document):
	...
	if TYPE_CHECKING:
		from saashq.core.doctype.sms_parameter.sms_parameter import SMSParameter
...
def send_sms(receiver_list, msg, sender_name="", success_msg=True):
	...
	arg = {
		"receiver_list": receiver_list,
		"message": saashq.safe_decode(msg).encode("utf-8"),
		"success_msg": success_msg,
	}

	if saashq.db.get_single_value("SMS Settings", "sms_gateway_url"):
...
def send_via_gateway(arg):
	...
	message = saashq.safe_decode(arg.get("message"))
	...
	if len(success_list) > 0:
		...
		if arg.get("success_msg"):
			saashq.msgprint(_("SMS sent successfully"))


def get_headers(sms_settings=None):
	if not sms_settings:
		sms_settings = saashq.get_doc("SMS Settings", "SMS Settings")
