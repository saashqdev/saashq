...
import saashq
from saashq.core.doctype.log_settings.log_settings import _supports_log_clearing, run_log_clean_up
...
class TestLogSettings(IntegrationTestCase):
	...
	def test_delete_logs(self):
		...
		activity_log_count = saashq.db.count("Activity Log", {"creation": ("<=", self.datetime.past)})
		error_log_count = saashq.db.count("Error Log", {"creation": ("<=", self.datetime.past)})
		email_queue_count = saashq.db.count("Email Queue", {"creation": ("<=", self.datetime.past)})
