...
from saashq.utils import cstr, get_site_url
...
class TestAccessLog(IntegrationTestCase):
	def setUp(self):
		...
		generate_keys(saashq.session.user)
		saashq.db.commit()
		generated_secret = saashq.utils.password.get_decrypted_password(
			"User", saashq.session.user, fieldname="api_secret"
		)
		...
		self.file_name = saashq.utils.random_string(10) + ".txt"
		self.test_content = saashq.utils.random_string(1024)
	...
	def test_make_full_access_log(self):
		...
		last_doc = saashq.get_last_doc("Access Log")
	...
	def test_private_file_download(self):
		...
		new_private_file = saashq.get_doc(
			{
				"doctype": self.test_doctype,
		...
		private_file_link = get_site_url(saashq.local.site) + new_private_file.file_url
