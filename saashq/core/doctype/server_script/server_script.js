...
saashq.ui.form.on("Server Script", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Compare Versions"), () => {
				new saashq.ui.DiffView("Server Script", "script", frm.doc.name);
			});
		}

		saashq
			.call("saashq.core.doctype.server_script.server_script.get_autocompletion_items")
			.then((r) => r.message)
			.then((items) => {
	...
	check_safe_exec(frm) {
		saashq.xcall("saashq.core.doctype.server_script.server_script.enabled").then((enabled) => {
			if (enabled === false) {
				let docs_link =
					"https://saashqframework.com/docs/user/en/desk/scripting/server-script";
...
if saashq.form_dict.message == "ping":
	saashq.response['message'] = "pong"
else:
	saashq.response['message'] = "ok"
</code></pre>
