...
from saashq.utils.safe_exec import (
	SaashqTransformer,
...
class ServerScript(Document):
	...
	def on_trash(self):
		...
		if self.script_type == "Scheduler Event":
			for job in self.scheduled_jobs:
				scheduled_job_type: "ScheduledJobType" = saashq.get_doc("Scheduled Job Type", job.name)
	...
	def sync_scheduled_job_type(self):
		...
		def get_scheduled_job() -> "ScheduledJobType":
			if scheduled_script := saashq.db.get_value("Scheduled Job Type", {"server_script": self.name}):
				return saashq.get_doc("Scheduled Job Type", scheduled_script)
			else:
				return saashq.get_doc({"doctype": "Scheduled Job Type", "server_script": self.name})
		...
		get_scheduled_job().update(
			{
				"method": saashq.scrub(f"{self.name}-{self.event_frequency}"),
		...
		saashq.msgprint(_("Scheduled execution for script {0} has updated").format(self.name), alert=True)
	...
	def check_if_compilable_in_restricted_context(self):
		...
		try:
			compile_restricted(self.script, policy=SaashqTransformer)
		except Exception as e:
			saashq.msgprint(str(e), title=_("Compilation warning"))

	def execute_method(self) -> dict:
		"""Specific to API endpoint Server Scripts.

		Raise:
		        saashq.DoesNotExistError: If self.script_type is not API.
		        saashq.PermissionError: If self.allow_guest is unset for API accessed by Guest user.

		Return:
		        dict: Evaluate self.script with saashq.utils.safe_exec.safe_exec and return the flags set in its safe globals.
		"""
	...
	def execute_scheduled_method(self):
		...
		Raises:
		        saashq.DoesNotExistError: If script type is not a scheduler event
		"""
		if self.script_type != "Scheduler Event":
...
def get_autocompletion_items():
	...
	e.g., ["saashq.utils.cint", "saashq.get_all", ...]
	"""

	return saashq.cache.get_value(
...
def execute_api_server_script(script: ServerScript, *args, **kwargs):
	...
	if saashq.session.user == "Guest" and not script.allow_guest:
	...
	return _globals.saashq.flags


@saashq.whitelist()
def enabled() -> bool | None:
	if saashq.has_permission("Server Script"):
