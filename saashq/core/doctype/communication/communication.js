saashq.ui.form.on("Communication", {
	onload: function (frm) {
		if (frm.doc.content) {
			frm.doc.content = saashq.dom.remove_script_and_style(frm.doc.content);
		}
			...
			fields: [
				{
					...
					get_query: function () {
						return { query: "saashq.email.get_communication_doctype" };
					},
				},
				{
	...
	show_move_dialog: function (frm) {
		var d = new saashq.ui.Dialog({
			...
			primary_action(values) {
				d.hide();
				saashq.call({
					method: "saashq.email.inbox.move_email",
					args: {
	...
	mark_as_read_unread: function (frm) {
		...
		return saashq.call({
			method: "saashq.email.inbox.create_email_flag_queue",
			args: {
				names: [frm.doc.name],
				action: action,
				flag: flag,
	...
	mark_as_closed_open: function (frm) {
		...
		return saashq.call({
			method: "saashq.email.inbox.mark_as_closed_open",
			args: {
				communication: frm.doc.name,
				status: status,
			},
	...
	get_mail_args: function (frm) {
		var sender_email_id = "";
		$.each(saashq.boot.email_accounts, function (idx, account) {
	...
	add_to_contact: function (frm) {
		...
		saashq.new_doc("Contact");
	},

	mark_as_spam: function (frm) {
		saashq.call({
			method: "saashq.email.inbox.mark_as_spam",
			args: {
				communication: frm.doc.name,
			...
			freeze: true,
			callback: function (r) {
				saashq.msgprint(__("Email has been marked as spam"));
			},
		});
	},
	...
	move_to_trash: function (frm) {
		saashq.call({
			method: "saashq.email.inbox.mark_as_trash",
			args: {
				communication: frm.doc.name,
			},
			freeze: true,
			callback: function (r) {
				saashq.msgprint(__("Email has been moved to trash"));
			},
		});
	},
});
