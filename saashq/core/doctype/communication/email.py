...
def make(
	...
	**kwargs,
	...
	"""
	if kwargs:
		from saashq.utils.commands import warn

		warn(
			f"Options {kwargs} used in saashq.core.doctype.communication.email.make "
			"are deprecated or unsupported",
			category=DeprecationWarning,
		)

	if doctype and name and not saashq.has_permission(doctype=doctype, ptype="email", doc=name):
		raise saashq.PermissionError(f"You are not allowed to send emails related to: {doctype} {name}")
...
def _make(
	...
	sender = sender or get_formatted_email(saashq.session.user)
...
def add_attachments(name: str, attachments: Iterable[str | dict]) -> None:
	...
	for a in attachments:
		if isinstance(a, str):
			attach = saashq.db.get_value("File", {"name": a}, ["file_url", "is_private"], as_dict=1)
			file_args = {
				"file_url": attach.file_url,
				"is_private": attach.is_private,
			}
		elif isinstance(a, dict) and "fcontent" in a and "fname" in a:
			# dict returned by saashq.attach_print()
			file_args = {
				"file_name": a["fname"],
				"content": a["fcontent"],
				"is_private": 1,
...
def mark_email_as_seen(name: str | None = None):
	saashq.request.after_response.add(lambda: _mark_email_as_seen(name))
...
def _mark_email_as_seen(name):
	try:
		...
		saashq.log_error("Unable to mark as seen", None, "Communication", name)

	saashq.db.commit()  # nosemgrep: after_response requires explicit commit
...
def update_communication_as_read(name):
	...
	communication = saashq.db.get_value("Communication", name, "read_by_recipient", as_dict=True)
