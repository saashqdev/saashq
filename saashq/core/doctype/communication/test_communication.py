...
import saashq
from saashq.core.doctype.communication.communication import Communication, get_emails, parse_email
from saashq.core.doctype.communication.email import add_attachments, make
...
if TYPE_CHECKING:
	from saashq.contacts.doctype.contact.contact import Contact
...
class TestCommunication(IntegrationTestCase):
	def test_email(self):
		...
		for i, x in enumerate(valid_email_list):
			with self.subTest(i=i, x=x):
				self.assertTrue(saashq.utils.parse_addr(x)[1])

		for i, x in enumerate(invalid_email_list):
			with self.subTest(i=i, x=x):
				self.assertFalse(saashq.utils.parse_addr(x)[0])
	...
	def test_name(self):
		...
		for x in valid_email_list:
			self.assertTrue(saashq.utils.parse_addr(x)[0])
	...
	def test_circular_linking(self):
		...
		a = saashq.get_doc("Communication", a.name)
		a.reference_doctype = "Communication"
		a.reference_name = c.name

		self.assertRaises(saashq.CircularLinkingError, a.save)

	def test_deduplication_timeline_links(self):
		saashq.delete_doc_if_exists("Note", "deduplication timeline links")
	...
	def test_contacts_attached(self):
		contact_sender: "Contact" = saashq.get_doc(
			{
				"doctype": "Contact",
		...
		contact_recipient: "Contact" = saashq.get_doc(
			{
				"doctype": "Contact",
		...
		contact_cc: "Contact" = saashq.get_doc(
			{
				"doctype": "Contact",
		...
		comm: Communication = saashq.get_doc(
			{
				"doctype": "Communication",
				"communication_medium": "Email",
	...
	def test_get_communication_data(self):
		from saashq.desk.form.load import get_communication_data

		saashq.delete_doc_if_exists("Note", "get communication data")

		note = saashq.get_doc(
			{"doctype": "Note", "title": "get communication data", "content": "get communication data"}
		).insert(ignore_permissions=True)

		comm_note_1 = saashq.get_doc(
			{
				"doctype": "Communication",
				"communication_type": "Communication",
	...
	def test_signature_in_email_content(self):
		...
		comm_with_signature = saashq.get_doc(
			base_communication
			| {
		...
		comm_without_signature = saashq.get_doc(
			base_communication
			| {
	...
	def test_mark_as_spam(self):
		...
		spam_comm: Communication = saashq.get_doc(
			{
				"doctype": "Communication",
				"communication_medium": "Email",
		...
		normal_comm: Communication = saashq.get_doc(
			{
				"doctype": "Communication",
				"communication_medium": "Email",
				"subject": "This is spam",
...
class TestCommunicationEmailMixin(IntegrationTestCase):
	...
	def test_add_attachments_by_filename(self):
		...
		attached_file_name, attached_content_hash = saashq.db.get_value(
			"File",
	...
	def test_add_attachments_by_file_content(self):
		...
		attached_file_name = saashq.db.get_value(
			"File",
			{"attached_to_name": comm.name, "attached_to_doctype": comm.doctype},
		)
		attached_file = saashq.get_doc("File", attached_file_name)
...
def create_email_account() -> "EmailAccount":
	saashq.delete_doc_if_exists("Email Account", "_Test Comm Account 1")
