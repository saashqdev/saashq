...
from saashq.utils import cstr, get_formatted_email, get_url, parse_addr
...
class CommunicationEmailMixin:
	...
	def mail_cc(self, is_inbound_mail_communcation=False, include_sender=False):
		...
		* Remove standard users from email list
		...
		if include_sender:
			...
			if saashq.session.user not in saashq.STANDARD_USERS:
				sender = saashq.db.get_value("User", saashq.session.user, "email")
			cc.append(sender)
		...
		self._final_cc = [m for m in cc if m and m not in saashq.STANDARD_USERS]
		return self._final_cc
	...
	def mail_bcc(self, is_inbound_mail_communcation=False):
		...
		* remove standard users.
		...
		self._final_bcc = [m for m in bcc if m not in saashq.STANDARD_USERS]
		return self._final_bcc
	...
	def get_content(self, print_format=None):
		if print_format and saashq.get_system_settings("attach_view_link"):
	...
	def get_attach_link(self, print_format):
		...
		return saashq.get_template("templates/emails/print_link.html").render(
			{
				"url": get_url(),
		...
		if not hasattr(self, "_outgoing_email_account"):
			if self.email_account:
				...
				if self.sent_or_received == "Sent" and self._outgoing_email_account:
					if saashq.db.exists("Email Account", self._outgoing_email_account.name):
	...
	def mail_attachments(self, print_format=None, print_html=None, print_language=None):
		...
		if print_format or print_html:
			d = {
				...
				"doctype": self.reference_doctype,
				"name": self.reference_name,
				"lang": print_language or saashq.local.lang,
			}
	...
	def filter_thread_notification_disbled_users(emails):
		...
		return saashq.get_all("User", pluck="email", filters={"email": ["in", emails], "thread_notify": 0})
	...
	def filter_disabled_users(emails):
		...
		return saashq.get_all("User", pluck="email", filters={"email": ["in", emails], "enabled": 0})
	...
	def send_email(
		...
		if input_dict := self.sendmail_input_dict(
			...
			saashq.sendmail(now=now, **input_dict)
