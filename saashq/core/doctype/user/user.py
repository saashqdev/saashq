...
import saashq.share
from saashq import STANDARD_USERS, _, msgprint, throw
from saashq.apps import get_default_path
from saashq.auth import MAX_PASSWORD_SIZE
from saashq.core.doctype.user_type.user_type import user_linked_with_permission_on_doctype
...
from saashq.utils.data import sha256_hash
from saashq.utils.password import check_password, get_password_reset_limit
from saashq.utils.password import update_password as _update_password
from saashq.utils.user import get_system_managers
from saashq.website.utils import get_home_page, is_signup_disabled
...
class User(Document):
	...
	if TYPE_CHECKING:
		...
		from saashq.core.doctype.defaultvalue.defaultvalue import DefaultValue
		from saashq.core.doctype.has_role.has_role import HasRole
		from saashq.core.doctype.user_email.user_email import UserEmail
		from saashq.core.doctype.user_role_profile.user_role_profile import UserRoleProfile
		from saashq.core.doctype.user_social_login.user_social_login import UserSocialLogin
	...
	def after_insert(self):
		...
		saashq.cache.delete_key("users_for_mentions")
		saashq.cache.delete_key("enabled_users")
	...
	def validate(self):
		...
		if (self.name not in ["Administrator", "Guest"]) and (not self.get_social_login_userid("saashq")):
			self.set_social_login_userid("saashq", saashq.generate_hash(length=39))
	...
	def populate_role_profile_roles(self):
		...
		for role_profile in self.role_profiles:
			role_profile = saashq.get_cached_doc("Role Profile", role_profile.role_profile)
	...
	from saashq.deprecation_dumpster import validate_roles
	...
	def validate_allowed_modules(self):
		if self.module_profile:
			module_profile = saashq.get_doc("Module Profile", self.module_profile)
	...
	def validate_user_image(self):
		if self.user_image and len(self.user_image) > 2000:
			saashq.throw(_("Not a valid User Image."))

	def on_update(self):
		# clear new password
		self.share_with_self()
		clear_notifications(user=self.name)
		saashq.clear_cache(user=self.name)
		now = saashq.flags.in_test or saashq.flags.in_install
		self.send_password_notification(self.__new_password)
		saashq.enqueue(
			"saashq.core.doctype.user.user.create_contact",
			user=self,
		...
		if self.name not in STANDARD_USERS and not self.user_image:
			saashq.enqueue(
				"saashq.core.doctype.user.user.update_gravatar",
				name=self.name,
				now=now,
				enqueue_after_commit=True,
			)
		...
		if self.time_zone:
			saashq.defaults.set_default("time_zone", self.time_zone, self.name)

		if self.has_value_changed("language"):
			locale_keys = ("date_format", "time_format", "number_format", "first_day_of_the_week")
			if self.language:
				language = saashq.get_doc("Language", self.language)
				for key in locale_keys:
					value = language.get(key)
					if value:
						saashq.defaults.set_default(key, value, self.name)
			else:
				for key in locale_keys:
					saashq.defaults.clear_default(key, parent=self.name)
	...
	def has_website_permission(self, ptype, user, verbose=False):
		...
		return self.name == saashq.session.user
	...
	def check_enable_disable(self):
		...
		if not cint(self.enabled) and self.name in STANDARD_USERS:
			saashq.throw(_("User {0} cannot be disabled").format(self.name))

		# clear sessions if disabled
		if not cint(self.enabled) and getattr(saashq.local, "login_manager", None):
			saashq.local.login_manager.logout(user=self.name)
	...
	def set_system_user(self):
		...
		if self.user_type and not saashq.get_cached_value("User Type", self.user_type, "is_standard"):
	...
	def set_roles_and_modules_based_on_user_type(self):
		user_type_doc = saashq.get_cached_doc("User Type", self.user_type)
	...
	def validate_share(self, docshare):
		...
		# 		if docshare.share != 1:
		# 			saashq.throw(_("Sorry! User should have complete access to their own record."))
		# 	else:
		# 		saashq.throw(_("Sorry! Sharing with Website User is prohibited."))
	...
	def password_reset_mail(self, link):
		reset_password_template = saashq.db.get_system_setting("reset_password_template")
	...
	def send_welcome_mail_to_user(self):
		...
		subject = None
		method = saashq.get_hooks("welcome_email")
		if method:
			subject = saashq.get_attr(method[-1])()
		if not subject:
			site_name = saashq.db.get_default("site_name") or saashq.get_conf().get("site_name")
			if site_name:
				subject = _("Welcome to {0}").format(site_name)
			else:
				subject = _("Complete Registration")

		welcome_email_template = saashq.db.get_system_setting("welcome_email_template")
	...
	def send_login_mail(self, subject, template, add_args, now=None, custom_template=None):
		...
		from saashq.utils import get_url
		from saashq.utils.user import get_user_fullname

		created_by = get_user_fullname(saashq.session["user"])
		if created_by == "Guest":
		...
		sender = (
			saashq.session.user not in STANDARD_USERS and get_formatted_email(saashq.session.user) or None
		)

		if custom_template:
			from saashq.email.doctype.email_template.email_template import get_email_template
	...
	def on_trash(self):
		...
		self.enabled = 0
		if getattr(saashq.local, "login_manager", None):
			saashq.local.login_manager.logout(user=self.name)

		# delete todos
		saashq.db.delete("ToDo", {"allocated_to": self.name})
		todo_table = DocType("ToDo")
		(
			saashq.qb.update(todo_table)
			.set(todo_table.assigned_by, None)
			.where(todo_table.assigned_by == self.name)
		).run()

		# delete events
		saashq.db.delete("Event", {"owner": self.name, "event_type": "Private"})

		# delete shares
		saashq.db.delete("DocShare", {"user": self.name})
		# delete messages
		...
		saashq.qb.update(table).where(table.user == self.name).set(table.user, None).run()

		# delete notification settings
		saashq.delete_doc("Notification Settings", self.name, ignore_permissions=True)
		...
		saashq.db.delete("User Permission", {"user": self.name})

		# Delete OAuth data
		saashq.db.delete("OAuth Authorization Code", {"user": self.name})
		saashq.db.delete("Token Cache", {"user": self.name})

		# Delete EPS data
		saashq.db.delete("Energy Point Log", {"user": self.name})

		# Remove user link from Workflow Action
		saashq.db.set_value("Workflow Action", {"user": self.name}, "user", None)

		# Delete user's List Filters
		saashq.db.delete("List Filter", {"for_user": self.name})

		# Remove user from Note's Seen By table
		seen_notes = saashq.get_all("Note", filters=[["Note Seen By", "user", "=", self.name]], pluck="name")
		for note_id in seen_notes:
			note = saashq.get_doc("Note", note_id)
	...
	def before_rename(self, old_name, new_name, merge=False):
		...
		if merge:
			saashq.delete_doc("Notification Settings", old_name, ignore_permissions=True)

		saashq.clear_cache(user=old_name)
	...
	def after_rename(self, old_name, new_name, merge=False):
		tables = saashq.db.get_tables()
		for tab in tables:
			desc = saashq.db.get_table_columns_description(tab)
		...
		if saashq.db.exists("Notification Settings", old_name):
			saashq.rename_doc("Notification Settings", old_name, new_name, force=True, show_alert=False)

		# set email
		saashq.db.set_value("User", new_name, "email", new_name)
	...
	def remove_disabled_roles(self):
		disabled_roles = [d.name for d in saashq.get_all("Role", filters={"disabled": 1})]
	...
	def validate_username(self):
		if not self.username and self.is_new() and self.first_name:
			self.username = saashq.scrub(self.first_name)
		...
		if self.username_exists():
			if self.user_type == "System User":
				saashq.msgprint(_("Username {0} already exists").format(self.username))
	...
	def suggest_username(self):
		...
		# @firstname
		username = _check_suggestion(saashq.scrub(self.first_name))

		if not username:
			# @firstname_last_name
			username = _check_suggestion(saashq.scrub("{} {}".format(self.first_name, self.last_name or "")))

		if username:
			saashq.msgprint(_("Suggested Username: {0}").format(username))

		return username

	def username_exists(self, username=None):
		return saashq.db.get_value("User", {"username": username or self.username, "name": ("!=", self.name)})
	...
	def validate_user_email_inbox(self):
		...
		if len(email_accounts) != len(set(email_accounts)):
			saashq.throw(_("Email Account added multiple times"))
	...
	def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
		...
		login_with_mobile = cint(saashq.get_system_settings("allow_login_using_mobile_number"))
		login_with_username = cint(saashq.get_system_settings("allow_login_using_user_name"))
		...
		users = saashq.get_all("User", fields=["name", "enabled"], or_filters=or_filters, limit=1)
		if not users:
			return
	...
	def check_roles_added(self):
		...
		saashq.msgprint(
			_("Newly created user {0} has no roles enabled.").format(saashq.bold(self.name)),
			title=_("No Roles Specified"),
			indicator="orange",
			primary_action={
				"label": _("Add Roles"),
				"client_action": "saashq.set_route",
...
def get_all_roles():
	...
	roles = saashq.get_all(
		"Role",
		filters={
			"name": ("not in", saashq.permissions.AUTOMATIC_ROLES),
			"disabled": 0,
		},
...
def get_roles(arg=None):
	...
	return saashq.get_roles(saashq.form_dict["uid"])


@saashq.whitelist()
def get_perm_info(role):
	"""get permission info"""
	from saashq.permissions import get_all_perms

	return get_all_perms(role)


@saashq.whitelist(allow_guest=True, methods=["POST"])
def update_password(
	...
	if len(new_password) > MAX_PASSWORD_SIZE:
		saashq.throw(_("Password size exceeded the maximum allowed size."))
	...
	if res.get("message"):
		saashq.local.response.http_status_code = 410
		return res["message"]
	else:
		user = res["user"]

	logout_all_sessions = cint(logout_all_sessions) or saashq.get_system_settings("logout_on_password_reset")
	_update_password(user, new_password, logout_all_sessions=cint(logout_all_sessions))

	user_doc, redirect_url = reset_user_data(user)

	# get redirect url from cache
	redirect_to = saashq.cache.hget("redirect_after_login", user)
	if redirect_to:
		redirect_url = redirect_to
		saashq.cache.hdel("redirect_after_login", user)

	saashq.local.login_manager.login_as(user)

	saashq.db.set_value("User", user, "last_password_reset_date", today())
	saashq.db.set_value("User", user, "reset_password_key", "")
...
def test_password_strength(new_password: str, key=None, old_password=None, user_data: tuple | None = None):
	from saashq.utils.password_strength import test_password_strength as _test_password_strength
	...
	enable_password_policy = saashq.get_system_settings("enable_password_policy")

	if not enable_password_policy:
		return {}

	if not user_data:
		user_data = saashq.db.get_value(
			"User", saashq.session.user, ["first_name", "middle_name", "last_name", "email", "birth_date"]
		)

	if new_password:
		result = _test_password_strength(new_password, user_inputs=user_data)
		password_policy_validation_passed = False
		minimum_password_score = cint(saashq.get_system_settings("minimum_password_score"))
...
def has_email_account(email: str):
	return saashq.get_list("Email Account", filters={"email_id": email})


@saashq.whitelist(allow_guest=False)
	...
	if key:
		...
		if result.user:
			reset_password_link_expiry = cint(
				saashq.get_system_settings("reset_password_link_expiry_duration")
			)
			if (
		...
		# verify old password
		saashq.local.login_manager.check_password(saashq.session.user, old_password)
...
def verify_password(password):
	saashq.local.login_manager.check_password(saashq.session.user, password)


@saashq.whitelist(allow_guest=True)
def sign_up(email: str, full_name: str, redirect_to: str) -> tuple[int, str]:
	if is_signup_disabled():
		saashq.throw(_("Sign Up is disabled"), title=_("Not Allowed"))

	user = saashq.db.get("User", {"email": email})
	if user:
		if user.enabled:
		...
		if saashq.db.get_creation_count("User", 60) > 300:
		...
		default_role = saashq.db.get_single_value("Portal Settings", "default_role")
		if default_role:
			user.add_roles(default_role)

		if redirect_to:
			saashq.cache.hset("redirect_after_login", user.name, redirect_to)
...
def reset_password(user: str) -> str:
	try:
		user: User = saashq.get_doc("User", user)
		...
		saashq.local.response["http_status_code"] = 404
...
def user_query(doctype, txt, searchfield, start, page_len, filters):
	from saashq.desk.reportview import get_filters_cond, get_match_cond

	doctype = "User"
	conditions = []

	user_type_condition = "and user_type != 'Website User'"
	if filters and filters.get("ignore_user_type") and saashq.session.data.user_type == "System User":
		user_type_condition = ""
    ...
    """.format(
			...
			standard_users=", ".join(saashq.db.escape(u) for u in STANDARD_USERS),
			key=searchfield,
...
def get_website_users():
	...
	return saashq.db.count("User", filters={"enabled": True, "user_type": "Website User"})
...
def get_permission_query_conditions(user):
	if user == "Administrator":
		...
		return """(`tabUser`.name not in ({standard_users}))""".format(
			standard_users=", ".join(saashq.db.escape(user) for user in STANDARD_USERS)
		)
...
def notify_admin_access_to_system_manager(login_manager=None):
	if (
		...
		and saashq.local.conf.notify_admin_access_to_system_manager
	):
		site = '<a href="{0}" target="_blank">{0}</a>'.format(saashq.local.request.host_url)
		date_and_time = "<b>{}</b>".format(format_datetime(now_datetime(), format_string="medium"))
		ip_address = saashq.local.request_ip
...
def handle_password_test_fail(feedback: dict):
	...
	saashq.throw(msg=" ".join([warning, *suggestions]), title=_("Invalid Password"))


def update_gravatar(name):
	gravatar = has_gravatar(name)
	if gravatar:
		saashq.db.set_value("User", name, "user_image", gravatar)
...
def throttle_user_creation():
	...
	if saashq.db.get_creation_count("User", 60) > saashq.local.conf.get("throttle_user_limit", 60):
		saashq.throw(_("Throttled"))


@saashq.whitelist()
def get_module_profile(module_profile: str):
	module_profile = saashq.get_doc("Module Profile", {"module_profile_name": module_profile})
...
def create_contact(user, ignore_links=False, ignore_mandatory=False):
	...
	if not contact_name:
		...
		except saashq.TimestampMismatchError:
			raise saashq.RetryBackgroundJobError
...
def generate_keys(user: str):
	...
	user_details: User = saashq.get_doc("User", user)
	api_secret = saashq.generate_hash(length=15)
	# if api key is not set generate api key
	if not user_details.api_key:
		api_key = saashq.generate_hash(length=15)
...
def switch_theme(theme):
	if theme in ["Dark", "Light", "Automatic"]:
		saashq.db.set_value("User", saashq.session.user, "desk_theme", theme)


def get_enabled_users():
	def _get_enabled_users():
		enabled_users = saashq.get_all("User", filters={"enabled": "1"}, pluck="name")
		return enabled_users

	return saashq.cache.get_value("enabled_users", _get_enabled_users)
...
def impersonate(user: str, reason: str):
	...
	impersonator = saashq.session.user
	saashq.get_doc(
		{
	...
	saashq.local.login_manager.impersonate(user)
