...
from saashq.core.doctype.user.user import (
	User,
...
from saashq.desk.notifications import extract_mentions
...
user_module = saashq.core.doctype.user.user
...
class TestUser(IntegrationTestCase):
	def tearDown(self):
		...
		saashq.db.set_single_value("System Settings", "enable_password_policy", 0)
		saashq.db.set_single_value("System Settings", "minimum_password_score", "")
	...
	def test_user_type(self):
		user_id = saashq.generate_hash() + "@example.com"
		new_user = saashq.get_doc(doctype="User", email=user_id, first_name="Tester").insert()
		self.assertEqual(new_user.user_type, "Website User")

		# social login userid for saashq
		self.assertTrue(new_user.social_logins[0].userid)
		self.assertEqual(new_user.social_logins[0].provider, "saashq")
		...
		saashq.delete_doc("User", new_user.name)

	def test_delete(self):
		saashq.get_doc("User", "test@example.com").add_roles("_Test Role 2")
		self.assertRaises(saashq.LinkExistsError, delete_doc, "Role", "_Test Role 2")
		saashq.db.delete("Has Role", {"role": "_Test Role 2"})
		delete_doc("Role", "_Test Role 2")

		if saashq.db.exists("User", "_test@example.com"):
			delete_contact("_test@example.com")
			delete_doc("User", "_test@example.com")

		user = saashq.copy_doc(self.globalTestRecords["User"][1])
		user.email = "_test@example.com"
		user.insert()

		saashq.get_doc({"doctype": "ToDo", "description": "_Test"}).insert()

		delete_contact("_test@example.com")
		delete_doc("User", "_test@example.com")

		self.assertTrue(
			not saashq.db.sql("""select * from `tabToDo` where allocated_to=%s""", ("_test@example.com",))
		)

		saashq.copy_doc(self.globalTestRecords["Role"][1]).insert()

	def test_get_value(self):
		self.assertEqual(saashq.db.get_value("User", "test@example.com"), "test@example.com")
		self.assertEqual(saashq.db.get_value("User", {"email": "test@example.com"}), "test@example.com")
		self.assertEqual(
			saashq.db.get_value("User", {"email": "test@example.com"}, "email"), "test@example.com"
		)
		self.assertEqual(
			saashq.db.get_value("User", {"email": "test@example.com"}, ["first_name", "email"]),
		...
		test_user = saashq.db.sql("select * from tabUser where name='test@example.com'", as_dict=True)[0]
		self.assertEqual(
			saashq.db.get_value("User", {"email": "test@example.com"}, "*", as_dict=True), test_user
		)

		self.assertEqual(saashq.db.get_value("User", "xxxtest@example.com"), None)

		saashq.db.set_single_value("Website Settings", "_test", "_test_val")
		self.assertEqual(saashq.db.get_value("Website Settings", None, "_test"), "_test_val")
		self.assertEqual(saashq.db.get_value("Website Settings", "Website Settings", "_test"), "_test_val")

	def test_high_permlevel_validations(self):
		user = saashq.get_meta("User")
		self.assertTrue("roles" in [d.fieldname for d in user.get_high_permlevel_fields()])

		me = saashq.get_doc("User", "testperm@example.com")
	...
	def test_delete_user(self):
		...
		delete_contact(new_user.name)
		saashq.delete_doc("User", new_user.name)
		self.assertFalse(saashq.db.exists("User", new_user.name))
	...
	def test_password_strength(self):
		...
		saashq.db.set_single_value("System Settings", "enable_password_policy", 1)
		saashq.db.set_single_value("System Settings", "minimum_password_score", 2)
		...
		self.assertRaises(saashq.exceptions.ValidationError, handle_password_test_fail, result["feedback"])
		self.assertRaises(
			saashq.exceptions.ValidationError, handle_password_test_fail, result
		...
		self.assertRaises(saashq.exceptions.ValidationError, user.save)
		user.reload()
	...
	def test_comment_mentions(self):
		...
		saashq.delete_doc("User Group", "Team")
		doc = saashq.get_doc(
			{
				"doctype": "User Group",
				"name": "Team",
	...
	def test_rate_limiting_for_reset_password(self):
		url = get_url()
		data = {"cmd": "saashq.core.doctype.user.user.reset_password", "user": "test@test.com"}

		# Clear rate limit tracker to start fresh
		key = f"rl:{data['cmd']}:{data['user']}"
		saashq.cache.delete(key)
	...
	def test_user_rename(self):
		...
		saashq.rename_doc("User", user.name, new_name)
		self.assertTrue(saashq.db.exists("Notification Settings", new_name))

		saashq.delete_doc("User", new_name)

	def test_signup(self):
		import saashq.website.utils

		random_user = saashq.mock("email")
		random_user_name = saashq.mock("name")
		# disabled signup
		with patch.object(user_module, "is_signup_disabled", return_value=True):
			self.assertRaisesRegex(
				saashq.exceptions.ValidationError,
				"Sign Up is disabled",
				sign_up,
				random_user,
				random_user_name,
		...
		self.assertEqual(saashq.cache.hget("redirect_after_login", random_user), "/welcome")

		# re-register
		self.assertTupleEqual(sign_up(random_user, random_user_name, "/welcome"), (0, "Already Registered"))

		# disabled user
		user = saashq.get_doc("User", random_user)
		user.enabled = 0
		user.save()
		...
		with patch.object(user_module.saashq.db, "get_creation_count", return_value=301):
			self.assertRaisesRegex(
				saashq.exceptions.ValidationError,
				"Throttled",
				sign_up,
	...
	def test_reset_password(self):
		...
		saashq.local.request_ip = "127.0.0.69"

		saashq.set_user("testpassword@example.com")
		test_user = saashq.get_doc("User", "testpassword@example.com")
		...
		self.assertRaises(saashq.exceptions.AuthenticationError, verify_password, old_password)
		...
		# test API endpoint
		with patch.object(user_module.saashq, "sendmail") as sendmail:
			saashq.clear_messages()
			test_user = saashq.get_doc("User", "test2@example.com")
			...
			update_password(old_password, old_password=new_password)
			self.assertEqual(
				saashq.message_log[0].get("message"),
...
class TestImpersonation(SaashqAPITestCase):
	def test_impersonation(self):
		with test_user(roles=["System Manager"], commit=True) as user:
			self.post(
				self.method("saashq.core.doctype.user.user.impersonate"),
				{"user": user.name, "reason": "test", "sid": self.sid},
			)
			resp = self.get(self.method("saashq.auth.get_logged_user"))
...
def test_user(
	*, first_name: str | None = None, email: str | None = None, roles: list[str], commit=False, **kwargs
	...
	try:
		first_name = first_name or saashq.generate_hash()
		email = email or (first_name + "@example.com")
		user: User = saashq.new_doc(
			"User",
		...
		commit and saashq.db.commit()


def delete_contact(user):
	saashq.db.delete("Contact", {"email_id": user})
	saashq.db.delete("Contact Email", {"email_id": user})
