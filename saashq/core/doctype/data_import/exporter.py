...
from saashq.model import display_fieldtypes, no_value_fields
from saashq.model import table_fields as table_fieldtypes
from saashq.utils import flt, format_duration, groupby_metric
from saashq.utils.csvutils import build_csv_response
from saashq.utils.xlsxutils import build_xlsx_response
...
class Exporter:
	...
	def get_all_exportable_fields(self):
		...
		exportable_fields = saashq._dict({})
	...
	def serialize_exportable_fields(self):
		...
		for key, exportable_fields in self.exportable_fields.items():
			for _df in exportable_fields:
				...
				if isinstance(_df, saashq.core.doctype.docfield.docfield.DocField):
					df = _df.as_dict()
				else:
					df = _df.copy()
	...
	def get_data_to_export(self):
		saashq.permissions.can_export(self.doctype, raise_exception=True)
		...
		for i, df in enumerate(self.fields):
			if df.parent == doctype:
				...
				if df.fieldtype == "Text Editor" and value:
					value = saashq.core.utils.html2text(value)
				row[i] = value
		return rows
	...
	def get_data_as_docs(self):
		...
		parent_data = saashq.db.get_list(
			self.doctype,
			filters=filters,
