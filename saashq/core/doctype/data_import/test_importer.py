...
from saashq.utils import format_duration, getdate
...
class TestImporter(IntegrationTestCase):
	...
	def test_data_import_from_file(self):
		...
		doc1 = saashq.get_doc(doctype_name, "Test")
		doc2 = saashq.get_doc(doctype_name, "Test 2")
		doc3 = saashq.get_doc(doctype_name, "Test 3")
	...
	def test_data_import_without_mandatory_values(self):
		...
		self.assertEqual(saashq.parse_json(import_log[0]["row_indexes"]), [2, 3])
		expected_error = (
			"Error: <strong>Child 1 of DocType for Import</strong> Row #1: Value missing for: Child Title"
		)
		self.assertEqual(
			saashq.parse_json(saashq.parse_json(import_log[0]["messages"])[0])["message"], expected_error
		)
		expected_error = (
			"Error: <strong>Child 1 of DocType for Import</strong> Row #2: Value missing for: Child Title"
		)
		self.assertEqual(
			saashq.parse_json(saashq.parse_json(import_log[0]["messages"])[1])["message"], expected_error
		)

		self.assertEqual(saashq.parse_json(import_log[1]["row_indexes"]), [4])
		self.assertEqual(
			saashq.parse_json(saashq.parse_json(import_log[1]["messages"])[0])["message"],
			"Title is required",
		)

	def test_data_import_update(self):
		existing_doc = saashq.get_doc(
		...
		updated_doc = saashq.get_doc(doctype_name, existing_doc.name)
...
def create_doctype_if_not_exists(doctype_name, force=False):
	if force:
		...
		saashq.delete_doc_if_exists("DocType", "Child 2 of " + doctype_name)
...
def get_import_file(csv_file_name, force=False):
	...
	_file = saashq.db.exists("File", {"file_name": file_name})
	if force and _file:
		saashq.delete_doc_if_exists("File", _file)

	if saashq.db.exists("File", {"file_name": file_name}):
		f = saashq.get_doc("File", {"file_name": file_name})
	else:
		full_path = get_csv_file_path(file_name)
		f = saashq.get_doc(
			doctype="File", content=saashq.read_file(full_path), file_name=file_name, is_private=1
		)
...
def get_csv_file_path(file_name):
	return saashq.get_app_path("saashq", "core", "doctype", "data_import", "fixtures", file_name)
