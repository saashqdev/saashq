...
from saashq.core.doctype.data_import.test_importer import create_doctype_if_not_exists
...
class TestExporter(IntegrationTestCase):
	...
	def test_exports_specified_fields(self):
		if not saashq.db.exists(doctype_name, "Test"):
			doc = saashq.get_doc(
			...
			doc = saashq.get_doc(doctype_name, "Test")
	...
	def test_export_csv_response(self):
		...
		self.assertEqual(saashq.response["doctype"], doctype_name)
