...
class TestDocumentNamingRule(IntegrationTestCase):
	def test_naming_rule_by_series(self):
		naming_rule = saashq.get_doc(
			doctype="Document Naming Rule", document_type="ToDo", prefix="test-todo-", prefix_digits=5
		).insert()

		todo = saashq.get_doc(
			doctype="ToDo", description="Is this my name " + saashq.generate_hash()
		).insert()
	...
	def test_naming_rule_by_condition(self):
		...
		# another rule
		naming_rule_1 = saashq.copy_doc(naming_rule)
		...
		naming_rule_2 = saashq.copy_doc(naming_rule)
		...
		todo = saashq.get_doc(
			doctype="ToDo", priority="High", description="Is this my name " + saashq.generate_hash()
		).insert()

		todo_1 = saashq.get_doc(
			doctype="ToDo", priority="Medium", description="Is this my name " + saashq.generate_hash()
		).insert()

		todo_2 = saashq.get_doc(
			doctype="ToDo", priority="Low", description="Is this my name " + saashq.generate_hash()
		).insert()
