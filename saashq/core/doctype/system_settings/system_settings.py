...
from saashq.utils import cint, today
...
class SystemSettings(Document):
	...
	def validate(self):
		from saashq.twofactor import toggle_two_factor_auth
		...
		if enable_password_policy and minimum_password_score <= 0:
			saashq.throw(_("Please select Minimum Password Score"))
		...
		if self.session_expiry:
			...
			if len(parts) != 2 or not (cint(parts[0]) or cint(parts[1])):
				saashq.throw(_("Session Expiry must be in format {0}").format("hh:mm"))

		if self.enable_two_factor_auth:
			if self.two_factor_method == "SMS":
				if not saashq.db.get_single_value("SMS Settings", "sms_gateway_url"):
					saashq.throw(
		...
		saashq.flags.update_last_reset_password_date = False
		if self.force_user_to_reset_password and not cint(
			saashq.db.get_single_value("System Settings", "force_user_to_reset_password")
		):
			saashq.flags.update_last_reset_password_date = True
	...
	def validate_user_pass_login(self):
		...
		social_login_enabled = saashq.db.exists("Social Login Key", {"enable_social_login": 1})
		ldap_enabled = saashq.db.get_single_value("LDAP Settings", "enabled")
		login_with_email_link_enabled = saashq.db.get_single_value("System Settings", "login_with_email_link")
	...
	def validate_backup_limit(self):
		if not self.backup_limit or self.backup_limit < 1:
			saashq.msgprint(_("Number of backups must be greater than zero."), alert=True)
			self.backup_limit = 1
	...
	def on_update(self):
		...
		saashq.cache.delete_value("system_settings")
		saashq.cache.delete_value("time_zone")

		if saashq.flags.update_last_reset_password_date:
			update_last_reset_password_date()

	def set_defaults(self):
		from saashq.translate import set_default_language

		for df in self.meta.get("fields"):
			if df.fieldtype not in no_value_fields and self.has_value_changed(df.fieldname):
				saashq.db.set_default(df.fieldname, self.get(df.fieldname))
...
@saashq.whitelist()
def load():
	from saashq.utils.momentjs import get_all_timezones

	if "System Manager" not in saashq.get_roles():
		saashq.throw(_("Not permitted"), saashq.PermissionError)

	all_defaults = saashq.db.get_defaults()
	defaults = {}

	for df in saashq.get_meta("System Settings").get("fields"):
