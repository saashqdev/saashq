saashq.ui.form.on("System Settings", {
	refresh: function (frm) {
		saashq.call({
			method: "saashq.core.doctype.system_settings.system_settings.load",
			callback: function (data) {
				saashq.all_timezones = data.message.timezones;
				frm.set_df_property("time_zone", "options", saashq.all_timezones);

				$.each(data.message.defaults, function (key, val) {
					frm.set_value(key, val, null, true);
					saashq.sys_defaults[key] = val;
				});
				if (frm.re_setup_moment) {
					saashq.app.setup_moment();
	...
	after_save: function (frm) {
		...
		const attr_tuples = [
			[frm.doc.language, saashq.boot.sysdefaults.language, saashq.boot.user.language],
			[frm.doc.rounding_method, saashq.boot.sysdefaults.rounding_method], // no user override.
		];
	...
	rounding_method: function (frm) {
		if (frm.doc.rounding_method == saashq.boot.sysdefaults.rounding_method) return;
		let msg = __(
		...
		saashq.confirm(
			...
			() => {},
			() => {
				frm.set_value("rounding_method", saashq.boot.sysdefaults.rounding_method);
			}
		);
	},
