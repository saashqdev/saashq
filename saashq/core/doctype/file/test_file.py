...
from saashq.core.api.file import (
	create_new_folder,
	get_attached_images,
	get_files_in_folder,
	move_file,
...
from saashq.core.doctype.file.exceptions import FileTypeNotAllowed
from saashq.core.doctype.file.utils import get_corrupted_image_msg, get_extension
...
from saashq.utils import get_files_path, set_request
...
def make_test_doc(ignore_permissions=False):
	...
	d.description = "Test"
	d.assigned_by = saashq.session.user
...
def make_test_image_file(private=False):
	file_path = saashq.get_app_path("saashq", "tests/data/sample_image_for_optimization.jpg")
	with open(file_path, "rb") as f:
		file_content = f.read()

	test_file = saashq.get_doc(
		{
			"doctype": "File",
	...
	# remove those flags
	_test_file: "File" = saashq.get_doc("File", test_file.name)
...
class TestSimpleFile(IntegrationTestCase):
	...
	def test_save(self):
		_file = saashq.get_doc("File", {"file_url": self.saved_file_url})
...
class TestFSRollbacks(IntegrationTestCase):
	def test_rollback_from_file_system(self):
		file_name = content = saashq.generate_hash()
		file = saashq.new_doc("File", file_name=file_name, content=content).insert()
...
class TestExtensionValidations(IntegrationTestCase):
	...
	def test_allowed_extension(self):
		...
		file_name = content = saashq.generate_hash()
		bad_file = saashq.new_doc("File", file_name=f"{file_name}.png", content=content)
		self.assertRaises(FileTypeNotAllowed, bad_file.insert)

		bad_file = saashq.new_doc("File", file_name=f"{file_name}.csv", content=content).insert()
		saashq.db.rollback()
...
class TestBase64File(IntegrationTestCase):
	def setUp(self):
		...
		_file: saashq.Document = saashq.get_doc(
			{
				"doctype": "File",
	...
	def test_saved_content(self):
		_file: saashq.Document = saashq.get_doc("File", {"file_url": self.saved_file_url})
		content = _file.get_content()
...
class TestSameFileName(IntegrationTestCase):
	def test_saved_content(self):
		...
		_file = saashq.get_doc("File", {"file_url": self.saved_file_url1})
		content1 = _file.get_content()
		self.assertEqual(content1, self.test_content1)
		_file = saashq.get_doc("File", {"file_url": self.saved_file_url2})
	...
	def test_saved_content_private(self):
		...
		_file = saashq.get_doc("File", {"file_url": _file1.file_url})
		self.assertEqual(_file.get_content(), test_content1)

		_file = saashq.get_doc("File", {"file_url": _file2.file_url})
...
class TestSameContent(IntegrationTestCase):
	...
	def test_attachment_limit(self):
		...
		self.assertRaises(saashq.exceptions.AttachmentLimitReached, file2.insert)
	...
	def test_utf8_bom_content_decoding(self):
		...
		_file.save()
		saved_file = saashq.get_doc("File", _file.name)
...
class TestFile(IntegrationTestCase):
	...
	def tearDown(self):
		try:
			saashq.get_doc("File", {"file_name": "file_copy.txt"}).delete()
		except saashq.DoesNotExistError:
			pass

	def delete_test_data(self):
		test_file_data = saashq.get_all(
			"File",
			pluck="name",
	...
	def tests_after_upload(self):
		...
		file_folder = saashq.db.get_value("File", self.saved_name, "folder")
		self.assertEqual(file_folder, _("Home/Test Folder 1"))

	def test_file_copy(self):
		folder = self.get_folder("Test Folder 2", "Home")

		file = saashq.get_doc("File", {"file_name": "file_copy.txt"})
	...
	def test_folder_copy(self):
		...
		file = saashq.get_doc("File", {"file_name": "folder_copy.txt"})
		file_copy_txt = saashq.get_value("File", {"file_name": "file_copy.txt"})
		if file_copy_txt:
			saashq.get_doc("File", file_copy_txt).delete()

		self.assertEqual(_("Home/Test Folder 1/Test Folder 3"), file.folder)

	def test_default_folder(self):
		d = saashq.get_doc({"doctype": "File", "file_name": _("Test_Folder"), "is_folder": 1})
		d.save()
	...
	def test_on_delete(self):
		...
		self.assertEqual(saashq.db.get_value("File", _("Home/Test Folder 1"), "file_size"), 0)
		...
		folder = saashq.get_doc("File", "Home/Test Folder 1/Test Folder 3")
	...
	def test_same_file_url_update(self):
		...
		file2 = saashq.get_doc("File", file2.name)
	...
	def test_make_thumbnail(self):
		# test web image
		test_file: "File" = saashq.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": saashq.utils.get_url("/_test/assets/image.jpg"),
			}
		...
		test_file = saashq.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": saashq.utils.get_url("/_test/assets/image"),
			}
		).insert(ignore_permissions=True)
		...
		test_file.db_set("thumbnail_url", None)
		test_file.reload()
		test_file.file_url = saashq.utils.get_url("unknown.jpg")
		test_file.make_thumbnail(suffix="xs")
		self.assertEqual(
			saashq.message_log[0].get("message"),
			f"File '{saashq.utils.get_url('unknown.jpg')}' not found",
		)
		self.assertEqual(test_file.thumbnail_url, None)

	def test_file_unzip(self):
		file_path = saashq.get_app_path("saashq", "www/_test/assets/file.zip")
		public_file_path = saashq.get_site_path("public", "files")
		try:
			import shutil
...
class TestAttachment(IntegrationTestCase):
	...
	def tearDownClass(cls):
		saashq.db.rollback()
		saashq.delete_doc("DocType", cls.test_doctype)

	def test_file_attachment_on_update(self):
		doc = saashq.get_doc(doctype=self.test_doctype, title="test for attachment on update").insert()
...
class TestAttachmentsAccess(IntegrationTestCase):
	def setUp(self) -> None:
		saashq.db.delete("File", {"is_folder": 0})
...
class TestFileUtils(IntegrationTestCase):
	def test_extract_images_from_doc(self):
		is_private = not saashq.get_meta("ToDo").make_attachments_public
		...
		self.assertTrue(saashq.db.exists("File", {"attached_to_name": todo.name, "is_private": is_private}))
		...
		filename = saashq.db.exists("File", {"attached_to_name": todo.name})
		self.assertIn(f'<img src="{saashq.get_doc("File", filename).file_url}', todo.description)
	...
	def test_extract_images_from_comment(self):
		...
		"""
		is_private = not saashq.get_meta("ToDo").make_attachments_public
		test_doc = saashq.get_doc(doctype="ToDo", description="comment test").insert()
		comment = add_comment(
			"ToDo",
			test_doc.name,
		...
		self.assertTrue(
			saashq.db.exists("File", {"attached_to_name": test_doc.name, "is_private": is_private})
		)
	...
	def test_extract_images_from_communication(self):
		...
		"""
		is_private = not saashq.get_meta("Communication").make_attachments_public
		communication = saashq.get_doc(
		...
		self.assertTrue(
			saashq.db.exists("File", {"attached_to_name": communication.name, "is_private": is_private})
		)
...
class TestFileOptimization(IntegrationTestCase):
	...
	def test_optimize_svg(self):
		file_path = saashq.get_app_path("saashq", "tests/data/sample_svg.svg")
		with open(file_path, "rb") as f:
			file_content = f.read()
		test_file = saashq.get_doc(
	...
	def test_optimize_folder(self):
		test_folder = saashq.get_doc("File", "Home/Attachments")
...
class TestGuestFileAndAttachments(IntegrationTestCase):
	...
	def tearDown(self) -> None:
		saashq.set_user("Administrator")
		saashq.db.rollback()
		saashq.delete_doc("DocType", "Test For Attachment")
	...
	def test_attach_unattached_guest_file(self):
		...
		d = saashq.new_doc("Test For Attachment")
	...
	def test_private_remains_private_even_if_same_hash(self):
		file_name = "test" + saashq.generate_hash()
		content = file_name.encode()

		doc_pub: "File" = saashq.new_doc("File")  # type: ignore
		doc_pub.file_url = f"/files/{file_name}.txt"
		doc_pub.content = content
		doc_pub.save()

		doc_pri: "File" = saashq.new_doc("File")  # type: ignore
