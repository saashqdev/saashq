...
import saashq
from saashq.cache_manager import clear_doctype_cache
...
from saashq.core.doctype.rq_job.test_rq_job import wait_for_completion
from saashq.custom.doctype.custom_field.custom_field import create_custom_fields
from saashq.desk.form.load import getdoc
from saashq.model.delete_doc import delete_controllers
from saashq.model.sync import remove_orphan_doctypes
...
class TestDocType(IntegrationTestCase):
	...
	def test_validate_name(self):
		self.assertRaises(saashq.NameError, new_doctype("_Some DocType").insert)
		self.assertRaises(saashq.NameError, new_doctype("8Some DocType").insert)
		self.assertRaises(saashq.NameError, new_doctype("Some (DocType)").insert)
		self.assertRaises(
			saashq.NameError,
		...
		for name in ("Some DocType", "Some_DocType", "Some-DocType"):
			if saashq.db.exists("DocType", name):
				saashq.delete_doc("DocType", name)

			doc = new_doctype(name).insert()
			doc.delete()

	def test_making_sequence_on_change(self):
		saashq.delete_doc_if_exists("DocType", self._testMethodName)
		...
		self.assertEqual(
			...
			"varchar" if saashq.db.db_type == "mariadb" else "character varying",
		)

	def test_doctype_unique_constraint_dropped(self):
		if saashq.db.exists("DocType", "With_Unique"):
			saashq.delete_doc("DocType", "With_Unique")

		dt = new_doctype("With_Unique", unique=1)
		dt.insert()

		doc1 = saashq.new_doc("With_Unique")
		doc2 = saashq.new_doc("With_Unique")
		...
		doc1.insert()
		self.assertRaises(saashq.UniqueValidationError, doc2.insert)
		saashq.db.rollback()
	...
	def test_sync_field_order(self):
		...
		saashq.delete_doc_if_exists("DocType", "Test Field Order DocType")
		if os.path.isfile(path):
			os.remove(path)

		try:
			saashq.flags.allow_doctype_export = 1
			test_doctype.save()

			# assert that field_order list is being created with the default order
			test_doctype_json = saashq.get_file_json(path)
			...
			with open(path, "w+") as txtfile:
				txtfile.write(saashq.as_json(test_doctype_json))
			...
			saashq.reload_doctype(test_doctype.name, force=True)
			test_doctype.reload()
			...
			saashq.flags.allow_doctype_export = 0
	...
	def test_cancel_link_doctype(self):
		...
		from saashq.desk.form.linked_with import cancel_all_linked_docs, get_submitted_linked_docs
		...
		# create doctype data
		data_link_doc = saashq.new_doc("Test Linked Doctype")
		data_link_doc.some_fieldname = "Data1"
		data_link_doc.insert()
		data_link_doc.save()
		data_link_doc.submit()

		data_doc = saashq.new_doc("Test Doctype")
	...
	def test_ignore_cancelation_of_linked_doctype_during_cancel(self):
		...
		data_doc = saashq.new_doc("Test Doctype 2")
		data_doc.some_fieldname = "Data1"
		...
		self.assertRaises(saashq.LinkExistsError, data_link_doc_1.cancel)
	...
	def test_create_virtual_doctype(self):
		...
		virtual_doc.reload()
		doc = saashq.get_doc("DocType", "Test Virtual Doctype")

		self.assertDictEqual(doc.as_dict(), virtual_doc.as_dict())
		self.assertEqual(doc.is_virtual, 1)
		self.assertFalse(saashq.db.table_exists("Test Virtual Doctype"))

	def test_create_virtual_doctype_as_child_table(self):
		"""Test virtual DocType as Child Table below a normal DocType."""
		saashq.delete_doc_if_exists("DocType", "Test Parent Virtual DocType", force=1)
		saashq.delete_doc_if_exists("DocType", "Test Virtual DocType as Child Table", force=1)
		...
		doc = saashq.get_doc("DocType", "Test Virtual DocType as Child Table")

		self.assertEqual(doc.is_virtual, 1)
		self.assertEqual(doc.istable, 1)
		self.assertFalse(saashq.db.table_exists("Test Virtual DocType as Child Table"))
		...
		self.assertRaises(saashq.exceptions.ValidationError, parent_doc.insert)
		parent_doc.is_virtual = 1
		parent_doc.insert(ignore_permissions=True)
		self.assertFalse(saashq.db.table_exists("Test Parent Virtual DocType"))
	...
	def test_autoincremented_doctype_transition(self):
		saashq.delete_doc_if_exists("DocType", "testy_autoinc_dt")
		dt = new_doctype("testy_autoinc_dt", autoname="autoincrement").insert(ignore_permissions=True)
		dt.autoname = "hash"

		dt.save(ignore_permissions=True)

		dt_data = saashq.get_doc({"doctype": dt.name, "some_fieldname": "test data"}).insert(
		...
		try:
			dt.save(ignore_permissions=True)
		except saashq.ValidationError as e:
			self.assertEqual(
				e.args[0],
	...
	def test_json_field(self):
		...
		json_doc.insert()
		json_doc.save()
		doc = saashq.get_doc("DocType", "Test Json Doctype")
		...
		test_json = saashq.get_doc("Test Json Doctype", doc.name)
	...
	def test_no_delete_doc(self):
		self.assertRaises(saashq.ValidationError, saashq.delete_doc, "DocType", "Address")
	...
	def test_custom_field_deletion(self):
		...
		saashq.delete_doc("DocType", child)
		self.assertFalse(saashq.get_meta(doctype).get_field(field))
	...
	def test_delete_doctype_with_customization(self):
		...
		# Create 1 record
		original_doc = saashq.get_doc(doctype=doctype, custom_field_name="wat").insert()
		...
		restored_doc = saashq.get_last_doc(doctype)
		...
		self.assertEqual(saashq.get_meta(doctype).get_field(field).default, "DELETETHIS")
		saashq.delete_doc("DocType", doctype)
	...
	def test_delete_orphaned_doctypes(self):
		...
		job = saashq.enqueue(remove_orphan_doctypes)
		wait_for_completion(job)

		saashq.db.rollback()
		self.assertFalse(saashq.db.exists("DocType", doctype.name))
	...
	def test_not_in_list_view_for_not_allowed_mandatory_field(self):
		...
		saashq.delete_doc("DocType", doctype.name)
	...
	def test_no_recursive_fetch(self):
		...
		self.assertRaises(saashq.ValidationError, recursive_dt.insert)
