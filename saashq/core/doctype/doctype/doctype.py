...
from saashq.cache_manager import clear_controller_cache, clear_user_cache
...
from saashq.database.schema import validate_column_length, validate_column_name
from saashq.desk.notifications import delete_notification_count_for, get_filters_for
from saashq.desk.utils import validate_route_conflict
from saashq.model import (
	child_table_fields,
	data_field_options,
	default_fields,
	no_value_fields,
...
from saashq.modules import get_doc_path, make_boilerplate
from saashq.modules.import_file import get_file_path
from saashq.permissions import ALL_USER_ROLE, AUTOMATIC_ROLES, SYSTEM_USER_ROLE
from saashq.query_builder.functions import Concat
from saashq.utils import cint, flt, get_datetime, is_a_property, random_string
from saashq.website.utils import clear_cache

if TYPE_CHECKING:
	from saashq.custom.doctype.customize_form.customize_form import CustomizeForm
...
class InvalidFieldNameError(saashq.ValidationError):
	pass


class UniqueFieldnameError(saashq.ValidationError):
	pass


class IllegalMandatoryError(saashq.ValidationError):
	pass


class DoctypeLinkError(saashq.ValidationError):
	pass


class WrongOptionsDoctypeLinkError(saashq.ValidationError):
	pass


class HiddenAndMandatoryWithoutDefaultError(saashq.ValidationError):
	pass


class NonUniqueError(saashq.ValidationError):
	pass


class CannotIndexedError(saashq.ValidationError):
	pass


class CannotCreateStandardDoctypeError(saashq.ValidationError):
	pass
...
class DocType(Document):
	...
	if TYPE_CHECKING:
		from saashq.core.doctype.docfield.docfield import DocField
		from saashq.core.doctype.docperm.docperm import DocPerm
	...
	def validate(self):
		...
		if not self.is_new():
			self.before_update = saashq.get_doc("DocType", self.name)
		...
		if self.default_print_format and not self.custom:
			saashq.throw(_("Standard DocType cannot have default print format, use Customize Form"))
	...
	def check_indexing_for_dashboard_links(self):
		...
		for d in self.fields:
			if d.fieldtype == "Link" and not d.unique and not d.search_index:
				referred_as_link = saashq.db.exists(
					"DocType Link",
	...
	def check_developer_mode(self):
		...
		if saashq.flags.in_patch or saashq.flags.in_test:
			return

		if not saashq.conf.get("developer_mode") and not self.custom:
			saashq.throw(
		...
		if self.is_virtual and self.custom:
			saashq.throw(_("Not allowed to create custom Virtual DocType."), CannotCreateStandardDoctypeError)

		if saashq.conf.get("developer_mode"):
	...
	def setup_fields_to_fetch(self):
		...
		try:
			old_meta = saashq.get_meta(saashq.get_doc("DocType", self.name), cached=False)
			old_fields_to_fetch = [df.fieldname for df in old_meta.get_fields_to_fetch()]
		except saashq.DoesNotExistError:
			old_fields_to_fetch = []

		new_meta = saashq.get_meta(self, cached=False)
	...
	def validate_website(self):
		...
		if self.has_web_view:
			...
			if "route" not in [d.fieldname for d in self.fields]:
				saashq.throw(_('Field "route" is mandatory for Web Views'), title="Missing Field")
	...
	def validate_virtual_doctype_methods(self):
		...
		from saashq.model.virtual_doctype import validate_controller
	...
	def ensure_minimum_max_attachment_limit(self):
		...
		from saashq.model import attachment_fieldtypes
		...
		if total_attach_fields > self.max_attachments:
			...
			field_label = saashq.bold(self.meta.get_field("max_attachments").label)
			saashq.msgprint(
	...
	def change_modified_of_parent(self):
		...
		if saashq.flags.in_import:
			return
		parent_list = saashq.get_all(
			"DocField", "parent", dict(fieldtype=["in", saashq.model.table_fields], options=self.name)
		)
		for p in parent_list:
			saashq.db.set_value("DocType", p.parent, {})
	...
	def on_update(self):
		...
		try:
			saashq.db.updatedb(self.name, Meta(self))
		...
		allow_doctype_export = (
			...
			and (saashq.conf.developer_mode or saashq.flags.allow_doctype_export)
		)
		...
		saashq.clear_cache(doctype=self.name)

		# clear user cache so that on the next reload this doctype is included in boot
		clear_user_cache(saashq.session.user)

		if not saashq.flags.in_install and hasattr(self, "before_update"):
	...
	def setup_autoincrement_and_sequence(self):
		...
		name_type = f"varchar({saashq.db.VARCHAR_LEN})"

		if self.autoname == "autoincrement":
			name_type = "bigint"
			saashq.db.create_sequence(self.name, check_not_exists=True)
	...
	def sync_global_search(self):
		...
		if set(global_search_fields_before_update) != set(global_search_fields_after_update):
			now = (not saashq.request) or saashq.flags.in_test or saashq.flags.in_install
			saashq.enqueue("saashq.utils.global_search.rebuild_for_doctype", now=now, doctype=self.name)
	...
	def set_base_class_for_controller(self):
		...
		scrubbed_name = saashq.scrub(self.name)
		scrubbed_module = saashq.scrub(self.module)
		controller_path = saashq.get_module_path(
			scrubbed_module, "doctype", scrubbed_name, f"{scrubbed_name}.py"
		)

		document_cls_tag = f"class {despaced_name}(Document)"
		document_import_tag = "from saashq.model.document import Document"
		website_generator_cls_tag = f"class {despaced_name}(WebsiteGenerator)"
		website_generator_import_tag = "from saashq.website.website_generator import WebsiteGenerator"
	...
	def before_rename(self, old, new, merge=False):
		...
		if not self.custom and saashq.session.user != "Administrator":
			saashq.throw(_("DocType can only be renamed by Administrator"))

		self.check_developer_mode()
		self.validate_name(new)

		if merge:
			saashq.throw(_("DocType can not be merged"))
	...
	def after_rename(self, old, new, merge=False):
		...
		if self.issingle:
			saashq.db.sql("""update tabSingles set doctype=%s where doctype=%s""", (new, old))
			saashq.db.sql(
	...
	def rename_files_and_folders(self, old, new):
		...
		for fname in os.listdir(new_path):
			if saashq.scrub(old) in fname:
				old_file_name = os.path.join(new_path, fname)
				new_file_name = os.path.join(new_path, fname.replace(saashq.scrub(old), saashq.scrub(new)))
				shutil.move(old_file_name, new_file_name)

		self.rename_inside_controller(new, old, new_path)
		saashq.msgprint(_("Renamed files and replaced code in controllers, please check!"))

	def rename_inside_controller(self, new, old, new_path):
		for fname in ("{}.js", "{}.py", "{}_list.js", "{}_calendar.js", "test_{}.py", "test_{}.js"):
			fname = os.path.join(new_path, fname.format(saashq.scrub(new)))
		...
		doctype_json_path = os.path.join(new_path, f"{saashq.scrub(new)}.json")
		current_data = saashq.get_file_json(doctype_json_path)
	...
	def preserve_naming_series_options_in_property_setter(self):
		...
		if not (
			saashq.db.table_exists(self.name)
			and saashq.get_all(self.name, fields=["name"], limit=1, as_list=True)
		):
			return

		existing_property_setter = saashq.db.get_value(
	...
	def make_controller_template(self):
		...
		if self.has_web_view:
			templates_path = saashq.get_module_path(
				saashq.scrub(self.module), "doctype", saashq.scrub(self.name), "templates"
			)
	...
	def export_types_to_controller(self):
		...
		from saashq.types.exporter import TypeExporter
		...
		if any(saashq.get_hooks("export_python_type_annotations", app_name=app)):
	...
	def make_repeatable(self):
		...
		if self.allow_auto_repeat:
			if not saashq.db.exists(
				"Custom Field", {"fieldname": "auto_repeat", "dt": self.name}
			) and not saashq.db.exists("DocField", {"fieldname": "auto_repeat", "parent": self.name}):
	...
	def validate_nestedset(self):
		...
		if not self.nsm_parent_field:
			field_label = saashq.bold(_("Parent Field (Tree)"))
			saashq.throw(_("{0} is a mandatory field").format(field_label), saashq.MandatoryError)

		# check if field is valid
		fieldnames = [df.fieldname for df in self.fields]
		if self.nsm_parent_field and self.nsm_parent_field not in fieldnames:
			saashq.throw(_("Parent Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def add_nestedset_fields(self):
		...
		parent_field_name = saashq.scrub(parent_field_label)
		self.append(
			"fields",
			{
	...
	def get_max_idx(self):
		...
		max_idx = saashq.db.sql("""select max(idx) from `tabDocField` where parent = %s""", self.name)
	...
	def validate_name(self, name=None):
		...
		max_length = saashq.db.MAX_COLUMN_LENGTH - 3
		...
		if WHITESPACE_PADDING_PATTERN.search(name):
			saashq.throw(_("DocType's name should not start or end with whitespace"), saashq.NameError)
	...
	def check_pending_migration(self) -> bool:
		...
		file = Path(get_file_path(saashq.scrub(self.module), self.doctype, self.name))
...
def validate_series(dt, autoname=None, name=None):
	...
	if not autoname and dt.get("fields", {"fieldname": "naming_series"}):
		...
		if not dt.get("fields", {"fieldname": fieldname}):
			saashq.throw(
				_("Fieldname called {0} must exist to enable autonaming").format(saashq.bold(fieldname)),
	...
	if autoname and autoname.startswith("field:"):
		...
		if not field or field not in [df.fieldname for df in dt.fields]:
			saashq.throw(_("Invalid fieldname '{0}' in autoname").format(field))
		else:
			for df in dt.fields:
	...
	if (
		...
		if used_in:
			saashq.throw(_("Series {0} already used in {1}").format(prefix, used_in[0][0]))
...
def validate_empty_name(dt, autoname):
	...
	if not autoname and not (dt.issingle or dt.istable):
		...
		if not controller or (not hasattr(controller, "autoname")):
			saashq.toast(_("Warning: Naming is not set"), indicator="yellow")
...
def validate_autoincrement_autoname(dt: Union[DocType, "CustomizeForm"]) -> bool:
	...
	def get_autoname_before_save(dt: Union[DocType, "CustomizeForm"]) -> str:
		if dt.doctype == "Customize Form":
			...
			if not property_value:
				return saashq.db.get_value("DocType", dt.doc_type, "autoname") or ""
	...
	if not dt.is_new():
		...
		if (
			...
			if dt.doctype == "Customize Form":
				saashq.throw(_("Cannot change to/from autoincrement autoname in Customize Form"))

			if saashq.get_meta(dt.name).issingle:
				return False

			if not saashq.get_all(dt.name, limit=1):
...
def change_name_column_type(doctype_name: str, type: str) -> None:
	...
	saashq.db.change_column_type(*args)


def validate_links_table_fieldnames(meta):
	"""Validate fieldnames in Links table"""
	if not meta.links or saashq.flags.in_patch or saashq.flags.in_fixtures or saashq.flags.in_migrate:
		return
	...
	for index, link in enumerate(meta.links, 1):
		...
		if not link.parent_doctype:
			...
			saashq.throw(message, saashq.ValidationError, _("Parent Missing"))

		if not link.table_fieldname:
			message = _("Document Links Row #{0}: Table Fieldname is mandatory for internal links").format(
				index
			)
			saashq.throw(message, saashq.ValidationError, _("Table Fieldname Missing"))

		if meta.name == link.parent_doctype:
			field_exists = link.table_fieldname in fieldnames
		else:
			field_exists = saashq.get_meta(link.parent_doctype).has_field(link.table_fieldname)

		if not field_exists:
			message = _("Document Links Row #{0}: Could not find field {1} in {2} DocType").format(
				index, saashq.bold(link.table_fieldname), saashq.bold(meta.name)
			)
			saashq.throw(message, saashq.ValidationError, _("Invalid Table Fieldname"))
...
def _test_connection_query(doctype, field, idx):
	...
	try:
		saashq.get_all(doctype, filters=filters, limit=1, distinct=True, ignore_ifnull=True)
	except Exception as e:
		saashq.clear_last_message()
		msg = _("Document Links Row #{0}: Invalid doctype or fieldname.").format(idx)
		msg += "<br>" + str(e)
		saashq.throw(msg, InvalidFieldNameError)
...
def validate_fields(meta: Meta):
	...
	:param meta: `saashq.model.meta.Meta` object to check."""
	...
	def check_link_table_options(docname, d):
		if saashq.flags.in_patch or saashq.flags.in_fixtures:
			return
		...
		if d.fieldtype in ("Link", *table_fields):
			...
			if d.options != d.parent:
				options = saashq.db.get_value("DocType", d.options, "name")
				if not options:
					saashq.throw(
	...
	def check_hidden_and_mandatory(docname, d):
		if d.hidden and d.reqd and not d.default and not saashq.flags.in_migrate:
			saashq.throw(
	...
	def check_width(d):
		if d.fieldtype == "Currency" and cint(d.width) < 100:
			saashq.throw(_("Max width for type Currency is 100px in row {0}").format(d.idx))
	...
	def check_precision(d):
		if (
			...
			saashq.throw(_("Precision should be between 1 and 6"))
	...
	def check_unique_and_text(docname, d):
		...
		if getattr(d, "unique", False):
			...
			if not d.get("__islocal") and saashq.db.has_column(d.parent, d.fieldname):
				has_non_unique_values = saashq.db.sql(
	...
	def check_fold(fields):
		...
		for i, f in enumerate(fields):
			if f.fieldtype == "Fold":
				if fold_exists:
					saashq.throw(_("There can be only one Fold in a form"))
				fold_exists = True
				if i < len(fields) - 1:
					nxt = fields[i + 1]
					if nxt.fieldtype != "Section Break":
						saashq.throw(_("Fold must come before a Section Break"))
				else:
					saashq.throw(_("Fold can not be at the end of the form"))
	...
	def check_search_fields(meta, fields):
		...
		for fieldname in search_fields:
			...
			if (fieldtype_mapper.get(fieldname) in no_value_fields) or (fieldname not in fieldname_list):
				saashq.throw(_("Search field {0} is not valid").format(fieldname))
	...
	def check_title_field(meta):
		...
		if meta.title_field not in fieldname_list:
			saashq.throw(_("Title field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_image_field(meta):
		...
		if not df:
			saashq.throw(_("Image field must be a valid fieldname"), InvalidFieldNameError)
		if df[0].fieldtype != "Attach Image":
			saashq.throw(_("Image field must be of type Attach Image"), InvalidFieldNameError)
	...
	def check_is_published_field(meta):
		...
		if meta.is_published_field not in fieldname_list:
			saashq.throw(_("Is Published Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_website_search_field(meta):
		...
		if meta.website_search_field not in fieldname_list:
			saashq.throw(_("Website Search Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_timeline_field(meta):
		...
		if meta.timeline_field not in fieldname_list:
			saashq.throw(_("Timeline field must be a valid fieldname"), InvalidFieldNameError)

		df = meta.get("fields", {"fieldname": meta.timeline_field})[0]
		if df.fieldtype not in ("Link", "Dynamic Link"):
			saashq.throw(_("Timeline field must be a Link or Dynamic Link"), InvalidFieldNameError)
	...
	def check_illegal_depends_on_conditions(docfield):
		...
		for field in depends_on_fields:
			...
			if depends_on and ("=" in depends_on) and DEPENDS_ON_PATTERN.match(depends_on):
				saashq.throw(_("Invalid {0} condition").format(saashq.unscrub(field)), saashq.ValidationError)
	...
	def validate_fetch_from(field):
		...
		if source_field == field.fieldname:
			...
			saashq.throw(msg, title=_("Recursive Fetch From"))
	...
	def validate_data_field_type(docfield):
		...
		if docfield.fieldtype == "Data" and not (docfield.oldfieldtype and docfield.oldfieldtype != "Data"):
			if docfield.options and (docfield.options not in data_field_options):
				df_str = saashq.bold(_(docfield.label, context=docfield.parent))
				text_str = (
				...
				saashq.msgprint(text_str + df_options_str, title="Invalid Data Field", alert=True)

	def check_child_table_option(docfield):
		if saashq.flags.in_fixtures:
			return
		if docfield.fieldtype not in ["Table MultiSelect", "Table"]:
			return

		doctype = docfield.options
		child_doctype_meta = saashq.get_meta(doctype)

		if not child_doctype_meta.istable:
			saashq.throw(
				_("Option {0} for field {1} is not a child table").format(
					saashq.bold(doctype), saashq.bold(docfield.fieldname)
				),
	...
	def check_max_height(docfield):
		if getattr(docfield, "max_height", None) and (docfield.max_height[-2:] not in ("px", "em")):
			saashq.throw(f"Max for {saashq.bold(docfield.fieldname)} height must be in px, em, rem")
	...
	def check_no_of_ratings(docfield):
		if docfield.fieldtype == "Rating":
			if docfield.options and (int(docfield.options) > 10 or int(docfield.options) < 3):
				saashq.throw(_("Options for Rating field can range from 3 to 10"))
	...
	for d in fields:
		...
		if not saashq.flags.in_migrate or in_ci:
...
def validate_permissions_for_doctype(doctype, for_remove=False, alert=False):
	...
	doctype = saashq.get_doc("DocType", doctype)
...
def validate_permissions(doctype, for_remove=False, alert=False):
	...
	if not permissions and alert:
		saashq.msgprint(_("No Permissions Specified"), alert=True, indicator="orange")
	...
	def check_atleast_one_set(d):
		if not d.select and not d.read and not d.write and not d.submit and not d.cancel and not d.create:
			saashq.throw(_("{0}: No basic permissions set").format(get_txt(d)))
	...
	def check_permission_dependency(d):
		if d.cancel and not d.submit:
			saashq.throw(_("{0}: Cannot set Cancel without Submit").format(get_txt(d)))

		if (d.submit or d.cancel or d.amend) and not d.write:
			saashq.throw(_("{0}: Cannot set Submit, Cancel, Amend without Write").format(get_txt(d)))
		if d.amend and not d.write:
			saashq.throw(_("{0}: Cannot set Amend without Cancel").format(get_txt(d)))
		if d.get("import") and not d.create:
			saashq.throw(_("{0}: Cannot set Import without Create").format(get_txt(d)))
	...
	def remove_rights_for_single(d):
		...
		if d.report:
			saashq.msgprint(_("Report cannot be set for Single types"))
			d.report = 0
			d.set("import", 0)
			d.set("export", 0)
	...
	def check_if_submittable(d):
		if d.submit and not issubmittable:
			saashq.throw(_("{0}: Cannot set Assign Submit if not Submittable").format(get_txt(d)))
		elif d.amend and not issubmittable:
			saashq.throw(_("{0}: Cannot set Assign Amend if not Submittable").format(get_txt(d)))

	def check_if_importable(d):
		if d.get("import") and not isimportable:
			saashq.throw(_("{0}: Cannot set import as {1} is not importable").format(get_txt(d), doctype))
		...
		if doctype.custom:
			if d.role in AUTOMATIC_ROLES:
				saashq.throw(
					...
					).format(d.idx, saashq.bold(_(d.role))),
					title=_("Permissions Error"),
				)

			roles = [row.name for row in saashq.get_all("Role", filters={"is_custom": 1})]
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
	...
	try:
		if (
			...
			and not saashq.db.exists("Domain", doc.restrict_to_domain)
		):
			saashq.get_doc(doctype="Domain", domain=doc.restrict_to_domain).insert()

		if "tabModule Def" in saashq.db.get_tables() and not saashq.db.exists("Module Def", doc.module):
			m = saashq.get_doc({"doctype": "Module Def", "module_name": doc.module})
			if saashq.scrub(doc.module) in saashq.local.module_app:
				m.app_name = saashq.local.module_app[saashq.scrub(doc.module)]
			else:
				m.app_name = "saashq"
			m.flags.ignore_mandatory = m.flags.ignore_permissions = True
			if saashq.flags.package:
				m.package = saashq.flags.package.name
				m.custom = 1
			m.insert()
		...
		for role in list(set(roles)):
			if saashq.db.table_exists("Role", cached=False) and not saashq.db.exists("Role", role):
				r = saashq.new_doc("Role")
				r.role_name = role
				r.desk_access = 1
...
def check_fieldname_conflicts(docfield):
	...
	doc = saashq.get_doc({"doctype": docfield.dt})
	...
	if docfield.fieldname in method_list + property_list:
		saashq.msgprint(msg, raise_exception=not docfield.is_virtual)


def clear_linked_doctype_cache():
	saashq.cache.delete_value("linked_doctypes_without_ignore_user_permissions_enabled")
...
def check_email_append_to(doc):
	...
	if doc.subject_field and not subject_field:
		saashq.throw(_("Select a valid Subject field for creating documents from Email"))
	...
	if subject_field and subject_field.fieldtype not in [
		...
		saashq.throw(_("Subject Field type should be Data, Text, Long Text, Small Text, Text Editor"))
	...
	if doc.sender_field and not sender_field:
		saashq.throw(_("Select a valid Sender Field for creating documents from Email"))

	if sender_field.options != "Email":
		saashq.throw(_("Sender Field should have Email in options"))
...
def get_row_size_utilization(doctype: str) -> float:
	...
	try:
		return flt(saashq.db.get_row_size(doctype) / saashq.db.MAX_ROW_SIZE_LIMIT * 100, 2)
	except Exception:
		return 0.0
