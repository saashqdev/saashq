...
saashq.ui.form.on("Installed Applications", {
	...
	show_update_order_dialog() {
		const dialog = new saashq.ui.Dialog({
			...
			primary_action: function () {
				...
				saashq.call({
					method: "saashq.core.doctype.installed_applications.installed_applications.update_installed_apps_order",
					freeze: true,
					args: {
						new_order: new_order,
					},
		...
		saashq
			.xcall(
				"saashq.core.doctype.installed_applications.installed_applications.get_installed_app_order"
			)
			.then((data) => {
				data.forEach((app) => {
				...
				grid.find(".grid-row-check:first").remove() &&
					grid.find(".grid-row-check").replaceWith(saashq.utils.icon("menu"));
				dialog.show();
			});
	},
});
