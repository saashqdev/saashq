saashq.pages["permission-manager"].on_page_load = (wrapper) => {
	let page = saashq.ui.make_app_page({
		parent: wrapper,
	...
	$(saashq.render_template("permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new saashq.PermissionEngine(wrapper);
};

saashq.pages["permission-manager"].refresh = function (wrapper) {
	wrapper.permission_engine.set_from_route();
};

saashq.PermissionEngine = class PermissionEngine {
	constructor(wrapper) {
	...
	setup_page() {
		this.doctype_select = this.wrapper.page.add_field({
			...
			options: "DocType",
			change: function () {
				saashq.set_route("permission-manager", this.get_value());
			},
		});
		...
		this.page.add_inner_button(__("Set User Permissions"), () => {
			return saashq.set_route("List", "User Permission");
		});
		this.set_from_route();
	}
	...
	set_from_route() {
		...
		if (saashq.get_route()[1]) {
			this.doctype_select.set_value(saashq.get_route()[1]);
		} else if (saashq.route_options) {
			if (saashq.route_options.doctype) {
				this.doctype_select.set_value(saashq.route_options.doctype);
			}
			if (saashq.route_options.role) {
				this.role_select.set_value(saashq.route_options.role);
			}
	...
	reset_std_permissions(data) {
		let doctype = this.get_doctype();
		let d = saashq.confirm(__("Reset Permissions for {0}?", [__(doctype)]), () => {
			return saashq
				.call({
					module: "saashq.core",
					page: "permission_manager",
		...
		data.message.forEach((d) => {
			let rights = this.rights
				.filter((r) => d[r])
				.map((r) => {
					return __(toTitle(saashq.unscrub(r)));
				});
	...
	add_delete_button(row, d) {
		$(
			`<button class='btn btn-danger btn-remove-perm btn-xs'>${saashq.utils.icon(
				"delete"
			)}</button>`
		)
			...
			.on("click", () => {
				return saashq.call({
					...
					callback: (r) => {
						if (r.exc) {
							saashq.msgprint(__("Did not remove"));
						} else {
							this.refresh();
						}
					},
	...
	add_check_events() {
		...
		this.body.on("click", ".show-user-permissions", () => {
			saashq.route_options = { allow: this.get_doctype() || "" };
			...
			() => {
				...
				d.set_primary_action(__("Add"), () => {
					...
					saashq.call({
						...
						callback: (r) => {
							if (r.exc) {
								saashq.msgprint(__("Did not add"));
							} else {
								this.refresh();
							}
						},
	...
	get_link_fields(doctype) {
		return saashq.get_children("DocType", doctype, "fields", {
			fieldtype: "Link",
