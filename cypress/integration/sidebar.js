...
context("Sidebar", () => {
	before(() => {
		...
		return cy
			...
			.its("saashq")
			.then((saashq) => {
				return saashq.call("saashq.tests.ui_test_helpers.create_blog_post");
			});
	});

	it("Verify attachment visibility config", () => {
		cy.call("saashq.tests.ui_test_helpers.create_todo", {
	...
	it("Verify attachment accessibility UX", () => {
		cy.call("saashq.tests.ui_test_helpers.create_todo_with_attachment_limit", {
