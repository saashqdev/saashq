context("Realtime updates", () => {
	...
	it("Recieves custom messages from server", { scrollBehavior: false }, () => {
		...
		cy.window()
			.its("saashq")
			.then((saashq) => {
				saashq.realtime.on(event, () => handler.handle());
			});
	...
	it("Progress bar", { scrollBehavior: false }, () => {
		...
		cy.call("saashq.tests.ui_test_helpers.publish_progress", { title }).then(() => {
...
function publish_realtime(args) {
	return cy.call("saashq.tests.ui_test_helpers.publish_realtime", args);
}
