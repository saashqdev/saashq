context("Table MultiSelect", () => {
	...
	it("select value from multiselect dropdown", () => {
		...
		cy.get("@input").type("test@erpnexus", { delay: 100 });
		cy.wait(500);
		cy.get("@input").type("{enter}");
		cy.get(
			'.saashq-control[data-fieldname="users"] .form-control .tb-selected-value .btn-link-to-form'
		).as("selected-value");
		cy.get("@selected-value").should("contain", "test@erpnexus.com");

		cy.intercept("POST", "/api/method/saashq.desk.form.save.savedocs").as("save_form");
		// trigger save
	...
	it("delete value using backspace", () => {
		...
		cy.get('.saashq-control[data-fieldname="users"] .form-control .tb-selected-value').should(
			"not.exist"
		);
	});
	...
	it("delete value using x", () => {
		cy.go_to_list("Assignment Rule");
		cy.get(`.list-subject:contains("table multiselect")`).last().find("a").click();
		cy.get('.saashq-control[data-fieldname="users"] .form-control .tb-selected-value').as(
			"existing_value"
		);
	...
	it("navigate to selected value", () => {
		...
		cy.location("pathname").should("contain", "/user/test%40erpnexus.com");
	});
});
