...
context("Grid Search", () => {
	before(() => {
		...
		return cy
			...
			.then((saashq) => {
				return saashq.xcall(
					"saashq.tests.ui_test_helpers.insert_doctype_with_child_table_record",
					{
	...
	it("Test search row visibility", () => {
		cy.window()
			.its("saashq")
			.then((saashq) => {
				saashq.model.user_settings.save("Doctype With Child Table", "GridView", {
		...
		cy.get('.saashq-control[data-fieldname="child_table_1"]').as("table");
