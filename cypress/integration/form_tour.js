context.skip("Form Tour", () => {
	before(() => {
		...
		return cy
			...
			.its("saashq")
			.then((saashq) => {
				return saashq.call("saashq.tests.ui_test_helpers.create_form_tour");
			});
	});
	...
	it("navigates a form tour", () => {
		...
		cy.get(".saashq-driver").should("be.visible");
		cy.get('.saashq-control[data-fieldname="first_name"]').as("first_name");
		cy.get("@first_name").should("have.class", "driver-highlighted-element");
		cy.get(".saashq-driver").findByRole("button", { name: "Next" }).as("next_btn");
		...
		cy.get('.saashq-control[data-fieldname="last_name"]').as("last_name");
		...
		// assert field is highlighted
		cy.get('.saashq-control[data-fieldname="phone_nos"]').as("phone_nos");
		...
		cy.get('.grid-row-open .saashq-control[data-fieldname="phone"]').as("phone");
		...
		cy.wait(500);
		cy.get(".saashq-driver").findByRole("button", { name: "Save" }).should("be.visible");
	});
});
