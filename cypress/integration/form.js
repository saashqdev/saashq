...
context("Form", () => {
	before(() => {
		...
		return cy
			...
			.its("saashq")
			.then((saashq) => {
				return saashq.call("saashq.tests.ui_test_helpers.create_contact_records");
			});
	});
	...
	it("create a new form", () => {
		...
		cy.intercept({
			method: "POST",
			url: "api/method/saashq.desk.form.save.savedocs",
		}).as("form_save");
	...
	it("validates behaviour of Data options validations in child table", () => {
		...
		cy.get('.saashq-control[data-fieldname="email_ids"]').as("table");
	...
	it("update docfield property using set_df_property in child table", () => {
		...
		cy.window()
			...
			.then((frm) => {
				...
				cy.get("@table-form")
					.find('.saashq-control[data-fieldname="is_primary_phone"]')
					.should("be.hidden");
