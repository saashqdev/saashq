			...
			return cy
				.request({
					...
					headers: {
						...
						"X-Saashq-CSRF-Token": csrf_token,
					},
				})
				.then((res) => {
...
Cypress.Commands.add("create_records", (doc) => {
	return cy
		.call("saashq.tests.ui_test_helpers.create_if_not_exists", { doc: JSON.stringify(doc) })
		.then((r) => r.message);
});

Cypress.Commands.add("set_value", (doctype, name, obj) => {
	return cy.call("saashq.client.set_value", {
		doctype,
		name,
		fieldname: obj,
	});
...
Cypress.Commands.add(
	...
	(tablefieldname, row_idx, fieldname, fieldtype = "Data") => {
		let selector = `.saashq-control[data-fieldname="${tablefieldname}"]`;
...
Cypress.Commands.add("dialog", (opts) => {
	return cy
		.window({ log: false })
		.its("saashq", { log: false })
		.then((saashq) => {
			Cypress.log({
				name: "dialog",
				displayName: "dialog",
				message: "saashq.ui.Dialog",
				consoleProps: () => {
					return {
						options: opts,
						dialog: d,
			...
			var d = new saashq.ui.Dialog(opts);
			d.show();
			return d;
		});
});
...
Cypress.Commands.add("save", () => {
	cy.intercept("/api/method/saashq.desk.form.save.savedocs").as("save_call");
...
Cypress.Commands.add("add_role", (user, role) => {
	cy.window()
		.its("saashq")
		.then((saashq) => {
			const session_user = saashq.session.user;
...
const add_remove_role = (action, user, role, session_user) => {
	...
	cy.call("saashq.tests.ui_test_helpers.add_remove_role", {
		action: action,
		user: user,
		role: role,
	});
...
Cypress.Commands.add("select_listview_row_checkbox", (row_no) => {
	cy.get(".saashq-list .select-like > .list-row-checkbox").eq(row_no).click();
});
