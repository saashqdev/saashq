...
const html_plugin = require("./saashq-html");
const vue_style_plugin = require("./saashq-vue-style");
const rtlcss = require("rtlcss");
const postCssPlugin = require("@saashq/esbuild-plugin-postcss2").default;
...
const argv = yargs
	...
	.option("skip_saashq", {
		type: "boolean",
		description: "Skip building saashq assets",
	})
	.option("files", {
		type: "string",
...
const APPS = (!argv.apps ? app_list : argv.apps.split(",")).filter(
	(app) => !(argv.skip_saashq && app == "saashq")
);
...
function get_files_to_build(files) {
	// files: ['saashq/website.bundle.js', 'erpnexus/main.bundle.js']
	let include_patterns = [];
	let ignore_patterns = [];
